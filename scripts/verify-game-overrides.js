// 게임정리(사진)+룰설명 관리자 입력(마이그레이션 019, game_overrides) — 실DB 왕복 검증
//
...
(async () => {
  ...
  try {
    ...
    console.log('\n=== ④ 빈 값으로 upsert — rule_note NULL 저장 확인 ===');
    await window.CottageDB.upsertGameOverride(TEST_KEY, { organizerPhotoUrls: [], ruleNote: '' });
