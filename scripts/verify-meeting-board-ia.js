...
check('모임 보드 미리보기 링크는 홈 코티지 모임 섹션 시작점으로 이동', src.includes("previewLink.href = '/?focus=meeting'")
  && homeSrc.includes("const target = document.getElementById('meeting');")
  && !homeSrc.includes('const target = preview?.firstElementChild;')
  && homeSrc.includes('target.getBoundingClientRect().top - headerHeight'));
