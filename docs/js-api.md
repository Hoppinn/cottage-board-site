...
| `getMeetingVotes(startDate, endDate)` | 모임 플래너: 날짜 범위 내 전체 투표 조회. startDate/endDate: 'YYYY-MM-DD'. 024부터 각 행에 `game_style`, `game_depth`, `play_traits`, `recruitment_message` 포함 |
...
| `openProfilePanel(autoSubsheet?, opts?)` | 프로필 보드 열기. `autoSubsheet`: `'taste'\|'records'\|'usage'\|'meeting'\|'voucher'\|'notif'`(자동 진입할 서브시트 — `[data-subsheet]` 요소를 클릭시킴. `'notif'`는 `_ro()`로 감싸져 **내 보드에만 존재**하므로 readOnly와 함께 쓰면 조용히 무시됨). **`opts={userId, readOnly, backTo, nickname}`**. `readOnly:true`면 대상 `userId`의 **공개 보드를 편집 컨트롤 없이** 표시(비공개 섹션=알림·교환권·함께한 시간 제외, 로그인 없이도 조회 가능). readOnly=false(기본)면 종전대로 `getKakaoUser()` 기준 내 보드(버튼 재클릭 토글). 편집 HTML은 내부 `_ro()`로 생략, `.profile-panel--readonly`/`.profile-subsheet--readonly` 클래스 부여. 🚨 **`opts.nickname`은 실제로 넘겨주는 호출부가 없다**(전부 `userId`만 전달) — 비워두면 패널이 `getProfileSnapshot(userId)`로 DB에서 자동 조회해 채운다(2026-07-28 추가). 이게 없던 예전엔 readOnly 보드가 전부 `nickname=''`였고, `getUserPlayedGames`/`getUserParticipationCount`(nickname 기반, player_names ilike 매칭)가 **본인이 직접 쓰지 않고 남에게 태그만 된 기록**을 못 잡아 게임도감·참여횟수가 실제보다 낮게(자주 0으로) 보였다. 패널 제목도 `nickname || '회원'`이라 "회원"으로만 뜨던 것도 같은 원인 |
