...
| `openProfilePanel(autoSubsheet?, opts?)` | 내 보드 통합 패널. 메인 카드 순서는 **프로필→모임→기록→함께한 시간**이며 `autoSubsheet`: `'taste'\|'records'\|'usage'\|'meeting'\|'voucher'\|'notif'`. 표시명은 프로필 보드지만 `'taste'` 내부 키와 `my-board-taste` 분석 키는 레거시 호환을 위해 유지한다. 프로필 보드는 `getProfileBoardData`를 읽어 핵심 요약→플레이 스타일/환경→게임 경험→게임 취향 순으로 표시한다. `opts={userId, readOnly, backTo, nickname}`이고 `readOnly:true`면 대상 `userId`의 공개 보드를 편집 컨트롤 없이 표시한다(비공개 섹션=알림·교환권·함께한 시간 제외, 로그인 없이도 조회 가능). 편집 HTML은 `_ro()`로 생략한다. `opts.nickname`이 없으면 `getProfileSnapshot(userId)`로 보충해 태그 참여 기록과 패널 제목을 보존한다 |
