...
| `getAllProfiles()` | 전체 프로필. 어드민 회원목록 + **identity 별칭→user_id 해석**(game-reviews 허브 `_profileNickMap`·index-page 홈 최근 플레이 참여자 이름 클릭). 현재 nickname과 `real_name`을 `normalizeNick` 키로 함께 쓰되, 키가 서로 다른 user_id에 중복되면 연결하지 않는다. ⚠️ 반환 행의 kakao 키는 **`user_id`**(profiles엔 `id` 컬럼 없음) |
...
| ↳ **참여자 identity 해소 (2026-09-10)** | `getMyStats`·태그 알림·`getUserPlayedGames`·`getUserParticipationCount`·`getUserUniqueDayCount`는 `player_names`를 쉼표 토큰으로 나눠 `normalizeNick`(trim·모든 공백 제거·소문자화) 키가 정확히 같은 행만 쓴다. `profiles.nickname`과 `profiles.real_name`은 같은 회원의 identity 별칭으로 인덱싱하므로, 가입 뒤 닉네임을 정해도 과거 본명 토큰이 유일하게 그 회원으로 해소되면 canonical nickname에 귀속된다. 후보 ILIKE는 공백 사이를 `%`로 넓히는 후보 수집일 뿐 최종 판정이 아니다. 어느 별칭 키든 다른 profiles와 충돌하면 그 키는 자동 연결하지 않고 현재 nickname의 원문 정확 토큰만 허용한다. 기록 원문과 모임 참석 판정은 바꾸지 않는다. |
...
| `window.normalizeNick` | supabase-client.js | play-records-utils.js, game-reviews.js, index-page.js, club-history.html. trim·모든 공백 제거·소문자화한 participant identity 키. 회원 링크와 player_names 정확 토큰 판정은 nickname·real_name 별칭을 함께 보되, 같은 키가 여러 회원이면 호출부는 자동 연결하지 않는다 |
