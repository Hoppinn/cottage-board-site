...
| `window.openDateMeetingModal` | day-detail.js | 날짜 전체 집계 모달 `(voteDate, votes, voteGames, opts?)`. 홈·본 플래너의 `이날 모임 상세`에서 호출. 날짜 헤더는 날짜만 표시하고, 참여 인원·최대 겹침·선호 유형·겹치는 게임은 `게임 조율` 카드에 모은다. 룰렛 다음에 로그인 본인에게만 접힌 `내 게임 조율`을 두고 기존 `meeting_vote_games`와 `setMeetingVoteGamePriority`/`setMeetingVoteGameCondition`을 재사용해 대표 게임(최대 2개)·희망 플레이 인원을 즉시 편집한다. 전원 참여자 목록은 계속 읽기전용이며 각 행 전체가 기존 `openDateScheduleModal(userId, voteDate)` 날짜별 개인 상세로 진입한다. **닫기 줄에 등록/수정 버튼**(`.dd-planner-btn`, 로그인 상태+`voteDate>=오늘`일 때만) — 게임 추가·삭제와 시간·오늘 상태를 수정하는 `openPlannerModal({edit:voteDate, onDirtyClose})`을 이 모달 **위에 겹쳐** 연다. 저장 후 최신 데이터로 이 모달을 다시 그린다. `opts`는 내부에서 안 읽고 재오픈 시 그대로 전달만 함(DD4). |
