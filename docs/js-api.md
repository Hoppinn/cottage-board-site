...
| `formatMemberIntroAvailability(days, times)` | 요일 요약·가능 시간·`available_days`의 `flexible`을 각각 분리해 `[요일 요약] · [가능 시간] · 일정 유동적`으로 조합한다. 시간·일정이 모두 유동적이면 중복된 `시간대 유동적` 대신 `시간대 · 일정 유동적`으로 압축한다. 기존 요일/시간 코드와 신규 `holiday`를 읽기 호환한다 |
