...
| `_confirmLinkOrPlain(record)` | (2026-07-31) 체크박스 미체크로 등록하려는데 연동 대상(내 미연동 기록 **정확히 1건**)이 있으면 **쓰기 전에** 뜨는 확인창. `#sheetLinkConfirmModal`, Promise로 `'cancel'`/`'plain'`/`'link'` 반환. 2건 이상이면 안 뜬다(애매해서 미대응). 게임평·사진(`onSubmitCommentModal`/`onSubmitPhotoModal`) 공용, 두 흐름 다 쓰기 전 분기라 [취소]는 아무 것도 안 쓴다 |
