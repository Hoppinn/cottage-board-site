...
| `window.openDateMeetingModal` | day-detail.js | 날짜 전체 집계 모달 `(voteDate, votes, voteGames, opts?)`. 홈·본 플래너의 `이날 모임 상세`에서 호출. 날짜·닫기는 본문 스크롤 밖의 컴팩트한 헤더에 두며, 닫기는 헤더 우측 × 하나다. 헤더 아래 본문은 `게임 조율`과 `참여자별` 두 섹션으로만 구성한다. 게임 조율에는 참여 인원·최대 시간 겹침·선호 유형·공통 게임 및 룰렛 최종 액션을 모으고, 참여자별 카드는 `getProfileBoardData`의 평소 유형·깊이·경험과 그날 시간·동반·오늘 성향·게임·희망 인원 조건을 바로 표시한다. 참여자 카드는 별도 `openDateScheduleModal`로 열리지 않는다. 로그인 본인 카드의 기존 오늘 게임 행에서만 `setMeetingVoteGameCondition`으로 희망 플레이 인원을 즉시 편집하며, 게임 추가·삭제·대표 지정은 플래너 편집에 남긴다. 로그인했지만 미참여이고 `voteDate>=오늘`일 때만 게임 조율 영역의 등록 버튼이 `openPlannerModal({edit:voteDate, onDirtyClose})`을 이 모달 위에 겹쳐 열며, 저장 후 최신 데이터로 다시 그린다. `opts`는 내부에서 안 읽고 재오픈 시 그대로 전달만 함(DD4). |
