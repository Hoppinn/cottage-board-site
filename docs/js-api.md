...
## window.CottageModalStack (header.js)

`modalStack=1` embed iframe에서만 의미가 있는 center-modal frame client다. 독립 URL과 일반 embed에서는 `request()`/`pop()`이 `false`를 반환해 기존 local modal 동작을 유지한다.
...
### Stable identity, public nickname, and historical participant contract

- **Identity SSOT is stable `user_id`.** Profile boards, statistics, codex, achievements, participation counts, and member links use that ID to identify a person; nickname strings must not become a substitute identity.
- **Public member display-name SSOT is `member_intros.nickname`.** In member-card and read-only profile contexts, this is the canonical public nickname for that `user_id`, even if `profiles.nickname` or `profiles.real_name` differs. The member card passes both the stable ID and this public nickname into the profile flow; the read-only panel fixes its title/display name from that same public value while querying data by ID.
- Historical `player_names` tokens can lack an ID, so the participant resolver recognizes only exact `normalizeNick` keys for `profiles.nickname`, a unique `profiles.real_name`, and `member_intros.nickname`. It never uses partial-substring matching; an alias key shared by multiple IDs is not auto-linked; and an alias never replaces a stable ID that already exists.
- ILIKE with `%` between normalized characters is candidate collection only. Parse candidate `player_names` back into comma tokens and apply the exact, unique identity check before display, count, or aggregation; never count a broad candidate result itself.
...
const { data } = await db.from('profiles').select('bio');
return data || [];          // → [] 반환, try/catch도 console.error도 안 울림

// ✅ error를 받아서 확인해야 함
const { data, error } = await db.from('profiles').select('bio');
...
- **현황(2026-07-17)**: `supabase-client.js`의 `await` **구조분해** 104곳은 전부 error를 받아 로그한다(2단계에서 59곳 추가). 신규 DB 함수도 이 규약을 지킬 것.
  ...
  - **로그 여부의 기준은 "고칠 게 있는가"** — 조용한 실패(화재를 못 봄)와 정상 경로 로그(가짜 경보가 진짜를 가림)는 **같은 원칙의 양쪽 위반**이다. `getUserPhotoCount`의 안쪽 `JSON.parse` catch처럼 실패가 정상 분기인 곳엔 로그 대신 주석을 단다(1단계가 기계적으로 붙여 실제로 가짜 에러 발생). 상세는 현재 사용하는 작업 규칙 파일([AGENTS.md](../AGENTS.md) 또는 [CLAUDE.md](../CLAUDE.md))의 DB 오류 처리 규칙.
...
| `getAllProfiles()` | 전체 프로필. 어드민 회원목록 + **identity 별칭→user_id 해석**(game-reviews 허브 `_profileNickMap`·index-page 홈 최근 플레이 참여자 이름 클릭). profiles 행에 같은 `user_id`의 `member_intros.nickname`을 `public_nickname`으로 보강한다. 참여자 링크는 `nickname`·`real_name`·`public_nickname`의 `normalizeNick` 키를 함께 쓰되, 키가 서로 다른 user_id에 중복되면 연결하지 않는다. ⚠️ 반환 행의 kakao 키는 **`user_id`**(profiles엔 `id` 컬럼 없음) |
...
| `getProfileUsage(userId)` | **한 회원**의 이용 누적 `{visit_count, total_minutes, today_seconds, today_date, last_seen_at, first_seen_at}` (P4 「이용 요약」 — 누적·방문은 R3대로 profiles가 정본). `total_minutes`는 **legacy DB 이름이고 실제 저장 단위는 seconds**이므로, 소비처는 읽은 직후 `totalUsageSeconds` 같은 초 단위 변수로 매핑한다. `getAllProfiles(*, 전원)`을 안 부른다 |
...
| `getMyNotifications(userId, nickname, notifSeenAt, newGameSeenAt)` | 최근 알림 목록 반환. ①태그된 기록(최근20) ②궁금해요 게임 코멘트(최근20) ③구매완료(최근10) ④new_game(newGameSeenAt 이후 추가된 게임) ⑤new_intro(타인 소개글, 로그인 회원 전체 수신, `{type:'new_intro', count, names, firstUserId, date, isNew}`) ⑥snack_done(내 간식·음료 요청 처리완료, 최근10 — `snack_requests.is_done`/`done_at`, 요청자 본인 전용, 015) ⑦new_member(신규 회원가입, 로그인 회원 전체 수신, `{type:'new_member', count, names, userIds, date, isNew}` — 2026-07-30. 새 테이블 없이 `page_events`의 `signup_complete`를 소스로 씀, 닉네임은 `profiles`에서 별도 in-조회. `userIds`는 2026-08-02 추가 — `names`와 같은 순서라 인덱스로 매칭, 이름마다 개별 클릭 시 그 사람 보드로 이동(`kakao-auth.js` `_renderNotifItem`의 `data-member-uid`, `new_intro`의 `firstUserId`는 1명뿐이라 여럿이 묶이면 첫 명만 갈 수 있었던 것과 다름)). notifSeenAt=null이면 지평선 없음(전체 기간). 반환: `[{type, key, keys?, ..., isNew}]` |
| ↳ **참여자 identity 해소** | 위 **Stable identity, public nickname, and historical participant contract**를 따른다. `getMyStats`·태그 알림·`getUserPlayedGames`·`getUserParticipationCount`·`getUserUniqueDayCount`는 historical token을 resolver로만 보조 해소하며, 기록 원문과 모임 참석 판정은 바꾸지 않는다. |
...
| ↳ **묶음 알림 4종과 묶는 축** | `new_intro`·`new_member`=유형 전체 / `voucher_granted`·`voucher_used`=유형+KST날짜 / **`tagged`=모임(`group_name`+날짜)**(2026-07-21 추가). 🚨 **`tagged`를 게임별로 묶지 말 것** — 실측에서 한 사람 16건 중 게임이 11종이라 거의 안 접힌다. 한 모임에서 게임 여러 개를 한 번에 기록하는 게 실제 패턴이라 **모임 축이어야 접힌다**(전체 91줄→44줄, 김기성 14→3). `new_intro`/`new_member`는 **신규+기존이 섞이면 신규만 센다**(기존 건은 묶음에서 빠짐) — 전부 기존일 때만 "최신 1건 기록용"으로 대체 표시 |
...
| `addNotifReadKeys(userId, keys[])` | **개별** 알림 읽음 — `profiles.notif_read_keys`에 키 배열을 합집합으로 추가(중복 제거). `notif_seen_at`(지평선)은 **건드리지 않음**. 반환 `{error}`. 소비처: kakao-auth.js `_markOneNotifSeen`. **`notice:fee`/`notice:voucher`도 같은 배열에 저장**(전체공지·교환권공지 확인, 소스 행 id가 없는 정적 singleton 키라 위 `${type}:id` 규칙의 예외) |
| `getNoticeAckKeys(userId)` | `profiles.notif_read_keys`를 그대로 반환 — `notice:fee`/`notice:voucher` 포함 여부 확인용(2026-07-30). `feeNoticeSeen`/`voucherNoticeSeen`이 localStorage(기기별)에만 있어 기기·브라우저를 바꾸면 재노출되던 문제를 `notif_seen_at`과 같은 방식(DB)으로 해소. 소비처: kakao-auth.js `_updateNotifBadge`(로그인 버튼 배지)·`openProfilePanel`(알림 서브시트) 둘 다 |
| `updateNotifSeenAt(userId, timestamp)` | **모두 읽기** — 지평선을 `timestamp`로 옮기고 **`notif_read_keys`에서 `notice:` 접두사가 아닌 키를 전부 비운다**(2026-07-18 도입, 2026-07-30 `notice:*` 보존 예외 추가 — 이 키는 시간 지평선과 무관한 1회성 ack라 "모두 읽기"에 휩쓸리면 안 됨). 소비처: kakao-auth.js `_markAllNotifSeen`(호출 시 `addNotifReadKeys(['notice:fee','notice:voucher'])`를 먼저 큐에 넣어야 이 함수가 보존할 값이 생김 — 같은 유저 write 큐로 직렬화되므로 호출 순서=실행 순서). ※2026-07-18 이전까지 이 표에 **누락돼 있던 항목**(문서 드리프트, 발견 즉시 보강) |
...
| `getMeetingVotes(startDate, endDate)` | 모임 플래너: 날짜 범위 내 전체 투표 조회. startDate/endDate: 'YYYY-MM-DD'. 각 행에 `game_style`, `game_style_custom`, `game_depth`, `play_traits`, `recruitment_message` 포함. 027부터 `game_style='other'`이면 `game_style_custom`에 최대 30자의 표시 문구가 있다 |
...
| `openProfilePanel(autoSubsheet?, opts?)` | 내 보드 통합 패널. 메인 카드 순서는 **프로필→모임→기록→함께한 시간**이며 `autoSubsheet`: `'taste'\|'records'\|'usage'\|'meeting'\|'voucher'\|'notif'`. 표시명은 프로필 보드지만 `'taste'` 내부 키와 `my-board-taste` 분석 키는 레거시 호환을 위해 유지한다. 프로필 보드는 `getProfileBoardData`를 읽어 평소 플레이어 정보를 표시한다. 모임 보드와 메인 모임 카드 모두 `meeting_votes`/`meeting_vote_games`를 오늘~180일의 같은 범위로 읽는다. 카드는 일정 날짜와 distinct 하고 싶은/배우고 싶은 게임 수만 요약하며 날짜별 판 성향은 하나로 합성하지 않는다. 모임 보드의 평소 정보는 프로필 요약만 참조한다. `opts={userId, readOnly, backTo, nickname, focusDate}`이고 `readOnly:true`면 대상 `userId`의 공개 보드를 편집 컨트롤 없이 표시한다(비공개 섹션=알림·교환권·함께한 시간 제외, 로그인 없이도 조회 가능). `focusDate`는 모임보드 진입 시 해당 날짜를 강조·스크롤하는 모임 활동 컨텍스트다. 편집 HTML은 `_ro()`로 생략한다. `opts.nickname`이 없으면 `getProfileSnapshot(userId)`로 보충해 태그 참여 기록과 패널 제목을 보존한다 |
...
| ↳ **서브시트 문자열 캐시 주의 (2026-07-18)** | 알림 서브시트는 `_notifInnerHtml` **문자열 캐시**로 재렌더되므로 **DOM만 바꾸는 변경은 재진입 시 되돌아간다.** 바꾸려면 `_openSubSheet`의 **`onLeave` 스냅샷**을 함께 걸 것(기록보드와 같은 방식). 같은 함정이 `_recordInnerHtml`·`_collectionBoardInnerHtml` 등 **오픈 시 1회 문자열로 만드는 서브시트 전부**에 적용된다(`_buildProfileBoardInnerHtml`·`_buildMeetingInnerHtml`은 함수라 해당 없음) |
| `openOtherProfileSheet(userId, opts?)` | 사람 이름·대표 아바타의 공통 진입점. 기본은 얇은 래퍼 → `openProfilePanel(null, {userId, readOnly:true, ...opts})`; 본인이면 내보드 메인, 타인이면 읽기전용 내보드 메인으로 연다. `opts.autoSubsheet`을 넘길 때만 그 서브보드(예: `'taste'`)에 직행한다. 모임원 프로필 카드의 본문·`프로필 자세히 보기 ›`는 이 옵션으로 프로필 보드에 가고, 이름/헤더·게임시트 아바타·게임평/기록 닉네임·참여자/리뷰어·홈 최근 플레이 작성자는 기본 내 보드 진입을 쓴다 |
| `openOtherMeetingSheet(userId, opts?)` | 모임 활동의 공통 진입점. `openProfilePanel('meeting', {userId, readOnly:true, ...opts})`로 모임 보드에 직행한다. 플래너 참여카드 일반 영역에서 사용하며 `opts.focusDate`가 있으면 해당 날짜를 모임보드에서 강조·스크롤한다. 사람 이름·아바타에는 사용하지 않는다 |
...
| `onSubmitCommentModal()` | link-mode(`linkCommentId`)면 `updateGamePlay`+원본 `deleteComment`. select 연동 분기: `join:` 옵션이면 세션 필드 복사한 `recordGamePlay`(남 세션 참여), 내 기록 id면 `updateGamePlay(review_text)`, 미선택이면 **내 기록이 정확히 1개일 때만** 쓰기 전에 `_confirmLinkOrPlain`으로 확인(2026-07-31, 사후확인 방식에서 변경) — [연동하기]면 `insertComment` 없이 곧바로 그 기록에 `updateGamePlay(review_text)`, [취소]면 아무 것도 안 씀, [그냥 남기기]/그 외엔 `insertComment`. **`modal.dataset.recordId` 있으면(첨부 모드, 014)** 이 확인 없이 바로 `insertComment(..., recordId)` → 그 기록에 매인 게임평 → `refreshPlayRecordsBoard`로 게시판 반영, 넛지 건너뜀. 내 기록 0개면 남 세션 있을 때 `_openJoinConfirm`(방금 쓴 게임평 이동), 없으면 넛지 토스트 |
...
| `_confirmLinkOrPlain(modalEl, record)` | (2026-07-31, 별도 팝업→같은 모달 내 전환으로 재설계) 체크박스 미체크로 등록하려는데 연동 대상(내 미연동 기록 **정확히 1건**)이 있으면 **쓰기 전에** 뜨는 확인. 새 오버레이를 안 띄우고 `modalEl`(호출한 게임평/사진 모달 그 자체) 안의 `.sheet-comment-modal-normal`을 숨기고 `.sheet-link-confirm-inline`을 그 자리에 펼친다 — 겉보기엔 모달 하나 유지. Promise로 `'cancel'`/`'plain'`/`'link'` 반환. 2건 이상이면 안 뜬다(애매해서 미대응). 게임평·사진(`onSubmitCommentModal`/`onSubmitPhotoModal`) 공용 |
| `onOpenPhotoInput(btn)` / `onSubmitPhotoModal()` | 사진 남기기 모달. "연동" select = 내 기록(선택 시 `updateGamePlay`로 photo_url 병합) + **남의 세션**(`data-join="1"`, `data-rec-ids`, `modal._joinSessions`; 선택 시 세션 필드 복사한 `recordGamePlay`로 내 새 사진 기록 = 세션 참여). `btn.dataset.recordId` 있으면 `_preselectLinkOption`으로 기본 연동. 미체크인데 내 미연동 기록이 정확히 1건이면 쓰기 전에 `_confirmLinkOrPlain`으로 확인(2026-07-31) — 그 외 미연동이면 사진만 담은 새 기록 생성 |
...
| `window.COTTAGE_GAMES` | 게임 플랫 배열 `{id, bggId, display, titleKo, titleEn, abbr, bestPlayers, recPlayers, thumbnail}`. 게임명 자동완성·인원 조건 표시용.<br>`thumbnail`: `getGameImage(g)` 재사용(images.thumbnail→images.main 순, 정규화됨), 없으면 `null`. day-detail.js 자세히/이날모임한눈에보기 모달의 작은 게임 썸네일에 사용(2026-07-15).<br>`abbr` 결정 3단계 (build-output.js): ① `game-abbr.json[bggId]` → ② `game-abbr-byname.json[ownedName]` → ③ `titleKo.slice(0,2)` 폴백.<br>`bestPlayers`/`recPlayers`: gameData.bgg.bestPlayers/recommendedPlayers 배열 원본. 데이터 없으면 `null`. `window.formatCondLabel`이 소비.<br>**abbr 소비처**: 막대 라벨(`resolveGameAbbr`), 룰렛 휠 SVG, 룰렛 후보, 룰렛 수동 추가. 보유 게임은 `COTTAGE_GAMES[i].abbr` 우선, 직접입력 게임은 `COTTAGE_GAME_ABBR_BY_NAME[custom_name]` 우선이며 둘 다 없을 때만 게임명 앞 2글자를 쓴다. `#` 접두 제거 후 조회·slice한다. |
| `window.COTTAGE_GAME_ABBR_BY_NAME` | `game-abbr-byname.json`을 `build-output.js`가 그대로 노출한 게임명별 수동 약칭 정본. BGG ID 없는 보유 게임뿐 아니라 모임 플래너의 직접입력 게임에도 사용한다. `day-detail.js`의 `_resolveNameAbbr(name)`가 정확한 게임명으로 조회하고, 없으면 앞 2글자로 폴백한다. |
...
| `window.normalizeNick` | supabase-client.js | play-records-utils.js, game-reviews.js, index-page.js, club-history.html. trim·모든 공백 제거·소문자화한 participant identity 키. 회원 링크와 player_names 정확 토큰 판정은 profiles의 nickname·real_name 및 같은 stable user_id의 `member_intros.nickname` 별칭을 함께 보되, 같은 키가 여러 회원이면 호출부는 자동 연결하지 않는다 |
...
| `window.openDateMeetingModal` | day-detail.js | 날짜 전체 집계 모달 `(voteDate, votes, voteGames, opts?)`, 화면명은 `모임 조율`. 홈·본 플래너의 `모임 조율`에서 호출한다. 날짜·닫기는 본문 스크롤 밖의 컴팩트한 헤더에 두며, 닫기는 헤더 우측 × 하나다. 헤더 아래 본문은 `모임 현황 → 인원 조율 → 참여자별 상세` 순서다. 모임 현황에는 참여 인원·최대 시간 겹침·선호 유형·공통 게임을 보이고, 인원 조율에는 기존 룰렛과 미참여자의 플래너 등록 진입을 둔다. 게임 투표는 DB/API 설계·승인 전까지 이 API의 범위가 아니다. 참여자별 상세 카드는 `getProfileBoardData`의 평소 유형·깊이·경험과 그날 시간·동반·오늘 성향·게임·희망 인원 조건을 바로 표시하며, 별도 `openDateScheduleModal`로 열리지 않는다. 로그인 본인 카드의 기존 오늘 게임 행에서만 `setMeetingVoteGameCondition`으로 희망 플레이 인원을 즉시 편집하며, 게임 추가·삭제·대표 지정은 플래너 편집에 남긴다. 로그인했지만 미참여이고 `voteDate>=오늘`일 때만 인원 조율 영역의 등록 버튼이 `openPlannerModal({edit:voteDate, onDirtyClose})`을 이 모달 위에 겹쳐 열며, 저장 후 최신 데이터로 다시 그린다. 외곽은 참여자 수와 무관하게 compact 고정 크기이며 본문만 스크롤한다. `opts`는 내부에서 안 읽고 재오픈 시 그대로 전달만 함(DD4). |
| `window.buildBarsInCard` | day-detail.js | 주간 카드/홈 미리보기 시간 막대 HTML 반환 `(dayVotes, voteGames, myVote, includeCoordinationSummary=false)`. 마지막 인자가 true인 플래너 날짜 카드는 참여자 카드 아래에 날짜별 `모임 현황` 요약을 붙이고, 그 아래 `모임 조율` 진입 버튼이 온다. myVote=null이면 is-mine 강조·수정삭제 버튼 없음. **지난 날짜(`vote_date < 오늘`)면 내 막대여도 ✎/✕를 렌더하지 않는다** — 플래너 iframe이 `cottage-edit`을 `ds >= 오늘`에서 버려 눌러도 무반응이던 자리(2026-07-22). 막대 자체는 그대로 그린다(보기 허용). club-schedule.html·index-page.js·openDatePreviewModal에서 호출. |
| `window.openDatePreviewModal` | day-detail.js | 하루치 미리보기 센터모달 `(dateStr, dayVotes, dayGames, myVote?, onChange?)` — buildBarsInCard 재사용, 그날 참여자 막대그래프. 우상단 ✕로 닫기. 현재 플래너·홈·하루치 카드 본문은 모임보드로 통일하며, 이 모달은 기존 호환 진입점/날짜 미리보기 용도로 유지한다. |
| `window.openPlannerModal` | day-detail.js | **공용** 모임 플래너 센터모달(전 페이지, `#__plannerModal`). `(opts)`: `weekOffset`(주차), `register`/`edit`(date), `onDirtyClose`(저장 후 닫힘 콜백). club-schedule.html?embed=true를 iframe으로 띄우고 open 시 목표 상태 전체 선언(cottage-reset-week/register/edit). 호출부: 모임보드 `＋ 참여 등록`(register)과 일정별 ✎(edit), `openDatePreviewModal`·`openDateMeetingModal`의 등록/수정 버튼. ⚠️ **`edit`/`register`가 있으면(quick-entry) 박스를 즉시 안 연다** — club-schedule.html이 시트를 실제로 그린 뒤 보내는 `cottage-sheet-shown`을 받은 시점에만 연다(`_pmAwaitingReveal`) + `.planner-modal-overlay`에 `is-quick-entry` 클래스를 얹어 박스를 풀블리드·투명화한다(안쪽이 하단 바텀시트뿐이라 고정 크기 박스를 그대로 쓰면 빈 카드처럼 남는 레이아웃 갭 방지, 2026-07-30). `weekOffset`만 있는 일반 오픈은 이 신호를 안 보내므로 기존처럼 즉시 연다. |
...
`openProfilePanel`의 네 보드 서브시트와 별도 기능인 `수집 보드`도 특정 사용자 보드 내부 화면이므로 `_openSubSheet`의 공통 사용자 identity back header(대표 캐릭터+닉네임)를 사용한다. 하위 보드 header의 왼쪽 전체 `< 캐릭터 닉네임` 영역은 해당 사용자 내보드 메인으로 돌아가며, 수집 보드를 메인 보드 IA 메뉴에 추가하는 계약은 아니다. 내보드 메인은 큰 identity로 시작하고 스크롤 후에만 `내 보드` compact header를 표시한다.
