...
| `openProfilePanel(autoSubsheet?, opts?)` | 내 보드 통합 패널. 메인 카드 순서는 **프로필→모임→기록→함께한 시간**이며 `autoSubsheet`: `'taste'\|'records'\|'usage'\|'meeting'\|'voucher'\|'notif'`. 표시명은 프로필 보드지만 `'taste'` 내부 키와 `my-board-taste` 분석 키는 레거시 호환을 위해 유지한다. 프로필 보드는 `getProfileBoardData`를 읽어 평소 플레이어 정보를 표시한다. 모임 보드와 메인 모임 카드 모두 `meeting_votes`/`meeting_vote_games`를 오늘~180일의 같은 범위로 읽는다. 카드는 일정 날짜와 distinct 하고 싶은/배우고 싶은 게임 수만 요약하며 날짜별 판 성향은 하나로 합성하지 않는다. 모임 보드의 평소 정보는 프로필 요약만 참조한다. `opts={userId, readOnly, backTo, nickname}`이고 `readOnly:true`면 대상 `userId`의 공개 보드를 편집 컨트롤 없이 표시한다(비공개 섹션=알림·교환권·함께한 시간 제외, 로그인 없이도 조회 가능). 편집 HTML은 `_ro()`로 생략한다. `opts.nickname`이 없으면 `getProfileSnapshot(userId)`로 보충해 태그 참여 기록과 패널 제목을 보존한다 |
...
| `openOtherProfileSheet(userId, opts?)` | 얇은 래퍼 → `openProfilePanel('taste', {userId, readOnly:true, ...opts})`. 본인이면 편집 가능한 프로필 보드다. 모임원 프로필 카드(club-intro.html), 게임시트 아바타/게임평/기록 닉네임, 플레이기록 참여자·리뷰어, 홈 최근 플레이 작성자·참여자 진입점으로 쓴다 |
| `openOtherMeetingSheet(userId, opts?)` | 얇은 래퍼 → `openProfilePanel('meeting', {userId, readOnly:true, ...opts})`. 알림 소개글과 **특정 일정의 모임 참여자**(`.sched-bar-name[data-uid]` — day-detail.js/index-page.js/club-schedule.html)가 가까운 일정 맥락을 유지할 때만 모임 보드로 직행한다 |
