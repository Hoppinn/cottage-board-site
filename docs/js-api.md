...
| `window.openDateMeetingModal` | day-detail.js | 날짜 전체 집계 모달 `(voteDate, votes, voteGames, opts?)`. 홈 미리보기 카드 클릭 시 index-page.js에서 호출. **닫기 줄에 등록/수정 버튼**(`.dd-planner-btn`, 로그인 상태+`voteDate>=오늘`일 때만) — 클릭 시 `openPlannerModal({edit:voteDate, onDirtyClose})`을 이 모달 **위에 겹쳐** 연다(모달을 안 닫음, 닉네임→보드와 동일한 레이어 방식). 저장 여부와 무관하게 플래너가 닫히면 최신 데이터로 이 모달을 다시 그린다(2026-07-30). `opts`는 내부에서 안 읽고 재오픈 시 그대로 전달만 함(DD4). |
