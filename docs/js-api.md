...
| ↳ **참여자 identity 해소 (2026-09-10)** | `getMyStats`·태그 알림·`getUserPlayedGames`·`getUserParticipationCount`·`getUserUniqueDayCount`는 `player_names`를 쉼표 토큰으로 나눠 `normalizeNick`(trim·모든 공백 제거·소문자화) 키가 정확히 같은 행만 쓴다. `profiles.nickname`·`profiles.real_name`과 같은 `user_id`의 `member_intros.nickname`(공개 canonical nickname)을 같은 identity 별칭으로 인덱싱한다. 따라서 모임원 카드와 read-only 상세가 같은 stable ID를 쓰고, 과거 본명 토큰도 유일하게 그 회원으로 해소되면 같은 회원 통계에 귀속된다. 후보 ILIKE는 공백 사이를 `%`로 넓히는 후보 수집일 뿐 최종 판정이 아니다. 어느 별칭 키든 다른 회원과 충돌하면 그 키는 자동 연결하지 않고 현재 canonical nickname의 원문 정확 토큰만 허용한다. 기록 원문과 모임 참석 판정은 바꾸지 않는다. |
