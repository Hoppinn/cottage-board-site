...
최종 갱신: 2026-08-30 (027 `meeting_votes.game_style_custom` 운영 적용·76행 보존, `other` 안정 코드와 최대 30자 사용자 문구 분리 저장·API 왕복·격리 행 삭제 확인) / 2026-08-30 (025·026 운영 적용 및 실DB 최초/재저장 검증: UUID 반환, 30분 슬롯·유동적·커스텀 유형 왕복, `intro_complete` 최초 true/재저장 false. 스냅샷 복구 후 테스트 쿠폰은 SQL Editor에서 제거해 profiles 41/member_intros 10/voucher_log 57 기준 복구) / 2026-08-29 (024 모임 플래너 날짜별 판 의도 필드 운영 반영·69행 보존·API 왕복 및 DB CHECK 검증)
