...
| 028·029 프로필 보드 IA 데이터 | ✅ **운영 DB 반영 + 검증 (2026-08-31)** — `profiles.preferred_game_depths TEXT[]`는 신규 선호 웨이트 `weight_intro\|weight_light\|weight_heavy\|weight_hardcore`와 기존 legacy `light\|medium\|deep`를 함께 허용한다. 기존 legacy 값은 자동 변환하지 않는다. `profile_hardest_games`(사용자별 순서 1~2), 원자 교체 RPC, `meeting_votes.play_traits`의 `hard_game_learning_ok` 확장을 포함한다. |
