...
최종 갱신: 2026-08-31 (028 프로필 보드 IA 마이그레이션 작성·API 모의검증 완료, **운영 DB 미적용**) / 2026-08-30 (027 `meeting_votes.game_style_custom` 운영 적용·76행 보존, `other` 안정 코드와 최대 30자 사용자 문구 분리 저장·API 왕복·격리 행 삭제 확인)
...
| 028 프로필 보드 IA 데이터 | ⚠️ **파일·API 구현 완료, 운영 DB 미적용 (2026-08-31)** — `profiles.preferred_game_depths TEXT[]`(복수 `light\|medium\|deep`, `any`·중복 없음), `profile_hardest_games`(사용자별 순서 1~2), 원자 교체 RPC, `meeting_votes.play_traits`의 `hard_game_learning_ok` 확장. `scripts/verify-profile-board-ia.js` 음성 대조군·모의 API 검증 통과. UI 착수 전에 028 적용과 실DB 왕복 검증이 필요하다. |
...
| `profile_hardest_games` (028 예정) | id, user_id, sort_order (1~2), game_id (nullable), custom_name (nullable), created_at | 해본 게임 중 가장 어려웠던 게임 최대 2개. 카탈로그 게임은 `game_likes`와 같은 COTTAGE_GAMES game key, 그 밖의 게임은 custom_name을 저장하고 둘 중 정확히 하나만 허용. `(user_id, sort_order)` UNIQUE로 최대 2개를 DB에서 보장. RLS OFF + anon 직접 접근 계약 |
...
| `replace_profile_hardest_games(p_user_id, p_games)` (028 예정) | 가장 어려웠던 게임 0~2개를 원자 교체. JSONB 배열 각 원소는 game_id/custom_name 중 정확히 하나, 각 100자 이하. 검증→기존 행 삭제→순서대로 삽입이 한 트랜잭션이므로 실패 시 기존 목록 보존 |
