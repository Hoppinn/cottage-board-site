...
| 020 `game_overrides.error_note` 컬럼 (에러로그) | ⚠️ **미실행 — 사용자가 Supabase에서 SQL 실행 필요.** 실행 전 코드 배포 시 `upsertGameOverride`가 없는 컬럼에 쓰려다 실패한다(014와 동일 순서 주의) |
...
| `game_overrides` | game_key (PK), organizer_photo_urls (text[]), rule_note, error_note, updated_at | 게임정리 사진·룰설명·에러로그(자주 하는 실수 메모) 관리자 입력(019 + 020, error_note는 ⚠️ 미실행). game_key = `COTTAGE_GAMES[].id`(= `game_likes.game_id`와 동일한 slugifyKorean(ownedName) 값, bggId 아님). `requests-admin.html` 「게임 관리」에서 편집, `game-sheet.js` `initSheetOrganizerContent`가 읽어 표시 |
