...
| 033 평소 플레이 습관 분리 | ✅ **운영 적용 완료 (사용자 확인, 2026-09-01)** — `member_intros.usual_play_days/usual_play_times`와 `submit_member_intro` 새 시그니처를 추가했다. 기존 `available_days/times`는 모임 참여 가능 일정으로 보존하며, hardest game은 기존 `profile_hardest_games`를 같은 RPC 트랜잭션에서 교체한다. |
| 034 가입 경로 `boardlife` 허용 | ✅ **운영 적용 완료 (사용자 확인, 2026-09-10)** — `submit_member_intro`의 `join_sources` 허용 코드에 `boardlife`만 추가했다. 기존 7개 코드와 저장값, upsert·hardest game 교체·최초 쿠폰 지급 트랜잭션은 변경하지 않는다. |
...
| 029 프로필 선호 웨이트 코드 | ✅ **운영 DB 적용 확인 (2026-08-31)** — `profiles.preferred_game_depths TEXT[]`는 신규 선호 웨이트 `weight_intro\|weight_light\|weight_heavy\|weight_hardcore`와 기존 legacy `light\|medium\|deep`를 함께 허용한다. 기존 legacy 값은 자동 변환하지 않는다. 적용 SQL의 `invalid_depth_rows = 0`을 확인했다. |
...
| `profiles` | user_id, nickname, real_name, last_seen_at, visit_count, total_minutes, is_banned, photo_url, today_seconds, today_date, rep_achievement_id, rep_title_id, first_source, bio (text), avoid_tags (text[]), notif_seen_at (timestamptz), **preferred_game_depths (text[], 028·029 legacy)** | 유저 계정 공통 프로필. **bio: 한줄소개 SSOT** — 취향보드/회원 자기소개(club-intro.html)/모임 보드가 공유 읽기·쓰기(`updateUserBio`)한다. `avoid_tags`와 `preferred_game_depths`는 과거 데이터 보존용 레거시 컬럼이며 **031 이후 회원 설문·프로필 보드·모임원 목록 카드의 게임 취향 정본이 아니다**. 새 설문은 `member_intros`만 읽고 쓴다. notif_seen_at: 알림 마지막 읽은 시각, notif_read_keys (jsonb, default `[]`): **개별** 알림 읽음 키 배열 — `${type}:${소스행 id}` 형식(예: `new_intro:42`). `updateNotifSeenAt`(모두 읽기)은 `notice:` 키 외 읽음 키를 비운다. |
...
| `member_intros` | id (**uuid**), user_id (**text, UNIQUE**), nickname, location, join_sources, companion_types, average_play_frequency, possible_frequency_min/max, desired_frequency_min/max, available_days, available_times, **preferred_game_types, game_type_range, avoid_game_types, preferred_game_depths, game_depth_range, avoid_game_depths** (031·032), clocktower_preference, expectation, questionnaire_completed_at | 회원 공개 프로필·게임 취향의 정본. `join_sources` 허용 코드는 `store_visit|friend_referral|cottage_homepage|open_chat_search|daangn|naver_place|boardlife|social_media`이며 기존 값은 정규화하지 않는다. 유형과 난이도 모두 `주 취향 ⊆ 전체 취향 범위`; 난이도는 `game_depth_range ∩ avoid_game_depths = ∅`이다. 유형의 명백한 충돌(`party`↔파티게임, `social_deduction`↔마피아류)도 차단한다. 가입 경로·거주 지역은 상세 프로필에 공개한다. |
...
- `profiles.total_minutes`는 **legacy column name**이며 실제 저장 단위는 **seconds**다. `increment_profile_counters`가 `p_secs`를 그대로 누적한다.
- 애플리케이션은 이 DB 필드를 읽은 직후 `totalUsageSeconds`처럼 초 단위가 드러나는 내부 변수로 매핑한다. DB 컬럼 rename과 기존 운영 데이터 변경은 별도 migration 작업이다.
- 이 전 기간 누적값은 내 보드 「함께한 시간」, 오너 전용 회원 분석의 누적 체류, 관리자 회원 카드의 총시간에 사용한다. 기간별 페이지 체류·분석은 `page_sessions`의 별도 지표이므로 서로 맞추거나 보정하지 않는다.
...
| `increment_profile_counters(p_user_id, p_secs, p_today, p_bump_visit)` | **profiles 카운터 원자적 증가**(012, #22). `total_minutes`(**legacy name; stored unit: seconds**) += `p_secs` / `today_seconds`는 `today_date`가 다르면 리셋 후 시작 / `p_bump_visit`일 때만 `visit_count` +1 / `last_seen_at` 갱신. **`returns setof profiles`** — 반환이 비면 "그 user_id의 행이 없다"는 뜻이다. 🚨 **UPDATE만 하고 upsert하지 않는다** — 행 생성은 `upsertProfile`의 몫이고, 여기서 만들면 프로필 없는 사용자에게 빈 행이 생겨 회원 수가 부풀려진다 |
| `submit_member_intro(...)` | 필수 자기소개 전체 제출. `p_user_id TEXT`는 카카오 사용자 ID, 반환 `intro_id UUID`는 `member_intros.id`다(025). 034의 `join_sources` 허용 코드는 `store_visit|friend_referral|cottage_homepage|open_chat_search|daangn|naver_place|boardlife|social_media`이며, 033의 `p_usual_play_days/times`는 평소 습관, 기존 `p_available_days/times`는 모임 참여 가능 일정이다. 030의 `holiday`, 031·032의 유형·난이도 3단 배열과 `location`을 검증한다. 유형·난이도 범위는 주 취향을 포함해야 하고, 난이도 범위와 꺼림은 겹칠 수 없다. **`member_intros` upsert + `profile_hardest_games` 교체 + `voucher_log intro_complete` insert**를 단일 트랜잭션으로 처리하며, `profiles.avoid_tags`·`profiles.preferred_game_depths`를 새 설문 값으로 갱신하지 않는다. partial unique index가 최초 저장 쿠폰을 사용자당 1회로 보장한다. |
| `member_intro_holiday_supported()` | 030 capability 확인. migration이 적용된 운영 DB에서만 `true`를 반환하며 anon wizard가 공휴일 입력을 노출할지 결정한다. |
