...
| `member_intros` | id, user_id (**UNIQUE**), nickname, favorite_games, available, location, travel_range, meeting_style (text[]), card_color, created_at, **join_sources, companion_types, average_play_frequency, possible_frequency_min/max, desired_frequency_min/max, available_days, available_times, preferred_game_types, clocktower_preference, expectation, questionnaire_completed_at** (023) | 회원 자기소개 + 모임 보드 공유 프로필. user_id당 1행. 023 구조화 답변은 전체 제출 RPC만 갱신하며 과거 행의 NULL은 허용한다. 빈도는 0~6 순위값(몇 달에 1회 이하~주 4회 이상), 최소≤최대 DB CHECK. 가입 경로는 관리자 화면에서만 표시하고 공개 보드에는 반환하지 않는다. 꺼리는 유형은 새 사본을 만들지 않고 `profiles.avoid_tags`를 계속 SSOT로 쓴다. 2026-05-27 이전 user_id=NULL 레거시는 연동 대상에서 제외 |
...
| `submit_member_intro(...)` | 023 필수 자기소개 전체 제출. 허용 코드·필수 배열·각오 10~500자·빈도 0~6·최소≤최대를 검증하고 `member_intros` upsert + `profiles.avoid_tags` update + `voucher_log intro_complete` insert를 단일 트랜잭션으로 처리. 반환 `{intro_id, voucher_granted}`. 기존 프로필 행이 없으면 ghost profile을 만들지 않고 전체 롤백 |
