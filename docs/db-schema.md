...
| 030 자기소개 가능 요일 `holiday` 호환 확장 | ⏳ **migration 파일 작성 완료, 운영 적용 후 실DB 왕복 검증 필요** — `available_days TEXT[]` 구조와 `submit_member_intro` 기존 시그니처는 유지하고 허용값에 `holiday`만 추가했다. 기존 행은 변환하지 않는다. `member_intro_holiday_supported()`가 `true`를 반환할 때만 wizard의 공휴일 입력을 노출한다. |
| 031 자기소개 취향 3단 구조 | ⏳ **운영 적용 대기** — `member_intros`에 유형 범위/꺼림과 난이도 3단 배열 5개를 추가하고, 주 취향⊆범위·난이도 범위∩꺼림=∅·명백한 유형 충돌을 DB 제약과 `submit_member_intro` RPC에서 함께 보장한다. 기존 취향 데이터만 초기화하며 닉네임·가입 경로·빈도·시간·시계탑·서술·거주 지역은 보존한다. 적용 순서: 030 → 031 → 코드 배포. |
| 032 취향 범위 `any` 보정 | ⏳ **운영 적용 대기** — 주 취향이 일반 유형이어도 취향 범위에서 `장르를 가리지 않음(any)`을 선택할 수 있게 031의 DB 제약·RPC를 교체한다. 저장값은 주 취향+`any`만 허용한다. 적용 순서: 030 → 031 → 032 → 코드 배포. |
...
| `member_intros` | id (**uuid**), user_id (**text, UNIQUE**), nickname, location, join_sources, companion_types, average_play_frequency, possible_frequency_min/max, desired_frequency_min/max, available_days, available_times, **preferred_game_types, game_type_range, avoid_game_types, preferred_game_depths, game_depth_range, avoid_game_depths** (031), clocktower_preference, expectation, questionnaire_completed_at | 회원 자기소개 + 모임 보드 공유 프로필. 주 취향 유형은 `preferred_game_types`, 전체 취향 범위는 `game_type_range`이며 전자는 후자의 부분집합이다. 난이도도 같은 3단 구조이고 범위와 꺼림은 겹치지 않는다. 가입 경로와 거주 지역은 상세 프로필에 공개한다. 031 전에는 새 필드를 읽는 코드를 배포하지 않는다. |
