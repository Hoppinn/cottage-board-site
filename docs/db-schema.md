...
| `submit_member_intro(...)` | 필수 자기소개 전체 제출. `p_user_id TEXT`는 카카오 사용자 ID, 반환 `intro_id UUID`는 `member_intros.id`다(025). 033의 `p_usual_play_days/times`는 평소 습관, 기존 `p_available_days/times`는 모임 참여 가능 일정이다. 030의 `holiday`, 031·032의 유형·난이도 3단 배열과 `location`을 검증한다. 유형·난이도 범위는 주 취향을 포함해야 하고, 난이도 범위와 꺼림은 겹칠 수 없다. **`member_intros` upsert + `profile_hardest_games` 교체 + `voucher_log intro_complete` insert**를 단일 트랜잭션으로 처리하며, `profiles.avoid_tags`·`profiles.preferred_game_depths`를 새 설문 값으로 갱신하지 않는다. partial unique index가 최초 저장 쿠폰을 사용자당 1회로 보장한다. |
