...
| 항목 | 우선순위 | 비고 |
|------|----------|------|
| 카카오 로그인 커스텀 닉네임 시스템 | 높음 | 로그인 후 사이트 닉네임 설정, 드롭다운(닉네임 변경/로그아웃) |
| 관리자 OWNER_KAKAO_ID 설정 | 높음 | 로그인 성공 후 콘솔에서 getKakaoUser().id 확인 → 코드에 입력 |
| 동호회(club.html) 중복 항목 제거 | 보통 | — |
| 요청하기(requests.html) 개선 | 보통 | — |
...
| 항목 | 상태 |
|------|------|
| 카카오 JS SDK 도메인 등록 | 🔧 진행중 (새 플랫폼 키 생성 중, 저장 후 키 값 확인 필요) |
