...
| 항목 | 우선순위 | 비고 |
|------|----------|------|
| **플레이기록·모임기록 디자인 연동** | 🔴 높음 | game-reviews ↔ club-history 색감·구조 통일 |
| **club-schedule.html 일정 투표** | 🟡 높음 | 플레이스홀더만 있음, 기능 설계·구현 필요 |
| 사진 업로드 | 🟡 중간 | play-photos 버킷 연동 (버킷은 이미 생성됨) |
...
- ✅ game-reviews.html 3단 계층 디자인 (차콜→테라코타→흰카드)
- ✅ club-history.html 디자인 개편 (동일 색감 적용)
- ✅ 관리자 페이지 섹션 접기/펴기 + 닉네임(실명) 표시 개선
- ✅ 게임 한국어 설명 133개 + summaryKo 185개 생성 완료
- ✅ 원드러스크리처 BGG 400366 연결 + 이미지
- ✅ 맥주와빵(천국과맥주) 이미지·한국어 데이터
- ✅ 플레이기록 참여자 이름 태그 입력 — 엔터로 추가, 다음 칸 이동 방지
