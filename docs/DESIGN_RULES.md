...
- `날짜·전체 요약 → 모임 전체 룰렛 → 참여자별 상세 → 내 액션` 순서를 유지한다. 이 구조의 공용 렌더러는 `day-detail.js`의 `openDateMeetingModal`이다.
- 참여자는 이름·시간·동반·게임이 한 사람에게 귀속되도록 얇은 구분선과 짧은 내부 여백으로 나눈다. 모달 안에 두꺼운 독립 카드를 반복해 쌓지 않는다.
