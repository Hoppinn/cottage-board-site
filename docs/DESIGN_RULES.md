...
- **배경 없는 순수 텍스트**(대표 캐릭터명·칭호 등)는 **박스가 아니라 글자 자체에 `text-shadow`로 펄스**한다(`textGlowPulse` 키프레임, `.profile-panel-rep-name`/`.profile-panel-title-name` 참고). **2026-08-02 정정**: 이전엔 "text-shadow만으론 안 보인다"며 투명 테두리+box-shadow로 박스 모양을 흉내냈으나, 사용자가 실제 화면을 보고 "박스 테두리 말고 글씨 자체가 반짝이는 게 낫다"고 판단 — 이중 레이어 `text-shadow`(`0 0 3px`+`0 0 6px`)로 교체. **배경 필(pill)로 채우는 방식은 그 이전에 시도했다가 "부담스럽다"는 피드백으로 폐기**됐던 것과는 다른 방향(필 대신 아예 박스 자체를 없앰). ⚠️ **버튼 요소(`<button>`)에 이 패턴을 쓸 땐 `border:none`을 명시할 것** — 안 그러면 브라우저 기본 버튼 테두리가 드러난다(`.profile-panel-title-name`에서 실제 발생, 검정 사각 테두리로 신고됨).
- **완전 소멸형 변주**(`.meeting-status-msg.is-glow`, `statusBlinkGlow` 키프레임): 이번 주 모임 상태 메시지는 위 규칙과 달리 **글자 자체가 완전히 나타났다 사라진다**(opacity 1↔0) — 사용자가 여러 안(그림자만/그림자 확대/투명도만/투명도+그림자)을 시안으로 비교해보고 직접 선택한 예외다(2026-08-02). **핵심 조건: opacity와 text-shadow의 피크·트로프가 정확히 같은 시점이어야 한다** — 어긋나면 "글자가 안 보일 때 그림자만 진해지는" 무의미한 상태가 된다(실제로 처음 구현이 이 실수였다). 트로프에서 `text-shadow:none` 대신 `0 0 0 rgba(...,0)`(레이어 수를 맞춘 명시적 투명값)을 쓸 것 — `none`은 셰이프가 안 맞아 부드럽게 보간되지 않는다. **이 변주는 "항상 읽혀야 하는 라벨"(캐릭터명·칭호)에는 쓰지 않는다** — 완전히 사라지는 구간이 있어도 괜찮다고 판단된 곳에서만.
