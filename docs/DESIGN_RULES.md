...
- 로그인한 본인 카드에만 짧은 `조율 수정` 액션을 두고 기존 대표 게임·희망 인원 편집을 그 안에 흡수한다. 다른 참여자 카드에는 수정 UI를 표시하지 않는다.
- 룰렛은 별도 CTA 블록으로 만들지 않고 게임 조율 영역의 마지막 액션으로 둔다. 하단 액션은 구분선과 공통 베이지 톤으로만 footer처럼 보이게 하고 `fixed`/`sticky`로 만들지 않는다.
- 날짜 제목과 닫기는 본문 스크롤 밖의 컴팩트한 모달 헤더에 두고, 본문 날짜를 반복하지 않는다. 헤더 아래에는 옅은 divider만 둔다.
- 360px·6명에서는 모달 외곽을 화면 안에 유지하고 본문만 스크롤한다. 가로 overflow와 하단 CTA의 본문 가림이 없어야 한다.
