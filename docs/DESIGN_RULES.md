...
## 12. 날짜 전체 모임 센터 모달 정보 위계 (2026-08-31 확정)

- 일정 미리보기의 펼쳐진 날짜는 `날짜/참여 인원 → 참여자별 카드 → 인원 조율 요약 → 이날 모임 상세 ›` 순서다. 인원 조율은 참여 인원·최대 시간 겹침·선호 유형·겹치는 게임의 핵심 요약만 보여 준다.
- 상세의 큰 섹션은 `인원 조율 → 참여자별` 두 개만 둔다. 날짜 헤더에는 날짜만 두고, `인원 조율`에는 참여 인원·최대 시간 겹침·선호 유형 분포·공통 게임·룰렛 최종 액션을 모은다. 공용 렌더러는 `day-detail.js`의 `openDateMeetingModal`이다.
...
- 룰렛은 별도 CTA 블록으로 만들지 않고 인원 조율 영역의 마지막 액션으로 둔다. 미참여자의 플래너 등록 진입도 같은 영역 안에서 룰렛보다 앞에 둔다. 하단 action footer는 만들지 않는다.
...
- 인원 조율의 참여/시간 겹침 요약 뒤와 룰렛 최종 액션 앞에는 정보 블록 간 여백을 둔다. 참여자 카드의 `오늘`에서도 플레이 성향·메시지와 게임 목록을 같은 간격으로 분리한다.
