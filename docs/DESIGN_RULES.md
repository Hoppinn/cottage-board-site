...
- 모임원 프로필 게시판 카드는 모바일에서 2열 compact 목록으로 둔다. 넓은 화면에서만 최소 220px을 보장하는 responsive grid로 열 수를 늘린다. 카드 전체는 읽기전용 내 보드 메인으로 가며, 카드 안 본인용 수정·삭제·색상 버튼만 별도 동작으로 유지한다.
...
- 목록의 축약/확장은 수집 보드의 섹션 헤더 패턴을 따른다. 제목과 건수는 상태 표시만 하고, 상태 변경은 헤더 안의 명시적 `전체보기`/`접기` 컨트롤만 담당한다. 헤더 전체를 버튼처럼 만들거나 chevron·pointer cursor로 accordion처럼 보이게 하지 않는다.
