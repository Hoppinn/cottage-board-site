...
- 룰렛은 별도 CTA 블록으로 만들지 않고 게임 조율 영역의 마지막 액션으로 둔다. 하단 액션은 구분선과 공통 베이지 톤으로만 footer처럼 보이게 하고 `fixed`/`sticky`로 만들지 않는다.
- 날짜 제목과 닫기는 본문 스크롤 밖의 컴팩트한 모달 헤더에 두고, 본문 날짜를 반복하지 않는다. 헤더 아래에는 옅은 divider만 둔다. 폭·헤더·최대 높이는 공용 플래너 센터모달 규격을 재사용하되, 회의 상세의 높이는 콘텐츠에 맞게 자동으로 결정하고 최대 높이를 넘을 때만 본문을 스크롤한다.
