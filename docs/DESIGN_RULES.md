...
- 내 보드의 프로필 보드 미리보기 정보 행은 라벨과 값을 공통 flex 행으로 분리한다. 라벨은 한 줄을 지키고, 값은 남은 폭을 우선 사용한 뒤에만 자연 줄바꿈한다. 특정 문구 길이·고정 폭·행 전체 `nowrap`으로 줄바꿈을 제어하지 않는다.
...
## 14. 모임보드 다가오는 모임·참여 페이스 (2026-09-01 확정)
...
- 모임 참여 페이스는 실제 일정이 아니라 평소 참여 가능/원하는 상태다. `참여 가능 빈도`, `원하는 참여 빈도`, `참여 가능한 요일`, `참여 가능한 시간대`를 표시하며, 참여 가능한 요일·시간대는 프로필 보드와 같은 원본 데이터를 중복 표시할 수 있다. 새 필드나 기존 필드 의미 변경은 하지 않는다.
