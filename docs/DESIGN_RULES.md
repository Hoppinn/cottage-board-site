...
Modal Stack Host child의 outer geometry/chrome CSS는 page-name·HTML tag selector가 아니라 `data-ui-geometry-owner`, `data-ui-geometry-variant`, `data-ui-geometry`, `data-ui-layer`, `data-ui-chrome`, `data-ui-functional-surface`, `data-ui-surface-geometry-owner` role marker를 사용한다. 현재 Host child route는 `recommend-all` 하나이며, Host가 그 outer geometry를 소유하고 `standard`/`compact` variant로 shell 형태를 고르며 child renderer는 iframe `0,0`의 기능 layout만 소유한다. functional surface는 별도 축이다: `game-info` 계열은 어느 parent에서도 기존 `game-sheet` bottom-sheet renderer를 유지하며 Host child frame으로 대체하지 않는다. game-sheet가 iframe 안에서 열릴 때는 its immediate surface geometry owner가 full available area를 제공해 renderer가 작은 parent modal 좌표계로 축소되지 않게 한다. 현재 `recommend-all` Host frame의 × close와 local surface의 ← one-step navigation은 서로 다른 owner 책임이다. 기능 header·tab·search/filter·sticky control과 profile local ←는 global/duplicate chrome 숨김에 포함하지 않는다. scroll owner marker는 책임을 나타낼 뿐 개별 overflow/overscroll을 Host scroll로 바꾸지 않는다.
...
- 게임명처럼 시트를 여는 텍스트 동작은 키보드로도 실행할 수 있는 버튼/링크를 사용하고, 모바일 탭 영역은 최소 36px 높이를 확보한다.
...
- 플레이기록의 사람 token(`.pr-rec-recorder`)은 게시판·홈 요약·게임정보의 목록/게임별 기록에서 공통으로 쓴다. 참가자는 12px 웜그레이, 작성자는 기존 `--green`과 `font-weight:600`만으로 강조하며 크기·line-height·padding·아이콘 정렬은 같다. participant identity icon은 모두 micro 12px이고, 인원 표지 `👥 N명`·시간은 같은 보조정보 톤의 nowrap token이다. 시간/점수 메타는 게임정보·플레이기록 게시판·홈 최근 플레이에서 `play-record-meta-time`(기존 `#8a7868`, 600)과 muted 점수/구분점의 공통 위계를 쓴다. 게임평·사진의 단독 작성자 meta는 이 계약과 별개로 standard 20px을 유지한다. 사람 행은 token 단위로 자연스럽게 줄바꿈한다.
- 내보드 메인은 별도 제목 헤더 없이 큰 사용자 identity로 시작하고, 큰 identity가 스크롤 밖으로 사라진 뒤에만 작은 캐릭터·닉네임·`내 보드`·닫기를 compact sticky header로 표시한다. 하위 보드(프로필·모임·기록·함께한 시간·수집)는 처음부터 기존 `< 내 보드` back UI 문법을 확장한 단일 행의 `< 캐릭터 닉네임 | 보드명 | 닫기` header를 사용한다. identity는 대표 캐릭터와 닉네임을 표시하고 보드 메인으로 돌아가며, 메인 보드 IA에 수집 보드를 추가하는 의미는 아니다.
- Modal Stack의 게임정보 하위 drill-down(게임위치·게임안내)에서는 parent shell의 공통 좌상단 `←`만 navigation으로 둔다. `←`와 ESC는 현재 top frame 한 장만 복귀한다. 별도 기능 center modal overlay는 같은 shell의 우상단 `×`를 쓰며, `×`는 현재 top modal 한 장만 닫아 바로 아래 modal의 DOM·scroll·선택 상태를 보존한다. child 안의 제목 전용 header/title row는 숨기되, 사용자 identity·보드 상태처럼 화면 자체의 정보를 담은 기존 header는 유지한다. 독립 페이지의 기존 header 동작은 바꾸지 않는다.
...
- 모임원 프로필 게시판 카드는 모바일에서 2열 compact 목록으로 둔다. 넓은 화면에서만 최소 220px을 보장하는 responsive grid로 열 수를 늘린다. 닉네임·헤더는 읽기전용 내 보드 메인, 카드 본문과 `프로필 자세히 보기 ›`는 프로필 보드로 가며, 카드 안 본인용 수정·삭제·색상 버튼은 별도 동작으로 유지한다.
- 설문의 2열 선택 카드는 체크박스와 핵심 제목을 첫 행에 두고, 수치·예시·설명 같은 보조정보는 제목 아래 행으로 분리한다. 카드별 예외 대신 `.intro-choice-grid`의 공통 grid·font hierarchy를 사용하며, 핵심 제목은 `min-width:0`과 nowrap/ellipsis로 세로 한 글자씩 깨지는 것을 막고 보조정보만 자연 줄바꿈한다.
- 내 보드의 프로필 보드 미리보기 정보 행은 라벨과 값을 일반 inline 흐름으로 둔다. 라벨은 한 줄을 지키되, 값은 첫 줄의 남은 폭부터 쓰고 다음 줄에서는 카드의 전체 폭을 사용해 자연 줄바꿈한다. 특정 문구 길이·고정 폭·행 전체 `nowrap`으로 줄바꿈을 제어하지 않는다.
- 목록의 축약/확장은 수집 보드의 섹션 헤더 패턴을 따른다. 제목과 건수는 상태 표시만 하고, 상태 변경은 헤더 안의 명시적 `전체보기 ▼`/`접기 ▲` 컨트롤만 담당한다. 컨트롤은 배경·테두리·밑줄 없는 헤더 보조 액션이며 충분한 padding만 둔다. 헤더 전체를 버튼처럼 만들거나 chevron·pointer cursor로 accordion처럼 보이게 하지 않는다. sticky 상태에서 접을 때는 해당 헤더의 원래 진입점으로 scroll container를 복원한다.
...
7. **공통 스타일의 기준은 선언값이 아니라 기준 화면의 실제 렌더 결과로 고정한다.** 같은 구조를 공유할 때는 공통 베이스에 구조·크기·box model을 두고, 화면별 클래스에는 배치만 둔다. 기준 요소가 사용자 확인 전이라면 “기준과 동일하게 맞췄다”고 판정하지 않으며, 기존 요소의 인라인 스타일·미디어쿼리·후속 cascade가 공통값을 덮지 않는지 함께 확인한다.

8. **반복 보고가 있는 시각 요소는 런타임 측정 전 추가 수치 패치를 금지한다.** 브라우저 연결이 없거나 대상 URL·뷰포트·계산된 `getBoundingClientRect()`를 확보하지 못한 상태에서는 헤더·sticky·position 차이를 해결로 판정하지 않는다. 이 경우에는 정적 DOM/CSS 원인만 기록하고, 사용자에게 필요한 측정 조건을 요청한 뒤 재개한다.

9. **2026-08-31 작업 사례 — 음료교환권 위치 이동과 참여자 액션 재배치.** 음료교환권 축소가 늦어진 원인은 카드 전용 규칙보다 뒤의 모바일 `.profile-card-grid .profile-card { min-height: 92px; }`가 같은 우선순위로 덮은 것이었고, 헤더가 반복해서 어긋난 원인은 내 보드·하위보드·플래너가 같은 구조임에도 기준 요소와 변형 요소를 분리하지 않은 채 수치를 번갈아 수정한 것이었다. 이번에는 기존 `data-subsheet="voucher"` 진입 경로를 상단 보조 버튼으로 옮기고, 참여자 액션은 기존 `sched-bar-item` 렌더러 안에서 순서를 바꿨다. 사례는 원인과 수정 경로만 기록하며, 실화면 확인 전에는 성공 판정으로 사용하지 않는다.

9-a. **2026-08-31 반복 실패 기록 — 같은 CSS를 고쳤는데 화면이 그대로였던 건.**
   - **증상:** 내 보드·하위 보드·플래너의 고정 헤더가 미세하게 달랐고, 모달 시작 공백·참여자 카드 간격·음료교환권 위치가 여러 번 변하지 않았다. 플래너에서는 참여 액션이 사라지거나 등록하지 않은 것처럼 `+ 이날 함께하기`가 순간 표시되기도 했다.
   - **원인:** (a) 공통 클래스는 바깥 상자만 공유하고 내부 DOM 슬롯·직접 자식·`display:contents`·페이지별 보정 규칙은 달랐다. (b) 음료교환권의 크기 문제는 후순위 모바일 `min-height` cascade가 덮었고, 위치 문제는 상위 액션 묶음이 아니라 버튼 자체의 padding/offset을 움직여야 했다. (c) 플래너 표시 상태는 `myVote`와 인증 갱신 시점에 의존해 재조회 전 임시 렌더가 생길 수 있었다.
   - **실패한 시도:** A) 기준 화면을 런타임 측정하지 않고 헤더 `height/padding/top`을 번갈아 수정했다. B) 버튼 자체가 아닌 부모의 `right`와 카드 여백을 조정했다. C) 아이콘·액션 마크업을 바꾼 뒤 정적 검사 통과를 화면 해결로 간주했다. D) 이전 사례를 문서에 적어두었지만, 이번 수정의 사전 진입 조건으로 강제하지 않았다.
   - **현재 결론:** 공통화는 선언값 복사가 아니라 같은 렌더 슬롯/renderer를 공유하고, 예외 cascade를 제거하는 것으로 해야 한다. 교환권 위치와 상세 모달 `✕`는 코드에 반영했지만, 이 기록 시점의 실화면 성공 여부는 사용자 검수 대기다. `+ 이날 함께하기`의 일시 표시는 인증·데이터 재조회 타이밍 후보만 확인됐고, 재현 없이 해결로 기록하지 않는다.
   - **재발 방지:** 반복 시도 후에는 수정 전에 대상 URL·뷰포트·기준 요소·실제 DOM 구조·활성 media query·`getBoundingClientRect()` 및 계산된 box model을 한 화면에서 대조한다. 브라우저 측정이 불가능하면 추가 수치 패치를 중단하고 정적 원인과 미검증 상태만 기록한다.

9-c. **실패 사례: 보드의 모임원 프로필 위저드 진입 (2026-09-01).**
   - **증상:** 프로필 보드의 `프로필 수정`이 위저드를 바로 열어야 했지만, 일반 프로필 페이지로 이동하거나 그 페이지 전체를 담은 센터모달을 열었다. 사용자가 같은 실패를 여러 번 확인했다.
   - **원인:** URL의 `?edit=1` 자동열기, 페이지 전체 iframe, 임베드 위저드 경로를 차례로 겹쳐 두었지만, 실제 정본 동작인 `#introWizardOpenBtn` 클릭을 직접 재사용하지 않았다. 페이지 데이터·카카오 세션이 준비되기 전에 URL 조건으로 열기를 시도한 것도 경로 충돌을 키웠다.
   - **해결:** 임베드 페이지가 `loadIntros()` 뒤 준비 신호를 보내면 부모 보드가 한 번만 `cottage-open-profile-intro-wizard` 메시지를 보내고, 페이지는 기존 `#introWizardOpenBtn`을 실제 클릭한다. 완료·닫기는 기존 `closeWizard()`에서 부모에게 닫기 신호를 돌려준다.
   - **실패한 시도:** (1) `?edit=1` 일반 페이지 이동, (2) 프로필 페이지 전체를 기록 센터모달 규격으로 감싸기, (3) URL 자동열기와 임베드 위저드 경로를 동시에 유지하기. 셋 모두 사용자가 보던 기존 버튼의 준비 시점·클릭 경로를 보존하지 못했다.
   - **재발 방지:** 다른 페이지의 기존 모달을 보드에서 재사용할 때는 URL 파라미터만 추측하지 말고, 실제 사용자 버튼의 이벤트 진입점과 데이터 준비 완료 시점을 확인한다. URL 자동열기·iframe 메시지·직접 클릭 중 하나만 단일 진입 경로로 남기고, 모달 크기는 기존 센터모달의 실제 폭·높이 규격과 먼저 대조한다.

9-b. **원칙 자체 점검 절차.** 사용자가 “원칙을 점검하라”고 하면 사례를 더하는 것에서 끝내지 않고, ① 규칙이 문서에 존재했는지, ② 작업 시작 시 해당 규칙을 읽었는지, ③ 수정 전에 실행을 막는 적용 조건이 있었는지, ④ 완료 판정에 필요한 증거를 요구했는지의 네 층을 각각 확인한다. 규칙이 있었지만 실패했다면 “미기록”과 “미적용”을 구분해 원인을 적고, 다음부터는 완료 보고·사례 기록 전에 검증 증거와 미검증 항목을 대조하는 게이트를 둔다.
...
## 12. 모임 조율 정보 위계 (2026-08-31 확정)

- 플래너의 펼쳐진 날짜는 `날짜/참여 인원 → 참여자별 카드 → 모임 현황 → 모임 조율 ›` 순서다. `모임 현황`은 참여 인원·최대 시간 겹침·선호 유형·겹치는 게임을 충분히 보여 주는 날짜 카드 내부 요약 블록이며, 모임 조율 화면 진입을 유도하려고 줄이지 않는다.
- 날짜 전체 화면의 이름은 `모임 조율`이다. 날짜 헤더는 `날짜 모임 조율`로 표시하고, 큰 섹션은 `모임 현황 → 인원 조율 → 참여자별 상세` 순서다. 공용 렌더러는 `day-detail.js`의 `openDateMeetingModal`이다.
- `모임 현황`은 상태 요약이고, `인원 조율`은 사용자가 함께 할 게임을 결정하는 행동 영역이다. 현재 인원 조율에는 기존 게임 룰렛을 두며, 게임 투표는 DB/API 설계와 별도 승인 전까지 UI·임시 저장 구조를 만들지 않는다.
- 참여자 카드는 이름·시간·동반과 함께, 이날 같이 게임할 판단에 필요한 평소 유형·깊이·경험 및 오늘 성향·게임·희망 인원 조건을 바로 표시한다. 카드 전체를 별도 날짜 상세로 연결하거나 chevron을 붙이지 않는다.
- 참여자 한 명은 연베이지 카드 하나로만 표현한다. 카드 안의 `평소`·`오늘`은 별도 테두리 카드가 아니라 짧은 간격과 아주 옅은 divider로 구분하며, 인원이 늘어도 내부 여백을 과도하게 키우지 않는다.
- 로그인한 본인 카드에서는 `오늘`의 기존 게임 행 오른쪽에 희망 인원 selector만 인라인으로 둔다. 게임 추가·삭제·대표 지정은 기존 플래너 편집에 남기고, 별도 `조율 수정` 영역이나 중복 게임 목록은 만들지 않는다. 다른 참여자 카드는 같은 인원 조건을 읽기 전용으로 표시한다.
- 룰렛은 인원 조율 영역의 현재 핵심 액션으로 둔다. 미참여자의 플래너 등록 진입도 같은 영역 안에서 룰렛보다 앞에 둔다. 하단 action footer는 만들지 않는다.
- 날짜 제목과 닫기는 본문 스크롤 밖의 컴팩트한 모달 헤더에 두고, 본문 날짜를 반복하지 않는다. 닫기는 헤더 우측의 충분한 hit area를 가진 × 하나로 통일하며, 헤더 아래에는 옅은 divider만 둔다. 폭·radius·헤더는 공용 플래너 센터모달 규격을 재사용하고, 외곽은 정사각형에 가까운 compact 세로형 고정 크기다. 콘텐츠가 넘칠 때만 본문을 스크롤한다.
- 모임 현황과 인원 조율, 인원 조율과 참여자별 상세 사이에는 정보 블록 간 여백을 둔다. 참여자 카드의 `오늘`에서도 플레이 성향·메시지와 게임 목록을 같은 간격으로 분리한다.
- 360px·6명에서는 긴 게임명이 최대 두 줄로 자연스럽게 줄고 인원 selector는 오른쪽에 남아야 한다. 가로 overflow와 하단 CTA의 본문 가림이 없어야 한다.

## 13. 모임원 프로필 30분 시간 막대 (2026-08-29 확정)

- 입력은 30분 슬롯이지만 48개 체크박스처럼 보이지 않게 인접 슬롯이 붙은 막대로 렌더한다. PC는 6시간 막대 4개를 2열, 360px은 1열로 배치해 슬롯 폭과 터치 높이를 확보한다.
- 시간 눈금은 3시간 간격만 표시하고, 선택 결과는 막대 아래에 `09시30분~12시 · 22시30분~01시` 형식으로 즉시 보여준다. 프로필 출력에서는 막대를 반복하지 않고 같은 텍스트 포맷만 쓴다.
- `시간대 유동적`은 막대 선택을 지우지 않는 독립 보조값이다. 요일의 `일정에 따라 유동적`도 선택 요일과 동시에 쓸 수 있다.

## 14. 모임보드 다가오는 모임·최근 참여 (2026-09-01 확정)

- 다가오는 모임 상단의 새 참여 행동은 `＋ 참여 등록` 하나다. 하고 싶은 게임·배우고 싶은 게임은 이 등록 또는 일정별 수정 안에서 입력하며, 독립 추가 버튼으로 분리하지 않는다.
- 일정 카드 헤더는 왼쪽 `날짜 + ✎ + ×`, 오른쪽 `참여 시간`이다. ✎/×는 작은 보조 아이콘 액션이지만 접근성 라벨을 유지한다. 본문에는 값이 있는 참여 스타일·게임·한마디만 보여 주고 개별 빈 항목을 반복하지 않는다.
- 같은 게임 표시는 regular 카드에서 약 24px, compact 플래너 카드에서 약 16~18px 썸네일을 기준으로 기존 렌더링·토큰을 재사용한다.
- 프로필성 모임 참여 페이스는 내 프로필 보드 정본에서만 보여준다. 모임 보드는 실제 일정·참여 등록·최근 참여에 집중하며, 같은 정보를 별도 섹션으로 반복하지 않는다.
- 모임 보드 미리보기 카드의 태그 뒤 첫 `하고 싶은 게임` 행은 다른 게임 행보다 소폭 큰 위 여백으로 태그 묶음과 구분한다. 이 여백은 해당 `want` 행에만 적용한다.
