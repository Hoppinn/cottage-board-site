...
Modal Stack Host child의 outer geometry/chrome CSS는 page-name·HTML tag selector가 아니라 `data-ui-geometry-owner`, `data-ui-geometry-variant`, `data-ui-geometry`, `data-ui-layer`, `data-ui-chrome`, `data-ui-functional-surface`, `data-ui-surface-geometry-owner` role marker를 사용한다. Host가 outer geometry를 소유하고 `standard`/`compact` variant로 shell 형태를 고르며 child renderer는 iframe `0,0`의 기능 layout만 소유한다. functional surface는 별도 축이다: `game-info` 계열은 어느 parent에서도 기존 `game-sheet` bottom-sheet renderer를 유지하며 Host child frame으로 대체하지 않는다. game-sheet가 iframe 안에서 열릴 때는 its immediate surface geometry owner가 full available area를 제공해 renderer가 작은 parent modal 좌표계로 축소되지 않게 한다. Host ×의 flow-close 책임과 ←의 one-step navigation 책임은 독립이다. 기능 header·tab·search/filter·sticky control과 profile local ←는 global/duplicate chrome 숨김에 포함하지 않는다. scroll owner marker는 책임을 나타낼 뿐 개별 overflow/overscroll을 Host scroll로 바꾸지 않는다.
