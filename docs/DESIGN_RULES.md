...
- 큰 섹션은 `게임 조율 → 참여자별` 두 개만 둔다. 날짜 헤더에는 날짜만 두고, `게임 조율`에는 참여 인원·최대 시간 겹침·선호 유형 분포·공통 게임·룰렛 최종 액션을 모은다. 공용 렌더러는 `day-detail.js`의 `openDateMeetingModal`이다.
...
- 룰렛은 별도 CTA 블록으로 만들지 않고 게임 조율 영역의 마지막 액션으로 둔다. 미참여자의 플래너 등록 진입도 같은 영역 안에서 룰렛보다 앞에 둔다. 하단 action footer는 만들지 않는다.
- 날짜 제목과 닫기는 본문 스크롤 밖의 컴팩트한 모달 헤더에 두고, 본문 날짜를 반복하지 않는다. 닫기는 헤더 우측의 충분한 hit area를 가진 × 하나로 통일하며, 헤더 아래에는 옅은 divider만 둔다. 폭·헤더·최대 높이는 공용 플래너 센터모달 규격을 재사용하되, 회의 상세의 높이는 콘텐츠에 맞게 자동으로 결정하고 최대 높이를 넘을 때만 본문을 스크롤한다.
- 게임 조율의 참여/시간 겹침 요약 뒤와 룰렛 최종 액션 앞에는 정보 블록 간 여백을 둔다. 참여자 카드의 `오늘`에서도 플레이 성향·메시지와 게임 목록을 같은 간격으로 분리한다.
