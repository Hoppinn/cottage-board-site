...
## 12. 날짜 전체 모임 센터 모달 정보 위계 (2026-08-29 확정)

- `날짜 → 게임 조율(참여 인원·최대 겹침·선호 유형·겹치는 게임) → 모임 전체 룰렛 → 참여자별 상세 → 내 액션` 순서를 유지한다. 날짜 헤더에는 날짜만 두며, 이 구조의 공용 렌더러는 `day-detail.js`의 `openDateMeetingModal`이다.
- 참여자는 이름·시간·동반·게임이 한 사람에게 귀속되도록 얇은 구분선과 짧은 내부 여백으로 나눈다. 행 전체는 기존 날짜별 개인 상세 진입점이며 오른쪽 chevron으로 이를 알린다. 모달 안에 두꺼운 독립 카드를 반복해 쌓지 않는다.
- 룰렛은 참여자 목록과 다른 `모임 전체` 기능임을 연한 CTA 영역으로 표시한다. 하단 액션은 구분선과 공통 베이지 톤으로만 footer처럼 보이게 하고 `fixed`/`sticky`로 만들지 않는다.
