...
- 큰 섹션은 `게임 조율 → 참여자별` 두 개만 둔다. 날짜 헤더에는 날짜만 두고, `게임 조율`에는 참여 인원·최대 겹침·선호 유형 분포·공통 게임·룰렛 최종 액션을 모은다. 공용 렌더러는 `day-detail.js`의 `openDateMeetingModal`이다.
...
- 날짜 제목과 닫기는 본문 스크롤 밖의 컴팩트한 모달 헤더에 두고, 본문 날짜를 반복하지 않는다. 헤더 아래에는 옅은 divider만 둔다. 외곽 크기는 공용 플래너 센터모달 규격을 재사용하고 본문만 스크롤한다.
