...
최종 구조 정리: 2026-09-10. 이 문서는 페이지·파일·흐름의 빠른 index다. 완료 작업의 경위는 git/DEBUGGING_HISTORY에, 현재 열린 작업은 PROJECT_STATE에, UI presentation/navigation 계약은 UI_PATTERNS에 둔다.
...
[내 활동 패널 (openProfilePanel)]
  ...
  - historical player_names 참여 기록은 js-api의 stable identity / participant resolver 계약으로만 보조 해소
...
[프로필·모임 보드 ↔ 회원 자기소개 연동]
  ...
  - 회원 자기소개 카드의 공개 표시명은 `member_intros.nickname`이고, card는 stable `user_id`와 함께 이를 전달한다. 닉네임·헤더 클릭 → `openOtherProfileSheet(userId)` → 읽기전용 내 보드 메인, 카드 본문과
...
일반적인 게임 추가·게임 위치 변경은 개별 소스 파일을 수동으로 추적하는 작업이 아니라 이 파이프라인을 가동하는 작업으로 취급한다. 먼저 `npm run check`로 필요한 단계만 확인하고, 정본 추가 명령과 필요한 BGG·번역 단계를 실행한 뒤 다음만 대조한다: 게임명·BGG ID, 사용자가 지정한 원본 위치와 출력 선반 그룹, 약칭·중복 경고 없음, 최종 `npm run check` 통과. 이 조건이 충족되면 코드·데이터 내부를 추가로 반복 추적하지 않는다.

실제 화면 검증이 필요한 경우에는 에이전트가 같은 화면을 중복 탐색하지 않고, 사용자에게 게임명·확인할 화면/위치·완료 판정을 한 번에 제시한다. 예: “게임 목록에서 `<게임명>`이 `<선반명>`에 표시되고 약칭이 의도대로 보이면 완료.” 파이프라인 검사 통과와 사용자 화면 확인은 서로 대체하지 않으며, 보고에서 구분한다.
