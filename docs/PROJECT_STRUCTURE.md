...
홈페이지 기능·플래너 보기·모임원 프로필의 iframe root는 `assets/js/modal-stack-host.js`의 공통 Modal Stack Host를 사용한다. `modalStack=1` frame에서 `openRecommendOverlay`만 `{ kind, payload, presentation }` child frame을 요청한다. parent는 source가 현재 top iframe인지 확인하고 같은 viewport 외곽의 child iframe shell을 필요한 depth만큼 쌓으며, 아래 frame은 inert 상태로 DOM·scroll을 보존한다. Host는 `stackGeometry` variant로 outer shell 형태를 선택하되 child가 inset을 다시 소유하지 않는다. 게임정보·기록·위치·안내(`game-sheet`), 내 보드/읽기 전용 프로필(`profile-panel`), 모임 조율(`meeting-adjust`)은 parent와 무관하게 현재 document의 registered canonical local surface flow를 사용한다. `prepare → owner geometry → ready → first visible paint`가 geometry first paint를 막고, active local surface의 close는 Host pop보다 우선한다. `meeting-adjust`의 compact variant와 profile 내부 local subsheet navigation은 각 feature renderer가 계속 소유한다. `data-ui-*` marker가 functional surface·geometry owner/variant·chrome·navigation·scroll boundary를 연결한다. `guide-child-mode`는 legacy compatibility class로만 남으며 CSS ownership은 data marker를 읽는다. 직접 URL과 일반 embed에는 이 위임을 적용하지 않는다.
