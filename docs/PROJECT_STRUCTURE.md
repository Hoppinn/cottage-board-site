...
홈페이지 기능·플래너 보기·모임원 프로필의 iframe root는 `assets/js/modal-stack-host.js`의 공통 Modal Stack Host를 사용한다. `modalStack=1` frame에서 `openRecommendOverlay`만 `{ kind, payload, presentation }` child frame을 요청한다. parent는 source가 현재 top iframe인지 확인하고 같은 viewport 외곽의 child iframe shell을 필요한 depth만큼 쌓으며, 아래 frame은 inert 상태로 DOM·scroll을 보존한다. Host는 `stackGeometry` variant로 outer shell 형태를 선택하되 child가 inset을 다시 소유하지 않는다. `game-sheet`, `profile-panel`, `game-location`은 feature-owned canonical renderer를 유지하되, Host iframe에서 independent overlay가 필요하면 `CottageFunctionalSurface.requestAncestor()`가 이미 로드된 ancestor document의 같은 renderer에 presentation만 위임한다. 이때 parent geometry/variant는 preserve이고 iframe의 fixed containing box를 넓히지 않는다. `meeting-adjust`의 compact variant와 profile 내부 local subsheet navigation은 각 feature renderer가 계속 소유한다. `data-ui-*` marker와 registry가 functional surface·geometry owner/variant·presentation owner·chrome·navigation·scroll boundary를 연결한다. `guide-child-mode`는 legacy compatibility class로만 남으며 CSS ownership은 data marker를 읽는다. 직접 URL과 일반 embed에는 이 위임을 적용하지 않는다.
