...
홈페이지 기능·플래너 보기·모임원 프로필의 iframe root는 `assets/js/modal-stack-host.js`의 공통 Modal Stack Host를 사용한다. `modalStack=1` frame의 기존 `openGameSheet`/`openGameRecordSheet`/`openRecommendOverlay`/`openShelfSheet`/룰 허브/`openDateMeetingModal`/`openProfilePanel`은 local center modal 대신 `{ kind, payload, presentation }` push를 요청한다. parent는 source가 현재 top iframe인지 확인하고 같은 viewport 외곽의 child iframe shell을 필요한 depth만큼 쌓으며, 아래 frame은 inert 상태로 DOM·scroll을 보존한다. Host는 `stackGeometry` variant로 outer shell 형태를 선택하되 child가 inset을 다시 소유하지 않는다. 게임 flow는 flow-close X와 one-step ←를 분리해 drilldown에서도 둘 다 유지한다; 일반 overlay ×와 ←/ESC는 top frame 하나를 처리한다. backdrop은 root overlay 하나만 소유한다. child iframe bootstrap은 기존 deep-link/component를 `runLocal()`로 한 번만 연다. child URL의 `stackPresentation`/`stackGeometry`와 Host shell/child body의 `data-ui-*` marker가 geometry owner·variant·chrome role·navigation semantics·scroll boundary를 연결한다. `guide-child-mode`는 legacy compatibility class로만 남으며 CSS ownership은 data marker를 읽는다. `game-location`의 page `main`은 태그 예외가 아니라 functional content source로 marker를 받는다. 직접 URL과 일반 embed에는 이 위임을 적용하지 않는다.
