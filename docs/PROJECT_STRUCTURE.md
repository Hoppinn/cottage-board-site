...
│   ├── verify-meeting-detail-game-editor.js # 모임 조율의 본인 게임 편집 이전 계약
...
│                               # openDateMeetingModal은 날짜 전체 「모임 현황」·「인원 조율」·「참여자별 상세」를
│                               #   세 섹션으로 보여 준다. 인원 조율은 기존 룰렛을 제공하고, 본인 카드의 오늘 게임에서 희망 인원만 즉시
...
홈페이지 기능·플래너 보기·모임원 프로필의 iframe root는 `assets/js/modal-stack-host.js`의 공통 Modal Stack Host를 사용한다. `modalStack=1` frame의 `openRecommendOverlay`/`openDateMeetingModal`/`openProfilePanel`은 `{ kind, payload, presentation }` push로 child frame을 요청한다. parent는 source가 현재 top iframe인지 확인하고 같은 viewport 외곽의 child iframe shell을 필요한 depth만큼 쌓으며, 아래 frame은 inert 상태로 DOM·scroll을 보존한다. Host는 `stackGeometry` variant로 outer shell 형태를 선택하되 child가 inset을 다시 소유하지 않는다. 반면 게임정보·기록·위치·안내는 parent와 무관하게 현재 document의 canonical `game-sheet` local flow를 사용한다. Host는 이 기능 surface를 새 iframe frame으로 대체하지 않는다. child URL의 `stackPresentation`/`stackGeometry`와 Host shell/child body의 `data-ui-*` marker가 geometry owner·variant·functional surface·chrome role·navigation semantics·scroll boundary를 연결한다. `guide-child-mode`는 legacy compatibility class로만 남으며 CSS ownership은 data marker를 읽는다. 직접 URL과 일반 embed에는 이 위임을 적용하지 않는다.
