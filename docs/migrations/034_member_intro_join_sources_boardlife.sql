-- 가입 경로에 boardlife를 추가한다.
-- 적용 순서: 033 이후, boardlife를 노출하는 프런트 배포 이전.
-- 기존 가입 경로 값은 정규화하거나 변경하지 않는다.
BEGIN;
...
BEGIN
  ...
  IF COALESCE(cardinality(p_join_sources),0)<1 OR NOT(p_join_sources <@ ARRAY['store_visit','friend_referral','cottage_homepage','open_chat_search','daangn','naver_place','boardlife','social_media']::TEXT[]) THEN RAISE EXCEPTION 'invalid_join_sources'; END IF;
