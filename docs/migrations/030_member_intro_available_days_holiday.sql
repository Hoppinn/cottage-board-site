-- 회원 자기소개 가능 요일에 공휴일 코드를 호환 확장한다.
-- 기존 member_intros.available_days 행은 변환하거나 갱신하지 않는다.
...
BEGIN
  ...
  IF COALESCE(cardinality(p_available_days), 0) < 1
    OR NOT (p_available_days <@ ARRAY['mon','tue','wed','thu','fri','sat','sun','holiday','flexible']::TEXT[]) THEN
...
CREATE OR REPLACE FUNCTION public.member_intro_holiday_supported()
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY INVOKER
...
GRANT EXECUTE ON FUNCTION public.member_intro_holiday_supported() TO anon;
