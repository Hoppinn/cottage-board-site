...
✅ **사용자 결정 (2026-07-21): 셋 다 당분간 이모지 폴백으로 둔다.** 그림은 나중에 새로 그린다.
→ **이건 「할 일」이 아니라 「현재 사양」이다.** 열린 항목으로 다시 등록하지 말 것. 그림이 들어오면 그때 `verify-character-assets.js`가 ✅로 바뀌는 것으로 확인된다.
→ 임계값이 record 400회 / photo 500장 / visit 500회로 **아직 아무도 근처에 없다**는 게 이 결정의 근거다. 달성자가 나오면 그 사람에게는 이모지가 보인다(깨진 이미지가 아니라).

| 항목 | 내용 |
|------|------|
| squirrel_lv5.png | record_400 보상. **이모지 폴백 유지 중**(사용자 결정 2026-07-21). |
| hedgehog_lv5.png | photo_500 보상. **이모지 폴백 유지 중**(사용자 결정 2026-07-21). |
| sparrow_lv5.png | visit_500 보상. **이모지 폴백 유지 중**(사용자 결정 2026-07-21). |
