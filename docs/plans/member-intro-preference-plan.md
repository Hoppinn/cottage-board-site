...
- 목표: 모임원 프로필의 게임 취향을 `주 취향 → 즐기는 범위 → 꺼림` 3단 구조로 바꾸고, 상세 프로필에서 게임 유형·난이도 전체를 명확하게 읽을 수 있게 한다.
...
- `member_intros.preferred_game_types`는 주 취향 게임 유형으로 재사용한다. 새 `game_type_range`는 **주 취향을 포함한 전체 즐기는 범위**를 저장한다.
...
- 게임 유형과 난이도는 한 `게임 취향` 단계 안에서 각각 주 취향·즐기는 범위·꺼림을 연속해서 입력한다.
- 주 취향은 즐기는 범위에서 자동 선택·잠금으로 표시하며 주 취향 변경 때 즉시 범위를 동기화한다.
...
- 요약 카드에는 주 취향 중심의 짧은 요약만 표시하며 즐기는 범위 전문은 넣지 않는다.
