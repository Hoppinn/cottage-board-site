...
3. 기록 보드 아코디언 — 날짜별·모임별의 열림/닫힘 헤더 `height`, `padding`, `margin`, `line-height`, border를 대조하고, 열림 상태의 margin/장식이 헤더 box를 바꾸지 않게 한다. **하단 위치 보정은 2026-09-07 사용자 확인으로 정상 동작하므로 범위에서 제외한다.**
4. 모달·바텀시트 배경 스크롤 — 센터모달·바텀시트·iframe에서 내부 scroll container의 상단/하단 wheel·touchmove가 부모 페이지로 체이닝되는 경로를 확인하고, 양방향 모두 차단한다. iPhone 실기기 확인이 필요하다. **프로필 수정 iframe footer는 같은 날 사용자 확인으로 정상이라 범위에서 제외한다.**
...
- 아코디언은 날짜별·모임별 헤더 높이를 확인한다. 하단 위치 보정은 이미 사용자 확인으로 닫았다.
