...
| `cottage_time_{userId}` | supabase-client.js | 누적 체류 시간(분) 임시 저장 | upsertProfile 시 DB 반영 후 삭제 |
...
auth-callback.html:
  ...
  4. 없으면 Supabase REST API로 DB 프로필 닉네임 조회 (최근 추가)
...
[시간 누적 (로컬)]
  ...
  _flushTime():
    ...
    if (elapsed <= 0) return  ← 1분 미만 완전 폐기
    cottage_time_{userId} += elapsed
    _sessionStart 리셋

[DB 반영]
  upsertProfile() 호출 시 (하루 첫 방문만)
    _popAccumulatedMinutes() → cottage_time_{userId} 읽고 즉시 삭제
    profiles.total_minutes += 누적 분

[결과]
  당일 시간은 다음날 첫 방문 때 DB 반영
  마지막 방문 이후 재방문 없으면 시간 누락
  1분 미만 세션은 모두 0으로 폐기
```
