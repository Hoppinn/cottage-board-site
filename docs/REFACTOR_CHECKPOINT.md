...
| 13 | **R10a** | **KA1 추출 — `openProfilePanel` 서브시트 6블록 모듈화** | **Opus xhigh** | 완료(플랜 승인 2026-07-16) | ✅ **완료 (2026-07-16, 커밋 c9ab2bd·fd2bd0c·e5e4bad·c6a0bff·682227e·b3ed30d, 스모크 통과)**. `openProfilePanel` **1,940→918줄**(1,043줄 추출). 서브시트별 원자 커밋 6개(usage 13·voucher 14·notif 30·record 101·taste 353·meeting 533줄). window 노출·시그니처 변화 0 → js-api.md 무변경. 상세는 아래 "R10a 결과 메모". |
| 13b | R10b | **크로스보드 stale 버그** — 방향 A(진입 시 DB 재조회 = 단일 소스)로 서브시트 데이터 로딩 재설계, 취향보드 스냅샷 임시방편(11e10b8) 대체. | **Opus xhigh** | 완료(플랜 승인 2026-07-17) | ✅ **완료, 브라우저 스모크 통과** (2026-07-17, 커밋 507f2e9·c0cd874·5f82aed·a03250c). 중복 쿼리 2개 제거 → `getMeetingProfile` 단일 소스(`avoidTags` 추가), HTML 빌드를 `_buildTasteInnerHtml`/`_buildMeetingInnerHtml` 함수화해 진입 시 재빌드, taste 스냅샷·`_currentBio` 접근자 제거. **착수 후 stale 당사자가 문서의 2종이 아니라 5종**(bio·avoid_tags·ruleSet 추가)임을 발견. 기록보드 스냅샷·패널 카드 요약은 **범위 밖으로 분리**(사용자 승인) → PROJECT_STATE §3. 스모크 포인트 8개는 PROJECT_STATE §0. |
| 13c | R10c | **네비게이션 스택 (신규기능)** — §3-1 알림→읽기전용 보드 복귀 + §3-2 좋아요 토스트→게임시트 복귀. `openProfilePanel(sub, {backTo:{...}})` 일반화. | **Opus xhigh** | **필요** | 🟡 코드 완료·스모크 대기 (2026-07-17, 6f506d8·9593237·ade71df). **"스택"은 불필요했음** — `_openSubSheet`에 서브시트→패널 뒤로가기가 이미 있어 깊이 1의 `backTo` 객체로 충분(자료구조 0, 복귀는 기존 `autoSubsheet` 재사용). 상세·교훈은 PROJECT_STATE §0 R10c 항목. |
| — | ~~R10(원안)~~ | ~~KA1 분리 + stale + 네비스택 한 항목~~ | — | — | **분할됨 (2026-07-16, 사용자 승인)** — 리팩토링·버그수정·신규기능이 한 단위에 섞여 CLAUDE.md "구현/리팩토링 분리" 위반 → R10a/R10b/R10c로 분리. 원안 설명은 아래 보존. |
| ~~13~~ | ~~R10 원안~~ | ~~**KA1 — `openProfilePanel` 1,972줄 분리** (최대·최고위험 단일함수, 서브시트별로 여러 세션 재분할 가능성 있음). **+ 크로스보드 stale 버그 동반 해결(2026-07-16)**: 취향보드(`likedGames`)와 모임보드(`_meeting.likedGames`)가 같은 `game_likes`를 패널오픈 시 각각 따로 불러와 **별도 배열 2개**로 들고 있어, 한쪽에서 게임 추가/삭제해도 반대 보드엔 새로고침 전까지 미반영(`getMeetingProfile`이 내부에서 `getUserLikedGamesAll` 재호출). 방향 A(진입 시 DB 재조회 = 단일 소스)로 서브시트/박스 데이터 로딩을 재설계 → 현재 취향보드 스냅샷 임시방편(커밋 11e10b8)도 이걸로 대체.~~ | — | — | **R10a/R10b/R10c로 분할됨** (위 3행) |
...
**R11c에서 함께 처리할 것 (R11b에서 이월)**: `initPlayWidget` 내부 `buildRecordItemHtml`(40줄) 추출 — 로컬 `escH`(2279) 캡처 때문에 IIFE 전에는 안전하게 못 올림(모듈 스코프 `function escH` = `window.escH` 덮어쓰기 → 타 파일 파손). IIFE 적용 시 이 위험이 사라지므로 그때 함께.

**보류 권고**: GS5(escH 사본 제거)는 감사 자체가 `"` 이스케이프 차이(PU4) 선행 정리를 조건으로 담. ⚠️ **IIFE화(R11c)만으로 GS5가 해결됐다고 착각하지 말 것** — IIFE는 로컬 escH를 안전하게 만들 뿐, 5개 사본 통합은 여전히 별건. GS7(난이도 헬퍼 전역결합)은 IIFE로 안 풀림 — `getDifficultyData`/`normalizeLevelValue`가 어차피 전역 유지 대상이라, 실제 해결은 헬퍼 별도 파일 분리(신규 파일 = 별도 결정).
...
**커밋**: 7d9fd24(IIFE 래핑+선별노출) · d4d1ac1(buildRecordItemHtml 모듈 추출). 두 원자 커밋으로 분리 — IIFE(교차파일 위험)를 격리해 스모크 회귀 시 이분 탐색 가능. **브라우저 클릭 스모크 통과 확인(사용자, 2026-07-16)** — 죽은 버튼 0. 스모크 중 나온 2건은 R11c 회귀가 아닌 별개 요청: ①표지모달 배경 어둡게(Green, 커밋 8b93e48 처리) ②좋아요 토스트→취향보드→뒤로가기 흐름(네비게이션 스택 = PROJECT_STATE §3-2, R10 병합 확정·사용자 승인).
...
- **`GameView`**(const=window.CottageGameView) — index-page·owned-games-page·script-nav 3파일이 `GameView.` 참조, 자체 정의 없음 → **window 노출**.
- **`gameSheet`**(재할당 let, 시트 요소) — owned-games-page.js:884 + 자기 인라인 onclick 2곳(스티키바 스크롤탑)이 bare 참조 → 스냅샷은 로드시 null이라 불가, **라이브 getter**(`Object.defineProperty`)로 노출 → 타 파일·인라인 무수정.

→ **다음 IIFE 작업(향후 game-* 파일) 시 반드시 "최상위 const/let의 크로스파일 bare 참조"도 함수와 함께 grep할 것.**

**노출 39 도출 방식**(재현 가능): `(파일 밖 js/html 참조) ∪ (자기 템플릿 on*= 속성 내 호출)`. 스크립트로 계산해 사전조사 39와 정확히 일치 확인. 함정1(한 onclick에 호출 2개, `event.stopPropagation();_openCoverModal(...)`)은 속성 전체를 스캔해 자동 포함.
...
**R10 원안 분할 (사용자 승인)**: 원안은 리팩토링 + 버그수정(stale) + 신규기능(네비스택)이 한 항목 = CLAUDE.md "구현/리팩토링 분리" 위반 → R10a(추출, 완료) / R10b(stale) / R10c(네비스택)로 분리. R10a가 서브시트 경계를 만들어 **R10b·R10c 착수 비용이 내려감**(각 서브시트가 자기 데이터를 스스로 로드하는 형태로 바꾸는 게 R10b의 방향 A와 그대로 맞물림).

**핵심 기법 — ctx + 첫 줄 구조분해로 본문 바이트 보존**: 캡처를 `ctx` 객체로 넘기고 추출 함수 첫 줄에서 `const { user, readOnly, ... } = ctx;`로 **원래 이름을 그대로 복원** → 이동 본문을 한 글자도 안 고침. R11a가 vm 객체안을 기각한 사유(`vm.` 접두사가 붙어 diff 검증 무력화)를 **구조분해로 회피**. 실측 캡처: usage 0 · voucher 1 · notif 4 · record 3 · taste 7+1 · meeting 9+1.

**⚠️ 재할당 캡처는 구조분해 금지 (R12 교훈의 쓰기판)**: 구조분해는 **복사**라 블록 안 재할당이 바깥에 전파되지 않는다. `node --check`·diff·**런타임 에러 셋 다 통과**하며 조용히 죽으므로 R12(ReferenceError로 즉시 터짐)보다 나쁘다. 실제 해당 2건 — 둘 다 **접근자 콜백으로 승격**(R12 `onDirty` 선례):
- `_currentBio`(taste): 한줄소개 저장값. 복사했으면 저장 후 재진입 시 옛 값으로 표시(1610줄 주석이 재진입 의존을 명시).
- `_pendingMeetingScrollTop`(meeting): 취향보드 편집 후 "‹ 모임 보드" 복귀 시 **다음 afterRender 호출**에서 읽음. 복사했으면 스크롤 복원만 조용히 실패.
- 반면 `_allPhotoData`(record)는 `splice` **변형**일 뿐 재할당이 아니라 참조 전달로 안전. `_tasteInnerHtml`/`_recordInnerHtml` 스냅샷 재할당은 호출부 `onLeave` 콜백에 남아 애초에 안 넘어감.

**⚠️ 검사기 자체가 3번 조용히 틀렸다 — 음성 대조군이 전부 잡아냄**: R12가 "검사기를 음성 대조군으로 검증하라"고 남긴 이유가 이번에 실증됨. 실제 발생 순서:
1. **bash heredoc이 정규식을 먹음** — `'\\b'`가 `'\b'`(백스페이스 문자)로 저장돼 **어떤 이름도 매치 불가** → 첫 실행이 "캡처 0개"로 나옴(= 검사기가 아무것도 안 보는 상태). **이 환경에선 정규식 스크립트를 heredoc·`node -e` 큰따옴표로 만들지 말 것**(같은 사고가 `node -e`에서 한 번 더 재발). Write 툴로 파일 생성이 안전.
2. **문자열·주석 내용을 코드로 오탐** — `'.profile-panel-stats'`의 `panel`/`stats`, `'profile-subsheet-body--records'`의 `body--`를 감소연산자로. → 문자열/주석 제거 스캐너 추가(단 template literal의 `${}` 안은 진짜 코드라 보존).
3. **프로퍼티 접근을 지역참조로 오탐** — `document.body`의 `body`. → 앞에 `.`이 오면 제외(`(?<![.\w$])`). 이걸 안 고쳤으면 taste ctx에 불필요한 `body`를 넣을 뻔.
4. **여러 줄 구조분해를 선언으로 못 읽음** → 이미 처리한 캡처 7개를 누수로 재보고.
→ **교훈은 CLAUDE.md "검증 결과가 0건·전부 통과면 검사기 자체를 먼저 의심한다"로 승격됨(2026-07-16, SSOT)** — 같은 날 DB 진단에서도 2번 더 재발(HTTP 400을 "0행"으로 오독, 컬럼명 추측으로 "마이그레이션 미적용" 오판)해 리팩토링 전용 교훈이 아님이 확인됨. 위 4건은 그 규칙의 실제 사례로 보존.

스크립트는 세션 스크래치패드에만 있음(`capture.js`/`writes.js`/`leak.js`/`move.js`/`bodydiff.js`) — R10b·R10c 착수 시 재작성 필요. 상설화하려면 `scripts/`에 신규 파일 = 별도 승인.

**검증**: 서브시트별로 ①`node --check` ②누수 검사(바깥 지역변수 무인자 참조) ③**HEAD 대조 본문 바이트 비교** — taste는 349줄 중 5줄(접근자 4 + 주석 1), meeting은 529줄 중 4줄(전부 접근자)만 변경, 나머지 4블록은 **본문 변경 0줄**. ④크로스파일 참조 0·window 노출 변화 0 확인(R11c 교훈: 함수만 세지 말 것 — 이번엔 새 전역 6개를 *추가*만 했고 IIFE화는 안 해 크로스파일 상수/변수 갇힘 위험은 없음).
...
→ **대응(재현 가능)**: 추출한 헬퍼별로 `(바깥 함수 지역변수 목록) − (헬퍼 파라미터 ∪ 헬퍼 내부 선언)` 을 정규식으로 계산해 0인지 확인. 파라미터 파싱 시 **구조분해(`{a,b}`)와 중첩 화살표 자체 파라미터**를 안 세면 오탐이 난다(실제로 `statChip = (icon,label,count)=>`의 `count`가 오탐). **검사기 자체를 음성 대조군으로 검증할 것** — 알려진 누수를 일부러 만들어 넣고 잡히는지 확인(안 하면 "전부 통과"가 검사기가 아무것도 안 보는 것과 구별 안 됨). **R10(KA1)은 지역변수가 훨씬 많아 이 검사 없이는 위험** — 스크립트는 세션 스크래치패드에만 있으므로 착수 시 재작성 필요(상설화하려면 `scripts/`에 신규 파일 = 별도 승인). ▶ **R10a(2026-07-16)에서 실제로 재작성해 적용** — 이 경고가 맞았고, 더해 **검사기 자체가 4번 조용히 틀렸다**(위 "R10a 결과 메모" 참조). 재할당 캡처는 이 검사(읽기 누수)로 안 잡히는 **별개 유형**이라 쓰기 검사도 함께 필요.
