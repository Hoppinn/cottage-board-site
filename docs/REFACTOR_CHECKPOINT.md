...
| 13 | **R10a** | **KA1 추출 — `openProfilePanel` 서브시트 6블록 모듈화** | **Opus xhigh** | 완료(플랜 승인 2026-07-16) | ✅ **코드 완료 (2026-07-16, 커밋 c9ab2bd·fd2bd0c·e5e4bad·c6a0bff·682227e·b3ed30d), 브라우저 스모크 대기**. `openProfilePanel` **1,940→918줄**(1,043줄 추출). 서브시트별 원자 커밋 6개(usage 13·voucher 14·notif 30·record 101·taste 353·meeting 533줄). window 노출·시그니처 변화 0 → js-api.md 무변경. 상세는 아래 "R10a 결과 메모". |
| 13b | R10b | **크로스보드 stale 버그** — 방향 A(진입 시 DB 재조회 = 단일 소스)로 서브시트 데이터 로딩 재설계, 취향보드 스냅샷 임시방편(11e10b8) 대체. **R10a가 서브시트 경계를 만들어 착수 비용 하락**. | **Opus xhigh** | **필요** | ⏳ 대기 (R10a 스모크 후) |
