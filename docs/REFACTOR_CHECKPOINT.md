...
| 13b | R10b | **크로스보드 stale 버그** — 방향 A(진입 시 DB 재조회 = 단일 소스)로 서브시트 데이터 로딩 재설계, 취향보드 스냅샷 임시방편(11e10b8) 대체. | **Opus xhigh** | 완료(플랜 승인 2026-07-17) | 🟡 **코드 완료, 브라우저 스모크만 남음** (2026-07-17, 커밋 507f2e9·c0cd874·5f82aed). 중복 쿼리 2개 제거 → `getMeetingProfile` 단일 소스(`avoidTags` 추가), HTML 빌드를 `_buildTasteInnerHtml`/`_buildMeetingInnerHtml` 함수화해 진입 시 재빌드, taste 스냅샷·`_currentBio` 접근자 제거. **착수 후 stale 당사자가 문서의 2종이 아니라 5종**(bio·avoid_tags·ruleSet 추가)임을 발견. 기록보드 스냅샷·패널 카드 요약은 **범위 밖으로 분리**(사용자 승인) → PROJECT_STATE §3. 스모크 포인트 8개는 PROJECT_STATE §0. |
