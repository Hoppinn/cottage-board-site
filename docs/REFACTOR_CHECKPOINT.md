...
→ **교훈: "0건 통과"를 신뢰하기 전에 알려진 누수를 심어 잡히는지 먼저 확인.** 스크립트는 세션 스크래치패드에만 있음(`capture.js`/`writes.js`/`leak.js`/`move.js`/`bodydiff.js`) — R10b·R10c 착수 시 재작성 필요. 상설화하려면 `scripts/`에 신규 파일 = 별도 승인.
