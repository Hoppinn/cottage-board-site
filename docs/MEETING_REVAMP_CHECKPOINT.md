...
> ✅ **완료 (2026-07-09)** — 1~7단계 전 단계 커밋 완료. PROJECT_STATE §0에서 해제.
> 이 문서는 이식 후보 보존 목적으로 삭제하지 않음 (§6.9 이식 후보 참조).
...
- 대표 게임 정렬 비대칭 (의도된 설계): 개인 막대=⭐ 우선(타입 무관)→want→learn(개인 강조 뷰), 집계(태그줄·센터모달)=투표수 먼저 대표수 2차(그룹 합의 뷰). 뷰어와 목적이 달라 비대칭이 정상.
...
2.7. [x] 대표 게임 (Red — DB Plan 필수, 마이그레이션 009 통합) — 완료
   - [x] Step 3 ⭐ 지정 — club-schedule.html 로컬 토글 + saveAll is_priority 반영 (2026-07-08)
   - [x] 표시/집계 반영 — buildBarsInCard 대표 우선, 센터모달 대표수 집계·칩 ⭐ 병기 (2026-07-08)
...
완료 조건: ✅ 달성 — 7단계 전부 커밋 + 문서 갱신 완료 (2026-07-09).
PROJECT_STATE §0에서 해제. 문서 삭제 대신 완료 상태로 보존 (이식 후보 때문).
