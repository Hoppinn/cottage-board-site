...
| ID | 위치 | 설명 |
|----|------|------|
| B-01 | supabase-client.js:486–490 | `_popAccumulatedMinutes()`가 localStorage 즉시 삭제 후 DB upsert → 네트워크 실패 시 시간 데이터 영구 소실 |
...
다른 기기로 로그인
  ...
  → 이후 원래 기기에서도 프로필 패널에서 원래 닉네임 보임 (localStorage)
    BUT 다음날 upsertProfile 재실행 시 DB 닉네임이 카카오명으로 확정
...
사진 삭제 버튼(🗑) → deletePlayPhoto() → photo_url = null
  ...
  → JSON 배열에서 특정 사진 하나만 삭제 불가 → 전체 삭제됨
```
...
- [ ] **B-01** 이용시간 localStorage 삭제 타이밍 수정 — DB upsert 성공 후 삭제
