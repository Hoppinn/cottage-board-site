...
| Surface geometry owner / context | the immediate iframe owner supplies a canonical surface's available coordinate area only when that surface explicitly opts in, without changing the preserved parent outer geometry. `profile-panel` may use that full available owner; `game-sheet` remains inside its parent modal context and never reclassifies the parent owner. |
...
| 프로필 작성/수정 wizard | 모임원 프로필 iframe entry | iframe-internal local layer | local component in `club-intro.html` | 모임원 프로필 root iframe | `.intro-wizard` local container (not promoted to Host frame) | `.intro-wizard-body` (`min-height:0`, `overflow-y:auto`, `overscroll-behavior:contain`) | iframe wizard close; parent X delegates while wizard open | `.intro-wizard`, `.intro-wizard-body`, `.intro-wizard-close` | `cottage-profile-wizard-state`, `cottage-close-profile-wizard` messages | active local exception | runtime close delegation 확인됨; scroll containment user verification needed |
