...
| 프로필 작성/수정 wizard | 모임원 프로필 iframe entry | iframe-internal local layer | local component in `club-intro.html` | 모임원 프로필 root iframe | wizard local container (not promoted to Host frame) | wizard body (runtime exact selector needed) | iframe wizard close; parent X delegates while wizard open | `.board-wizard-modal`, `.intro-wizard-close` (documented measured selectors) | `cottage-profile-wizard-state`, `cottage-close-profile-wizard` messages | active local exception | runtime close delegation 확인됨; geometry/scroll inventory runtime 필요 |
...
| 모임 조율 | planner day/detail entry | local `dd` overlay, or Host child overlay when requested by stack iframe | same document `day-detail.js` renderer; Host child iframe `club-schedule.html` | standalone planner or Host root | local `.dd-meeting-modal`; Host-owned **compact** shell + child content fill | local modal body (runtime selector needed) | local `.dd-x-btn`; Host child hides it and Host × owns close | `#__ddModal`, `.dd-overlay`, `.dd-meeting-modal` | `openDateMeetingModal()`, `stackKind=meeting` | dual | 코드 확인됨 / runtime 필요 |
...
| `data-ui-surface-geometry-owner` + active `data-ui-functional-surface="game-sheet"` | root/Host iframe shell that directly contains the game-sheet document | parent/child message bridge makes this owner transparent full-available-area only while the canonical surface is active, so the same `position:fixed` game-sheet does not become a modal-inside-modal. |
