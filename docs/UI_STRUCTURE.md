...
| Surface geometry owner / context | the immediate iframe owner supplies a canonical surface's available coordinate area without changing the preserved parent outer geometry. `profile-panel` may use that full available owner; `game-sheet` remains inside its parent modal context. |
