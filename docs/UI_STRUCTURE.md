...
| 모임 조율 | planner day/detail entry | local `dd` overlay, or Host child overlay when requested by stack iframe | same document `day-detail.js` renderer; Host child iframe `club-schedule.html` | standalone planner or Host root | local `.dd-meeting-modal`; Host child shell + child content fill | local modal body (runtime selector needed) | local `.dd-x-btn`; Host child hides it and Host × owns close | `#__ddModal`, `.dd-overlay`, `.dd-meeting-modal` | `openDateMeetingModal()`, `stackKind=meeting` | dual | 코드 확인됨 / runtime 필요 |
