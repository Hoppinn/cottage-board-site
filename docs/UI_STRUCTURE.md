...
| `data-ui-presentation-layer-owner="host"` + `presentationStack: 'above-requesting-host'` | `ModalStackHost` container and profile portal request | the Host container supplies the requesting layer's computed z-index; profile-panel is placed exactly one layer above it. This is a semantic relative stack position, not a route selector or global z-index escalation. |
