...
| Presentation / layer owner | current document for local surfaces, or the nearest already-loaded ancestor canonical renderer for a registry surface with `presentationOwner: 'ancestor'`. This is independent from geometry owner and never changes the preserved parent outer geometry. |
...
| 내 보드 | header/login/profile entry | local profile panel; Host iframe에서는 ancestor presentation layer | canonical `openProfilePanel()` renderer, reused without a Host child document | standalone page, planner/member-profile/guide root context | `.profile-panel-box` is feature layout; parent shell geometry remains unchanged | `.profile-panel-body` | local `.profile-panel-close`; parent outer × remains parent-owned | `#profilePanel`, `.profile-panel`, `.profile-panel-box`, `.profile-panel-body` | `openProfilePanel()` | canonical local + ancestor presentation | code confirmed; user speed/visual verification needed |
