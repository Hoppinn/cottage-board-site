...
**다음 작업 후보**
1. **다른 플레이어 프로필 보기** — 닉네임/아바타 클릭 시 읽기전용 취향보드 시트
2. **플레이기록 → game_curious 알림 (4-1)** — 기록 저장 시 해당 게임 curious 유저에게 인앱 알림
3. **게임시트 curious 상태 변경 (4-2)** — 이미 플레이한 게임이면 "해보고싶어요" 버튼 다르게 표시
