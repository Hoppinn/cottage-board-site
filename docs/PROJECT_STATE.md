...
**진행 중 작업 없음** (2026-07-21 세션 ⑰ 종료 시점, **CLAUDE.md 경량화·정합성 종결**. 코드 변경 0) · **열린 스모크 3건**(§0 아래 「열린 스모크」).
...
- **세션 ⑰**: 「외부 의견은 감사 입력이지 구현 지시가 아니다」 발동 — **이 파일에서 처음으로 "있어서 사고를 막았다"가 성립한 기록.** 모임 플래너 확장 스펙(GPT 작성)이 **플래너·상세 모달 2곳**만 고치게 돼 있었는데, 재검증하니 `getMeetingVotes` 소비처는 **5곳**이었다. 그대로 구현했으면 같은 날짜가 **플래너 9명·홈 6명**으로 갈렸다(#15 `visitorKey`와 동형). 「닫기 우선」 **A-3**도 발동 — 요금 문구와 플래너를 "같은 정책"이라는 이유로 묶으려다 **파일·검증 환경이 다름**을 근거로 분리했다.
- **재감사 카운터**: 감사 시행이 2026-07-20(세션 ⑭), 세션 ⑰이 **3회차**. 트리거는 5회. ⚠️ **세션 ⑰은 카운트하되 감사 자체를 손댄 세션**이다 — CLAUDE.md에서 2건을 커맨드로 분리했다가 `wrap`은 되돌렸고(매 세션 발동 규칙은 상주), 그 판정 기준이 「재감사 때 볼 것」 **4번**으로 들어갔다.
...
> 🔀 **2026-07-21 재분류.** 사용자 요청으로 순서를 다시 짰다. 기준은 CLAUDE.md B-5·B-6 — **①확인·문구로 닫히는 것 → ②판단이 필요한 버그 → ③기능·구조(Plan) → ④기획**, 그 안에서 **지금 사용자의 관심(모임팟·홈 첫 화면)이 먼저**다. 아래 1~3이 이번에 새로 들어온 것이고, 옛 1~5는 4~8로 밀렸다(내용 변경 없음).

| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 🔴 **홈 히어로 「최근 플레이」 미갱신** — 사진+게임평이 둘 다 있는 새 기록이 안 올라온다 | §2 | 버그 | **홈 첫 화면이고 지금 발현 중.** 다만 원인 미확정이라 **재현·실측이 첫 동작** |
| **2** | 📄 **동호회 소개·회칙 요금 문구 정합** — 회칙 77행이 확정 정책과 **정반대로 읽힌다** | §3 P1 | 문구 | **①형**: 정책이 이미 확정돼 코드 없이 닫힌다. 회원이 읽는 규정이라 틀린 채 두면 그대로 오안내 |
| **3** | 🎲 **모임 플래너 동반 지인 인원(`guest_count`)** | §3 P1 | 기능+DB·Plan | **지금 사용자의 관심사**(모임팟 수요가 실제로 발생). 2번과 같은 정책에서 나왔지만 **파일·검증이 달라 묶지 않는다**(A-3) |
| 4 | 🚫 **「프로필 보드 4건 묶음」은 묶음이 아니었다** (2026-07-21 독립 진단) — 4건 중 **2건은 이미 닫혀 있었고**(수집보드 스크롤=커밋 db08f332에서 해결됨, 카드 요약=2026-07-21 수정), 남은 2건은 **원인·코드 경로·검증 환경을 하나도 공유하지 않는다**: 박스모달 닫힘은 z-index 레이어 문제(§3), 타인 보드 네비는 표현 통일 문제(§3). **따로 처리할 것** | §3 | 개별 판단 | 진행 중이던 대기열 선두. 새 3건이 앞서면서 밀림 |
| 5 | 🔔 **관심 기반 묶음 알림** | §3 | Red+Plan | 무거운 신규 기능 |
| 6 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 | 전부 **재현부터** 필요. 하나씩 독립 |
| 7 | 🟡 **#28 `page_sessions.referrer`도 라벨/pathname 저장** — 외부 유입 체류가 전부 「직접 방문」으로 접힌다. #14와 같은 파일이지만 **다른 화면·다른 판정**이라 분리 | [admin-analytics.md](admin-analytics.md) §4 | 옐로 | 관리자만 보는 화면 |
| 8 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan | 사용자 눈에 안 보이는 리팩토링 |

⚠️ **1·2·3은 서로 묶지 않는다.** 2와 3은 같은 요금 정책에서 나왔지만 **2는 정적 HTML 문구, 3은 DB+JS 5개 파일**이라 검증 환경이 다르다. 1은 완전히 별개다.
