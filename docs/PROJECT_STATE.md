...
>
> 🔍 **세션 기록을 쓸 때의 검사 문장 (2026-07-20 추가)**: 한 줄을 적기 전에 **"이게 도메인 문서나 git log에 이미 있나?"**를 묻는다. **있으면 안 쓰고 거기를 가리킨다.** 남길 건 **"그 문서들을 다 읽어도 복원되지 않는 판단"**뿐이다.
> > **사건**: 2026-07-20 "누락없이 정리"를 **"빠짐없이 §0에 적기"**로 읽고 닫은 5건을 22줄로 늘어놨는데, **내용의 거의 전부가 `admin-analytics.md` 종결표·`achievement-system.md`·git log에 이미 있었다.** 같은 사실이 두 곳에 있으면 다음 세션엔 "어느 쪽이 최신이지"가 된다. 8줄로 줄였다.

> 📍 **관리자 분석을 만질 땐 [admin-analytics.md](admin-analytics.md)를 먼저 읽을 것** — 표시 원칙 5개·발견 대장 #1~#26·부위별 실행계획 P1~P7·재조사 금지 가설이 전부 거기 있다. 아래 §0 세션 요약의 `#숫자`도 그 문서의 발견 번호다. **최근 세션(⑭·⑬) 요약은 §0 상단.** 이전 세션(①~⑪) 상세는 git log + [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md), 그때 발생한 열린 항목·교훈은 전부 §3·CLAUDE.md·js-api.md·PROJECT_STRUCTURE.md로 이관 완료.
...
**세션 ⑭ (2026-07-20)** — 관리자 잔여 **5건 전부 종결**(#25·#18·#3·#26·교환권 도배) + 업적 재지급 낭비 제거. 커밋 10개. **무엇을 어떻게 닫았는지는 [admin-analytics.md](admin-analytics.md) §4 종결표**(업적은 [achievement-system.md](achievement-system.md), 검증 방법은 git log).

> **한 줄 요약**: **5건 중 3건은 「고칠 것」이 아니라 「대장 기재가 틀린 것」**이었다. 관리자 페이지에 남은 건 **#14**(Plan) 하나이고, 목표를 닫는 건 **P3b**다.
>
> 🚨 **여기 남길 값이 있는 판단 셋** — 나머지는 도메인 문서·git log에 있다.
> 1. **「0건」·「발현 없음」은 발견이 아니라 질문이다.** #18·#3이 연달아 뒤집혔다 → 아래 시작점 표의 경고.
> 2. **도형도 주장이다.** 화살표·트리는 포함관계를 주장하는데 이 데이터는 보장하지 않는다(#26) → P3b 착수 경고.
> 3. **외부 지적은 문제 가설이지 해결안이 아니다.** 같은 자리를 하루 세 번 고쳤고 2·3차가 같은 실패였다 → CLAUDE.md 「외부 의견」.
