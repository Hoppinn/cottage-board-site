...
- 다음 작업: 플래너 과거 인원 수정 커밋 뒤 P1 기능 버그를 한 건씩 수정·검증·커밋한다.
...
2. **P0 플래너 과거 인원** — 본체가 오늘 기준 -7~+62일만 읽어 과거 주·달을 0명으로 보이던 문제를 수정했다. 이동한 주/달 범위의 실제 `meeting_votes`를 재조회·캐시해 주간/달력이 같은 집계를 쓴다. 브라우저 수동 확인 후 닫는다.
