...
**Step 1 — CSS 아키텍처 재구조화 (style.css) — 반드시 먼저**
- 모든 `@media(min-width:900px)` 블록을 파일 맨 끝 단일 블록으로 통합
- `!important` 전부 제거 (cascade 순서로 해결)
- 결과: 모바일 기본 스타일 앞 / PC 오버라이드 파일 끝 단일 블록
- **스케일 작업은 이 재구조화 완료 후 해당 단일 블록 안에서만 진행**

**Step 2 — 균일 비율 PC 스케일 적용 원칙**
- **방법**: 모바일 기준값(style.css 기본 스타일) 대비 일정 배율을 정해서 **모든 요소에 같은 비율로** 적용
  - 예: 모바일 `font-size: 14px` → PC `font-size: 17.5px` = 25% 상향이면 다른 모든 값도 ×1.25
  - 세션 7에서 적용된 값 중 올바르게 커진 항목(예: req-*)을 기준 비율로 삼아 역산
- **하나씩 페이지별 패치 금지** — 비율 계산 먼저, 일괄 적용
- 페이지별 사용자 피드백(아래 목록)은 비율이 제대로 적용됐는지 **검증 기준**이지 개별 수정 지시가 아님

**Step 2 적용 대상 — 모든 페이지 전체 구성요소 (메인 + 하부패널 + 바텀시트 + 연동페이지)**

| 페이지 | 검증 포인트 |
|--------|------------|
| game-reviews (pr-*) | 현재 `!important` 패치로 너무 크게 된 항목 → 균일 비율에 맞게 재조정. `pr-rec-thumb`(게임플레이 사진 썸네일)은 모바일(64px)보다 크게 |
| owned-games | `.og-hero-count`("~종의 게임이 기다리고 있어요"), 정렬 버튼 텍스트 아직 모바일 크기와 같음 → 비율 적용. 필터 섹션은 오히려 조금 줄여도 됨 |
| index (추천 게임) | 필터 텍스트, 게임카드 전체 |
| admin 페이지들 | 전체 텍스트/버튼 PC 확대 미적용 |
| price-rules.html | 전체 텍스트/요금표 PC 확대 미적용 |
| club-history.html | history-* 클래스 텍스트 PC 확대 점검 |
| 바텀시트 (game-sheet) | PC에서 텍스트/버튼 크기 점검 |
| **req-* (요청하기)** | **이미 올바르게 적용됨 — 수정 금지, 비율 역산 기준으로 활용** |
...
### 주의
- Step 1(재구조화) 전에 절대 스케일 값 수정 금지
- style.css 전체 재구조화이므로 작업 전 git commit 상태 확인
- `!important` 한 줄도 남기지 말 것
- 재구조화 후 모든 페이지 브라우저 확인 필수
