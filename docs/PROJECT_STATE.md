...
>
> 📌 **다음 목표는 미정.** 아래 표는 우선순위 순 대기열이며, **#14는 관리자 화면 건이지만 소급 결정이 걸린 별건**이라 위 목표에 포함되지 않았다.

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🟡 **#14 `page_sessions.page`가 라벨 저장** — 저장 경로가 **둘**(`script-nav.js` 세션 트래커=한글 라벨 / `_startAnonHeartbeat`=슬러그)이라는 정체가 2026-07-20에 밝혀졌다. 통일 + **과거 행 소급**(불가분 감안) 결정이 필요 | [admin-analytics.md](admin-analytics.md) §4 | 옐로+Plan |
| 2 | 🧩 **프로필 보드 4건 묶음** *(관리자 페이지 종료 후)* — 카드 요약이 편집 미반영 · 박스모달이 닫힘 · 타인 보드 네비게이션 통일 · 수집보드 스크롤 진입점. 네 건 다 같은 서브시트 구조 | §3 | 옐로 |
| 3 | 🔔 **관심 기반 묶음 알림** *(교환권 도배는 2026-07-20 종결)* | §3 | Red+Plan |
| 4 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 |
