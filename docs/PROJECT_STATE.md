...
- **1단계 (구조)** — 이번 세션 작업 범위
  - 오늘 요약 / 분석 탭 / 그래프 재사용 영역 / 이벤트 탭 / 상세 접힘 구조
  - 기존 데이터만 연결: 방문(chartDaily+시간대+회원비율) / 회원(방문자목록, 최근3건+전체보기) / 유입(chartReferrer+교차분석 상세) / 페이지(chartPages+표) / 이벤트(추천게임·플레이기록 기존 퍼널만)
- **2단계 (데이터)** — 보류, 다음 세션
