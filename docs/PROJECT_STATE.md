...
- [ ] **게임시트 상단 레이아웃 개편** (Yellow, JS+CSS) — ① `sheet-img-bgg`(썸네일 아래 별점) 제거. ② `sheet-en-title`을 `sheet-header` 밖→`sheet-title-block` 최상단으로 이동. ③ 버튼 한 줄 재배치: 현재 `[A-1 ← 꽂혀있는 책장 보기 전체폭] / [룰영상 보기][정리법 보기]` → `[꽂혀있는 책장 보기][룰영상 보기] / [정리법 보기(있을때)]`. 텍스트 길이 문제 있으면 `📍 책장 위치 보기`로 단축 검토. 관련 파일: `assets/js/script.js` ~L1288~1319, `assets/css/style.css` `.sheet-en-title/.sheet-loc-btn/.sheet-links-block`.
