...
- 현재 단계: 상세 Plan, 031 마이그레이션, 설문·프로필 출력·계약 검증을 구현했다. 운영 DB 적용과 360px 실화면 검수만 남았다.
- 다음 작업: 사용자가 `030 → 031`을 운영 DB에 적용한 뒤 새 코드를 배포한다. 적용 후 신규/수정 저장, 가입 경로·거주 지역·3단 취향 출력, 쿠폰 재지급 방지를 실계정으로 확인한다.
