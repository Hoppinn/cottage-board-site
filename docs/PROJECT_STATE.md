...
| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🟡 **관리자 잔여 2건** — `page`가 라벨 저장(#14, **Plan 필요** — 저장 경로 2개 통일 + 과거 행 소급) · 알림 교환권 도배(원인 조사부터). 서로 독립. *(2026-07-20: #25·#3·#26 종결 / #18 판정종결)* | [admin-analytics.md](admin-analytics.md) §4 | 옐로 |
...
>
> 🚨 **#18에서 얻은 것 — "0건"은 발견이 아니라 질문이다.** 대장의 「DB 0건」이 **낡은 값**이었고(실측 5건, 2026-07-19부터) 그동안 **없는 병의 원인 후보를 넷 세워두고** 있었다. 착수 첫 동작은 가설이 아니라 **그 숫자를 지금 다시 재는 것**이다. **같은 날 #3에도 그대로 적중했다** — "현재 발현 0건"이라 적혀 있었으나 실제로는 326세션 중 13개에서 발현 중이었다. **두 번 연속이면 우연이 아니다.**
