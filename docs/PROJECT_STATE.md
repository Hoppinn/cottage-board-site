...
>
> 🏁 **종료 조건 = P3b(드릴다운)까지.** ⚠️ **이건 사용자 말의 뜻이 아니라 내 구현 판단이다.** 원문은 *"유효 데이터들이 더 직접적이고 **유기적으로** 드러났으면"*이고, **그 상태를 현재 구조에서 충족하려면 드릴다운이 필요하다고 판단**해 완료 조건에 넣었다. "유기적"이 다른 뜻일 수도 있다(지표 간 연결·시각 구조 정리 등) — **사용자가 다르게 말하면 종료 조건을 다시 정한다.**
>
...
>
> 🔍 **#18은 "고칠 것"이 아니라 "잴 것"이다** — `home_record_write_click`은 **이미 정상 배선돼 있다**(`index-page.js:1239`, 버튼도 `index.html:224`에 존재). 그런데 DB가 0건이다. 즉 **코드 결손이 아니라 "아무도 안 누른다"일 수 있고, 그러면 퍼널의 0은 거짓말이 아니라 사실**이다. **먼저 왜 0인지 재고**(버튼이 안 보이나·관리자만 눌러 트래킹 제외됐나) 그다음에 고칠지 정한다.
