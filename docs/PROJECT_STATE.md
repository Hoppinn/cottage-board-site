...
| 항목 | 남은 확인 | 재개·판정 경로 |
|---|---|---|
| 이날 모임 상세 UI (2026-08-31) | 날짜 고정 헤더, 게임 조율·참여자별 두 섹션, 참여자 한 명=한 카드 위계가 모바일에서 자연스러운지 | 최근 `cf6d1441`, `f1038820` 및 후속 CSS 커밋. 이상이 있으면 스크린샷의 URL·뷰포트·비교 기준부터 고정한다. |
...
| 모임 데이터 쓰기 보호 | Kakao 인증 구조 때문에 meeting 테이블은 현재 UNRESTRICTED이며 문서화 후 보류 | 범위(meeting만/전체)와 Edge Function 배포 환경을 확정한 뒤 Plan |
