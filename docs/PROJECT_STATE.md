...
>
> 📐 **순서**: 목표 화면 **안에서** 쉬운 것부터. 끝나면 다음 화면으로. **"쉬운 것부터"와 "목표 먼저"는 충돌이 아니다** — 같은 화면 안에도 쉬운 게 5건 있다.

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🟡 **관리자 잔여 5건** — `record_start` 유령 발사(#25) · 퍼널 `home_record_write_click` 0건(#18) · `anonTodayS`(#3) · `page`가 라벨 저장(#14) · 알림 교환권 도배. 전부 같은 화면 + 🟡 + 서로 독립 | [admin-analytics.md](admin-analytics.md) §4 | 그린~옐로 |
| **2** | 🎯 **P3b 드릴다운 — 관리자 페이지 완료 조건.** 이벤트·페이지 숫자에서 **"이걸 한 사람들"**로 내려가는 길. 무겁고 Plan이 필요하지만 **이걸 해야 목표가 닫힌다.** 선행(#22·#24·R3) 완료 | 〃 §5 | Plan |
| 3 | 🧩 **프로필 보드 4건 묶음** *(관리자 페이지 종료 후)* — 카드 요약이 편집 미반영 · 박스모달이 닫힘 · 타인 보드 네비게이션 통일 · 수집보드 스크롤 진입점. 네 건 다 같은 서브시트 구조 | §3 | 옐로 |
| 4 | 🔔 **알림 2건 묶음** — 교환권 도배 + 관심 기반 묶음 알림 | §3 | Red+Plan |
...
> 🚨 **1번 5건은 묶지 말고 하나씩 닫는다.** 특히 **#25와 #18을 묶지 말 것** — 2026-07-20 코드 실측 결과 **공유하는 게 없다**: 파일이 다르고(`game-reviews.js:119` vs `index-page.js:1239`), 원인 유형이 다르고, 검증 방법이 다르다. "둘 다 기록 계열"은 **주제 유사성이지 묶음 근거가 아니다**(CLAUDE.md 「닫기 우선」 A-3).
>
> 🔍 **#18은 "고칠 것"이 아니라 "잴 것"이다** — `home_record_write_click`은 **코드상 정상 배선**돼 있다(`index-page.js:1239`, 버튼도 `index.html:224`). 따라서 **"코드 결손"은 아닐 가능성이 높다** — 그 이상은 아직 모른다. 🚨 **"아무도 안 누른다"로 단정하지 말 것**(2026-07-20에 실제로 그렇게 단정했다가 정정). 후보가 최소 넷이다: ①정말 클릭이 없다 ②**관리자만 눌렀고 관리자는 트래킹 영구 제외**(`_shouldSkipAnalytics`) ③실서버 배포본이 로컬과 다르다 ④DOM 교체로 런타임에 리스너가 안 붙는다. **먼저 어느 것인지 재고**, 그다음에 고칠지 정한다.
