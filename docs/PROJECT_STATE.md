...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **모임시간 30분단위 / QR 추천게임 집계** | §3 P1 1·3번 | 기능 | 명확한 스펙, 서로 독립 — 순서 무관하게 진행 가능 |
...
- **📵 간식·음료/건의 요청 카카오톡(Make) 알림 미수신 — 진단 소진, 재활성화 실행만 남음 (2026-07-27)** — Make.com 시나리오 `5213346`("Integration Webhooks")이 **6/30 밤 에러 1건 직후 Make가 자동 비활성화(Inactive)** 시켰고, 그 뒤로 알림 44건이 대기열에 쌓인 채 방치돼 있었다(사용자가 대시보드 스크린샷으로 확인). 코드는 무관 — Supabase Database Webhook → 이 시나리오 → 카카오 "나에게 보내기" 전부 대시보드 설정(`voucher_log`도 같은 방식, [db-schema.md](db-schema.md)).
  - **진단 경로 3개를 다 확인했고 더 볼 게 없다**: ①History의 그 Error 실행 상세 로그 — **"Log details doesn't exist"**(Make가 ~30일 지나 로그를 지움, 6/30 건이라 만료). ②Credentials/Connections/Keys — **전부 비어있음**. 원인은 이 시나리오가 카카오를 Make의 OAuth "Connection"이 아니라 **HTTP 모듈에 토큰을 직접 박아넣는 방식**으로 쓰기 때문 — 그래서 토큰 상태를 확인할 방법 자체가 이 화면들엔 없다. ③INCOMPLETE EXECUTIONS — 빈 목록(그 큐와 별개).
  - **내가 대시보드/시나리오 내부(HTTP 모듈 설정)에 접근할 수단이 없어 여기서부터는 사용자만 진행 가능.**
  - **남은 유일한 다음 수**: "Show queue"로 44건 먼저 보고 지울지 결정 → **"Inactive" 토글을 켜서 재활성화** → 다음 실행이 Success면 일회성 에러였던 것으로 종결. **또 에러 나면 이번엔 로그가 신선하니 HTTP 모듈 상세보기에서 실제 에러 메시지를 볼 수 있다** — 그때 스크린샷 주면 재진단.
  - 에러가 반복되는 근본 원인(토큰 만료 등)으로 확인되면 Discord 웹훅 전환이 근본 해결책(Make 시나리오 HTTP 모듈 교체만 하면 됨, 예전부터 나온 안).
  - **재개 조건**: 사용자가 재활성화 완료 + 이후 실행 결과(Success/Error) 알려주면 종결.
