...
최종 갱신: 2026-07-17 — **R10c 핵심 스모크 ✅ 통과**(뒤로가기 2경로 정상). 스모크 중 발견한 **버그 2건 수정**(✕→햄버거 자동열림 affec36 / 참여자 태그 닉네임 클릭 6f3652a) + **스크롤 복원**(22d326b). **`REFACTOR_CHECKPOINT.md` 정리 ✅ 완료**(474→90줄). **다음 세션**: ①**R10c 잔여 스모크 5포인트**(회귀 2 + 후속 3, §3) ②감지기 3단계 ③이날모임 상세 클릭 2건(§3, 사용자 요청).
...
**다음 세션 시작점 (2026-07-17 세션 ③ 종료)**: **①R10c 잔여 스모크 5포인트**(§3 — 회귀 2건 + 이번에 고친 스크롤/햄버거/닉네임 3건) ②**이날모임 상세 클릭 진입점 2건**(§3, 사용자 요청, Yellow) ③**감지기 3단계**(§3, 선행이던 R10b·R10c 완료로 착수 가능) ④R번호 미배정 열린 항목(GS4·GS5·GS7·DD4·IP1~3 — REFACTOR_CHECKPOINT.md) 또는 감사 잔여 중 GDA3(유일한 버그성).
...
- [ ] **[verify] R10c 잔여 스모크** — **핵심 3건은 ✅ 통과**(2026-07-17 사용자 확인): 게임시트→토스트→취향보드→뒤로→내 보드→뒤로→**원래 게임시트 복귀** / 궁금해요 동일 / 알림→남의 모임보드→되돌아오기. **남은 것은 회귀 확인 + 그 자리에서 나온 후속 3건**:
  - **회귀 1순위**: ①평소 경로('내 활동' 클릭)로 연 보드에 **뒤로가기가 없고 제목이 왼쪽 정렬 그대로**인지(헤더가 backTo일 때만 grid 3열) ②닉네임 클릭으로 연 남의 보드도 뒤로가기 없음.
  - **후속 3건(2026-07-17 스모크 중 발견·수정, 확인 필요)**: ③**게임시트 복귀 시 보던 스크롤 지점 복원**(22d326b — `closeGameSheet`가 스크롤을 저장 안 하고 있었음. 좋아요/궁금해요 둘 다) ④**✕로 닫을 때 햄버거가 저절로 열리던 것**(affec36 — 이제 햄버거→내 활동으로 연 경우만 메뉴 복귀, 토스트·알림 경로는 닫힌 채) ⑤**참여자 태그 닉네임 클릭**(6f3652a — 기록을 올린 적 없는 회원도 이제 눌림. 게스트 이름은 여전히 안 눌리는 게 정상).
