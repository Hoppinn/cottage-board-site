...
- 현재 작업: 모임원 프로필 페이지 목록 카드 정리 완료
- 상세 Plan: [member-intro-play-pattern-plan.md](plans/member-intro-play-pattern-plan.md)
- 현재 단계: 프로필 보드 미리보기 카드와 모임원 목록 카드는 같은 핵심 프로필 요약을 쓰며, 평소 플레이·상세 취향은 내 프로필 보드 정본에서만 제공한다.
- 다음 작업: 기록보드 버그 수정 + 더보기/접기 공통 UI + 모임보드 후속 개편
...
1. 기록보드 버그 수정 + 더보기/접기 공통 UI + 모임보드 후속 개편의 범위와 관련 화면을 확인한다.
