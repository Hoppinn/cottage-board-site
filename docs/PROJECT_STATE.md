...
최종 갱신: 2026-07-22 (세션 ㉘ — 「진입 N회」 2중 계상 종결, iframe 사후 확인 재측정). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
- [ ] **접근성 개선 — 절반 남음: 폼 요소 이름 34건** (아이콘 버튼 44건은 2026-07-22 종결, git log). 전체 폼 요소 88개 중 **34개가 접근 가능한 이름이 없다**(`<label for>`·`aria-label`·`aria-labelledby`·`title`·`placeholder` 전부 없음 기준 — placeholder도 인정해서 센 값이라 실제 정석 기준으론 더 많다). 분포: `game-sheet.js` 10 · `owned-games.html` 7 · `club-intro.html` 5 · `club-schedule.html` 4 · `game-reviews.js` 3 · 나머지 5.
  - ⚠️ **버튼과 달리 일괄 처리하면 안 된다** — 상당수가 필터 체크박스·라디오라 **바로 옆에 보이는 글자가 이미 있다**. 그 경우 정답은 `aria-label` 신설이 아니라 **그 글자를 `<label for>`로 묶는 것**이다(보이는 이름과 읽히는 이름을 일치시켜야 음성 조작이 작동한다 — 버튼 작업에서 실제로 이걸 어겨 1건 되돌렸다). **항목마다 옆 텍스트를 보고 판정할 것.**
