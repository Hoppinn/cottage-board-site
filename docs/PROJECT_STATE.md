...
- 현재 단계: 2. 실제 모임보드 내부 수정 완료·사용자 화면 확인 대기
- 다음 작업: 모바일 364px 기준 0/1/여러 일정, 게임 유무, 긴 한마디, 모임 상세 진입을 확인하고 PC 화면도 점검한다. 확인 전에는 3번으로 진행하지 않음.
- 인수인계: 1번 미리보기는 확정됨. 2번은 `다가오는 모임`에 일정별 당일 참여 정보 카드, `참여 페이스`에 장기 빈도, `최근 참여`에 기존 기록을 유지한다. 상세 진입 동선은 기존 모임 상세 열기를 재사용한다.
