...
최종 갱신: 2026-07-22 (세션 ㊱ — **P1 구현: 한 플레이기록에 여러 사람 게임평(`game_comments.record_id`, 마이그레이션 014). 코드 완료, 사용자 실행 3단계 대기**). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중: P1 게임평 다중화 — 코드 완료, 사용자 실행 3단계 대기**(§3 P1: ①마이그레이션 014 실행 ②배포 ③link 스크립트 --commit). 내 코드 몫은 끝났고 남은 건 사용자 실행·실기기 확인. · **열린 버그 0건**(§2) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0** · **열린 스모크 6건**(§0 아래 「열린 스모크」) + P1 실기기 확인 1건.
...
- [ ] 🟡 **[코드 완료 · 사용자 실행 3단계 대기] 한 플레이기록에 여러 사람의 게임평** (2026-07-15 요청 → 2026-07-22 구현) — `game_comments`에 nullable `record_id`(마이그레이션 014)를 붙여, 게임평을 특정 기록에 매달아 그 기록 아래 표시한다. 새 기록을 만들지 않는다(옛 「남 세션 참여」 삽입 접근은 되돌림). 설계·검증은 커밋 참조.
  - **닫으려면 사용자 3단계** (코드는 다 됐고 내 몫은 끝, ②③은 실기기): ① **Supabase SQL Editor에서 마이그레이션 014 실행** — 안 하면 배포 후 게임시트 「게임평」 목록이 400으로 빈다(`getGameComments`가 `record_id`를 select). 컬럼 추가는 nullable이라 현재 라이브 코드엔 무해. ② **코드 배포(push)**. ③ **`node scripts/link-bbok-0711-comments.js --commit`** (014 적용 후) — 뽁님 게임평 2건을 호핀 7/11 기록에 매단다.
  - 📌 **뽁님 3건 중 2건만 연결된다(재측정 확정)**: 레비아탄와일드→기록97, 원더랜드워-풀확→기록96. **에버델은 호핀의 7/11 기록이 없어** 매칭 대상이 없다(그대로 독립 게임평으로 남음). 옛 삽입 접근도 2건만 처리했으니 동일 결과다. 에버델까지 원하면 「호핀의 에버델 기록이 어느 날짜인지」가 먼저 필요.
  - 📌 **실기기 확인(위 ③ 이후)**: 플레이기록 게시판에서 호핀 7/11 기록 아래 뽁님 이름의 게임평이 뜨는지 + 아무 기록 ⋯메뉴 「게임평 추가」로 남의 기록에 게임평이 붙는지.
