...
| 모바일 "이번주모임 미리보기" 일요일 레이아웃 밀림 (2026-07-15 발견) | 모바일 접속 시 홈 "이번주모임 미리보기"에서 **이번 주**만 일요일 칸이 한 칸 아래로 밀려 표시됨. 이전 주/다음 주는 정상. 컴퓨터(데스크톱)에서는 미재현 — 모바일 뷰포트 전용 증상 추정. 원인 미조사(재현 확인 안 됨, 실제 조사 시작 전 런타임 값 확인 필요 — 반복 패치 정지 규칙 적용). 관련 코드: `index-page.js`(홈 미리보기) / `day-detail.js`(막대 렌더) 추정, 확인 필요 |
| 이번 주 하고싶은 게임에 취소된 테스트 게임 잔존 (2026-07-15 발견) | 이번 주 금요일 "하고 싶은 게임"에 예전에 테스트용으로 넣었다가 취소한 "얼티밋 레일로드"가 계속 표시됨. 해당 게임을 넣은 참여자(투표)가 현재 없는데도 목록에 남아있음 — `meeting_vote_games` orphan 행 추정. `deleteMeetingVote`의 cascade 삭제(커밋 de89af8, 2026-07-14)가 **이후 취소부터만** 적용되므로 그 이전에 생긴 orphan은 소급 정리되지 않았을 가능성. 원인 미조사(DB 데이터 직접 확인 필요, 코드 수정 전 실제 `meeting_vote_games` 행 존재 확인 우선) |
