...
- 활성 코드 작업: 없음. 최근 이날 모임 상세의 구조·CSS 정리는 구현·문법 검사를 마쳤고 사용자 화면 확인만 남았다.
- 새 작업의 목표·종료 조건·범위·검증 기준은 착수 시 이 절에 한 번만 기록하고, 완료되면 항목을 삭제한다.
- 현재 버그: 새로 보고된 열린 버그 없음.

### 다음 시작점

1. 사용자 확인 대기 항목의 결과가 오면 해당 행만 닫거나 원인 조사로 전환한다.
2. 코드 작업을 재개한다면 약칭 출처 분류가 가장 작은 독립 작업이다. 착수 전 [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) §8의 약칭 정본·migration invariant를 읽는다.
3. 그 외에는 사용자가 지정한 새 목표를 우선한다.

## 1. 사용자 확인 대기

| 항목 | 남은 확인 | 재개·판정 경로 |
|---|---|---|
| 이날 모임 상세 UI (2026-08-31) | 날짜 고정 헤더, 게임 조율·참여자별 두 섹션, 참여자 한 명=한 카드 위계와 홈/플래너 미리보기의 인원조율 요약·compact 센터모달이 모바일에서 자연스러운지 | 최근 `cf6d1441`, `f1038820` 및 후속 CSS 커밋. 공용 `day-detail.js`로 미리보기·플래너·상세의 참여 인원/최대 시간 겹침을 통일했고, 2026-08-31 후속으로 햄버거 active 점을 화면 중앙 섹션 기준으로 조정하고 모달/내 보드 패널을 compact 기준으로 맞췄다. 추가 검증에서 확인된 box-sizing·cascade·합산 여백 원인을 수정해 고정 헤더·참여자 카드·모달 시작 여백을 재조정했다. 브라우저 연결이 되면 URL·뷰포트·계산된 크기와 참여자 헤더 정렬을 확인한다. |
