...
| 5 | **간식·음료 요청 Part2(관리자 처리됨 표시 + 요청자 완료알림)** | §3 P1 2번 | 기능/Plan | DB 컬럼 신규 — Plan 모드 먼저 |
...
**열린 버그 4건** (2026-07-27 세션 ㊴ 사용자 제보 일괄 등록·일부 원인 확인). 우선순위·착수 순서는 §0 표.

1. **새 게임평 작성해도 홈 최근기록이 새로고침 전엔 안 바뀜** — 원인 미조사. 캐시/이벤트 무효화 쪽으로 추정. ⚠️ **"최근소식 읽음"·"알림배지"와는 다른 코드 경로**(그 둘은 notif_read_keys 경합이었고 이건 game_play_records→홈 렌더 쪽) — 따로 조사할 것, 같은 원인으로 묶지 말 것.
2. **게임기록페이지 ⋯(햄버거) 메뉴를 열면 아래가 잘림** — 미수정. `DESIGN_RULES.md` 6-a에 같은 유형(overflow:hidden이 팝업을 자름) 해법이 이미 문서화돼 있어 그대로 적용 가능해 보임 — 실제 선택자 확인 필요.
3. **플래너 "지난 모임에서 하고 싶었던 게임" 탭 정렬이 최신순 역순이 아님** — 원인 미조사.
4. **PC 페이지(1440px) 가로폭 — `game-reviews.html`만 `.inner-page-body` `max-width:760px`, 다른 내부 페이지는 전부 720px** — Playwright 실측(owned-games/about/guide/club/game-location/price-rules/club-*/admin/requests 전수, 1440px 기준)에서 나머지는 모두 정확히 720px로 일치, game-reviews만 760 이상치. ⚠️ **단 1440px에선 시각적 차이가 없었다**(부모가 이미 720으로 죄고 있어서) — 사용자가 실제로 본 폭 문제가 이건지, 아니면 다른 화면·다른 해상도인지 재확인이 먼저다.
