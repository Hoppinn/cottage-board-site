...
최종 갱신: 2026-07-27 (세션 ㊶ — **간식·음료 요청 Part2 완료**(마이그레이션 015·016, 상태 2단계 드롭다운, 실사용 화면 `requests.html`+분석대시보드 `requests-admin.html` 양쪽 구현) + **요청관리 아코디언 드롭다운 잘림 CSS 버그 당일 종결**(play-records와 동일 패턴, DESIGN_RULES 6-a). **⚠️ 로컬이 origin/main보다 5커밋 앞서 있음(push 안 됨) — 지난 세션 종료 기록의 "push 완료"가 실제로는 안 됐던 것으로 확인(`git fetch` 후 `git log origin/main..HEAD` 대조). 사용자가 직접 push 필요.** 열린 버그 0건. 카카오 Make 알림 미수신 스모크는 "예전엔 됐었다"는 새 단서로 재정리(§0 아래). 다음 시작점은 §0 표 1번(모임시간 30분단위/QR 추천게임 집계). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음.** **열린 버그 0건**(§2) · **열린 기능 3건**(§3, Plan 필요 2건 — 게임정리·룰설명 관리자입력, 플래너 다른날짜 추가 UX. 간식·음료 Part2는 2026-07-27 종결) · **열린 스모크 7건**(§0 아래, 카카오 Make 알림은 "예전엔 됐다"는 단서로 재정리) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0**. **🚨 push 안 됨 — origin보다 5커밋 앞섬, 사용자 직접 push 필요.**
...
> ✅ **2026-07-27 세션 ㊵ 종료 시 압축.** 사용자 제보 6건(캡션복사 배치·관리자 권한·PC 레이아웃 4건) 전부 조사 즉시 당일 종결(§2 참조) — 표에서 제거. **항목이 하나씩 닫힐 때마다 이 표에서 그 행을 지운다** — 완료 요약을 표에 쌓지 않는다(CLAUDE.md 「저장 위치」).

| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **게임정리(사진)+룰설명(신규 텍스트) 관리자입력 (2026-07-27 범위 확정 — 룰영상 유튜브링크는 무관·그대로)** | §3 P1 2번 | 기능/Plan | 하드코딩 메커니즘 확인됨 — Supabase 테이블화 설계 필요, Plan 모드 |
| 2 | **플래너 「다른 날짜에도 추가」 confirm→토글+일괄 (2026-07-27 사양 확정)** | §3 P1 4번 | 기능/Plan | DB 변경 없음, UX 엣지케이스 결정 필요 — Plan 권장 |
...
**열린 버그 1건** — 플레이 기록 다른 날짜 저장 안 됨(아래). 나머지는 2026-07-28 당일 종결(git log 참조).

- [ ] **플레이 기록을 다른 날짜로 추가해도 등록일로 저장된다는 제보 (2026-07-28) — 코드·실측 둘 다 재현 안 됨.** 게임시트 "+ 남기기" 모달(`game-sheet.js:2644` `playedAt` → `recordGamePlay`)과 플레이기록허브 기록입력 탭(`game-reviews.js:437` `dateVal` → 동일 경로) 둘 다 Playwright로 날짜를 바꿔 저장한 뒤 **실제 네트워크 POST 페이로드**를 가로채 확인했는데 `played_at`이 선택한 날짜 그대로 전송됨. `supabase-client.js`의 `recordGamePlay`/`updateGamePlay`도 인자를 그대로 insert에 씀 — 덮어쓰는 트리거·기본값도 DB 스키마에 없음. **재개 조건**: 사용자에게 ①정확히 어느 화면(게임시트 모달 vs 기록허브)·버튼인지 ②모바일/PC, 브라우저 종류 ③새벽 시간대였는지 재확인 요청 필요 — 지금은 재현 정보 부족.
  - ✅ 조사 중 **관련 있지만 다른 버그**를 하나 찾아 즉시 고침(2026-07-28): `game-sheet.js`의 날짜 입력 기본값·상한(`sheetPlayModalDate`)이 `new Date().toISOString()`(UTC)을 썼다 — 한국시간 새벽 0~9시엔 "오늘"이 실제보다 하루 전으로 잡힌다(day-detail.js에 이미 있는 경고 주석과 동일 함정, game-reviews.js는 `todayKst()`로 이미 피하고 있었음). `+9시간` 보정 추가. **이건 제보된 증상과 별개** — 새벽 시간대에 날짜 입력칸의 초기값·최대값만 하루 어긋나는 것이지, 직접 고른 날짜가 무시되는 문제는 아니었다.
