...
| 이번 주 하고싶은 게임에 취소된 테스트 게임 잔존 (2026-07-15 발견) | 이번 주 금요일 "하고 싶은 게임"에 예전에 테스트용으로 넣었다가 취소한 "얼티밋 레일로드"가 계속 표시됨. **원인 확인됨** (2026-07-15, 읽기전용 SELECT로 검증): `meeting_vote_games`에 orphan 행 2개 존재 — id=22(vote_date 2026-07-08, user_id=4916417947), id=36(vote_date 2026-07-17, user_id=4916417947) — 둘 다 대응하는 `meeting_votes` 행이 없음(참여 자체가 취소됨). `deleteMeetingVote`의 cascade 삭제(커밋 de89af8, 2026-07-14)가 이후 취소부터만 적용돼 그 이전 생성분(created_at 07-07·07-13)이 소급 정리 안 된 것으로 확정. **삭제는 미실행** — DB 행 삭제라 사용자 승인 후 진행 |
