...
최종 갱신: 2026-07-19 (세션 ⑪ — 관리자 화면 육안 점검 → 발견 #5·#6·#10·#13 수정 → **관리자 분석 전체를 [admin-analytics.md](admin-analytics.md)로 통합**. §3에서 127줄 감소).

> 📍 **관리자 분석을 만질 땐 [admin-analytics.md](admin-analytics.md)를 먼저 읽을 것** — 표시 원칙 5개·발견 대장 #1~#14·부위별 실행계획 P1~P7·재조사 금지 가설이 전부 거기 있다. 아래 §0 세션 요약의 `#숫자`도 그 문서의 발견 번호다. **최근 세션(⑪·⑩) 요약은 §0 상단.** 이전 세션(①~⑨) 상세는 git log + [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md), 그때 발생한 열린 항목·교훈은 전부 §3·CLAUDE.md·js-api.md로 이관 완료.
...
**세션 ⑫ (2026-07-19)** — 관리자 분석 **P1 도달률** 완료. 상세는 [admin-analytics.md](admin-analytics.md) §5 「이미 끝난 것」·§5-1.
...
- ✅ **P1 도달률** — 이벤트 탭 퍼널 위에 "방문 67명 중 43명 64% · 회원 11/11(100%) · 비회원 32/56(57%)". 절대수만으론 많고 적음을 판단할 수 없다는 원칙 ②의 직접 구현.
  ...
  - 📌 **부산물 발견 2건** → [admin-analytics.md](admin-analytics.md) §4. **#15**(방문자 도넛이 비회원을 행 단위로 세서 754명 vs 실제 348명, 2.2배) · **#16**(90일 컷이 2026-08-29경부터 물려 '전체' 도달률이 조용히 과대해짐, 현재 미발현).
...
**세션 ⑪ (2026-07-19)** — 「관리자 화면 눈으로 보기」로 시작해 **감사 → 수정 → 기획 → 문서 통합**까지 갔다. 상세는 전부 [admin-analytics.md](admin-analytics.md), 여기엔 흐름과 판단만 남긴다.

> **흐름**: ①6탭 육안 점검 → 표시층 문제 5건(#5~#9) 등록 ②#5·#6 수정(원인 하나) → 그 결과로 #10~#12 새로 드러남 ③사용자 요청("계열별로 묶어달라") 착수 중 **#13(이벤트 77% 미조회)** 발각 ④사용자 제안 3건("사람 단위") 기획 → **A·B단계 구현**, #10 부산물 해결 ⑤흩어진 6곳을 **admin-analytics.md로 통합**.
>
> **커밋 9개**: `abc7640`(하니스) `7f9cd7d`(발견 등록+프루닝) `3c1068f`(#5·#6) `552314a`(종결+신규3건) `c460d4c`(계열묶기+#13) `2f29389`(사람단위 기획) `fabb49e`(A단계) `a3e1844`(B단계) `580776b`(SSOT 통합)
>
> **코드가 바뀐 파일은 2개뿐**: `pages/admin/requests-admin.html`(대부분) · `assets/js/supabase-client.js`(`getEventCounts` select 확장). **변경 범위와 이유는 [admin-analytics.md §5-1](admin-analytics.md) 표에 전부 기재** — 다시 만질 때 그걸 볼 것.
...
- ✅ **"사람 단위" B단계 완료 — 방문자 카드에 페이지 분포 펼침** (사용자 제안 ②) — 회원·비회원 카드 공통 `📄 페이지 N개 ▾` → `페이지/진입/체류`(체류순 상위 10 + 외 N개). **죽은 렌더러를 그대로 되살리지 않고 계산부만 재사용**(옛 것은 전체 유저 아코디언이라 카드 UI와 형태 불일치) → 옛 렌더러 3개는 이제 **삭제 대상**. `dedupUserPageDay` 제외로 **체류 과소집계도 함께 해소**(회원/비회원 기준 일치). 파생 발견 #14(`page` 저장값이 라벨) 등록.
- ✅ **"사람 단위" A단계 완료 — 이벤트에 명·평균 + 전환율 사람 기준화** (사용자 제안 ①, 사용자가 착수 판단을 위임) — `getEventCounts`가 `user_id, session_key`를 함께 반환하도록 확장(가산적, 기존 소비처 무영향). 요약 계열 카드 `🤝 모임 1,050회 · 46명`, 퍼널 `132회 / 23명 · 평균 5.7`. **발견 #10이 부산물로 해결**(사람 집합 교집합 → 100% 초과 원리상 불가, 16칸 검사 0건). 회귀 가드를 하네스에 추가.
  - 📌 **A를 먼저 고른 근거**: #10은 **지금 화면에 떠 있는 틀린 숫자**였고, B는 정보를 더할 뿐이다. **거짓말 고치기 > 기능 추가.** 게다가 A의 식별자가 B·C의 토대라 순서상으로도 앞.
...
- ✅ **문서 통합 — [admin-analytics.md](admin-analytics.md) 신설** (사용자 지시: "전체 정리 및 통합 정리, 이후에 플랜, 부위별로 분리 후 하나씩") — 같은 주제가 **6곳**(§3 3곳 + 「론칭 후 이월」 + `PLAN_funnel_analytics.md`)에 흩어져 다음 세션의 판단을 막고 있었다. 도메인 SSOT로 합치고 **PROJECT_STATE는 549→424줄**. CLAUDE.md 필독표·갱신 트리거표에도 등재(다음 세션이 자동으로 읽게).
  - 📌 **원칙 5개를 도출했고 그게 이 세션의 진짜 산출물이다** — ①기준 표기 ②분모(같은 기간·모집단) ③파생값 중복 금지 ④한 화면 한 질문 ⑤드릴다운. **근거는 관념이 아니라 발견 #1~#14 자체**: 14건 중 "지표가 부족하다"는 **하나도 없었고 전부 "이 숫자가 무엇을 센 것인가"** 문제였다. 사용자가 말한 "유기적이지 않다"의 정체가 이것.
  - 📌 **`DESIGN_RULES.md`에 넣지 않은 이유(사용자 지적으로 교정)**: 그 문서는 팔레트·모바일·`border-radius`·sticky 등 **순수 시각 규칙**이라 범주가 다르다. ①②는 데이터 정의, ③④⑤는 정보 구조(IA)다.
...
- 🔍 **표시 층 문제 5건 실측 확정** → [admin-analytics.md](admin-analytics.md) §4 **발견 #5~#9**에 등록. 요지: **데이터가 없는 게 아니라, 있는 데이터를 잘못 보여준다.**
...
- 🔥 **관리자 페이지 감사 — "조용한 실패" 2건 규명 ✅** (단일버그 추적을 감사로 전환한 사용자 결정의 성과):
  ...
  - 상세·미수정 발견 2건(#3 `anonTodayS` 잠재버그 · #4 dead code)은 [admin-analytics.md](admin-analytics.md) §4.
...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🔴 **발견 #15 — 방문자 도넛 비회원 과대집계** (754명 표시 vs 실제 348명). **지금 화면에 떠 있는 틀린 숫자**라 P2보다 앞. legacy 585행 `session_key` 부재로 소급 dedup 불가 → "식별불가 N명" 표기 설계 필요. P1에서 분리하기로 한 그 건 | [admin-analytics.md](admin-analytics.md) §4 | 옐로 / Sonnet high |
| 2 | 🎯 **P2 — 기준 표기 정리** (#9 요약 카드 기간·말줄임 / #11 "7일" 버튼이 실제 이번 주 / #8 축 라벨 / #16 '전체'가 실은 90일). 전부 원칙 ①(무엇을·언제·누구를 셌는지) 위반 | 〃 | 그린·옐로 / Sonnet |
| 3 | 🎯 **P3 — 드릴다운** (이벤트·페이지 → **그걸 한 사람들**로). 원칙 ⑤의 최대 결손, "유기적"의 핵심. 그다음 **P4=C단계**(보드 연동, Red) | 〃 | Plan / Opus |
