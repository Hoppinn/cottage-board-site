...
**다음 세션 시작점**: **R8(SC4/SC5 — `getVisitorStats`/`getUserFirstRecordCount` 성능 개선, Opus xhigh, Plan 필수)** — RPC 신설 시 DB 변경 동반이라 Red. 이후 R9~R13은 REFACTOR_CHECKPOINT.md "처리 계획" 표 순서대로. (R7은 순수 삭제·behavior-preserving이라 별도 스모크 불필요, 다만 R1~R6 누적 브라우저 스모크는 여전히 권장.)
