...
- ~~**[4] 게임 상세시트 기록 미리보기 섹션 표시 버그**~~ — sheet-records-group(플레이기록·게임평·사진 미리보기 박스) 제거 (142차-107). openGameSheet에서 기록 섹션이 별도 시트처럼 보이는 문제. 게임 상세시트는 게임 정보만 표시, "기록 보기 & 남기기 →" 버튼으로 openGameRecordSheet 진입. initSheetCommentsPreview/PlayPreview/PhotoPreview 호출 제거.
...
- **[8] 더루프 사진 미표시** — try-catch 에러처리 추가됨. 실제 원인 미파악. DB에서 더루프 기록의 game_id 값과 photo_url 컬럼 직접 확인 필요
