...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🎯 **[기획] 관리자 분석에 "사람" 단위 도입 — A단계**(이벤트에 명·평균 붙이기). 사용자 제안 3건의 공통 의도. **발견 #10(전환율 100% 초과)이 여기서 함께 풀린다** — 사람 집합 교집합으로 세면 100%를 못 넘음. 실현가능성·자산·주의사항 실측 완료 | §3 | Plan+옐로 / Opus |
| 2 | 위 **B단계**(회원별 페이지 상세) — **렌더러가 이미 구현돼 있고 죽어 있음**(`renderUserDetailAll`), 카드 클릭에 연결만 하면 됨 | §3 | 옐로 / Sonnet high |
| 3 | **발견 #9(요약 카드 기간 표기·말줄임, Green)** + **#11(기간 버튼 라벨 "7일"인데 실제 이번 주)** — 둘 다 "화면이 기준을 잘못 말함" 같은 뿌리 | §3 | 그린·옐로 / Sonnet |
| 4 | **[감사] 관리자 페이지 — 잔여분**. 남은 것: #3(`anonTodayS`)·#4(dead code)·#7·#8(차트 축)·#12(회원탭 차트 기간 조작 수단 없음)·**요청관리 영역 미착수** | §3 | 감사 / Opus |
| 5 | 남은 버그: 기록보드 플레이기록 09:00 고정 / 서브시트 모서리 음영 / 단기방문 시간 미표시(`duration_sec=0` 14.7%) | §2 | 버그 / 개별 판단 |
| 6 | 기록게시판 디자인 개선 (+ GS5 esc 2사본 겸사겸사) | §3 | design·feat |
| 7 | GS5(escH ~11개) — **보류**(선행: 로드순서), 착수 시 REFACTOR_CHECKPOINT 필독 | §3·[REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) | 보류 / Opus xhigh+Plan |
...
- [ ] 🎯 **[기획] 관리자 분석에 "사람" 단위 도입 — 세 요청의 공통 의도** (2026-07-19 사용자 제안 3건, 미착수) — 사용자 원문: ①"단순 클릭수만 세지 말고 몇 명이 클릭했는지도. 평균 클릭수 정도?" ②"회원별로 어떤 사람이 어떤 페이지를 많이 보는지. 회원 카드 안에 넣든 닉네임 누르면 모달이 뜨든" ③"남의 보드에 들어가면 관리자에게만 그 사람이 어떤 페이지 많이 봤는지 보이는 'xx보드'". 사용자 단서: **"이렇게 해야 한다가 아니라 이 의도를 적중시킬 시스템이면 됨"**.
  - **공통 의도 = 지금 관리자 화면엔 "사람"이라는 단위가 없다.** 전부 행(이벤트·세션) 카운트라 "몇 건"만 보이고 "누가/몇 명"이 안 보인다. 세 요청은 같은 것의 세 표면이다.
  - ✅ **실현 가능성 실측 완료 (2026-07-19)**: `page_events.session_key` **97% 채워짐**(2026-07부터 **100%**, 06월은 8%뿐이라 그 구간 "명" 집계는 부정확 → 표기하거나 제외할 것), `user_id` 79%. 고유 user_id 14명·session_key 70개. `page_sessions` 11,523행 중 user_id 보유 10,352행, 고유 회원 21명·고유 페이지 35종.
    - **실측 예시(이 요청의 근거)**: 홈 모임 날짜칩 **443회/38명(평균 11.7)** · record_start **336회/36명(9.3)** · 모임 프로필 클릭 **75회/10명(7.5)**. 소수가 반복 클릭하는 패턴이 실재 → 클릭수만 보면 과대평가된다.
  - ⚠️ **선행 제약**: `getEventCounts`는 현재 `select('event_type, created_at')`뿐이라 **식별자를 안 가져온다**. `user_id, session_key` 추가 필요 = **반환 계약 변경**([js-api.md](js-api.md) 갱신 대상).
  - 🔑 **핵심 설계 판단 — 집계 로직을 한 벌로 유지할 것**: ③을 별도 "xx보드"로 따로 만들면 같은 집계가 두 벌이 되고, **2026-07-19에 겪은 드리프트가 그대로 재발한다**(조회 목록이 두 벌이라 이벤트 77%가 안 보였던 건 = 발견 #13). → **`buildMemberProfile(uid)` 한 개 + 뷰 한 개**를 만들고 ②③이 같은 걸 재사용한다.
  - 🎁 **이미 만들어져 있는 자산 (재작성 금지)**: ②의 렌더러가 **이미 구현돼 있고 죽어 있다** — `buildUserMap`([:929](../pages/admin/requests-admin.html#L929))·`renderUserDetailAll`([:969](../pages/admin/requests-admin.html#L969))·`renderUserDetailDay`([:1013](../pages/admin/requests-admin.html#L1013))가 회원별 `페이지/방문/체류` 아코디언을 그린다. 계산은 매번 돌지만(`userDayParts`·`userAllDetail`, [:1121·1129](../pages/admin/requests-admin.html#L1121)) **DOM에 안 붙는다**(참조 1회 = 선언뿐).
    - **왜 끊겼나**: `b3ea437`("어드민 비주얼 분석 UI 개선")이 회원 탭을 카드 UI로 바꾸며 `subSection('전체', userAllDetail, false) + userDayParts` 렌더 호출을 지웠고, **계산은 안 지웠다**. 의도적 교체였지 사고는 아니다 — 다만 그때 버린 정보가 지금 다시 필요해진 것. (별건: `39eb7e2`는 **페이지별** 테이블을 제거한 커밋으로 이것과 다름.)
  - **제안 단계 (각 단계가 독립적으로 가치 있음, A→B→C 순 추천)**:
    - **A. 이벤트에 "명" 붙이기** (요청①) — `getEventCounts` select 확장 → 요약 계열 카드에 `🤝 모임 1,050회 · 68명`, 이벤트 탭 각 단계에 `443회 / 38명 (평균 11.7)`. **가장 싸고, 발견 #10(전환율 100% 초과)이 여기서 같이 풀린다** — 사람 집합의 교집합으로 계산하면 100%를 넘을 수 없다("1-1 누른 사람 중 몇 %가 1-2도 눌렀나"). [PLAN_funnel_analytics.md](PLAN_funnel_analytics.md) 2단계의 "unique 카운트 기반 전환율"이 정확히 이것.
    - **B. 회원별 페이지 상세 되살리기** (요청②) — 죽은 렌더러를 회원 카드 클릭 펼침(또는 모달)에 연결. **이미 검증된 로직이라 비용 최소.**
    - **C. 남의 보드에 관리자 전용 섹션** (요청③) — B의 뷰를 재사용. **가장 비싸다**: 집계가 `requests-admin.html` 안에 있고 보드는 `kakao-auth.js`라 **공용 모듈로 추출**해야 재사용 가능. C를 먼저 하면 A·B 없이 로직이 흩어진다.
  - ⚠️ **주의 3가지**: ①**관리자(호핀)는 트래킹에서 영구 제외**(`_shouldSkipAnalytics`)라 이 지표에 절대 안 잡힌다 — 읽을 때 전제로 깔 것. ②**06월 구간은 session_key가 8%**라 "명" 수치가 과소집계된다. ③요청③은 **오너 가드가 필수**(데이터 자체를 비오너에게 fetch하지 않도록) — 회원 21명 규모라 개인 식별이 사실상 실명이다.
