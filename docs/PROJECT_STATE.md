...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🔴 **#21 회원 카드 총체류가 두 소스로 갈림** — 헤더=`profiles.total_minutes` vs 펼침=`page_sessions`. 회원 5/5 불일치(최대 −431분). **어느 쪽이 옳은지부터 판정**해야 하고 둘 다 편향돼 있어 조사 단계 분리 필요 | [admin-analytics.md](admin-analytics.md) §4 | 원인조사 / Opus |
