...
- 현재 단계: 사용자가 032를 운영 적용·확인하고 코드를 push했다. 난이도 선택 카드의 제목/범위/예시를 공통 2행 선택 카드 규칙으로 분리했고, 질문 블록 간격도 보정했다. 브라우저 인스턴스 부재로 360px 실화면 검수는 미실행이다.
- 다음 작업: 배포된 360px 화면에서 난이도 3개 그룹, 유형 카드, 선택·잠금·비활성 상태 및 마지막 카드와 고정 액션의 겹침을 확인한다.
