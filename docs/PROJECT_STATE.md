...
- 현재 작업: 회원 보드·플레이 기록·동호회 메뉴 연계 보강 조사/Plan 작성
- 상세 기획: [docs/plans/member-board-navigation-and-records-plan.md](plans/member-board-navigation-and-records-plan.md)
- 작업 순서: 1) 플레이 기록·동호회 기록 공통 아코디언 및 날짜별 클릭 버그 → 2) 햄버거 메뉴 active/ancestor → 3) 내·타인 보드 게시판 이동 CTA → 4) 게임도감 간격
- 현재 단계: 사용자 요구사항 1~8 접수 완료, 구현 전 영향 경로·우선순위 정리 완료
- 다음 작업: Plan 1단계로 플레이 기록 미리보기와 본 페이지의 renderer·이벤트 경로를 조사하고, 같은 원인/검증 경로끼리 묶어 기능별로 구현·검증한다.
...
- 현재 버그: 프로필 수정 버튼 불능, 보드 게시판 이동 CTA 부재, 플레이 기록 날짜별 클릭/아코디언 불일치, 동호회 기록 아코디언 부재, 동호회 메뉴 active/자동열림 오류, 게임도감 과도한 간격.
