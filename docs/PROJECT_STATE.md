...
**진행 중 작업 없음** (2026-07-18 세션 ⑧ 종료 시점) · **열린 스모크 2건**(플래너 깜빡임 수정·IP2 추천버튼 배선 — 아래 「열린 스모크」).

**세션 ⑧ (2026-07-18)**: ①**문서-코드 참조 정합성 감사 ✅ 종결** — 인덱스 3파일을 rg로 실코드 대조, 음성 대조군 검증 후 드리프트 3건 수정(header.js 역할목록 누락, CottageDB·getKakaoUser 소비처 script-nav.js 오기재). §3 「문서-코드 참조 정합성 감사」. ②**플래너 깜빡임 fix(커밋 4ab32d8)** — 홈 미리보기 빠른진입 시트 닫을 때 주간뷰가 한 프레임 켜지던 것을 `is-quick-entry` 토글 위치 이동으로 제거(close→cottage-reset-week). **시각 스모크 대기**. §2 참조. ③**IP2 ✅ 종결(2번 방안)** — 감사 전제("15+개 onclick") 틀림(실측 onclick 2개), 전체 IIFE 래핑은 R11c 위험 대비 이득 미미라 기각. 대신 onclick 2개를 addEventListener로 전환해 배선 균일화. **스모크 대기**(더보기→오버레이·✕닫기). REFACTOR_CHECKPOINT 「Phase 3」 IP2 행.
...
- **플래너 깜빡임 수정 (세션 ⑧, 2026-07-18)** — 홈 미리보기에서 ①닉네임 옆 ✎(수정) ②"+ 이날 참여 등록" ③빈 일정 "+ 플래너에서 등록하기" 각각으로 시간설정 시트를 연 뒤 **✕(또는 dim)로 닫을 때 주간뷰가 깜빡이지 않고 바로 닫히는지** 확인. 추가로 "모임 플래너 열기"(정상 오픈)에서 주간뷰가 정상 표시되는지도 함께(같은 클래스 토글 이동 영향). 모임보드 ✎ 편집 닫기도 여력 되면. 상세는 §2.
...
| ~~"이날 참여 등록"/"플래너에서 등록하기"/홈 미리보기 ✎ 닫을 때 플래너 깜빡임~~ | 🟡 **수정 적용, 시각 스모크 대기** (2026-07-16 등록 → 2026-07-18 근본원인 규명+수정). 재현 경로 추가 확인: 홈 미리보기 닉네임 옆 **✎(수정)** → 시간설정 시트 → **✕** 닫을 때도 동일 깜빡임(`.sched-bar-edit-btn`→`__openPlannerFor(ds,true)`→`cottage-edit`). **근본원인**: [club-schedule.html:898](../pages/club/club-schedule.html#L898) `close()`가 편집시트를 숨긴 **직후** `is-quick-entry`를 제거해 `#viewWeek`(주간뷰)가 `display:''`로 한 프레임 그려지고, **그 다음에** 비동기 `postMessage`로 부모가 모달을 닫음 → async 간극이 "켜졌다 꺼짐". 사용자 요구("아예 안 켜지게")대로 **클래스 토글 위치 이동**으로 해결: `close()`에서 제거 삭제, `cottage-reset-week`(주간뷰를 실제 보여주는 유일 진입점)에서 제거. 백지 배경(74f6345)과 별개 잔여였음. 공유 iframe이라 홈·모임보드 양쪽에 적용. **⚠️ paint 타이밍이라 시각 확인 필수** — 사용자 스모크 전 종결 금지. §0 열린 스모크 참조. |
