...
**진행 중 작업 없음** — 2026-07-27 세션 ㊴에서 사용자가 제보 15건을 일괄 등록, 5건 종결(직접방문 집계=정상, 이용시간 카드/막대 불일치=버그아님, 참여자 카운팅 누락=수정 `a7a1880a`, 간식알림 Part1=구현 `10839a00`, referrer 4곳 통일=수정 `2f0e2618`). **열린 버그 7건**(§2) · **열린 기능 5건**(§3, Plan 필요 2건 포함) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0** · **열린 스모크 6건**(§0 아래 「열린 스모크」). **미배포 커밋 있음(push는 사용자)**.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **업적 달성 알림 안 뜸** | §2 버그 1번 | 버그 | 그룹 C 잔여, 원인 미조사 |
| 2 | **최근소식 읽음처리 / 게임평→홈 반영 / 알림배지 — 새로고침해야 반영되는 3건** | §2 버그 2~4번 | 버그 | 각각 원인 미조사, 사용자 체감 빈도 높음 |
| 3 | **햄버거(⋯) 메뉴 잘림** | §2 버그 5번 | 버그(CSS) | 해법 패턴이 DESIGN_RULES.md 6-a에 이미 있어 적용만 하면 됨 — 쉬운 것 먼저 |
| 4 | **플래너 지난모임 탭 정렬 순서** | §2 버그 6번 | 버그 | 미조사, 범위 좁아 보임 |
| 5 | **PC 페이지 폭 — game-reviews.html 760 vs 720** | §2 버그 7번 | 확인 우선 | 1440px에선 시각적 차이 없었음 — **사용자가 본 화면이 이건지부터 재확인**, 그 다음 수정 여부 판단 |
| 6 | **간식·음료 알림 Part2(요청자 완료알림)** | §3 P1 2번 | 기능/Plan | DB 컬럼 신규 — Plan 모드 먼저 |
| 7 | **모임시간 30분단위 / 게임정리·룰설명 관리자추가 / QR 추천게임 집계** | §3 P1 3~5번 | 기능 | 명확한 스펙, 서로 독립 — 순서 무관하게 진행 가능 |
| 8 | 🚫 **「이날모임 상세창에 등록·수정 버튼」** — 승인 없이 만들었다가 2026-07-22 되돌림(git `d315b314`). 되살리려면 먼저 기획(진입점 개수·복귀 동선) | — | 기획 | 오늘 안건과 무관한 기존 항목, 결정 대기라 낮은 순위 유지 |
| 9 | **유형검사(MBTI식) 신규 기능** | §3 P2 1번 | 기획/Plan | 규모 최대 — 다른 항목 정리 후 |
...
**열린 버그 7건** (2026-07-27 세션 ㊴ 사용자 제보 일괄 등록·일부 원인 확인). 우선순위·착수 순서는 §0 표.

1. **업적 달성해도 알림이 안 뜸** — 원인 미조사. `achievements.js` 토스트 트리거 경로부터 확인 필요.
2. **최근소식(공지) 읽음 처리가 저장 안 됨 — 다시 뜸** — 원인 미조사.
3. **새 게임평 작성해도 홈 최근기록이 새로고침 전엔 안 바뀜** — 원인 미조사. 캐시/이벤트 무효화 쪽으로 추정.
4. **알림 확인해도 배지(점)가 새로고침 전엔 안 사라짐** — 원인 미조사. #2·#3와 "새로고침해야 반영" 패턴은 같지만 원인 공유 여부 미확인 — 따로 조사.
5. **게임기록페이지 ⋯(햄버거) 메뉴를 열면 아래가 잘림** — 미수정. `DESIGN_RULES.md` 6-a에 같은 유형(overflow:hidden이 팝업을 자름) 해법이 이미 문서화돼 있어 그대로 적용 가능해 보임 — 실제 선택자 확인 필요.
6. **플래너 "지난 모임에서 하고 싶었던 게임" 탭 정렬이 최신순 역순이 아님** — 원인 미조사.
7. **PC 페이지(1440px) 가로폭 — `game-reviews.html`만 `.inner-page-body` `max-width:760px`, 다른 내부 페이지는 전부 720px** — Playwright 실측(owned-games/about/guide/club/game-location/price-rules/club-*/admin/requests 전수, 1440px 기준)에서 나머지는 모두 정확히 720px로 일치, game-reviews만 760 이상치. ⚠️ **단 1440px에선 시각적 차이가 없었다**(부모가 이미 720으로 죄고 있어서) — 사용자가 실제로 본 폭 문제가 이건지, 아니면 다른 화면·다른 해상도인지 재확인이 먼저다.
