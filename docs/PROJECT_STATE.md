...
최종 갱신: 2026-07-22 (세션 ㉛ — A-8 캡션 종결, 데이터 판단 4건 결정, 뽁님 기록 연동 실행, iframe 표본 5/11 유지). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음** (2026-07-22 세션 ㉛ 종료 시점) · **바로 착수할 요청 2건**(표 0-a·0-b — 세션 끝에 들어와 미착수) · **열린 버그 0건**(§2) · **관리자 분석 발견 대장 열림 0건** · **열린 스모크 6건**(§0 아래 「열린 스모크」).
...
> 📌 **현재 목표**: **표 0-a·0-b부터 한다**(2026-07-22 사용자 요청, 세션 ㉛ 종료 직전에 들어와 미착수). 둘 다 사용자 입력이 이미 끝났고 바로 코드로 열 수 있다. 그다음 큰 덩어리는 **「관리자 분석 P3(드릴다운)」**이고 착수하려면 사용자의 「하자」가 필요하다(Plan 필수, 세션 하나 규모 — 회원 탭 기간 선택·탭 구조 압축이 **같은 UI를 공유**하므로 따로 하면 기간 선택기가 두 벌 된다).
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **0-a** | 🆕 **캡션복사를 플레이기록 「모임별 보기」에도 — 모든 모임에**(2026-07-22 사용자 요청). 지금은 `club-history.html` 안에만 있다 | [game-reviews.js](../assets/js/game-reviews.js) 모임별 뷰 | feat | **먼저 공용 함수로 뺄 것 — 복사 금지.** `buildCaption`/`formatPlayTime`/`formatScore`가 `club-history.html`에 인라인이라 그대로 베끼면 **양식이 두 벌**이 되어 한쪽만 고쳐진다(#15와 같은 병). ⟹ 영향: 모임 기록 페이지 + 플레이기록 모임별 뷰. ⟹ 경로: 세 함수를 `play-records-utils.js`로 이관 → 양쪽에서 호출 → `verify-history-caption.js`가 **이관 후에도 통과**하는지 확인(그게 회귀 검사다) |
| **0-b** | 🆕 **동호회 「기록 & 사진」에서 「보러가기」를 누르면 페이지가 통째로 넘어간다 — 돌아올 길이 없다**(2026-07-22 사용자 요청). 뒤로가기든 모달이든 필요 | `club-history.html` | feat | ⚠️ **「닫고 전환」인지 「겹쳐 쌓기」인지 먼저 판정할 것** — [PROJECT_STRUCTURE §2-A 6](PROJECT_STRUCTURE.md)의 기준은 **"닫은 뒤 무엇을 기대하는가"** 하나다. 2026-07-22에 이 판정을 틀려 이날모임 상세 버튼을 되돌렸다(표 3번). ⟹ 영향: 동호회 기록&사진 → 플레이기록 이동 동선 |
...
| 2 | **§5에 남은 코드 작업 1건 — 착수 전 사용자 입력이 필요하다.** 게임 검색 영어 제목 인식 → **선행 작업 「게임시트 영어제목 독립 표시」가 미착수**. 그 외 §5는 기획·디자인 판단 | §5 | 기획 | A-8(캡션)·A-10(지난 일정)은 2026-07-22 종결(git log). 0-a·0-b까지 닫고 나면 **선행 입력 없이 혼자 열 수 있는 코드 작업이 다시 0이 된다** — 그때는 「열린 스모크」(실기기·비관리자 로그인)나 P3 착수 승인이 필요하다 |
