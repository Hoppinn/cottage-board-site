...
**세션 ⑩ 진행 중** (2026-07-18) — 아래 「세션 ⑩」 참조. **열린 스모크 0건**.
...
- **개별 '읽음' 버튼이 실제로는 '모두 읽기' 🔴 Plan 진행 중** — [kakao-auth.js:1588](../assets/js/kakao-auth.js#L1588)이 `_markAllNotifSeen`을 호출. 읽음 상태가 `notif_seen_at` **타임스탬프 하나**뿐이고 알림도 DB 행이 아닌 파생 생성물이라 개별 읽음을 저장할 자리가 구조적으로 없음. **사용자 결정: 개별 읽음을 제대로 구현**(Red) → **코드 완료, ⏳ 마이그레이션 실행 + 스모크 대기**.
  ...
  - ✅ **마이그레이션 010 실행 완료** (2026-07-18 실측 — 컬럼 존재, 5행 전부 `[]`).
  - ✅ **엔드투엔드 스모크 통과 (2026-07-18, Playwright + 실운영 DB, 14케이스 전원 PASS)**. 알림 35건(미읽음 35) 상태에서:
    - **개별 읽음 → 35 → 34** (원래 버그면 0이 됐을 자리). 나머지 34개 읽음 버튼 유지.
    - **묶음(`new_intro`) 키 7개가 한 번에** `notif_read_keys`에 저장됨. `notif_seen_at`은 `null` 그대로 = 지평선 미변경 확인.
    - **뒤로가기→재진입 34 유지**(onLeave 스냅샷 실동작 확인) / **새로고침 후에도 34 유지**(DB 왕복 확인).
    - **모두 읽기 → 미읽음 0 + `notif_read_keys`가 `[]`로 비워짐**(무한증가 상한선 실동작 확인), `notif_seen_at`은 새 지평선으로 갱신.
    - 콘솔 에러 0건. 테스트가 건드린 운영 DB 행은 **원본값으로 복원 완료**.
    - 🔗 **부수 확인**: 이때 35건 중 27건이 '더보기' 목록(`.profile-notif-more-list`)에 있었고 모두 읽기 후 전부 0이 됨 → **커밋 8b5d9f7(8건 초과 버그)도 같은 실행에서 함께 검증**됐다.
  - 📌 **관찰(미처리)**: 알림 35건 중 **30건이 `voucher:*`** — 관리자만 받는 교환권 지급/사용 로그가 알림 목록을 지배한다. 의도된 사양인지 확인 필요. 확인 후 필요하면 §3에 정식 등록.
...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | **[감사] 관리자 페이지 전체** — 착수 시 **범위부터 합의**(요청관리/방문·이용분석/이벤트퍼널 3덩어리, 2100줄+) | §3 | 감사 / Opus |
...
- [ ] **[감사] 관리자 페이지(`requests-admin.html`) 전체** (2026-07-18 등록 — 사용자 결정: "페이지 전체 리팩토링 한다고 생각하고 감사부터") — 「금일이용데이터 간헐 미표시」 단일버그 추적을 **종료하고 대체**한 항목. 전환 사유: 증상이 오래돼 재현 조건이 불확실("어떤 에런지 긴가민가")하고, 콘솔 에러 0건 + 가설 3건 기각으로 **단일 원인 추적의 기대값이 낮아짐**.
  ...
  - 🔴 **감사 발견 #2 — PostgREST `max-rows`(1000) 절단으로 관리자 분석이 데이터의 9%만 본다 (미수정, 최우선)**:
    ...
    - ✅ **결정: Ⓐ `max-rows` 상향 (2026-07-18 사용자 승인)**. 코드 변경 0·회귀 위험 0으로 **정확성부터 즉시 회복**하고, 구조 개선(Ⓒ)은 트리거를 걸어 분리.
    ...
    - ⏳ **Ⓒ(집계 RPC) — 트리거 도달 시 착수. "언젠가" 아님**:
      - **착수 트리거(둘 중 하나)**: `page_sessions` **5만 행 초과** 또는 관리자 페이지 로딩 **3초 초과**. (※주당 ~750행 기준 5만까지 약 2년 추정 — 추정이지 보장 아님)
