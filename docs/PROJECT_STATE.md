...
| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🟡 **관리자 잔여 4건** — 퍼널 `home_record_write_click` 0건(#18) · `anonTodayS`(#3) · `page`가 라벨 저장(#14) · 알림 교환권 도배. 전부 같은 화면 + 🟡 + 서로 독립. *(#25는 2026-07-20 종결)* | [admin-analytics.md](admin-analytics.md) §4 | 그린~옐로 |
...
> 🚨 **1번 4건은 묶지 말고 하나씩 닫는다.** #25를 닫을 때 확인된 대로 **주제가 비슷해도 파일·원인 유형·검증 방법이 다르면 묶음 근거가 아니다**(CLAUDE.md 「닫기 우선」 A-3).
>
> 🔍 **#18은 "고칠 것"이 아니라 "잴 것"이다** — `home_record_write_click`은 **코드상 정상 배선**돼 있고(`index-page.js:1239`, 버튼 `index.html:224`), **2026-07-20 Playwright 실클릭에서 정상 발사됨을 실측**했다(#25 검증의 부산물). 따라서 코드 결손이 아니다. 🚨 **"아무도 안 누른다"로 단정하지 말 것**(2026-07-20에 실제로 그렇게 단정했다가 정정). 남은 후보 셋: ①정말 클릭이 없다 ②**관리자만 눌렀고 관리자는 트래킹 영구 제외**(`_shouldSkipAnalytics`) ③실서버 배포본이 로컬과 다르다. **먼저 어느 것인지 재고**, 그다음에 고칠지 정한다.
