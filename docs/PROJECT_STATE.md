...
- 현재 작업: 메인 보드 메뉴 카드 헤더 정리
- 상세 기획: 없음. 완료된 이전 작업의 정본은 [docs/plans/member-board-navigation-and-records-plan.md](plans/member-board-navigation-and-records-plan.md)
- 현재 단계: 게임도감 정렬·세로 우선 2열·무모션 토글 위치 보정은 사용자 확인 완료. 메인 보드의 프로필·모임·수집·기록·함께한 시간 카드 헤더를 아이콘 + `XX 보드 >` 한 줄 구조로 통일하고, 아이콘 간격 6px·헤더 아래 여백 추가를 반영했으며 사용자 확인 대기
- 다음 작업: 사용자가 메인 보드의 다섯 카드 헤더 배치와 수집 메뉴 표기를 확인한다. 로컬 브라우저 연결이 없어 자동 360px 화면 검수는 미실행.
- 인수인계: 기존 모임보드 개편 Plan의 1·2단계와 모임원 프로필 게시판 작업은 기존 기록을 보존한다. 이번 작업은 메인 보드 메뉴 카드 헤더의 문구·배치만 다룬다.
- 승인 대기: 구현 전 사용자 확인이 필요한 사업·권한 판단은 없음. DB/API·localStorage·인증 계약 변경이 발견되면 별도 Plan 승인 대상으로 전환한다.
- 현재 버그: 없음. 메인 보드 카드 헤더의 사용자 확인 대기.
