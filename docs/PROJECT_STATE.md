...
>
> 🏁 **종료 조건 = P3b(드릴다운)까지.** 사용자 요청 원문이 *"유효 데이터들이 더 직접적이고 **유기적으로** 드러났으면"*이었고, **"유기적" = 숫자에서 그걸 만든 사람으로 내려가는 길 = P3b**다. **P3b는 후속 확장이 아니라 완료 조건이다** — 이걸 안 하면 관리자 페이지는 미완이다.
>
...
> 🚨 **1번에 착수할 때 반드시 확인할 것 — 퍼널은 `record_*`를 쓰지 않는다.**
> 퍼널(`renderFunnel`)이 부르는 건 `home_*` 클릭 8개뿐이다(2026-07-20 코드 실측). 따라서 **#25(`record_start` 유령 발사)를 고쳐도 퍼널 숫자는 안 변한다** — 그건 요약 탭 계열 카드 합계와 히어로 통계에만 영향을 준다. 퍼널을 고치는 건 **#18 하나**다.
> ⚠️ **이 착각은 이미 두 번 났다** — 2026-07-19에 문서 전제를 믿고 착수했다가 틀린 걸 알았고, 2026-07-20에 다음 세션이 **이 줄을 읽고도** 다시 "#25와 #18은 퍼널의 분모·분자"라고 판단했다. **묶는 것 자체는 맞다**(둘 다 "기록 계열 신호가 사용자 행동과 안 맞는다"는 한 문제) — 틀리는 건 **완료 조건**이다.
