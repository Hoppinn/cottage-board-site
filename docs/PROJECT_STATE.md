...
- 현재 단계: 위저드·내 프로필 보드 정본·프로필 보드 미리보기 카드·모임 보드의 명칭·역할 규칙을 동기화했다. 프로필 보드 미리보기 카드는 게임 취향과 모임 참여 가능성만 압축하고, 모임 보드는 실제 일정·참여 등록·최근 참여만 보여준다. `usual_play_days/times`는 평소 습관, 기존 `available_days/times`는 모임 참여 가능 일정 정본으로 분리했다.
- 다음 작업: 360px 실화면에서 위저드·프로필 보드 미리보기 카드·모임 보드의 줄바꿈과 간격을 확인한 뒤, 모임원 목록 카드의 출력 밀도를 정리한다.
