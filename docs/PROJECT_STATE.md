...
- 현재 단계: 위저드 완료. 내 프로필 보드 정본에서 평소 플레이와 모임 플레이를 분리 출력하도록 1차 정리 완료. `usual_play_days/times`는 평소 습관, 기존 `available_days/times`는 모임 참여 가능 일정 정본으로 분리했다.
- 다음 작업: 내 프로필 보드 정본의 실화면 확인 후 모임원 목록 카드, 프로필 보드 미리보기 카드 순으로 출력 밀도를 정리한다.
