...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🎯 **"사람" 단위 B단계** (사용자 요청②: 회원별로 어떤 페이지를 많이 보는지) — **렌더러가 이미 구현돼 있고 죽어 있음**(`renderUserDetailAll`/`renderUserDetailDay`), 회원 카드 클릭 펼침 또는 모달에 연결. A단계 완료로 토대는 깔림 | §3 | 옐로 / Sonnet high |
| 2 | 🎯 **"사람" 단위 C단계** (요청③: 남의 보드에 관리자 전용 섹션) — B의 뷰 재사용. **집계를 공용 모듈로 추출해야** 함(`requests-admin.html` ↔ `kakao-auth.js`). **오너 가드 필수**(비오너에게 fetch 자체 금지) | §3 | Red+Plan / Opus |
