...
> 📌 **관리자 분석에 남은 큰 덩어리는 둘 다 Red+Plan이다** — **P4**(남의 보드에 관리자 전용 섹션, 오너 가드 필수) · **P6**(집계 정확도 = 추적 로직 재설계). 그 외 **「탭 구조 압축안」은 적용 여부만 결정하면 되는 상태**이고 [admin-analytics.md](admin-analytics.md) §5 「조건부 트리거」에 있다 — **내 추천은 적용 안 함**(원칙 ④를 이미 지키는 화면이고, 합치면 P3b 드릴다운까지 다시 손봐야 한다).
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | **관리자 분석 P6(집계 정확도) — "재설계"가 아니었음(2026-07-22 재측정).** ①0초=설계값 · ③기기중복=iframe#24 종결 · **④자정넘김=2026-07-22 수정** · **⑥로그인 잔여만 남음**. ⑥은 unload가 `page_sessions`를 안 써 로그인 세션 마지막 구간이 누락/지연되는 것 → `sendBeacon` 필요(저영향, 코드-only). Red+Plan 딱지는 과대책정이었다(DB·인증 무변경) | [admin-analytics.md](admin-analytics.md) §4 ④⑥ | 구조 | 남은 건 ⑥ 하나, unload 신뢰성 때문에 신중히 |
| 2 | **관리자 분석 P4(보드 연동) — Red+Plan, 실행 전 승인 필요.** 남의 보드에 관리자 전용 섹션, 오너 가드 필수(프라이버시) | [admin-analytics.md](admin-analytics.md) §5 | 구조 | 되돌리기 어려움 + 프라이버시 위험, P6 뒤 |
