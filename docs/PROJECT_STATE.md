...
🚧 **진행 중 (세션 ⑲)**: 모임 플래너 동반 지인 인원(§3 P1). **헬퍼·표시 8곳 완료(커밋 8e7481f3, 동작 무변경)** → ⏳ **다음 동작은 `docs/migrations/013`을 SQL Editor에서 실행하는 것**이고 그건 사용자만 할 수 있다. 실행 확인 후 ①`getMeetingVotes` select에 `guest_count` ②`upsertMeetingVote` 인자 ③입력 UI(스테퍼+직접입력, 상한 없음) ④막대·미니바 「+N」 표기. ⚠️ **실행 전에 select를 건드리면 PostgREST 400으로 플래너가 백지가 된다.**

(이전 상태: 2026-07-21 세션 ⑱ 종료 시점, **홈 히어로 정렬 버그 · 동호회 문구 플래너 기준 전면 교체 · 요금 변경 전체 공지 3건 종결**) · **열린 스모크 4건**(§0 아래 「열린 스모크」).
...
- **세션 ⑱**: 「검사기를 먼저 의심한다」가 **또 사고를 막았다** — 검증 스크립트에서 `setInterval`을 스텁했더니 undici가 `.unref()`를 못 불러 **모든 쿼리가 죽었는데 결과는 「0행」으로 보였다.** DB 에러 로그(감지기)가 없었으면 빈 결과를 정상으로 읽었을 것이다. 음성 대조군(`--negctl`)도 발동 — 기대값을 비틀어 그 줄만 🔴이 뜨는 걸 본 뒤에 통과를 믿었다. 「0건은 발견이 아니라 질문이다」도 발동: **문서가 "가장 유력"이라 적어둔 가설(게임평이 별도 테이블)이 실측으로 기각**됐고 실제 원인은 2순위였던 NULL 정렬이었다. **「닫기 우선」 A-7**(근원을 고치면 그 자리를 정리까지) 발동 — 같은 폴백 규칙이 세 곳에 흩어져 있던 것을 공용 헬퍼로 합쳤다(refactor 별도 커밋). 공지 카드 작업에선 **「신규 생성 전 기존 검색」**이 잠복 버그를 잡았다 — 기존 공지 장치를 재사용하려고 읽다가 `_markRewardCardSeen`이 `querySelector` **단수**임을 발견했다. 새 장치를 따로 만들었으면 안 봤을 자리고, 그대로 뒀으면 「모두 읽기」가 두 카드 중 하나를 조용히 남겼다.
- **재감사 카운터**: 감사 시행이 2026-07-20(세션 ⑭), 세션 ⑱이 **4회차**. 트리거는 5회 → **다음 세션이 재감사 시점**이다. ⚠️ **세션 ⑰은 카운트하되 감사 자체를 손댄 세션**이다 — CLAUDE.md에서 2건을 커맨드로 분리했다가 `wrap`은 되돌렸고(매 세션 발동 규칙은 상주), 그 판정 기준이 「재감사 때 볼 것」 **4번**으로 들어갔다.
...
>
> 🏁 **1번(`guest_count`)의 종료 조건**: 플래너에서 동반 지인 수를 입력하면 **「N명」을 보여주는 5개 화면이 전부 같은 숫자**를 낸다(등록 1건·방문 3명이 어디서나 3명). **DB 변경이라 착수 첫 동작은 Plan이고, 코드보다 `calcOverlap`의 `names`/`count` 분리가 먼저다** — §3 P1의 「수정·검증 경로」 참조.

> 🔀 **2026-07-21 재분류.** 사용자 요청으로 순서를 다시 짰다. 기준은 CLAUDE.md B-5·B-6 — **①확인·문구로 닫히는 것 → ②판단이 필요한 버그 → ③기능·구조(Plan) → ④기획**, 그 안에서 **지금 사용자의 관심(모임팟·홈 첫 화면)이 먼저**다. (세션 ⑱에서 옛 1·2번(홈 히어로·요금 문구)이 닫혀 번호가 두 칸 당겨졌다.)

| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 🎲 **모임 플래너 동반 지인 인원(`guest_count`)** | §3 P1 | 기능+DB·Plan | **지금 사용자의 관심사**(모임팟 수요가 실제로 발생). ⚠️ **회칙 문구는 이미 확정 정책대로 고쳐졌다**(세션 ⑱) — 이 작업은 회칙 75행의 `지인 2인 포함 3인` **텍스트 신고를 입력 필드로 대체**하는 것이고, 완료 시 그 줄을 "플래너에 등록"으로 줄일 수 있다 |
| 2 | 🚫 **「프로필 보드 4건 묶음」은 묶음이 아니었다** (2026-07-21 독립 진단) — 4건 중 **2건은 이미 닫혀 있었고**(수집보드 스크롤=커밋 db08f332에서 해결됨, 카드 요약=2026-07-21 수정), 남은 2건은 **원인·코드 경로·검증 환경을 하나도 공유하지 않는다**: 박스모달 닫힘은 z-index 레이어 문제(§3), 타인 보드 네비는 표현 통일 문제(§3). **따로 처리할 것** | §3 | 개별 판단 | 원래 대기열 선두였다가 밀린 것 |
| 3 | 🔔 **관심 기반 묶음 알림** | §3 | Red+Plan | 무거운 신규 기능 |
| 4 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 | 전부 **재현부터** 필요. 하나씩 독립 |
