...
최종 갱신: 2026-07-22 (세션 ㉖ — 홈 센터모달 레이어 · 회원 탭 정렬). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
>
> 📌 **현재 목표**: 표 1번은 **데이터 대기**라 사람이 할 게 없고, 남은 실착수 후보는 **2번(GS5) 하나**다. 다른 걸 하고 싶으면 §5(기획·디자인)에서 고르면 된다.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | ⏳ **iframe 3배 계상 수정의 사후 확인 — 재개 조건이 숫자로 확정됨.** `node scripts/verify-iframe-precondition.js`가 **「로그인 상태 홈 방문 11건」**이 찼는지까지 알려준다. 그게 차면 `verify-iframe-rows.js`(음성 대조군 `--negctl` 먼저) | [admin-analytics.md](admin-analytics.md) §4 ①·⑦ | 검증 | **2026-07-22 측정: 표본 2/11건이라 판정 불가.** ⚠️ `verify-iframe-rows.js`는 이날 **"✅ 0건"을 내놨지만 그건 무증상 구간이라는 뜻**이었다 — 그 스크립트만 보고 종결하지 말 것. **#28 수정도 같은 대기 중**이라 한 번에 잰다 |
| 2 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan | 사용자 눈에 안 보이는 리팩토링 |
