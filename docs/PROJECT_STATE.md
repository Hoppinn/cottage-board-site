...
최종 갱신: 2026-07-22 (세션 ㉜ — 게시판 요청 5건 종결: 닉네임 클릭·복귀 링크·닉네임 공백·캡션 자리·⋯ 메뉴 잘림). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음** (2026-07-22 세션 ㉜ 종료 시점) · **열린 버그 0건**(§2) · **관리자 분석 발견 대장 열림 0건** · **열린 스모크 7건**(§0 아래 「열린 스모크」).
...
> 🔴 **가장 먼저: 8/01 시행이 열흘 앞이다** — 아래 「열린 스모크」의 **①회칙 전문 반영(사용자 작업) ②요금 공지 실기기 확인** 둘 다 **8/01 전에** 끝나야 한다. 날짜가 박힌 항목이 쉬움·어려움보다 앞이다(CLAUDE.md B-9). **세션 첫 보고에서 이 둘을 사용자에게 먼저 꺼낼 것.**
> 📌 **그다음 큰 덩어리**: **「관리자 분석 P3(드릴다운)」** — 착수하려면 사용자의 「하자」가 필요하다(Plan 필수, 세션 하나 규모. 회원 탭 기간 선택·탭 구조 압축이 **같은 UI를 공유**하므로 따로 하면 기간 선택기가 두 벌 된다).
...
| 2 | **§5에 남은 코드 작업 1건 — 착수 전 사용자 입력이 필요하다.** 게임 검색 영어 제목 인식 → **선행 작업 「게임시트 영어제목 독립 표시」가 미착수**. 그 외 §5는 기획·디자인 판단 | §5 | 기획 | **혼자 열 수 있는 코드 작업은 현재 0건이다** — 위 8/01 마감 2건, 「열린 스모크」(실기기·비관리자 로그인), P3 착수 승인 중에서 사용자에게 꺼내 놓을 것. **그게 세션의 끝이 아니다**(B-9) |
...
- [ ] 🔴 **[Plan 필수] 한 플레이기록에 여러 사람의 게임평** (2026-07-15 요청 → 2026-07-22 재정의) — 원래 요청은 「뽁님 7/11 게임평을 호핀 기록에 연결」이었고, **사용자가 원한 모양은 「기록 1개에 뽁님 게임평만 달리는 것」**이다.
  - ⚠️ **앱에 그 자리가 없다(2026-07-22 실측)**: 화면은 **기록 1개당 게임평 1개**만 그리고 그 이름은 **기록 주인**으로 나온다([game-reviews.js:1271](../assets/js/game-reviews.js#L1271)의 `r.review_text`+`r.nickname`). ⋯메뉴 「게임평 추가」도 **"내 기록 중" 게임평 없는 것**만 후보로 올린다([game-sheet.js:1690](../assets/js/game-sheet.js#L1690)) → 남의 기록을 고를 수 없다.
  - ⚠️ **「남의 세션 참여」로는 안 된다 — 실행해보고 되돌렸다.** 그 경로는 **새 기록을 만드는** 모델이라 같은 게임이 두 번 뜨고 원본이 `(2번째 플레이)`로 밀린다(`orderMap`, [:1294](../assets/js/game-reviews.js#L1294)). 2026-07-22 삽입 후 사용자 확인으로 삭제(`scripts/link-bbok-0711-records.js --undo`).
  - ⟹ **영향**: 플레이기록 게시판의 기록 카드(모임별·게임별 양쪽), 그리고 게임시트 게임평 목록.
  - ⟹ **경로**: 저장 자리 신설이 선행이다 — `game_comments`에 `record_id`를 붙일지 별도 테이블을 둘지 = **DB 변경이라 Plan 필수**. 그 뒤 ①기록 아래 여러 게임평 렌더 ②남의 기록 ⋯에서 「게임평 추가」 진입 허용.
  - 📌 **원본 데이터는 그대로다** — 뽁님 `game_comments` 3건은 한 번도 지우지 않았다.
  - 🚨 **이 건에서 내가 네 번 어긋났다.** ①「남의 세션 참여」의 경로별 동작은 [js-api.md](js-api.md) `onSubmitCommentModal` 줄에 **이미 정확히 적혀 있었는데** 코드만 읽고 대표 경로로 요약하다 두 번 틀렸다 → **game-sheet 계열은 js-api.md의 해당 줄을 먼저 읽을 것.** ②그 오해 위에서 사용자 요청을 「안 해도 된다」로 닫았다. ③"애초에 그거 하려고 적힌 TODO"라는 지적을 받고서야 실행했다. ④**그러고도 「무엇이 만들어지는지」를 화면으로 확인하지 않고 삽입**해, 사용자가 스크린샷으로 잡아줬다. **운영 DB에 쓰기 전에 「그래서 화면이 어떻게 되는가」를 먼저 그려볼 것.**
