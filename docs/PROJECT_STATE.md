...
> ✅ **「캡션복사를 플레이기록 모임별 보기에도」(0-a)는 2026-07-22 「넣지 않는다」로 결정 — 재등록 금지.** 캡션은 **하루치 기록** 단위인데 플레이기록의 타이틀은 **모임** 단위라 「타이틀에 하나」가 성립하지 않고(어느 날짜인지 못 정한다), 날짜마다 붙이면 버튼이 수십 개가 되어 「내 기록 관리」 화면의 성격이 흐려진다. **공유는 동호회 기록&사진 페이지가 하는 일**이고 거기엔 이미 있다. 대신 **두 게시판을 오가는 길(0-b)을 고쳐** 건너갔다 돌아올 수 있게 했다. 되살리려면 「동호회 페이지를 거치지 않고 캡션이 필요한 상황」을 먼저 대야 한다.
...
- **🙋 「덕지」가 회원인지 확인 — 사용자만 답할 수 있다 (2026-07-22)** — 참여자 태그 177개 중 154개(87%)가 회원과 연결되는데, 안 열리는 이름 6종 중 **「덕지」만 13회로 유독 많다**(나머지는 2회 이하 = 비회원 손님으로 보임). **닉네임을 바꾼 회원이면** 그 13개 태그가 계속 죽어 있으므로 `profiles`에서 현재 닉네임을 찾아 이어주면 되고, **비회원이면 지금이 정상**이라 할 일이 없다. 재측정: `node scripts/audit-nick-click.js`.
