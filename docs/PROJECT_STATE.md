...
**진행 중 작업 없음** — 2026-07-27 세션 ㊴ 사용자 제보 15건 중 9건 종결(완료 내역은 git log, `a7a1880a`~`2c8e6552`). **열린 버그 2건**(§2) · **열린 기능 4건**(§3, Plan 필요 2건 포함) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0** · **열린 스모크 6건**(§0 아래 「열린 스모크」). **미배포 커밋 있음(push는 사용자)**.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **플래너 지난모임 탭 정렬 순서** | §2 버그 1번 | 버그 | 미조사, 범위 좁아 보임 |
| 2 | **PC 페이지 폭 — game-reviews.html 760 vs 720** | §2 버그 2번 | 확인 우선 | 1440px에선 시각적 차이 없었음 — **사용자가 본 화면이 이건지부터 재확인**, 그 다음 수정 여부 판단 |
| 3 | **간식·음료 요청 Part2(관리자 처리됨 표시 + 요청자 완료알림)** | §3 P1 2번 | 기능/Plan | DB 컬럼 신규 — Plan 모드 먼저 |
| 4 | **모임시간 30분단위 / 게임정리·룰설명 관리자추가 / QR 추천게임 집계** | §3 P1 3~5번 | 기능 | 명확한 스펙, 서로 독립 — 순서 무관하게 진행 가능 |
...
**열린 버그 2건** (2026-07-27 세션 ㊴ 사용자 제보 일괄 등록·일부 원인 확인). 우선순위·착수 순서는 §0 표.

1. **플래너 "지난 모임에서 하고 싶었던 게임" 탭 정렬이 최신순 역순이 아님** — 2026-07-27 실DB로 정렬·중복제거 로직을 그대로 재현했더니 **날짜 순서 자체는 정확했다**(신규순 확인). 코드 주석에 이미 적힌 별개 위험: 슬러그(구)/bggId(신) 표기가 섞이면 정규화 실패 시 **같은 게임이 두 줄로 중복 표시**될 수 있음 — 사용자가 본 "이상함"이 순서가 아니라 이 중복일 가능성. 이 UI는 날짜 선택 후에만 열리는 다단계라 자동화로 재현 못 함 — **재현 스크린샷 또는 재현 단계(날짜) 필요**.
2. **PC 페이지(1440px) 가로폭 — `game-reviews.html`만 `.inner-page-body` `max-width:760px`, 다른 내부 페이지는 전부 720px** — Playwright 실측(owned-games/about/guide/club/game-location/price-rules/club-*/admin/requests 전수, 1440px 기준)에서 나머지는 모두 정확히 720px로 일치, game-reviews만 760 이상치. ⚠️ **단 1440px에선 시각적 차이가 없었다**(부모가 이미 720으로 죄고 있어서) — 사용자가 실제로 본 폭 문제가 이건지, 아니면 다른 화면·다른 해상도인지 재확인이 먼저다.
