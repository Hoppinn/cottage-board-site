...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🎯 **"사람" 단위 C단계** (요청③: 남의 보드에 관리자 전용 섹션) — B의 표(`_visitorPagesHtml`) 재사용. **집계를 공용 모듈로 추출해야** 함(`requests-admin.html` ↔ `kakao-auth.js`). **오너 가드 필수**(비오너에게 fetch 자체 금지) | §3 | Red+Plan / Opus |
| 2 | **발견 #9·#11**(기간 표기·버튼 라벨) + **#14**(page 저장값이 라벨이라 이름 바꾸면 집계 분산) — "화면이 기준을 잘못 말함" 계열 | §3 | 그린·옐로 / Sonnet |
| 3 | **[감사] 관리자 페이지 — 잔여분**. 남은 것: #3(`anonTodayS`)·#4(dead code)·#7·#8(차트 축)·#12(회원탭 차트 기간 조작 수단 없음)·**B가 대체한 죽은 렌더러 3개 삭제**·**요청관리 영역 미착수** | §3 | 감사 / Opus |
...
- [ ] **[감사] 관리자 페이지(`requests-admin.html`) 전체** (2026-07-18 등록 — 사용자 결정: "페이지 전체 리팩토링 한다고 생각하고 감사부터") — 「금일이용데이터 간헐 미표시」 단일버그 추적을 **종료하고 대체**한 항목. 전환 사유: 증상이 오래돼 재현 조건이 불확실("어떤 에런지 긴가민가")하고, 콘솔 에러 0건 + 가설 3건 기각으로 **단일 원인 추적의 기대값이 낮아짐**.
  ...
  - [x] ~~🔴 **발견 #10 — 전환율이 100%를 넘는다**~~ — ✅ **2026-07-19 해결**. 「사람 단위 도입」 **A단계에서 함께 풀렸다** — 별도 수정이 아니라 부산물이다. 전환율을 **사람 집합의 교집합**으로 계산(`|앞∩뒤| / |앞|`)하니 **교집합 ⊆ 앞단계라 원리상 100%를 넘을 수 없다**. 실측 16칸 검사 **초과 0건**(이전 167%·118%).
    - **수치가 달라졌고 그게 정상**: `3-1→3-2`가 건수 기준 94%였는데 **사람 기준 70%**(히어로 모임을 누른 23명 중 16명이 플래너도 봄). 건수 기준은 "많이 누른 사람"이 비율을 부풀렸던 것.
    - ⚠️ **한계 명시**: 순서를 보지 않는다(= "둘 다 한 사람" 비율). 엄밀한 "앞 단계 **이후에** 했나"는 세션·시각 비교가 필요 → 필요해지면 별건.
    - 🧰 회귀 가드: `scripts/shot-admin-tabs.js`가 **100% 초과를 자동 검출**(넘으면 건수 기준으로 회귀했다는 신호).
    - ⚠️ **이전 세션의 내 제안은 철회됨**: "전환율 표기를 버리고 건수 비교로 개명(ⓑ)"을 권했으나, A단계 덕분에 **버릴 필요 없이 제대로 고쳤다**(ⓐ에 가까움). 아래 원래 기술은 대조용 보존.
...
- [ ] 🎯 **[기획] 관리자 분석에 "사람" 단위 도입 — 세 요청의 공통 의도** (2026-07-19 사용자 제안 3건, 미착수) — 사용자 원문: ①"단순 클릭수만 세지 말고 몇 명이 클릭했는지도. 평균 클릭수 정도?" ②"회원별로 어떤 사람이 어떤 페이지를 많이 보는지. 회원 카드 안에 넣든 닉네임 누르면 모달이 뜨든" ③"남의 보드에 들어가면 관리자에게만 그 사람이 어떤 페이지 많이 봤는지 보이는 'xx보드'". 사용자 단서: **"이렇게 해야 한다가 아니라 이 의도를 적중시킬 시스템이면 됨"**.
  ...
  - **제안 단계 (각 단계가 독립적으로 가치 있음, A→B→C 순 추천)**:
    - [x] ~~**A. 이벤트에 "명" 붙이기**~~ (요청①) — ✅ **2026-07-19 완료·검증 통과**. `getEventCounts` select에 `user_id, session_key` 추가(**가산적이라 기존 소비처 무영향** — `index-page.js`는 `event_type`/`created_at`만 읽음, [js-api.md](js-api.md) 갱신). 요약 계열 카드 `🤝 모임 1,050회 · 46명`, 퍼널 각 단계 `132회 / 23명 · 평균 5.7`, **전환율을 사람 집합 교집합으로** → 발견 #10 동반 해결(위 참조). 사람 식별 `user_id || session_key`, 식별자 없는 3%는 명 집계 제외.
      - **왜 A를 먼저 했나**(사용자 "판단하에 더 나은 것"): #10이 **지금 화면에 떠 있는 틀린 숫자**(167%)였다 — B는 정보를 더하지만 A는 거짓말을 고친다. 게다가 A의 식별자 데이터가 B·C의 토대다.
    - [x] ~~**B. 회원별 페이지 상세**~~ (요청②) — ✅ **2026-07-19 완료·검증 통과**. 방문자 카드(회원·비회원 공통)에 `📄 페이지 N개 ▾` 토글 + 펼치면 `페이지 / 진입 / 체류` 표(체류 내림차순, 상위 10개 + "외 N개" 합계). 위임 리스너는 밴 버튼과 같은 방식(`el.addEventListener`, 카드가 매 렌더 새로 생기므로).
      - ⚠️ **죽은 렌더러를 그대로 되살리지 않았다** — `renderUserDetailAll`은 전체 유저를 아코디언 목록으로 그리는 **별도 섹션**이라 지금의 카드 UI와 형태가 안 맞는다. 계산부(`buildUserMap`)만 재사용하고 표현은 카드에 맞게 새로 썼다. **옛 렌더러 3개는 여전히 죽어 있다**(`renderUserDetailAll`·`renderUserDetailDay`·`userDayParts`/`userAllDetail`) → 이제 대체됐으므로 **삭제 대상**, §3 「감사 잔여」에 함께 처리.
      - 🔑 **`dedupUserPageDay`를 쓰지 않은 이유(중요)**: 옛 코드는 `buildUserMap(dedupUserPageDay(rows))`였는데, dedup이 (유저·페이지·일)을 한 행으로 접어서 **체류시간이 그날 첫 세션 것만 합산**된다(= 과소집계). 비회원 경로(`buildAnonUserMap`)는 원본을 쓰고 있어 **회원/비회원 간 기준도 어긋났다**. 새 코드는 양쪽 다 **원본 `rows`** 사용.
      - **검증**: 토글 286개(페이지 데이터 있는 카드), 초기 열림 0, 펼침·접힘 정상, **밴 confirm 오발동 없음**(같은 카드 안 버튼이라 확인함), 콘솔 에러 0.
  ...
  - 🟡 **파생 발견 #14 — `page_sessions.page`가 슬러그가 아니라 "표시 라벨"을 저장한다** (2026-07-19 B단계 중 실측, 미수정) — 고유 `page` 값 42종 중 대부분이 이미 한글(`메인` 2,869 · `플레이 기록` 1,677 …)이다. 그래서 `_pageLabels[r.page]` 조회는 **거의 항상 실패하고 폴백(`|| r.page`)이 결과적으로 잘 보이게** 만든다. 두 가지 부작용:
    - ① **라벨을 바꾸면 과거 데이터가 쪼개진다** — `가격 & 규칙`(174행)과 `가격·이용안내`(65행)가 **같은 페이지인데 별개 버킷**으로 남아 있다(이름 변경의 흔적). 집계가 조용히 분산된다.
    - ② **경로가 그대로 노출되는 잔여 행**: `/pages/info/guide.html`(153) · `/pages/club/club-rules.html`(46) · `/pages/club/club-meeting.html`(16) 등 **약 224행(전체의 2%)**. 그중 `/pages/cottage/…`는 **존재하지 않는 디렉터리**(현재 `pages/club/`)라 옛 URL 구조의 레거시.
    - **성격**: 소급 보정은 어렵고(과거 행), 근본 해결은 "저장은 안정적 슬러그로, 표시할 때만 라벨" 구조로 바꾸는 것 → 「[통합] 방문/이용 집계 전면 점검」과 같은 성격. **지금 급하지 않음**(2%).
    - ⚠️ **검사기 주의**: "라벨 매핑이 없는 값"을 세면 8,897행이 나오는데 **대부분은 이미 한글이라 화면상 멀쩡하다**. 실제 문제는 `/`로 시작하는 값뿐 — 이 구분 없이 세면 문제를 40배 과대평가한다(실제로 이번에 그랬다).
