...
**진행 중 작업 없음** (2026-07-21 세션 ㉑ 종료 시점, **「프로필 보드 4건 묶음」 전부 소멸** — 박스모달 수정 + 타인 보드 네비 오탐 확인) · **열린 스모크 5건**(§0 아래 「열린 스모크」).
...
>
> 📌 **다음 목표(2026-07-21)**: 관리자 화면에서 **회원이 보는 화면**으로 옮겨간다 — 홈 첫 화면과 동호회 모임 운영. 아래 표는 우선순위 순 대기열이다. (#14는 2026-07-20 종결 — [admin-analytics.md](admin-analytics.md) §4.)
>
> 🔀 **2026-07-21 재분류.** 사용자 요청으로 순서를 다시 짰다. 기준은 CLAUDE.md B-5·B-6 — **①확인·문구로 닫히는 것 → ②판단이 필요한 버그 → ③기능·구조(Plan) → ④기획**, 그 안에서 **지금 사용자의 관심(모임팟·홈 첫 화면)이 먼저**다. (세션 ⑱·⑲에서 옛 1~3번(홈 히어로·요금 문구·`guest_count`)이 닫혀 번호가 세 칸 당겨졌다.)

> ✅ **홈 상태 문구도 동반 인원 포함으로 닫힘 (2026-07-21, 사용자 결정)** — 규칙은 `sumWeeklyPartySize`(유저별 최대치 합산)이고 [js-api.md](js-api.md)가 SSOT. **"이번 주 최대 인원(날짜별 max)"은 채택하지 않았다** — 그 방식은 동반 0일 때도 숫자가 달라져 회귀가 난다.

| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 🎲 **플래너 Step2 게임 등록창 2건** — ①「배우고 싶은 게임」 토글이 위 목록에 붙어 보임(간격) ②지난 모임의 want/learn 불러오기 버튼 신설 | §3 P1 | 그린+옐로 | **같은 파일·같은 렌더 함수(`renderStep2Games`)·같은 검증 화면**이라 A-3 근거 충족 → 묶는다. 사용자 눈에 바로 보이고 ①은 CSS 한 줄 |
| **2** | 🧑 **대표 캐릭터·칭호 변경이 즉시 반영 안 됨**(새로고침해야 바뀜) | §2 | 개별 판단 | 매번 재현되는 회귀 + 사용자가 방금 겪음. 재현부터 |
| 3 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 | 전부 **재현부터** 필요. 하나씩 독립 |
| 4 | 🔔 **관심 기반 묶음 알림** | §3 | Red+Plan | 무거운 신규 기능 |
| 5 | 🟡 **#28 `page_sessions.referrer`도 라벨/pathname 저장** — 외부 유입 체류가 전부 「직접 방문」으로 접힌다. #14와 같은 파일이지만 **다른 화면·다른 판정**이라 분리 | [admin-analytics.md](admin-analytics.md) §4 | 옐로 | 관리자만 보는 화면 |
| 6 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan | 사용자 눈에 안 보이는 리팩토링 |

> 🔀 **2026-07-21 2차 재분류 (신규 3건 등록에 따른 것)**: 새 1·2번이 옛 1~4번 앞으로 갔다. 근거는 B-6 「목표 안에서 쉬운 것부터」 — **현재 사용자의 관심이 모임 플래너·프로필 화면**이고, 새 1번은 CSS 한 줄이 섞인 그린이며, 새 2번은 **사용자가 방금 실제로 겪은 회귀**다. 관리자 화면(#28)과 리팩토링(GS5)은 그대로 뒤에 남긴다.
> ⚠️ **1번에 착수하면 「열린 스모크」의 「동반 인원 실기기 확인」도 같은 화면에서 함께 볼 것**(플래너를 만질 때가 조건이었다).

> ✅ **「프로필 보드 4건 묶음」은 2026-07-21 전부 소멸했다** — 4건 중 2건은 이전에 닫혔고, 박스모달은 이날 수정, **타인 보드 네비는 실측 결과 이미 해소돼 있었다**(아래 §3). 묶음으로 다뤘으면 남은 1건이 "미착수"로 계속 끌려다녔을 항목이다.
...
- **🎲 동반 인원 실기기 확인 (다음에 플래너를 만질 때)** — 스테퍼는 로그인 없이 DOM만 띄워 봤다(쓰기 차단). **실제 로그인해 동반 2명으로 등록**한 뒤 ①플래너 카드·월간칩·홈 칩·이날 상세가 전부 **3명**인지 ②수정 진입 시 스테퍼에 **2가 그대로** 떠 있는지(0으로 덮이면 동반 인원이 사라진다) 확인할 것. ⚠️ **기존 데이터에 우회 사례가 있다** — 7/20 등록이 게임명 칸에 「지인팟 4인」이라 적어 신고 중이고 화면엔 1명으로 뜬다. 그 등록의 `guest_count`를 채울지는 **사용자 판단**(남의 등록).
...
| **대표 캐릭터·칭호 변경이 즉시 반영 안 됨** (2026-07-21 사용자 보고) | 성장보드에서 대표 캐릭터를 바꾸면 **새로고침 전까지 옛 캐릭터**가 남는다. 사용자는 칭호도 같을 것으로 추정 — **확인 대상**.<br>⟹ **영향 대상**: 프로필 패널 아바타·햄버거 메뉴 아바타, 그 외 아바타/칭호를 그리는 화면 전부(미확정 — 실측 필요).<br>⟹ **수정·검증 경로**: 저장 핸들러는 [achievements.js:863](../assets/js/achievements.js#L863) `handleRepCardSelect` / [:588](../assets/js/achievements.js#L588) `handleRepTitleSelect`. 🚨 **"핸들러가 갱신을 안 한다"가 아니다** — 둘 다 이미 패널 아바타·메뉴 아바타(`#kakaoProfileImg`)·패널 칭호 텍스트를 **그 자리에서 갱신하는 코드를 갖고 있다**. 따라서 유력 가설은 ①경로 계산(`_charImgPath`/`getCharacterPath`)이 null을 반환해 `if (… && _repCharPath)`가 통째로 스킵 ②갱신 뒤 **다른 렌더러가 낡은 캐시(`userStats`/`_boardData`)로 다시 그림** ③사용자가 본 곳이 그 두 자리가 아닌 제3의 화면. **착수 첫 동작은 가설이 아니라 재현** — 어느 화면이 안 바뀌는지부터 특정한다. 완료 확인: 새로고침 없이 캐릭터·칭호를 바꾼 뒤 그 화면들이 즉시 바뀐다 |
...
- [ ] **🎲 플래너 Step2 게임 등록창 2건** (2026-07-21 사용자 요청, 스크린샷 있음) — **같은 렌더 함수 `renderStep2Games`([club-schedule.html:1368](../pages/club/club-schedule.html#L1368) `sectionHtml`)를 고치고 같은 화면에서 검증**하므로 묶는다(A-3).
  - **①「배우고 싶은 게임」 토글이 위 목록에 붙어 보인다** (그린) — 두 섹션이 `.sm-game-toggle-wrap`의 `gap:8px` 하나로만 떨어져 있어([club-schedule.html:498](../pages/club/club-schedule.html#L498)), **want 섹션이 펼쳐지면** 그 목록 바로 밑에 learn 토글이 붙어 **learn이 want의 일부처럼 읽힌다.**<br>⟹ **영향 대상**: 플래너 멀티스텝 Step2 「게임 선택」 화면 하나. 데이터·저장 로직 무관.<br>⟹ **수정·검증 경로**: `.sm-game-toggle-wrap` gap 또는 `.sm-game-section`([:511](../pages/club/club-schedule.html#L511)) 아래 여백. **육안 확인 필수**(CLAUDE.md 「자동 검증 통과는 육안 확인을 대체하지 않는다」) — 두 섹션 다 접힘/한쪽만 펼침/둘 다 펼침 **3가지 상태**를 다 볼 것. 접힘 상태에서 간격이 벌어지면 그건 회귀다.
  - **②「지난 모임에서 하고 싶었던/배우고 싶었던 게임」 불러오기 버튼 2개** (옐로) — 사용자 요구: 위치는 `게임 이름 직접 입력` + `추가` **같은 줄 오른쪽 빈 칸**, **두 줄로 넘기지 말 것**(길면 입력칸을 줄인다). 각 섹션에 자기 타입(want/learn)의 버튼 하나씩.<br>⟹ **영향 대상**: 같은 Step2 화면. 눌렀을 때 추가되는 건 기존 `addGameStep2`를 타므로 **저장 스키마 변경 없음**.<br>⟹ **수정·검증 경로**: 마크업은 `.sm-game-input-row`([:1383](../pages/club/club-schedule.html#L1383), CSS [:545](../pages/club/club-schedule.html#L545) `display:flex`)에 버튼 추가. 표시 방식은 **기존 `pickerHtml`/`_pickerCache` 구조를 재사용**할 것 — 지금 좋아요/궁금한 게임 목록이 정확히 같은 일을 하고 있다(신규 생성 전 기존 검색). ⚠️ **데이터 출처가 새것이다** — 현재 `fetchPickerData`는 `like`/`curious`만 안다. "지난 모임"은 `meeting_vote_games`를 **내 user_id + 지난 날짜**로 조회해야 하고, 그런 조회 함수가 [supabase-client.js](../assets/js/supabase-client.js)에 있는지부터 확인(없으면 신규). **범위·중복 규칙 미정 → 착수 시 결정**: 몇 주치까지, 중복 게임 합칠지, 이미 담긴 건 `is-added`로 막을지(기존 picker와 동일 처리가 자연스러움).
