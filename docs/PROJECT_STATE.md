...
- **플래너 깜빡임 2차 수정 (세션 ⑧, 2026-07-18)** — ⚠️1차(4ab32d8) 실패 후 재수정. 홈 미리보기에서 ①닉네임 옆 ✎ ②"+ 이날 참여 등록" ③"+ 플래너에서 등록하기" 각각으로 시간설정 시트를 연 뒤 **✕/dim으로 닫을 때 흰 박스·플래너 헤더 깜빡임 없이 바로 닫히는지** 확인. + "모임 플래너 열기"(정상 오픈)에서 주간뷰 흰박스가 정상 표시되는지 + **모임보드 ✎ 편집**(day-detail, 공유 CSS 영향)이 시각적으로 안 깨졌는지. 상세·원인은 §2.
