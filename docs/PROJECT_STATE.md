...
🚨 **§0이 「P3(드릴다운) 미착수」를 가리키고 있었으나 그건 2026-07-20에 이미 완료된 것이었다** — admin-analytics.md와 정면으로 충돌했고 코드로 해소했다(`ddPanelHtml` 실재). **§0의 「다음 큰 덩어리」를 적을 땐 그 자리가 정말 열려 있는지 도메인 문서와 대조할 것.** 착수 첫 동작인 재측정이 아니었으면 있는 기능을 다시 만들 뻔했다.
