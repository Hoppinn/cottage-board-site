...
최종 갱신: 2026-07-20 세션 ⑭ (**관리자 잔여 5건 전부 종결** — #25·#18·#3·#26·교환권 도배. 관리자 페이지에 남은 건 #14 하나이고 목표를 닫는 건 P3b).
...
**진행 중 작업 없음** (2026-07-20 세션 ⑭ 종료 시점) · **열린 스모크 1건**(아래 ⑬ 참조).

**세션 ⑭ (2026-07-20)** — 관리자 잔여 **5건 전부 종결**(#25·#18·#3·#26·교환권 도배). 커밋 8개.

> **한 줄 요약**: 목표 화면 안의 작은 것부터 닫았고 **5건 중 3건은 「고칠 것」이 아니라 「대장 기재가 틀린 것」**이었다. 관리자 페이지에 남은 건 **#14(Plan)** 하나이고, 목표를 닫는 건 **P3b**다.
>
> 🚨 **이 세션의 최대 교훈 — 대장의 「0건」·「발현 없음」은 발견이 아니라 질문이다.** #18(「DB 0건」→실제 5건)과 #3(「발현 0건」→실제 13세션)이 **연달아** 뒤집혔다. 착수 첫 동작은 가설 세우기가 아니라 **그 숫자를 다시 재는 것**이다.

- ✅ **#25 `record_start` 유령 발사** — 홈이 기록 iframe을 `?tab=input`으로 프리로드하면서 `tryInit()`이 돌아 **방문마다** 발사됐다. 발사를 **사용자가 기록 입력 화면에 들어온 3지점**으로 이동. Playwright 4경로 + **음성 대조군**(옛 구현에서 4건 FAIL) 확인.
  - 📌 **소급 보정은 안 했다** — 과거 `record_start` 367건은 부푼 채로 남는다. 앞으로 쌓이는 값만 정확해진다.
- ✅ **#18 `home_record_write_click` 0건 → 판정종결(코드 변경 없음)** — **실제로는 5건**이 쌓여 있었고 그중 둘은 `record_complete`까지 이어진 진짜 흐름이었다. 후보 4개 중 ③(실서버 배포본)·④(리스너 미부착)·②(관리자 전용)를 각각 실측으로 탈락. **추적 결손이 아니라 클릭이 적은 것**이고, ⚠️ **왜 적은지는 미확정**(방문 모수·노출·다른 경로 등). 사용량 분석이 필요해지면 **UX 항목으로 따로** 세울 것.
- ✅ **#3 비회원 카드 「오늘」이 0에 덮임** — 대장엔 「현재 발현 0건」이었으나 **326세션 중 13개에서 발현 중**. 원인은 **두 소스 중 하나만 고른 것**이고, `Math.max`로 변경 + 출처 툴팁.
  - 🚨 **내 첫 가설이 데이터와 충돌해 스스로 철회했다** — "비회원 `page_sessions`는 항상 0"이라는 **문서 기재를 믿고** 그쪽을 버리려다, 0이 아닌 값이 잔뜩 있는 걸 보고 멈췄다. 파고드니 **INSERT 경로가 둘**이었다(아래).
  - 📌 **부산물 — 문서의 미해결 질문 해소**: "비회원 행 699개가 `duration_sec>0`인 이유" = `script-nav.js` 세션 트래커(실제 초·한글 라벨·3초 미만 미전송)와 `_startAnonHeartbeat`(0 하드코딩·슬러그). **이게 #14의 정체이기도 하다.**
- ✅ **#26 이벤트 탭이 퍼널이 아닌데 퍼널로 그림** — 화살표·트리는 「B를 한 사람은 A를 거쳤다」는 **포함관계를 주장**하는데 **5쌍 중 4쌍에서 깨진다**(홈 버튼은 스크롤·딥링크 등 여러 경로로 도달). 순서도 못 센다(교집합이라 시각 미고려 — 세션·시각까지 보면 3-1→3-2는 17명이 아니라 **13명**). **도형을 없애고 「행동 묶음 + 겹침」**으로 재정의.
  - 🚨 **같은 자리를 하루에 세 번 고쳤고 2·3차가 같은 실패였다** — 둘 다 **관계를 데이터로 확인하지 않고 그림부터** 바꿨다. 3차는 외부 감사의 "선형으로 보인다"는 지적이 준 **프레임(분기)을 그대로 해답으로** 삼은 것. **지적은 맞았고 내 해답이 틀렸다.** → CLAUDE.md 「외부 의견」에 **「지적은 문제 가설이지 해결안이 아니다」 + 판정법** 추가.
- ✅ **교환권 알림 도배** — 사양(관리자 전용 감시 피드)은 맞았고 **원인이 두 겹**이었다: ①`dev_test` 13건 → 제외 + 30일 제한(30→19) ②그래도 **13건이 한 사람의 하루치** → **유형+날짜 묶음**(`new_intro` 방식, 렌더러가 이미 `keys[]` 지원). **19행 → 7줄.** `voucher_log` 원장은 무변경.
  - 📌 **첫 수정으로 안 끝난 지점에서 멈추고 가설을 다시 세웠다** — "오래된 데이터"가 아니라 "같은 유형이 한 날에 몰림"이었다.
- 🧰 **검사기 점검(외부 감사 트리거)** — 일회성 스크립트가 **HTTP 401을 「RPC 없음」으로 오독**한 걸 상태 코드 확인으로 잡았다. 그 스크립트는 리포 밖이라 고칠 대상이 아니었고, 대신 **재사용 하니스 15개를 전수 점검**해 `verify-lost-update.js`의 `read()`가 `const { data }`만 받던 것을 수정(#22 손실률을 만들어내는 도구다).
- 🧰 **브라우저 검증에 쓰기 차단 도입** — `ctx.route`로 **GET만 통과**시키면 실데이터로 화면을 보면서 쓰기가 0이 된다. ⚠️ **이번 건은 차단이 없었어도 오염되진 않았을 것**(관리자가 이미 보유한 업적이라 UNIQUE가 튕김) — **"지급될 뻔했다"고 적었다가 실측하고 정정**했다. 값은 **무엇이 쓰려 했는지 보인다**는 데 있다.
- ✅ **업적 재지급 낭비 제거** — `checkAchievements`가 조건 충족 업적 **전부**를 접속마다 INSERT하고 중복을 DB UNIQUE가 튕겨내는 구조였다. 보유 목록을 먼저 읽어 빼도록 변경 — **관리자 홈 1회 로드에 쓰기 10건 → 0건**, 음성 대조군(옛 코드에서 10건 재현) 확인, 알림 화면 회귀 없음.
  - 🚨 **"사용자 영향 0이니 급하지 않다"고 판단해 사용자에게 물었다가 반려됐다** — ①**판단해서 제안**할 자리였고(D-11) ②**불필요한 것은 영향이 0이어도 고쳐나가는 게 기준**이다. 영향 크기는 **순서**를 정하는 근거지 **안 고칠** 근거가 아니다.
- 📝 **문서 충돌 해소** — 마이그레이션 012가 `db-schema.md`엔 「미실행」, `PROJECT_STATE`엔 「완료」로 갈려 있었다. 012 파일이 지정한 무해 검증(없는 `user_id`로 RPC 호출)으로 **실행돼 있음을 확인**하고 `db-schema.md` 정정.
...
> 🚩 **진행 중인 목표: 「관리자 페이지 정리」** (2026-07-19 시작)
>
> 🏁 **종료 조건 = P3b(드릴다운)까지.** ⚠️ **이건 사용자 말의 뜻이 아니라 내 구현 판단이다.** 원문은 *"유효 데이터들이 더 직접적이고 **유기적으로** 드러났으면"*이고, **그 상태를 현재 구조에서 충족하려면 드릴다운이 필요하다고 판단**해 완료 조건에 넣었다. "유기적"이 다른 뜻일 수도 있다(지표 간 연결·시각 구조 정리 등) — **P3b를 만든 뒤 그 결과가 원래 목적("직접적이고 유기적으로 드러남")을 실제로 충족하는지 내가 먼저 대조하고, 부족하면 종료 조건이나 구현 방식을 다시 판단한다.** 사용자가 의도를 다르게 밝히면 그때도 갱신한다.
>
> 📐 **순서**: 목표 화면 **안에서** 쉬운 것부터. **그 쉬운 것은 2026-07-20에 전부 닫혔다**(5건) — 이제 남은 건 무거운 둘뿐이라 **P3b가 1번**이다. #14는 소급 결정이 걸린 별건이라 목표를 닫는 데 필수가 아니다.

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🎯 **P3b 드릴다운 — 목표를 닫는 유일한 항목.** 이벤트·페이지 숫자에서 **"이걸 한 사람들"**로 내려가는 길. 무겁고 Plan 필요. 선행(#22·#24·R3) 완료. **착수 전 [admin-analytics.md](admin-analytics.md) §1 원칙 ⑤ + §5 P3b를 읽을 것** | 〃 §5 | Plan |
| 2 | 🟡 **#14 `page_sessions.page`가 라벨 저장** — 저장 경로가 **둘**(`script-nav.js` 세션 트래커=한글 라벨 / `_startAnonHeartbeat`=슬러그)이라는 정체가 2026-07-20에 밝혀졌다. 통일 + **과거 행 소급**(불가분 감안) 결정이 필요 | [admin-analytics.md](admin-analytics.md) §4 | 옐로+Plan |
| 3 | 🧩 **프로필 보드 4건 묶음** *(관리자 페이지 종료 후)* — 카드 요약이 편집 미반영 · 박스모달이 닫힘 · 타인 보드 네비게이션 통일 · 수집보드 스크롤 진입점. 네 건 다 같은 서브시트 구조 | §3 | 옐로 |
| 4 | 🔔 **관심 기반 묶음 알림** *(교환권 도배는 2026-07-20 종결)* | §3 | Red+Plan |
| 5 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 |
| 6 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan |

> 🚨 **P3b 착수 전 읽을 것 — 이 화면의 이벤트 데이터는 순서를 모른다.** 2026-07-20 #26에서 확정: 계산이 **무순서 사람 집합 교집합**이라 "A 다음에 B"를 못 센다. **드릴다운에서 "이걸 한 사람들"로 내려가는 건 되지만, "이 순서로 한 사람들"은 안 된다** — 필요하면 같은 `session_key` 안 시각 비교를 새로 만들어야 한다.
>
