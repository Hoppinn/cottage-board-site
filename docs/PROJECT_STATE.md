...
- 수정/삭제 버튼: 사진 전체보기 시 플레이기록 수정/삭제(···) 버튼이 다른 페이지(기록게시판 등)와 동일하게 표시돼야 하는데 현재 안 뜸.
- 전적: 이전에도 4번째 칸에서만 한 줄 펼침 등 유사 오작동 발생 → 수 차례 수정 후 정상화한 이력 있음. 재발 가능성 주의.

**[BUG-B] 기록보드 게임평 미표시**
- 증상: 게임평을 여러 개 작성했는데 더루프 게임평만 표시됨. 나머지 연동 안 된 것으로 보임.

**[BUG-C] 기록보드 플레이기록 카드 클릭 범위 과도**
- 증상: 플레이기록 카드에서 게임이름을 눌러 상세페이지(게임시트)가 뜨는 건 정상. 게임이름이 아닌 영역(카드 전체)을 눌러도 게임시트가 열리는 건 의도치 않은 동작. 게임이름(링크)만 클릭 범위로 제한해야 함.
