...
- [ ] **관심 기반 묶음 알림** (Red, Plan 필수)
  - 개별 알림 → 유형별 묶음 방식 전환 / notifSeenAt → `{ tagged, review, play_record, purchased }` 확장. **함께 볼 것**: 위 「관리자 알림이 교환권 로그로 도배됨」 — 같은 화면·같은 함수라 묶을 근거가 있다.
  - 🚨 **착수 전 재측정할 것 — 이 항목의 절반은 이미 구현돼 있다** (2026-07-21 실측). 자동 메모리에 "다음 작업(승인됨)"으로 **끝난 일이 미착수인 척** 남아 있었고(39일 전 기록, 그 메모리는 삭제함) 하마터면 다시 만들 뻔했다. **지금 코드에 이미 있는 것**: `isNew` 플래그([supabase-client.js:1463](../assets/js/supabase-client.js#L1463) — seenAt 이후만 조회하던 걸 전체 조회 + 플래그로 바꾼 그 작업) · 개별 읽음(`readKeys`) · `newGameSeenAt` 4번째 인자 · 알림 5종(tagged/curious/구매완료/new_game/new_intro). → **남은 진짜 범위는 「유형별 묶음 표시」와 seenAt의 유형별 분리뿐**이고, 그것도 착수 첫 동작으로 다시 확인할 것.
