...
| 2 | **§5에 남은 코드 작업 2건 — 둘 다 착수 전 사용자 입력이 필요하다** ①캡션 복사 인스타/단톡 분리(A-8) → **두 포맷의 문구를 사용자가 정해야 한다**(취향 결정, D-14) ②게임 검색 영어 제목 인식 → **선행 작업 「게임시트 영어제목 독립 표시」가 미착수**. 그 외 §5는 기획·디자인 판단 | §5 | 기획 | A-10(지난 일정 수정불가)은 2026-07-22 종결(git log). **선행 입력 없이 혼자 열 수 있는 코드 작업은 없다** — 없으면 「열린 스모크」(실기기·비관리자 로그인)를 소화할 것 |
