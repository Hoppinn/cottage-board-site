...
- [ ] **[feat] 기록입력 '최신 기록' 버튼 → 최신순 토글로 선택** (2026-07-18 등록·**구현 적용, 스모크 대기**) — 1번 행 `↑ 최신 기록` 버튼([game-reviews.js:190](../assets/js/game-reviews.js#L190))이 최신 1건만 채우던 것을 **누를 때마다 다음 세팅으로 순회**하도록. **구현**: `_myLatest[0]`(단일) → `_prRecents`(내 기록 최신순 dedup 배열)+`_prRecentIdx`([:505~](../assets/js/game-reviews.js#L505) 빌드, [:295](../assets/js/game-reviews.js#L295) 토글). ⓐ**dedup**: `(group,count,names)` 시그니처로 서로 다른 세팅만(같은 모임 여러 게임의 중복 제거) ⓑ**라벨**: 클릭 시 버튼에 `↑ 2/5 · 그룹 · 4명` 표시(맹목 순회 방지) ⓒ끝에서 최신으로 wrap. DB 추가조회 0. `fillCountAndNames`가 칩 재생성이라 재클릭 안전. **스모크**: 기록입력 1번 행 '최신 기록' 반복 클릭 → 서로 다른 최근 세팅이 순서대로 채워지고 버튼에 n/total·그룹·인원 표시되는지. ⚠️ 라벨 길면 버튼 줄바꿈 가능(CSS 미세조정 여지).
