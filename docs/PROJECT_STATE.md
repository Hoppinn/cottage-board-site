...
최종 갱신: 2026-07-22 (세션 ㉞ — iframe 사후확인 종결·영어검색 복구·탭압축 기각·P6 집계정확도 해소). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
> 📌 **관리자 분석에 남은 큰 덩어리는 P4 하나다**(Red+Plan — 남의 보드에 관리자 전용 섹션, 오너 가드 필수, 프라이버시 사안이라 착수 시 Plan부터). **P6(집계 정확도)는 2026-07-22 해소**(①설계값·③=iframe#24 종결·④자정넘김 수정·⑥로그인잔여 의도적 보류 — [admin-analytics.md](admin-analytics.md) §4 ④⑥), **「탭 구조 압축안」은 기각**(§5 조건부 트리거 — 유입+페이지 병합이 원칙 ④를 깬다).
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | **관리자 분석 P4(보드 연동) — Red+Plan, 실행 전 승인 필요.** 남의 보드에 관리자 전용 섹션, 오너 가드 필수(프라이버시). 착수 시 Plan부터 | [admin-analytics.md](admin-analytics.md) §5 | 구조 | 되돌리기 어려움 + 프라이버시 위험 |
| 2 | 🚫 **「이날모임 상세창에 등록·수정 버튼」은 승인 없이 만들었다가 2026-07-22 되돌렸다(git `d315b314`).** 되살리려면 **먼저 기획**할 것 — ①등록·수정 진입점을 어디에 몇 개 둘 것인가(지금도 홈 카드 ✎ / 「+ 이날 참여 등록」 / 개인막대 모달로 갈라져 있다) ②등록을 마치거나 뒤로 갔을 때 **보던 상세창으로 복귀**하는 동선. 이 둘 없이 버튼만 붙이면 미완성이다 | — | 기획 | **교훈**: 내가 「결정해달라」고 올린 항목을 내가 집어가면 안 된다(D-14) |
...
- [ ] 🎯 **관리자 분석 (감사·사람단위·집계정확도) → [admin-analytics.md](admin-analytics.md)로 통합 이관** (2026-07-19) — 이 주제가 §3 3곳 + 「론칭 후 이월」 + `PLAN_funnel_analytics.md`로 **6곳에 흩어져** 다음 세션에 무엇부터 볼지 판단이 안 되던 것을, 사용자 지시("전체 정리 및 통합 정리, 이후에 플랜, 부위별로 분리 후 하나씩")로 도메인 SSOT를 신설해 옮겼다. **여기엔 포인터만 둔다 — 상세를 여기에 다시 쓰지 말 것.**
  ...
  - **남은 것은 P4(C단계, Red+Plan) 하나** — P1·P2·P3·P3b·P5·P7 완료, **P6(집계 정확도)는 2026-07-22 해소**(①설계·③=iframe#24 종결·④자정넘김 수정·⑥로그인잔여 의도적 보류, 상세는 admin-analytics §4 ④⑥). 탭 구조 압축안은 기각(§5).
