...
**진행 중 작업 없음** — 2026-07-27 세션 ㊴ 사용자 제보 15건 처리 완료(완료 내역은 git log, `a7a1880a`~`c2260c14`). **열린 버그 0건**(§2) · **열린 기능 3건**(§3, Plan 필요 1건 포함) · **열린 스모크 7건**(§0 아래, 카카오 Make 알림 신규 포함) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0**. **미배포 커밋 있음(push는 사용자)**.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **캡션 복사 — 게임 단위(기존 유지) + 날짜·모임 단위(신설) 역할 분리** | §3 P1 6번 | 기능 | 설계 확정·조사 완료 — 착수만 하면 됨 |
| 2 | **간식·음료 요청 Part2(관리자 처리됨 표시 + 요청자 완료알림)** | §3 P1 2번 | 기능/Plan | DB 컬럼 신규 — Plan 모드 먼저 |
| 3 | **모임시간 30분단위 / 게임정리·룰설명 관리자추가 / QR 추천게임 집계** | §3 P1 3~5번 | 기능 | 명확한 스펙, 서로 독립 — 순서 무관하게 진행 가능 |
...
**열린 버그 없음** (2026-07-27 세션 ㊴ 사용자 제보 15건 전부 처리 — 종결/원인규명/사용자확인대기로 분류 완료).
...
- [ ] **캡션 복사 — 역할 분리(게임 단위 + 날짜·모임 단위 둘 다 유지) (2026-07-27 사용자 요청+설계 확정, 조사 완료·미착수)**
  - **확정 설계**: 게임별 ⋯ 메뉴의 "캡션 복사"는 **없애지 않고 그 게임 하나짜리 캡션으로 재정의**(`buildRecordCaption([r], getGameName)` — 배열에 기록 하나만 넣으면 됨, 신규 로직 없음). **추가로** 날짜별 탭은 모임명(`.pr-moim-header`) 오른쪽 끝, 모임별 탭은 날짜 토글(`.pr-sub-hd`) 왼쪽에 ⋯을 새로 달아 **그 날짜+그 모임 전체 캡션**(기존 `_dayCaption` 그대로)을 담당하게 한다. 좋아요·궁금해요·게임평추가·사진추가는 게임별 메뉴에 그대로 유지.
  - **막히는 지점**: `.pr-moim-header`(game-reviews.js 1439줄 근처)와 `.pr-sub-hd`(`renderMoimView`의 `renderDateBlock`, 1210~1218줄)가 지금 전부 `<button>` 하나로 돼 있어 그 안에 별도 `<button>`(⋯)을 못 넣는다(HTML 무효 + 클릭 이벤트 충돌). **헤더를 감싸는 wrapper로 구조 변경 필요** — 토글 버튼과 ⋯ 메뉴를 형제 요소로 분리하고, 기존 클릭 바인딩([game-reviews.js:954](../assets/js/game-reviews.js#L954), 960줄)과 CSS(`.pr-sub-hd`/`.pr-moim-header` 관련 padding·background 규칙, style.css)를 그 구조에 맞게 조정.
  - **재사용 가능**: 새 ⋯ 트리거·메뉴는 기존 `.pr-rec-more`/`.pr-rec-more-btn`/`.pr-rec-more-menu` 클래스와 `copyCaption`/`trackMoreMenu` 함수를 그대로 재사용(신규 CSS 최소화).
  - ⚠️ **2026-07-22 "캡션 복사는 양쪽 다 ⋯ 메뉴 안으로 확정 — 재논의 금지" 결정과 다른 방향이다.** 그때 "모임 타이틀에 하나"를 기각한 이유는 "캡션이 하루치인데 타이틀은 모임 단위라 날짜를 못 정한다"였는데, **같은 날 이후 카드 구조가 "날짜 하나 = 카드 하나"로 바뀌어 그 문제가 해소됐다** — 재논의 금지를 어기는 게 아니라 **구조 변경으로 전제가 달라진 것**.
