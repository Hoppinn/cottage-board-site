...
최종 갱신: 2026-07-17 (**감지기 2단계 ✅ 완료** — supabase-client.js 구조분해 59곳에 error 수신+로그 추가 → 104곳 전부 감지. 실제 대상은 문서의 40곳이 아니라 **59곳**이었음(`{count}`·별칭형 누락 = 두 번째 오집계, 정정 완료). 실DB 화재 테스트 42개 **불난 곳 0건**. 1단계가 만든 노이즈·오라벨도 정리(019bd7c) → 화재 테스트 울림 0건 = 콘솔이 조용해 "울리면 진짜"가 성립. 교훈은 CLAUDE.md 규칙으로 승격. / **R10a ✅ 완료 — 스모크 통과** — openProfilePanel 1,940→918줄, 서브시트 6블록 추출, 커밋 c9ab2bd~b3ed30d. R10 원안은 사용자 승인 하에 R10a(추출)/R10b(stale 버그)/R10c(네비스택)로 **분할**. **다음 세션 = R10b**)
