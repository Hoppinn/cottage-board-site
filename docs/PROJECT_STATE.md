...
- [x] ~~**[Yellow] 이날모임 상세 모달 클릭 진입점 2건**~~ — ✅ **완료** (2026-07-17, 커밋 d7a0a88[닉네임]·8957e8b[게임 행]). **스모크 대기** — 아래 포인트 참조.
  - **구현**: `_buildParticipantsHtml`의 닉네임에 `.dd-nick-link`+`data-uid` → `openOtherMeetingSheet` / 게임 행 `<li>`에 `.dd-game-item--clickable`+`data-game-id` → `getGameKeyById` 변환 후 `openGameSheet`. **둘 다 모달을 닫지 않고 위에 겹쳐 띄운다**(아래 교훈 참조).
  - 클릭 영역은 **썸네일+게임명 묶음**(썸네일이 13px라 터치 타겟 미달, 사용자 승인). ⚠️ `<li>`가 블록이라 처음엔 **오른쪽 빈 공간까지 눌렸다**(사용자 지적) → `width: fit-content`로 내용 폭에 맞춤.
  - **미해결로 남긴 것**: 사용자가 언급한 **"배경 클릭으로 보드 닫기"는 현재 기능이 없다** — `.profile-panel`엔 ✕([kakao-auth.js:1716](../assets/js/kakao-auth.js#L1716))만 있고 백드롭 클릭 핸들러가 없다. **모든 보드에 영향**을 주는 신규 동작이라 이 항목에 넣지 않음 → 원하면 별도 Yellow로.
  - **알려진 엣지케이스**(기존 구조, 미수정): 이 모달과 `openDatePreviewModal`이 **같은 `__ddModal` id**를 쓴다 → 이날모임 상세 → 닉네임 → 남의 모임보드 → 거기서 "자세히"를 열면 원래 모달이 제거된다(보드를 닫으면 preview 모달이 대신 남음). 실사용 빈도 낮다고 판단해 보고만.
  - ⚠️ **여기서 얻은 교훈 — 이 항목의 착수 메모 3건 중 2건이 틀렸다**(전부 코드 확인 전엔 그럴듯했음). 219행 e2bfefb 교훈("동기화 경로를 문서 메모로 믿지 말 것")과 **같은 뿌리이고 또 맞았다**:
    ...
    - ②"z-index 확인 선행(박스모달 9700 > 게임시트 9500)" → **방향이 반대**. `.dd-overlay`는 9200이었고 게임시트(9500)가 이미 위라 썸네일은 `close()`가 **불필요**했다.
  - ⚠️⚠️ **가장 큰 교훈 — "닫고 전환"과 "겹쳐 쌓기"를 혼동했다** (2026-07-17 사용자 지적으로 발견): 닉네임 건은 보드(9100)가 모달(9200) 아래라 처음에 **모달을 닫고 보드를 열었는데**, 사용자가 보드를 ✕로 닫자 **돌아갈 화면이 사라졌다**. R10c의 `backTo`(뒤로가기 버튼)를 끌어오려 했으나 **그것도 오답** — 사용자 지적대로 이건 패널→패널 **전환**이 아니라 모달 위에 보드가 **겹쳐 뜨는** 것이고, 그러면 닫았을 때 아래 화면이 그대로 있는 게 당연하다. **해법은 복귀 장치가 아니라 레이어 순서**였다 → `openDateMeetingModal`의 오버레이에만 `.dd-overlay--under-board`(9050)를 부여해 보드 아래로 내리고, 모달은 **닫지 않는다**.
    - 📌 **`.dd-overlay` 기본값(9200)을 낮추면 안 된다** — `openDatePreviewModal`은 모임보드 서브시트(9200) **안에서** 열리므로(kakao-auth.js 1565~1566) 낮추면 서브시트 뒤에 깔린다. 그래서 **이 모달에만** 클래스를 준 것. 낮춘 9050은 헤더(1000)·게임시트(9500)·플래너(`--z-shelf` 9600) 전부와 무관함을 확인.
    - 📌 **CSS 선언 순서 주의**: `.dd-overlay--under-board`는 `.dd-overlay`와 우선순위가 같아 **뒤에 와야** 이긴다(작업 중 실제로 앞에 뒀다가 발견).
