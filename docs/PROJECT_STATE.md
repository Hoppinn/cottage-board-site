...
최종 갱신: 2026-07-08 (일별 뷰 제거 + 달력 날짜 클릭 → 해당 주차 이동)
...
| 커밋 | 내용 |
|------|------|
| (이번) | 일별 뷰 제거 + 달력 날짜 클릭 → 해당 주차 이동 — club-schedule.html: renderCalendar·openDay~removeVote (~359줄) + #viewDay HTML + btnBackCal + 일별뷰 전용 CSS ~230줄 제거. navigateToDate(ds)/getWeekOffset(ds) 추가, renderWeekView에 targetDate 파라미터+is-target CSS 신규. 진입점 2곳(요일 헤더·월간 셀) → navigateToDate로 교체. db-schema.md meeting_profile_click 설명 정리 |
