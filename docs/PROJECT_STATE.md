...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **캡션 복사 위치 이동(게임별 ⋯ → 탭별 헤더)** | §3 P1 6번 | 기능 | 조사 완료·구조 변경 지점까지 특정됨 — 착수만 하면 됨 |
...
- [ ] **캡션 복사 위치를 게임별 ⋯ 메뉴 → 탭별 헤더로 이동 (2026-07-27 사용자 요청, 조사 완료·미착수)**
  - **요구사항**: 날짜별 탭 — 카드 안 모임명(`.pr-moim-header`, 예 "코티지보드 동호회") 오른쪽 끝에 ⋯ 신설. 모임별 탭 — 날짜 토글 줄(`.pr-sub-hd`) 왼쪽에 ⋯ 신설. 게임별 행의 ⋯ 메뉴에서는 캡션 복사만 제거(좋아요·궁금해요·게임평추가·사진추가는 그대로 유지).
  - **캡션 내용 계산은 이미 올바른 자리에 있다** — `game-reviews.js` `buildSessionBody`의 `_dayCaption`(1360줄)이 "그 날짜+그 모임" 단위로 이미 정확히 계산됨. 새로 계산할 것 없음, **트리거 버튼 위치만 이동**.
  - **막히는 지점**: `.pr-moim-header`(1439줄 근처)와 `.pr-sub-hd`(`renderMoimView`의 `renderDateBlock`, 1210~1218줄)가 지금 전부 `<button>` 하나로 돼 있어 그 안에 별도 `<button>`(⋯)을 못 넣는다(HTML 무효 + 클릭 이벤트 충돌). **헤더를 감싸는 wrapper로 구조 변경 필요** — 토글 버튼과 ⋯ 메뉴를 형제 요소로 분리하고, 기존 클릭 바인딩([game-reviews.js:954](../assets/js/game-reviews.js#L954), 960줄)과 CSS(`.pr-sub-hd`/`.pr-moim-header` 관련 padding·background 규칙, style.css)를 그 구조에 맞게 조정.
  - **재사용 가능**: ⋯ 트리거·메뉴는 기존 `.pr-rec-more`/`.pr-rec-more-btn`/`.pr-rec-more-menu` 클래스와 `copyCaption`/`trackMoreMenu` 함수를 그대로 재사용(신규 CSS 최소화).
  - ⚠️ **2026-07-22 "캡션 복사는 양쪽 다 ⋯ 메뉴 안으로 확정 — 재논의 금지" 결정과 다른 방향이다.** 그때 "모임 타이틀에 하나"를 기각한 이유는 "캡션이 하루치인데 타이틀은 모임 단위라 날짜를 못 정한다"였는데, **같은 날 이후 카드 구조가 "날짜 하나 = 카드 하나"로 바뀌어 그 문제가 해소됐다** — 그래서 사용자가 다시 요청한 것이 근거 있음. 재논의 금지를 어기는 게 아니라 **구조 변경으로 전제가 달라진 것**.
