...
- [ ] **[verify] R10c 스모크** (2026-07-17, 코드는 6f506d8·9593237·ade71df로 완료) — 브라우저 확인만 남음.
  - **①3-2**: 게임시트 → 좋아요 → 토스트 '취향 보드 →' → 취향보드 → 뒤로 → 내 보드 → **뒤로('‹ 게임명')** → 원래 게임시트 ②궁금해요도 동일 ③**3-1**: 내 보드 → 🔔 알림 → 소개글 알림 → 남의 모임보드 → 뒤로 → 남의 보드 → **뒤로('‹ 최근 소식')** → 내 보드 + 알림 서브시트
  - **회귀 1순위**: ④평소 경로('내 활동' 클릭)로 연 보드에 **뒤로가기가 없고 제목이 왼쪽 정렬 그대로**인지(헤더가 backTo일 때만 grid 3열로 바뀜) ⑤닉네임 클릭으로 연 남의 보드도 뒤로가기 없음 ⑥✕ 닫기는 backTo 유무와 무관하게 패널 전체 닫힘.
...
- [x] **[Yellow] 검색 모달 "추가됨" 항목 재클릭 = 취소** — 🟡 **코드 완료, 브라우저 스모크만 남음** (2026-07-17 사용자 제안, 커밋 9d8d06f[refactor]·0db1f03[feat]).
