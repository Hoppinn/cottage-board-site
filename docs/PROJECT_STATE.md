...
최종 갱신: 2026-07-16 (R11a 코드 완료 — openGameSheet 321→187줄, 브라우저 스모크 대기. R11은 a/b/c로 분할)
...
**다음 세션 시작점**: ① **R11a 브라우저 스모크** (게임시트 — 아래 포인트). ② 이후 **R11b**(GS6 나머지 과대함수 4개, R11a와 동일 패턴·교차파일 위험 없음) → **R11c**(GS2 IIFE화, 단독 세션 필수) → R12 → R10.

**R11a 스모크 포인트** (코드 2026-07-16 완료, UI라 자체확인 불가): 게임시트 열기 → **표지 이미지 클릭(확대)** · 스티키바 · **같은 디자이너 카드 클릭(게임→게임 이동)·뒤로가기** · 게임설명/메커니즘 더보기 토글 · **좋아요·궁금해요(+▾ 패널)** · 게임평/사진/플레이기록 미리보기 3종 로딩 · "기록 페이지에서 보기" · 게임위치/룰영상 버튼 · 미보유 게임 클릭 시 기록시트로 라우팅.
