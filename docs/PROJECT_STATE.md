...
**[BUG-A] 기록보드 사진 +N 토글 오작동 → 143차-136 부분 수정**
- `+N 인라인 확장`: 전체 사진 DOM 포함 + 숨김 클래스 방식으로 변경. badge 클릭 시 라이트박스 대신 인라인 확장. ✅ 완료
- 미완: 수정/삭제 버튼 - 사진 전체보기 시 ··· 버튼 표시 (각 사진에 record 컨텍스트 필요, 별도 작업)
- 전적: 수 차례 수정 이력 있음. 재발 가능성 주의.
