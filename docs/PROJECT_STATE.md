...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 | 전부 **재현부터** 필요. 하나씩 독립 |
...
- [ ] **[문서-코드 불일치] `squirrel_lv5` 캐릭터 파일 미제작인데 경고가 없음** (Phase 1 감사 A1, 2026-07-02 발견 → **처리 현황 표에 등재된 적 없는 고아 항목**, 2026-07-17 REFACTOR_CHECKPOINT 압축 검토 중 재발견·실측 확인) — 실재하는 lv5 파일은 `bear_lv5`·`hamster_lv5`·`rabbit_lv5` **3개뿐**. 🔎 **2026-07-21 실측(브라우저 404 수집)으로 빠진 건 3개 확인 — `squirrel_lv5`·`sparrow_lv5`·`hedgehog_lv5`.** `hedgehog_lv5`는 여기에도 achievement-system.md에도 없던 **네 번째 고아**다(수집 보드를 열면 매번 404 3건이 난다. `onerror` 폴백이 이모지로 가려 화면상 티가 안 남). `sparrow_lv5`(visit_500)는 [achievement-system.md](achievement-system.md) 234·271행에 **⚠️미완성으로 명시**돼 있으나, **`squirrel_lv5`(record_400)는 167행에 아무 경고 없이 정상 보상처럼 적혀 있고 271행 미완성 목록에도 없음**. → record_400 달성자가 나오면 **깨진 이미지**가 뜬다(sparrow와 달리 아무도 모르는 상태). 조치: ①파일 제작 or ②171행/271행에 미완성 표기 추가. **record_400은 먼 임계값이라 급하진 않으나, 달성 후엔 사용자 자산(캐릭터) 영역이라 사후 대응이 어렵다.**
