...
최종 갱신: 2026-07-17 — **모임보드 박스모달 삭제버튼 🟡 코드 완료·스모크 대기**(e2bfefb, §3). **다음 세션 후보**: ①위 스모크 7포인트 ②R10c(네비스택) ③감지기 3단계 ④`REFACTOR_CHECKPOINT.md` 정리(§0에 절차, R10c 종료 후 권장).
...
**다음 세션 시작점 (2026-07-17 세션 ③ 종료 시점)**: **①모임보드 박스모달 삭제버튼 스모크 7포인트(§3, 코드는 e2bfefb로 완료)** ②R10c(네비스택, Opus xhigh + Plan) ③감지기 3단계 ④REFACTOR_CHECKPOINT 정리(R10c 종료 후 권장).
...
- [x] **[Yellow] 모임보드 취향 박스모달에 게임 삭제(✕) 추가** — 🟡 **코드 완료, 브라우저 스모크만 남음** (2026-07-17, 커밋 e2bfefb). 확인 3건 사용자 확답: ①confirm 띄움 ②readOnly 숨김(`_ro()`) ③룰 배지는 표시 전용 유지·삭제만 추가. CSS 변경 없음(`.taste-game-del` 전역 선택자 재사용).
  - **구현**: `_boxItemHtml`에 `_ro()` 감싼 ✕ + `data-custom-name`(직접입력 게임은 식별 속성이 없어 삭제 자체가 불가였음), `renderList`에 삭제 핸들러(`_openBoxAddSearch` `onAdd`의 역순).
  - ⚠️ **문서에 없던 단계를 발견 — 같은 착각을 반복하지 말 것**: 이 항목의 옛 메모는 "①splice+renderList ②`_emitLikesChanged` 이 둘이면 stale이 안 되살아난다"고 적었으나 **틀렸다**. `_emitLikesChanged`([1616](../assets/js/kakao-auth.js#L1616))는 **이벤트를 dispatch할 뿐**이고, 그걸 듣는 취향보드 리스너([869~](../assets/js/kakao-auth.js#L869))는 앵커 `#tastelikedList`가 **모임보드엔 없어서 스스로 해제**된다 → **모임보드의 `_likedSlugSet`/`_curiousSlugSet`은 아무도 안 고친다**. 그래서 삭제 시 `.delete(String(gameId))`를 직접 해야 이번 주 리스트의 ❤️/👀 마커가 안 남는다(추가 경로 `onAdd`가 `.add`를 직접 하는 이유도 이것). **"이벤트를 쐈으니 동기화됐겠지"라는 추론이 이 코드베이스에선 성립하지 않는 자리**.
  - **스모크 포인트**: ①BGG 게임 ✕ → 모달에서 즉시 사라짐 + 카운트 감소 ②이번 주 일정에 올라간 게임을 원천에서 ✕ → 뒤 이번주 리스트의 ❤️/👀 마커 사라짐(일정 항목 자체는 남는 게 정상 — 저장 분리 사양) ③직접입력 게임 ✕ ④마지막 1개 삭제 → '아직 없어요' ⑤남의 보드(readOnly)엔 ✕ 없음 ⑥confirm 취소 시 무변경 ⑦삭제 후 취향보드 서브시트에도 반영.
