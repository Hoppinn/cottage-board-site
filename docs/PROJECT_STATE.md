...
**진행 상황**: R1~R4 ✅ · R5 ✅ (2026-07-16) · R6 ✅ (2026-07-16) · R7 ✅ (2026-07-16) · R8 ✅ (2026-07-16, 코드변경 없음) — R8: SC4는 이미 count/head(행 미반환)로 해결됨. SC5는 `game_play_records` 전체 **60행**(실측) 규모 + limit(2000) 有라 성능 문제 부재 → RPC는 선제적 과최적화로 보류(A안 승인). 재방문 임계값 ~기록 1500행 기록. R7: GDA2는 이미 IIFE 적용돼 있어(감사 stale) 실질 작업은 GS3 — adapter의 죽은 `window.getAllGamesArray` 노출 제거. 전 14페이지 adapter→game-sheet 로드라 game-sheet 전역版이 항상 덮어써온 것 확인, 런타임 무변화. GDA6 함께 종결. 아래는 R6 상세: —— R6: ACH5 `buildAchievementsSection` 숨은 소급지급 side-effect 분리. `grantRetroAchievements(userId,stats)` public 승격 + 순수 빌드화, kakao-auth.js에서 `!readOnly` 가드로 명시 호출. **부수 버그 수정**: readOnly로 남의 보드 열람만 해도 대상 유저 DB에 소급 insert 되던 것 차단. node --check 통과, 로직추적 4경로 검증. **⚠️ 실서버 스모크 대기 (R1~R6 누적분, 2026-07-16)**: R7(GDA2 IIFE, Plan 필수)부터 위험도 상승이라 그 전에 브라우저 확인 권장. R6 추가 확인 포인트: 내 성장보드 열 때 업적/캐릭터/칭호 카운트 정상 + (있다면)소급지급 반영, 남의 보드 열람 시 정상 표시. **✅ R3/R6 확인 완료 (2026-07-16).** R4(사진첨부 후 새로고침해야 표시·사진클릭 시 썸네일/삭제버튼 없음)·R2(취향 게임추가 새로고침해야 반영)는 둘 다 크로스보드 stale/리로딩 문제(버그2-b)와 같은 뿌리라 **R10 동반 검증으로 이월**. R1(알림 읽음)은 알림 부재로 보류(다음 알림 발생 시 확인). 확인 포인트 ①**성장보드(R3)**: 내 보드 열어 캐릭터/업적/칭호/도감 카운트가 `N/M`으로 정상 표시되는지(regex 파싱→직접전달로 바꿔서 여기가 깨지면 0/0 등으로 보임) ②**사진 첨부(R4)**: 게임시트 사진/플레이 기록 모달에서 사진 여러 장 추가→취소/저장→다시 열기 반복 시 정상, 콘솔 에러 없음 ③**취향 게임추가(R2)**: 취향보드·모임보드 취향박스에서 게임 검색·추가가 되는지(공용 헬퍼로 합침) ④**알림 읽음(R1 KA5)**: 알림 '모두 읽기'/교환권 확인 동작. 이상 없으면 R6 진행.
...
- ✅ **[버그2-a 해결]** 취향/기록보드 **같은 서브시트 내부** 변경이 뒤로가기→재진입 시 되돌아가던 문제(커밋 11e10b8). 원인: `_openSubSheet`가 패널오픈 때 만든 정적 HTML 문자열을 재진입마다 재주입. 해결: `onLeave` 콜백으로 나갈 때 현재 DOM을 문자열변수(let)로 스냅샷→재진입 시 복원. `currentAvoidTags`는 DOM 기준 도출로 정합.
...
**다음 세션 시작점**: **R9(GR3 — `game-reviews.js` 과대함수 3개 `renderRecords` 277줄 등 분리, Opus xhigh, Plan 필수)**. 이후 R11~R13은 REFACTOR_CHECKPOINT.md "처리 계획" 표 순서대로. (R7·R8은 코드 무변화라 별도 스모크 불필요, 다만 R1~R6 누적 브라우저 스모크는 여전히 권장.)
