...
- 현재 작업: 홈 코티지 모임 미리보기의 `플래너 보기` 주변에 모임원 프로필 페이지 모달 진입 링크를 추가하는 범위로 복원했다. 아래 NEXT/BACKLOG/DEFERRED 항목은 승인 없이 착수하지 않는다.
...
- 승인 대기: 가입경로 `보드라이프` 추가 — `member_intros.join_sources` 허용 목록·입력 UI·표시 라벨·최신 RPC 마이그레이션을 함께 변경하는 Red Plan. 홈 모임원 프로필 페이지 모달 링크는 UI 진입점·모달 재사용 범위 확인 후 별도 판단.
...
1. **P3 기능·콘텐츠** — 홈 코티지 모임 섹션의 기존 `플래너 보기` 버튼 왼쪽/아래에 모임원 프로필 페이지를 여는 별도 링크·CTA를 추가한다. 기존 이름 클릭(프로필 보드)과 다른 진입점이며, 구현 전 모달/iframe 재사용 경계를 확인한다. 이후 게임 위치 2건·보유게임 1건을 별도 범위로 처리한다.
