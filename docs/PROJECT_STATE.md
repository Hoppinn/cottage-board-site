...
### ✅ 종료: 전체 리팩토링 순차 처리 (2026-07-15 ~ 07-17 — **R1~R12 전부 완료·스모크 통과**)

R1~R12(+R10a/b/c) 전부 완료. **세션별 결과 요약은 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 「진행 요약」 표**, 상세는 git log. 이 체크포인트에서 파생돼 **아직 열린 것은 둘**:
1. **R번호 미배정 항목** — GS4·GS5·GS7·DD4·IP1~3. REFACTOR_CHECKPOINT.md 「Phase 3 — 열린 항목만」
2. **감사 잔여 항목** — §3 「Phase 1~3 감사 잔여 항목」(2026-07-17 CHECKPOINT 압축 시 이관, 실측 결과 포함)
...
> ⚠️ **분류 정정 (2026-07-16)**: R4에 함께 묶여 있던 **"사진클릭 썸네일/삭제버튼 없음"은 stale과 무관**했음 — 라이트박스 컴포넌트는 정상이고 호출부가 옵션을 안 넘기는 문제(버그2-c로 분리 → **✅ 종료, 커밋 fe98add 2026-07-16 스모크 통과**). R10은 `kakao-auth.js`(KA1)만 건드리므로 `game-reviews.js`·`club-history.html` 호출부는 손대지 않음 → **그대로 뒀으면 R10에서 조용히 누락될 항목이었음**(분리한 게 값을 한 사례).
...
- **R10c ✅ 완료·스모크 통과 (2026-07-17, 커밋 6f506d8·9593237·ade71df + 후속 22d326b·affec36·6f3652a·e886af1·5838e1b)**: `openProfilePanel(sub, {backTo})` 일반화로 §3-1(알림 복귀)·§3-2(게임시트 복귀) 해결. **스모크에서 버그 3건이 딸려 나왔다** — ✕→햄버거 자동열림, 참여자 태그 닉네임 일부 클릭 불가, 게임시트 닉네임→보드가 시트 뒤에 깔림. 셋 다 R10c가 만든 게 아니라 **원래 있었는데 안 보이던 것**(진입 경로가 늘면서 드러남). 판단 근거는 §3 R10c 인용절.
...
**다음 세션 시작점 (2026-07-17 세션 ③ 종료 — 스모크 대기 0건, 진행 중 작업 없음)**: **①이날모임 상세 클릭 진입점 2건**(§3, 사용자 요청, Yellow — 착수 시 재사용할 패턴·z-index 주의 기록해 둠) ②**감지기 3단계**(§3, 선행이던 R10b·R10c 완료로 착수 가능. 실대상은 achievements 5 + game-sheet 9 + `Promise.all` 갭 16곳) ③**GDA3**(§3 감사 잔여 중 유일한 버그성 — 검색 가중치 2배) ④R번호 미배정(GS4·GS5·GS7·DD4·IP1~3 — REFACTOR_CHECKPOINT.md) ⑤`_restoreMenuExpanded` 제거 검토(§3).
...
> - 📌 **햄버거 깜빡임의 근본**(5838e1b): script-nav.js의 "메뉴 밖 클릭 시 닫기"가 보드 ✕ 클릭을 메뉴 밖으로 오인 → 닫힘. `_restoreMenuExpanded`의 30ms는 그 뒤에 다시 여는 **우회책**이었다(= 그 함수가 "무조건 add"였던 이유). 이제 보드 클릭을 제외해 애초에 안 닫힌다.
