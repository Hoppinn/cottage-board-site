...
최종 갱신: 2026-07-09 (모임보드 개편 최종 — 미러링 전환 + 주간 배지 + 룰설명 토글 + 론칭 후 이월)
...
| (이번) | feat: 모임보드 📖 룰설명 per-chip 토글 + 타인 뷰 뱃지. _buildMeetingGameItems 신규(📖 버튼 내장), _ruleSet(can_explain_rules Set), 클릭 시 addMeetingGamePref/removeMeetingGamePref 토글. 타인 뷰에 mb-rule-badge 표시 |
