...
> ✅ **「타인 보드 내부 네비게이션 통일」은 2026-07-21 종결 — 착수가 아니라 실측으로 닫혔다(오탐).** 기재는 "모임보드→취향보드 전환이 바텀시트로 뜬다"였으나 실측한 현재 코드는 그렇지 않다. 근거 3개:
> - ① **서브시트 생성기가 `_openSubSheet` 하나뿐**이다([kakao-auth.js:2479](../assets/js/kakao-auth.js#L2479)). 내 보드·타인 보드가 같은 함수를 쓰고 헤더(‹뒤로가기 + 제목 + ✕)도 같다 → **통일 대상이 애초에 갈라져 있지 않다.**
> - ② 문제의 전환(`.mb-pref-edit`)은 **R10b가 이미 통일**해뒀다([:1055](../assets/js/kakao-auth.js#L1055)) — 기존 `.profile-card` 경로를 재사용하고 뒤로가기 라벨만 '‹ 모임 보드'로 재지정한다. 항목이 원하던 형태 그대로다.
> - ③ 그 버튼 2개는 **`_ro()`로 감싸져 있어 타인 보드엔 렌더되지 않는다**([:2387](../assets/js/kakao-auth.js#L2387)·[:2391](../assets/js/kakao-auth.js#L2391)) → 「타인 보드에서」라는 전제 자체가 성립하지 않는다.
> - 📌 **부수 확인(고치지 않음)**: `.profile-subsheet--readonly`·`.profile-panel--readonly`는 **CSS 정의가 0건**이다. 죽은 코드가 아니라 [js-api.md](js-api.md)에 계약으로 적힌 **스타일 훅**이고, 타인 보드에 시각 차이를 안 두기로 한 현재 설계에선 비어 있는 게 정상이다.
> - ⚠️ **교훈**: 이 항목은 "PC 리팩토링"이라 무거워 보여 대기열 선두에서 계속 밀렸는데, 실제로는 **확인 5분짜리**였다. CLAUDE.md B-5(①확인만 하면 닫히는 것부터)가 가리키던 바로 그 유형이다.
