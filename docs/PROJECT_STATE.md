...
- 현재 단계: 2. 모임 카드 UI 통일 및 중복 상세 제거 구현 완료·화면 확인 대기
- 다음 작업: 모바일 364px 기준 참여자 카드 클릭 무반응, 플래너→이날 모임 상세, regular/compact 게임 썸네일 크기와 PC 화면을 확인한다. 확인 전에는 3번으로 진행하지 않음.
- 인수인계: 1번 미리보기는 확정됨. 2번은 `다가오는 모임`에 일정별 당일 참여 정보 카드와 수정·삭제 액션, `참여 페이스`에 장기 빈도, `최근 참여`에 기존 기록을 유지한다. 참여자 카드 자체가 최종 정보 화면이며 개인 참여 상세 모달로 재진입하지 않는다.
