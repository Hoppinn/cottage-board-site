...
- 현재 작업: 플래너 과거 인원 집계 수정 완료. 사진 sticky 접기 보정을 반영했고, 다음은 P1 시각 회귀다.
...
- 다음 작업: P1 시각 회귀를 한 건씩 조사·수정·검증·커밋한다.
...
2. **P1 시각 회귀** — 사진 sticky 접기, 모임 보드→홈 미리보기 focus, 기록 보드 아코디언 헤더 높이, 센터모달·바텀시트 배경 스크롤 차단. 상세 판단·검증은 [보드 상호작용 회귀 Plan](plans/board-interaction-regressions-plan.md)을 따른다.
3. **P2 데이터 정합성** — 누적 체류시간 원인 감사, `덕지` 기록을 `덕 지` 회원 보드로 연결하는 안전한 닉네임 해소 규칙 조사.
4. **P3 기능·콘텐츠** — 메인 모임 미리보기의 모임원 프로필 링크, 게임 위치 2건·보유게임 1건, 가입경로 보드라이프.
...
| 사진 sticky 접기 | 360px에서 사진 헤더가 sticky인 상태로 `접기 ▲`를 눌러도 헤더 위치가 유지되는지 | 코드가 일반 목록과 동일하게 클릭 전 viewport·scroll 값을 읽어 원래 진입점으로 복원한다. 이 환경에서는 앱 브라우저 연결이 없어 사용자 화면 확인이 남음. |
