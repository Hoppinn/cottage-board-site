...
최종 갱신: 2026-07-17 — **R10c ✅ 스모크 통과**(뒤로가기 2경로 + 회귀 확인). 스모크 중 **버그 3건 발견·수정**(affec36·6f3652a·e886af1) + **UX 3건**(스크롤 복원 22d326b, 복귀 애니·햄버거 깜빡임 5838e1b). **`REFACTOR_CHECKPOINT.md` 정리 ✅ 완료**(474→90줄). **다음 세션**: ①**후속 4건 스모크**(§3 R10c 항목) ②이날모임 상세 클릭 2건(§3, 사용자 요청) ③감지기 3단계.
...
> **R10a ✅ 완료** — `openProfilePanel` 1,940→918줄, 서브시트 6블록 추출(c9ab2bd~b3ed30d), 스모크 통과. R10 원안은 사용자 승인 하에 R10a(추출)/R10b(stale)/R10c(네비스택)로 **분할**.
...
- ✅ **[버그2-b] 크로스보드 stale — 해결 완료 (2026-07-17 R10b, 스모크 통과)**. 방향 A(진입 시 DB 재조회 = 단일 소스)로 처리, 스냅샷 임시방편(11e10b8) 제거. 상세는 아래 R10b 항목.
...
- **R10b ✅ 완료 (2026-07-17, 커밋 507f2e9·c0cd874·5f82aed·a03250c, 브라우저 스모크 통과)**: 크로스보드 stale을 방향 A(진입 시 DB 재조회 = 단일 소스)로 해결. 취향보드 스냅샷(11e10b8)·`_currentBio` 접근자 제거. **스모크 전 항목 정상**(사용자 확인 2026-07-17) — 아래 8포인트 전부, 비동기 렌더로 바뀐 '비선호 수정 →' 스크롤(회귀 1순위)·스크롤 복원 포함.
  ...
  - **스모크 ✅ 통과 (2026-07-17)**: ①취향→모임 게임 추가/삭제 반영 ②모임→취향 ③한줄소개·피하는유형 → 모임보드 칩 반영 ④📖 룰 토글 → 룰 배지 반영 ⑤'비선호 유형 수정 →' 진입 시 피하는유형 섹션 스크롤(비동기 렌더로 바뀐 자리) ⑥'‹ 모임 보드' 복귀 시 스크롤 복원 ⑦게임시트 좋아요 토스트→취향보드 직행 ⑧읽기전용 남의 보드.
  - ⚠️ **R2·R4는 이번 스모크에서 명시적으로 확인되지 않았다**(사용자는 "스모크 다 작동"이라 했으나 위 8포인트 기준). **§0 '남은 스모크'의 R2(취향 게임추가 새로고침 필요)·R4(사진첨부 후 새로고침 필요)는 열린 채로 유지** — 다음에 해당 화면을 만질 때 재현 여부부터 확인할 것. R2는 R10b가 고친 경로와 겹쳐 해소됐을 가능성이 높고, R4(기록보드)는 **범위 밖이었으므로 그대로 남아 있을 것**으로 예상.
- **R10c 🟡 코드 완료·스모크 대기 (2026-07-17, 커밋 6f506d8·9593237·ade71df)**: `openProfilePanel(sub, {backTo})` 일반화로 §3-1(알림 복귀)·§3-2(게임시트 복귀) 해결. 스모크 6포인트는 §3 「[verify] R10c 스모크」.
  - **설계 정정 — 스택은 필요 없었다**: 문서가 "네비게이션 히스토리 스택 필요"로 적어둔 건 과대평가였다. `_openSubSheet`([2420](../assets/js/kakao-auth.js#L2420))에 **서브시트→패널 뒤로가기가 이미 있어** 빠진 건 패널 한 칸(깊이 1)뿐이었고, 체인이 생겨도 각 패널 클로저가 자기 `backTo`를 들고 있어 **자연 스택**이 된다 → 자료구조 0.
  - **복귀도 신규 로직 0**: `autoSubsheet`([2621](../assets/js/kakao-auth.js#L2621))가 `[data-subsheet]`를 클릭시키는 기존 경로를 그대로 재사용(`openProfilePanel('notif')`).
...
**다음 세션 시작점 (2026-07-17 세션 ③ 종료)**: **①R10c 후속 4건 스모크**(§3 — 스크롤·햄버거·참여자태그·게임시트닉네임. 회귀 확인은 완료) ②**이날모임 상세 클릭 진입점 2건**(§3, 사용자 요청, Yellow) ③**감지기 3단계**(§3, 선행이던 R10b·R10c 완료로 착수 가능) ④R번호 미배정 열린 항목(GS4·GS5·GS7·DD4·IP1~3 — REFACTOR_CHECKPOINT.md) 또는 감사 잔여 중 GDA3(유일한 버그성).
...
- [ ] **[PC 리팩토링] 타인 보드 내부 네비게이션 통일** — 모임보드→취향보드 등 전환이 바텀시트로 뜸. 내 보드와 동일한 센터모달 + 고정 헤더 + 뒤로가기로 통일.
  ...
  - [x] ~~**3-1 알림 → 남의 보드 복귀**~~ — 🟡 **코드 완료, 스모크 대기** (2026-07-17 R10c, 커밋 ade71df).
  - [x] ~~**3-2 게임시트 → 취향보드 복귀**~~ — 🟡 **코드 완료, 스모크 대기** (2026-07-17 R10c, 커밋 9593237).
- [ ] **[verify] R10c 잔여 스모크** — **핵심 3건은 ✅ 통과**(2026-07-17 사용자 확인): 게임시트→토스트→취향보드→뒤로→내 보드→뒤로→**원래 게임시트 복귀** / 궁금해요 동일 / 알림→남의 모임보드→되돌아오기. **남은 것은 회귀 확인 + 그 자리에서 나온 후속 3건**:
  - ✅ **회귀 확인 완료** (2026-07-17): 평소 경로 보드 정상. **패널 제목은 원래부터 왼쪽 정렬**(`.profile-panel-header`는 flex `space-between`, backTo가 있을 때만 `--with-back` grid 3열) — 가운데 정렬은 **서브시트 헤더**(grid 3열)와 혼동한 것. 회귀 아님.
  - **후속 4건(2026-07-17 스모크 중 발견·수정, 확인 필요)**: ③**게임시트 복귀 시 보던 스크롤 지점 복원**(22d326b — `closeGameSheet`가 스크롤을 저장 안 하고 있었음. 좋아요/궁금해요 둘 다) ④**✕로 닫을 때 햄버거가 저절로 열리던 것**(affec36 — 이제 햄버거→내 활동으로 연 경우만 메뉴 복귀, 토스트·알림 경로는 닫힌 채) ⑤**참여자 태그 닉네임 클릭**(6f3652a — 기록을 올린 적 없는 회원도 이제 눌림. 게스트 이름은 여전히 안 눌리는 게 정상) ⑥**게임시트 닉네임/아바타 클릭 → 보드가 시트 뒤에 깔리던 것**(e886af1 — 아래 참조).
  - 📌 **z-index를 올려 고치려 하지 말 것 (e886af1 판단 근거)**: `--z-profile`(9100)·`--z-subsheet`(9200) < `--z-sheet`(9500)은 **의도된 순서**다 — 취향보드에서 게임 썸네일을 누르면 게임시트가 패널 **위에** 떠야 하기 때문(박스모달이 z 9700>9500이라 `close()` 먼저 하는 것도 같은 이유). 패널을 9500 위로 올리면 그쪽이 깨진다. 그래서 **게임시트 → 보드 경로는 전부 "시트를 닫고 `backTo`로 복귀"로 통일**(R10c 인프라 재사용). 게임시트 닉네임/아바타 4곳 적용.
  - **알려진 차이**: 기록시트(`openGameRecordSheet`)에서 닉네임을 누르면 복귀 대상이 기록시트가 아니라 **게임시트** — 둘 다 `_currentSheetGameKey`를 쓰기 때문. 게임 정보로 돌아가는 것이라 무해로 판단. 거슬리면 `backTo.type` 분화 필요.
  - ⑦**복귀가 "새로 열리는 것처럼" 보이던 것 2건 수정**(5838e1b, 확인 필요): 게임시트는 `.no-anim`으로 sheetUp 억제 → 그 자리에 그대로 있던 것처럼 / 햄버거는 **깜빡임 없이** 열린 채 유지. 확인 포인트 — 뒤로가기 복귀 시 시트가 안 올라오는지 / **새로 여는 게임시트는 종전대로 올라오는지**(회귀) / 기록시트 '← 게임 정보'도 종전대로 / 햄버거→내 활동→✕가 깜빡임 없이 메뉴 유지.
...
- [ ] **[기술부채] 감지기 갭 — `Promise.all` + 비구조분해 결과는 2단계가 통째로 지나쳤다** (2026-07-17 R10b 중 발견) — 2단계는 `const { data, error } = await`(구조분해) 형태 59곳을 고쳤는데, **`const [aRes, bRes] = await Promise.all([...])` 후 `aRes.data`만 읽는 형태는 대상이 아니었다**. 이 자리들은 `.error`를 아무도 안 봐서 **컬럼 오타·RLS 차단이 여전히 조용히 빈 값**이 된다(`catch`도 안 울림).
  ...
  - ✅ `getMeetingProfile`(profileRes·introRes 2곳)은 R10b가 그 함수를 건드리는 김에 **선처리 완료**(커밋 507f2e9).
  - **교훈**: "위반 N곳" 숫자를 좁은 grep으로 세면 안 된다는 기존 경고(§3 3단계 항목)의 **세 번째 재발**. 이번엔 `} = await` 패턴만 세어 `Promise.all` 계열을 통째로 놓쳤다. 3단계 착수 시 **`.data` 참조 전수**로 다시 셀 것.
...
- [ ] **[기술부채] DB 조회 에러 삼키기 — 1·2단계 완료(supabase-client.js), 나머지 파일 반응형** (2026-07-16 등록 / 2026-07-17 1·2단계 완료) — CLAUDE.md 「DB 함수 에러 처리」가 `catch (_) { return []; }`를 금지하는데 기존 코드에 소급 적용이 안 돼 있던 건. 감사(Phase 2 SC1~SC8)도 놓쳤음.
  ...
  - ⏳ **남음**: `achievements.js` 5곳·`game-sheet.js` 9곳(미착수, 우선순위 낮음). **`kakao-auth.js` 24곳은 R10b에서 재검토 후 "대상 아님"으로 정정 (2026-07-17)** — 이 자리들은 raw supabase가 아니라 **`CottageDB` 래퍼**를 부르므로 `{data, error}`를 받을 대상이 없다(래퍼가 내부에서 이미 수신). 삼키는 건 JS 예외뿐이고 대부분의 래퍼는 reject하지 않아 `.catch()`가 아예 안 울린다 = 저가치. **원래 지시("`{data, error}` 형태로 바꿔야 실효")는 레이어를 잘못 짚은 것**이었음.
