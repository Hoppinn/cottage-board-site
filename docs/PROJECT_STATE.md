...
**진행 중 작업 없음** (2026-07-21 세션 ⑲ 종료 시점, **모임 플래너 동반 지인 인원 종결** — 013 적용·검증 완료) · **열린 스모크 5건**(§0 아래 「열린 스모크」).
...
> 📌 **미결 판단 하나 남음 (`guest_count` 종결분에서 파생)**: 홈 상태 문구 *"N명이 기다리고 있어요"*는 **주간 등록자 수**라 날짜별 인원과 다른 질문이어서 지인을 더하지 않았다. 더하려면 "이번 주 최대 인원"으로 의미를 바꿔야 하고 그러면 **지인 0명일 때도 지금과 다른 숫자**가 나온다(회귀). 바꿀지는 문구의 목적(모집 독려 vs 인원 보고)에 달렸다.
...
- **🎲 동반 지인 실기기 확인 (다음에 플래너를 만질 때)** — 스테퍼는 로그인 없이 DOM만 띄워 봤다(쓰기 차단). **실제 로그인해 지인 2명으로 등록**한 뒤 ①플래너 카드·월간칩·홈 칩·이날 상세가 전부 **3명**인지 ②수정 진입 시 스테퍼에 **2가 그대로** 떠 있는지(0으로 덮이면 지인이 사라진다) 확인할 것. ⚠️ **기존 데이터에 우회 사례가 있다** — 7/20 등록이 게임명 칸에 「지인팟 4인」이라 적어 신고 중이고 화면엔 1명으로 뜬다. 그 등록의 `guest_count`를 채울지는 **사용자 판단**(남의 등록).
...
(비어 있음 — 「동반 지인 인원」은 2026-07-21 종결. 구조는 [js-api.md](js-api.md) `getPartySize`/`sumPartySize`, 컬럼은 [db-schema.md](db-schema.md) 013)
