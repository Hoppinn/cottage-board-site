...
| 2 | **§5 게임 검색 영어 제목 인식 — 선행 「게임시트 영어제목 독립 표시」부터.** | §5 | 기능 | ③기능 |
| 3 | **관리자 분석 P4·P6 — Red+Plan, 실행 전 승인 필요**(P4 남의 보드 관리자 섹션·오너 가드 / P6 집계 정확도 재설계) | [admin-analytics.md](admin-analytics.md) §5 | 구조 | 되돌리기 어려움 |
| 4 | 🚫 **「이날모임 상세창에 등록·수정 버튼」은 승인 없이 만들었다가 2026-07-22 되돌렸다(git `d315b314`).** 되살리려면 **먼저 기획**할 것 — ①등록·수정 진입점을 어디에 몇 개 둘 것인가(지금도 홈 카드 ✎ / 「+ 이날 참여 등록」 / 개인막대 모달로 갈라져 있다) ②등록을 마치거나 뒤로 갔을 때 **보던 상세창으로 복귀**하는 동선. 이 둘 없이 버튼만 붙이면 미완성이다 | — | 기획 | **교훈**: 내가 「결정해달라」고 올린 항목을 내가 집어가면 안 된다(D-14) |
