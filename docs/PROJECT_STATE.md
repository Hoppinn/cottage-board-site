...
최종 갱신: 2026-07-17 — **R10b ✅ 완료(스모크 통과)**. **다음 세션 후보**: ①모임보드 박스모달 삭제버튼(§3, Yellow, 사용자 요청·착수 전 확인 1건 있음) ②`REFACTOR_CHECKPOINT.md` 정리(§0에 절차 있음, R10b에서 이월) ③R10c(네비스택) ④감지기 3단계.
...
- **R10c ⏳ 대기**: 네비게이션 스택(§3-1 알림 복귀 + §3-2 좋아요 토스트→게임시트 복귀, `openProfilePanel(sub, {backTo:{...}})` 일반화). 신규기능. **R10b 구조 메모(아래)를 먼저 읽을 것** — 같은 파일·같은 라우터를 건드린다.
...
**다음 세션 시작점 (2026-07-17 세션 종료 시점)**: **①모임보드 박스모달 삭제버튼(§3, Yellow, 사용자 요청, 착수 전 확인 3건)** ②R10c(네비스택, Opus xhigh + Plan) ③감지기 3단계 ④REFACTOR_CHECKPOINT 정리(R10c 종료 후 권장).
...
- [ ] **[Yellow] 모임보드 취향 박스모달에 게임 삭제(✕) 추가** (2026-07-17 사용자 요청, 미착수 — **다음 세션 1순위 후보**) — 모임보드 `❤️ 좋아하는 게임`/`👀 해보고 싶은 게임` 링크로 여는 센터모달에 **＋게임 추가는 있는데 삭제가 없다**. 취향보드와 동일하게 ✕ 버튼을 달아 달라는 요청.
  - **대상**: [kakao-auth.js:1357](../assets/js/kakao-auth.js#L1357) `_openTasteBoxModal(listType)` — 아이템을 그리는 [`_boxItemHtml`(1381~1390)](../assets/js/kakao-auth.js#L1381)이 `썸네일+이름+룰배지`만 렌더하고 삭제 버튼이 없음. 진입점은 `.mb-taste-link`(`meetinglikedBoxBtn`/`meetingcuriousBoxBtn`, [2263·2267](../assets/js/kakao-auth.js#L2263)).
  - **참고할 기존 구현(취향보드)**: 마크업은 [`_buildTasteGameItems`:2063](../assets/js/kakao-auth.js#L2063)의 `<button class="taste-game-del" type="button" title="삭제">✕</button>`, 핸들러는 [848~867](../assets/js/kakao-auth.js#L848) — `confirm('…목록에서 뺄까요?')` → `removeGamePref(userId, gameId, customName, table)` → `item.remove()` → 0개면 빈 상태 문구 → 카운트 갱신 → **`_emitLikesChanged(table, gameId, false)`**.
  - ⚠️ **착수 전 확인 필요 (사용자에게 질문할 것)**: "똑같이"의 범위가 어디까지인가 — ①삭제 시 **confirm 창**을 띄울지(취향보드는 띄움) ②`readOnly`(남의 보드)에서는 숨길지(취향보드는 `_ro()`로 숨김 — 박스모달의 ＋추가 버튼도 이미 `_ro()` 처리돼 있으니 같이 가는 게 자연스러움) ③📖 룰 배지는 박스모달에선 **표시 전용**인데 삭제만 추가하는 게 맞는지.
  - 🔗 **R10b와 맞물리는 지점(중요)**: 이 모달의 `games`는 [1360](../assets/js/kakao-auth.js#L1360)에서 `_meeting.likedGames`를 잡는데, **R10b 이후 `_meeting`은 진입 시 재조회된 `_boardData`**다. 삭제 구현 시 ①로컬 `games` 배열에서 splice + `renderList()` 재호출(모달 즉시 반영) ②**`_emitLikesChanged` 반드시 호출**(열려 있는 이번주 리스트·`_likedSlugSet`·취향보드가 이걸로 동기화됨) — 이 둘을 빼먹으면 **R10b가 방금 없앤 stale이 이 경로로 되살아난다**. 서브시트를 다시 들어가면 재조회로 맞춰지므로 영구 손상은 아니지만, 모달 안에서 즉시 안 사라지면 버그로 보임.
