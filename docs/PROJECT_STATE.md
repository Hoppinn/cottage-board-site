...
- [ ] **메인 최근 플레이 미리보기 클릭 비활성** (2026-07-16 기록 → 2026-07-18 **수정+1차 스모크 반영, 재스모크 대기**) — 메인 "최근 플레이 미리보기" 카드 클릭 불가 건. [index-page.js `initRecentPlay`](../assets/js/index-page.js#L1017). **1차 스모크 피드백 4건 반영**: ①썸네일→게임시트(`openGameRecordSheet`) 유지 ②**게임명은 클릭 대상 아님**(1차엔 눌렸음, 제거) ③**참여자 이름→각 회원 읽기전용 보드**(`openOtherProfileSheet`) — 이름 태그에 `data-nick` 부여, **이미 로드한 기록에서 nickname→user_id 해석**(recorder로 등장한 이름만 클릭 가능, 미등장 이름은 비활성 = graceful) ④**사진 삭제권한**: 이 기록이 내 것/오너면 `data-record.mine=true`로 라이트박스에서 삭제 노출(`openRecordLightbox` onDelete). 리뷰어 닉네임→프로필 유지. ⚠️ **알려진 한계**: 라이트박스 삭제 후 홈 카드는 새로고침 전까지 stale(onAfterDelete 미전달). **재스모크**: 썸네일→시트 / 게임명 안 눌림 / 참여자 이름→그 사람 보드 / 내 사진 라이트박스에 삭제버튼.
