...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🎯 **P2 — 기준 표기 정리** (#9 요약 카드 기간·말줄임 / #11 "7일" 버튼이 실제 이번 주 / #8 축 라벨 / #16 '전체'가 실은 90일). 전부 원칙 ①(무엇을·언제·누구를 셌는지) 위반. **#15·P1이 값을 맞췄으니 이제 이름을 맞출 차례** | [admin-analytics.md](admin-analytics.md) §4·§5 | 그린·옐로 / Sonnet |
| 2 | 🎯 **P3 — 드릴다운** (이벤트·페이지 → **그걸 한 사람들**로). 원칙 ⑤의 최대 결손, "유기적"의 핵심. 그다음 **P4=C단계**(보드 연동, Red) | 〃 | Plan / Opus |
