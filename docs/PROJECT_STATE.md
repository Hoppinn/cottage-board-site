...
**진행 중 작업 없음** (2026-07-21 세션 ⑱ 종료 시점, **홈 히어로 정렬 버그 + 동호회 요금 문구 2건 종결**) · **열린 스모크 3건**(§0 아래 「열린 스모크」).
...
- [ ] **[기능+DB] 모임 플래너 — 동반 지인 인원 지원** (2026-07-21 사용자 요청. 계기: 홈페이지를 **모임팟 잡는 용도**로 쓰려는 사람이 실제로 나타남) — 등록·수정에 **동반 지인 수**만 추가. `meeting_votes`에 `guest_count integer not null default 0` 하나 → **착수 시 Plan 필수**(DB 변경).
  ...
  - 📌 **회칙에 남은 정합 1건 (세션 ⑱ 문구 개편 중 발견, 미판정)**: [club-rules.html:71](../pages/club/club-rules.html#L71) *"지인끼리만: `지인팟 3인` 형식으로"*가 그대로 있다. **확정 정책은 「지인팟은 플래너에 등록하지 않는다」**이므로 이 신고가 향하는 곳이 플래너인지 카톡방인지 문서에 없다. ⟹ **영향**: 회칙 1줄. ⟹ **경로**: 카톡방 관례면 그대로 두고 「카톡방에」를 명시, 아니면 줄 삭제. **`guest_count` 착수 시 같은 블록을 만지므로 그때 함께 판정**(70행은 이 작업이 입력 필드로 대체한다).
