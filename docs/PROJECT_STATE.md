...
최종 갱신: 2026-07-21 (세션 ㉑). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음** (2026-07-21 세션 ㉒ 종료 시점) · **열린 스모크 5건**(§0 아래 「열린 스모크」).
...
> 📌 **현재 목표**: 관리자 화면을 떠나 **회원이 보는 화면**을 손본다 — 홈 첫 화면과 동호회 모임 운영. 아래 표가 우선순위 대기열이다.
> 🏁 **종료 조건**: 표의 1번(단독 버그 6건)이 **각각 재현 → 판정 → 닫힘**까지 가면 이 목표는 닫힌다. 2~4번은 그다음 목표의 후보다.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 단독 버그: 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 | §2 | 개별 판단 | 전부 **재현부터** 필요. 하나씩 독립 |
...
| 4 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan | 사용자 눈에 안 보이는 리팩토링 |
...
- **🎲 플래너 실기기 확인 — 동반 인원 + 「지난 모임」 저장 (다음에 플래너를 만질 때)** — 둘 다 **쓰기를 차단한 채 DOM만** 봤다. **「지난 모임」 버튼으로 담은 게임이 저장 후에도 남는지**를 한 번 봐야 한다(피커 → `addGameStep2` → `saveAll` 경로는 미검증). 이어서 **실제 로그인해 동반 2명으로 등록**한 뒤 ①플래너 카드·월간칩·홈 칩·이날 상세가 전부 **3명**인지 ②수정 진입 시 스테퍼에 **2가 그대로** 떠 있는지(0으로 덮이면 동반 인원이 사라진다) 확인할 것. ⚠️ **기존 데이터에 우회 사례가 있다** — 7/20 등록이 게임명 칸에 「지인팟 4인」이라 적어 신고 중이고 화면엔 1명으로 뜬다. 그 등록의 `guest_count`를 채울지는 **사용자 판단**(남의 등록).
