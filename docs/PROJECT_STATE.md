...
### 인프라
- [x] 방문자 통계 — `__visitor__` 마커 방식 (113차 버그 수정, 118차 데이터 복구 완료)
...
| 비주얼분석 유입카운팅 오류 | 어드민 비주얼분석 유입 집계 수치 잘못됨. 원인 미조사 |
| 페이지별방문 유입 항목 중복 | 같은 메뉴(유입)가 두 개 표시됨 |
...
- [ ] **업적 8축 순서 재배열** — 비슷한 항목끼리 근처에 배치 (현재 순서 불규칙)
- [ ] **달성 업적 아이콘 컬러 적용** — 현재 달성/미달성 모두 흑백. 달성 시 컬러 아이콘으로 전환
- [ ] **업적명·내용·아이콘 불일치 전수조사** — 업적명, 설명 텍스트, 아이콘이 서로 맞지 않는 경우 찾아서 수정
- [ ] **캐릭터-업적 불일치 전수조사** — 토끼↔다람쥐, 참새↔곰돌이, 너구리→고슴도치 등. 전체 훑어보며 수정

### P2 — 기능 (선택)

- [ ] **게임 위치 페이지** (game-location.html) — 책장 위치 기능 본격 구현
- [ ] **게임위치 협력게임 난이도 분류** — 난이도 2.5 이상 → 어려운 협력게임 / 미만 → 쉬운 협력게임
- [ ] **페이지별방문 내 보드 + 서브시트 카운팅** — 내 보드 패널 및 서브시트(성장/교환권/이용기록) 접근도 페이지별 방문에 포함
- [ ] **방문자목록 회원/비회원 분류 버튼** — 현재 전체순(비회원 포함) 유지 + 버튼으로 회원/비회원 분류 토글
