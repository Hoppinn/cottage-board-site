...
- **후속 3 (2026-07-15)**: ①헤더 줄바꿈 수정(커밋 a81ec1f) — 이모지 복원 후 "💡 이번 주 배우고 싶은 게임" 라벨이 "❤️ 하고 싶은 게임"보다 길어져 우측 "궁금한 게임" 버튼이 줄바꿈되던 것을 간격/크기 값 축소로 해결. ②이번주 게임 목록에 인원조건 표시(커밋 b5e5c0f) — "자세히"(막대그래프 클릭 → openDateScheduleModal)에서 게임별로 설정한 인원조건(베스트/추천/N인, `meeting_vote_games.player_condition`)이 모임보드 게임 목록엔 표시 안 되던 것을 게임명 옆에 표시(`window.formatCondLabel` 재사용, 신규 조회 없음). 같은 게임이 여러 날 등록되고 조건이 다르면 첫 발견값을 대표로 단순화(문서화된 가정, 실사용은 대부분 1일 참여라 영향 적음).
- **스모크 확인 필요**: ①본인 보드 참여일 1일뿐일 때 (요일) 배지 사라짐 ②취향 외 게임 추가 시 ✨ 뜸/취향 게임엔 마크 없음 ③막대 밑 요약칩 사라짐 ④모임보드 라벨 글자만 ⑤남의 보드 자세히 = 전원 막대 차트(편집 버튼 없음).
