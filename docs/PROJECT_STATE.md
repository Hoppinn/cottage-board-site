...
- [ ] **음료교환권 시스템** (진행 중)
  ...
  - [ ] 6단계: 로그 항목에 당시 잔액 표시 (`bal` 역산 방식, 사용자 패널 + 관리자 패널 양쪽)
    - 사용자 패널: `bal`(현재) 기준 hist(DESC 5건) 역산 → 각 시점 잔액 정확
    - 관리자 패널: limit 제거 + ASC 정렬 → user별 Map 누적합 계산 → DESC 재정렬
    - DB 스키마 변경 없음, balance 컬럼 추가 없음
...
- [ ] **페이지/회원 분석 UX 개선** (requests-admin, 설계 검토 완료)
  - 운영 요약 카드 추가: 24h 방문자 / 7일 방문자 / 30일 신규 방문자 (기존 데이터 재활용, 추가 쿼리 없음)
  - 회원 + 비로그인 방문자 통합 목록 → 최근 활동순 정렬, 회원/비회원 뱃지로 구분
  - 이용 상세 접힘 유지
  - 변경 위치: `requests-admin.html` `loadAnalytics()` `membersBody.innerHTML` 블록

- [ ] **업적 목록 UI 개선** — "✅ 달성" → "✓" (도감/RPG 느낌 강화, 정보 중복 제거)
  - 미달성은 현재 그대로 `25/50`
