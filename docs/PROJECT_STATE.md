...
> 🚩 **진행 중인 목표는 「관리자 페이지 정리」다.** 2026-07-19 시작 — **숫자를 맞추는 절반은 끝났고**(P1 도달률·P2 기준표기·P3 시간 단일화·P5 청소), **눈에 보이게 만드는 절반(P3b 드릴다운)이 남았다.**
>
> 📐 **닫는 순서 기준(사용자 확정)**: **쉬운 것 → 묶인 것 → 무거운 것.** 목표가 관리자 페이지라고 해서 **제일 무거운 P3b를 먼저 하는 게 아니다** — 같은 화면 안에서도 쉬운 것부터 닫는다.

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🟡 **관리자 잔여 5건** — `record_start` 유령 발사(#25) · 퍼널 `home_record_write_click` 0건(#18) · `anonTodayS`(#3) · `page`가 라벨 저장(#14) · 알림 교환권 도배. **전부 같은 화면 + 🟡 + 서로 독립.** 목표 화면을 닫아가면서 동시에 가장 쉬운 것부터 | [admin-analytics.md](admin-analytics.md) §4 | 그린~옐로 |
| 2 | 🧩 **프로필 보드 4건 묶음** — 카드 요약이 편집 미반영 · 박스모달이 닫힘 · 타인 보드 네비게이션 통일 · 수집보드 스크롤 진입점. **네 건 다 같은 서브시트 구조**라 한 번에 볼 값이 있다 | §3 | 옐로 |
| 3 | 🔔 **알림 2건 묶음** — 교환권 도배 + 관심 기반 묶음 알림. 도배를 고치려면 어차피 묶음 구조를 본다 | §3 | Red+Plan |
| 4 | 🎯 **P3b — 드릴다운 (관리자 페이지의 나머지 절반)** — 이벤트·페이지 숫자에서 **"이걸 한 사람들"**로 내려가는 길. 사용자가 처음 말한 **"유기적으로 드러났으면"**이 이것이다. **남은 것 중 유일하게 Plan이 필요한 큰 건**이라 앞의 것들을 닫아 목록이 짧아진 뒤에 제대로 본다. 선행(#22·#24·R3)은 완료 | [admin-analytics.md](admin-analytics.md) §5 | Plan |
| 5 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 |
| 6 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독. **로드순서 선행조건은 회피 가능**하다는 결론이 거기 있다 | §3 | Plan |

> 📌 **이 표는 2026-07-20에 두 번 뒤집혔다.** ①보드 4건을 1번으로 → ②"관리자 페이지 목표"에 겁먹고 P3b를 1번으로(**사용자 기준에 어긋남**) → ③현재. **③의 근거**: P3b가 남은 것 중 **유일하게 Plan이 필요한 건**이라는 걸 그때 처음 따졌다. 순서를 또 바꾸려면 [CLAUDE.md 「입장을 바꿀 땐 무엇을 새로 알았는지를 댄다」](../CLAUDE.md)에 따라 **새로 안 것을 한 줄로 적을 것.**
