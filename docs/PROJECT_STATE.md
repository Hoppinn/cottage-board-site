...
최종 갱신: 2026-06-16 (58차 — 캐릭터 크롭 좌표 재검증, 재실행 대기)

---

## 0. 진행 중 작업 (세션 시작 시 확인)

### C작업: 캐릭터 PNG 재크롭 (좌표 재검증 완료, 실행 대기)

**현재 상태**: crop-characters.js 좌표 v2로 업데이트됨. 진단 이미지로 4개 계열 완전체 확인 완료.  
**다음 작업**: `node scripts/crop-characters.js` 실행 → 17종 재출력 → 브라우저에서 내 활동 패널 확인

**수정된 핵심 좌표 (v2)**:
| 계열 | top | height | 변경 이유 |
|------|-----|--------|----------|
| 토끼 | 25 (← 52) | 130 (← 100) | 머리/귀 상단 27px 잘렸음 |
| 다람쥐 | 295 (← 325) | 120 (← 100) | 머리 상단 30px 잘렸음 |
| 고슴도치 | 755 (← 775) | 90 (← 65) | 위 20px + 아래 잘렸음 |
| 햄스터 | 755 (← 775) | 90 (← 50) | 위 20px + 아래 크게 잘렸음 |

**보류 항목**: 기존 플레이 기록에 대한 업적 수동 부여 (SQL 실행됨 확인, 새싹 토끼 1개 지급됨)
