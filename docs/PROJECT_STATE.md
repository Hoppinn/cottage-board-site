...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | ⏳ **iframe 3배 계상 수정의 사후 확인 — 데이터가 쌓이면 즉시.** `node scripts/verify-iframe-rows.js` 한 줄이면 판정된다(음성 대조군 `--negctl` 먼저) | [admin-analytics.md](admin-analytics.md) §4 ①·⑦ | 검증 | **2026-07-21 재측정: 수정 커밋(19:00) 이후 들어온 행이 0행**이라 아직 판정 불가. 사람이 할 일은 없고 시간만 필요하다. **#28 수정도 같은 대기 중**이라 한 번에 잰다 |
