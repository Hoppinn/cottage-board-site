...
최종 갱신: 2026-07-22 (세션 ㉟ — **P4 완료: 회원 보드 오너 전용 「회원 분석」 섹션 + 집계 단일 소스화. 관리자 분석 큰 덩어리 전부 종결**). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음** (2026-07-22 세션 ㉟ 종료 시점 — P4 완료, 관리자 분석 큰 덩어리 전부 종결) · **열린 버그 0건**(§2) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0** · **열린 스모크 6건**(§0 아래 「열린 스모크」).
...
- [ ] 🔴 **[Plan 필수] 한 플레이기록에 여러 사람의 게임평** (2026-07-15 요청 → 2026-07-22 재정의) — 원래 요청은 「뽁님 7/11 게임평을 호핀 기록에 연결」이었고, **사용자가 원한 모양은 「기록 1개에 뽁님 게임평만 달리는 것」**이다.
  ...
  - 📌 **착수 전 2가지**: ①game-sheet 계열은 동작을 코드로 요약하지 말고 [js-api.md](js-api.md) `onSubmitCommentModal` 줄을 먼저 읽을 것(경로별 동작이 거기 정확히 적혀 있다) ②운영 DB에 쓰기 전 「그래서 화면이 어떻게 되는가」를 먼저 그려볼 것.
