...
>
> 📐 **순서**: 목표 화면 **안에서** 쉬운 것부터 — 그 쉬운 것은 2026-07-20에 전부 닫혔다. **지금 1번은 #27**(사용자가 보고 있는 숫자가 틀렸고, P3b가 그 위에 얹히므로 먼저다). 그다음 P3b가 목표를 닫는다. #14는 소급 결정이 걸린 별건이라 목표를 닫는 데 필수가 아니다.

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🔴 **#27 「비회원 N명」이 부풀어 있다** *(2026-07-20 사용자 신고)* — 요약 탭은 오늘 비회원 14명인데 회원 탭엔 3명. 실측: 마커 14개 중 **11개가 세션 표 어디에도 없음**. **07-19부터 급변**(유령 비율 0% → 79%)이라 설계가 아니라 최근 변화다. **원인 미확정 — 착수 첫 동작은 원인 가르기**(새 크롤러냐 vs 500ms 레이스냐) | [admin-analytics.md](admin-analytics.md) §4 #27 | 조사 |
