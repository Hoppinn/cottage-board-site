...
| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🟡 **관리자 잔여 4건** — 퍼널 2-2→2-3이 순차가 아님(#26) · `anonTodayS`(#3) · `page`가 라벨 저장(#14) · 알림 교환권 도배. 전부 같은 화면 + 🟡 + 서로 독립. *(#25 종결 / #18 판정종결 — 2026-07-20)* | [admin-analytics.md](admin-analytics.md) §4 | 그린~옐로 |
...
>
> 🚨 **#18에서 얻은 것 — "0건"은 발견이 아니라 질문이다.** 대장의 「DB 0건」이 **낡은 값**이었고(실측 5건, 2026-07-19부터) 그동안 **없는 병의 원인 후보를 넷 세워두고** 있었다. 착수 첫 동작은 가설이 아니라 **그 숫자를 지금 다시 재는 것**이다. 이건 #3의 "현재 발현 0건"에도 그대로 적용된다.
