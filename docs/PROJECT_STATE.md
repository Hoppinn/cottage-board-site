...
| c2ae239 | feat: 추천게임찾기 난이도 제한 토글 칩 (기본 ON=2.5 제한, OFF=해제). 난이도 필터(헤비/하드코어) 선택 시 토글 무관하게 5.0 유지. 360px 확인 완료 |
| 5c81f78 | docs: 론칭 후 이월 섹션 신설 + MEETING_REVAMP_CHECKPOINT 완료 처리 (13개 항목 이월 표 형식으로 분리) |
| 7929080 | docs: CLAUDE.md UI 요소 안보임 버그 디버깅 순서 원칙 추가 |
...
**확정 설계 (사용자 승인)**:
- 모임보드 하고싶은=meeting_vote_games(이번주,want)/배우고싶은=(learn). 게임별 그룹+요일배지. **이번 주만** 스코프.
- 두 섹션(이번주일정 미니바 + 게임리스트) **유지**, 단 플래너 fetch 1회로 공유(재조회 금지).
- 📖 can_explain_rules → 취향·모임 **양방향 공유**(meeting_game_prefs).
- 칩: 취향보드=[📖][✕], 모임보드=[📖][⋯]. ⋯={이번주 일정 수정(요일 픽커), 이번주에서 빼기}.
- 모임보드 추가=**날짜 필수** 플래너 등록 + (미등록 게임이면) "좋아하는 게임에도 추가?" 옵션. 미정 제거.

**추가 확정 (2026-07-13 판단)**:
- **금요일 orphan 방지**: 데이픽커 요일 선택 = **내가 이번 주 참여 등록한 요일(meeting_votes)** 중에서만. 참여 등록 없으면 "먼저 참여 요일 등록 → 플래너 열기" 안내. 게임 요일 ⊆ 참여 요일.
- **1-1 문구**: "미정" 문구 삭제(날짜 필수). 대신 "☑ 좋아하는 게임에도 추가할까요?"(미등록 게임일 때만).
- **1-2 좋아요 관계 = M2**: 좋아요 추가는 옵션. 이번주 하고싶은 게임 칩 중 **좋아하는 게임이면 ❤️ 작은 표시** → ❤️ 없으면 "이번만" 게임으로 자연 구분. (자동 필수 추가=취향 오염이라 반대.)
- **모임프로필 한줄소개(bio) 행 삭제**(위 선호 스타일과 중복). 나머지(활동지역/참여시간/이동범위)는 맨 아래 그대로 유지, 전부 표시.
...
**뒤집히는 이전 작업**: #5-1(데이픽커 미정→필수), 631a5ba(미러링 소스 교체). 완료 시 해당 문서 상태 갱신.

---

### 신규 작업 분류 (2026-07-13, E그룹 — 썸네일/모임보드 후속)
...
| [x] E-4 모임플래너 편집 버튼 이동+아이콘화+센터모달 | 모임보드 모달이식 | 완료(사용자 결정: 플래너 iframe 센터모달 1개 + 공용유틸 + open시 목표상태 전체선언). "모임 플래너에서 수정하기 →" 하단 CTA 제거 → "📅 이번 주 일정" 타이틀 옆 "✎ 편집" 아이콘. `window.openPlannerModal(opts)` 공용유틸 신설(day-detail.js, 전 페이지 호출) — club-schedule.html?embed=true를 iframe 센터모달로, open 시 cottage-reset-week(weekOffset)/register/edit 전체 선언, cottage-planner-ready 핸드셰이크, 조건부 재로드(contentWindow.pathname 확인), cottage-meeting-saved→dirty→onDirtyClose. 미니바 로드를 `_loadMeetingWeek()`로 감싸 편집 후 재조회. z=--z-shelf. 하니스 검증: 아이콘·CTA제거·모달생성·iframe 실제 로드+ready+주간뷰 렌더 확인. js-api.md 갱신. ⚠️ 실서버 편집→저장→미니바 갱신은 사용자 브라우저 최종 확인 권장 |
...
### 작업 우선순위 분류 (2026-07-12, 모델/effort 배정 포함)

> A = 2026-07-12 세션 신규 보고, B = 론칭 후 이월(§0 하단), C = 현재 버그(§2), D = P1~P3 추후 작업(§3). 상세 원문은 각 섹션 참조.

**🔴 레드 (Opus xhigh)**

| 항목 | 소요 | 비고 |
|------|------|------|
| A-1 c2daace 미반영 원인 파악 | 1~2h | 최우선. 수정 아닌 조사만 — 조사 스냅샷은 본 문서 하단 "c2daace 조사 스냅샷" 참조. 브라우저 콘솔 확인부터 |
| C 기록보드 시간 전부 09:00 | 1h | KST+9 냄새. DB 저장값(date형 여부) 확인 선행 |
| C 관리자 금일데이터 간헐적 미표시 | 1~2h | 간헐적=원인불명, 재현조건 수집 먼저 |
| C 서브시트 음영(3가설 실패) | 1h | 원인불명, 영향도 낮아 후순위 |
| A-9 에러로그 + 게임정리법 등록 시스템 | 기획 별도 | 기획은 대화로, 실행만 Opus |
| D meeting RLS(Edge Function) | 2~3h+ | Plan 모드 필수 |
| D 관심 기반 묶음 알림 | 2~3h+ | Plan 모드 필수 |
| D 이용시간 기기 중복 방지(서버 세션) | 2h+ | C의 중복 카운트와 통합 처리 |
| B 관리자 분석 2·3단계 / 퍼널 시스템 | 각 2h+ | 급하지 않음 |

**🟡 옐로 (Sonnet high / Opus medium)**

| 항목 | 소요 |
|------|------|
| A-4 주 계산 오류(지난 날짜 노출) | 30m~1h — D의 KST 비교 버그와 같은 뿌리일 수 있어 한 세션에 같이 |
| A-7 게임평→캐릭터/업적 미반영 | 30m~1h |
| A-10 지난 일정 흐리게+수정불가 | 30m |
| A-8 캡션 복사 인스타/단톡 분리 | 30m~1h |
| D KST 비교 버그 | A-4와 통합 |
| D 타인 보드 네비 통일 | 1h |
| D 게임 검색 영어 제목 | 30m~1h |
| D 동호회 가입 추적 | 1h |
| C 사진 개별 삭제 | 30m~1h |
| B 집계 모달 리디자인 / 내 등록 관리 동선 / 소개글 알림 분리 / 여러 주차 사전 등록 | 각 1h± |
| D price-rules 사진 재구성 / 기록게시판 디자인 / 모임보드 빈 상태 | 각 1h± |
...
**이번 주 세션 순서 제안**: ① A-1 조사(Opus xhigh, 결과에 따라 A그룹 재작업 범위 결정되므로 최우선) → ② A-1 결과 반영 + 그린 4종(Sonnet) → ③ A-4 + KST 버그 통합(옐로) → ④ A-7, A-10(옐로)

### 🔵 진행 중: 모임보드 재설계 (2026-07-13 시작, 3커밋 계획)

**배경**: c2daace 4건 미작동 조사 → 재설계로 흡수. 사용자 결정: #2 썸네일만 클릭 / 연동버튼 제거+게임추가 센터모달 / 선호·비선호 상단 요약 / 섹션 순서 변경.

**헤드리스 하니스** (`_harness.html`, 작업 후 삭제): getKakaoUser localStorage + CottageDB/gameData 목킹으로 openProfilePanel('meeting') 렌더. game_likes=한글슬러그 id, meeting_vote_games=숫자 bggId로 목킹.

**c2daace 최종 판정 (하니스 검증)**: ①프로덕션 배포 확인(cottageboard.com fetch, 코드 마커 전부 존재) ②하니스에서 c2daace 로직 **정상 작동** — 플래너 전용 항목 append + # 제거 + 요일배지 모두 정상. → 코드 자체는 건강. 사용자 실환경 실패는 **특정 데이터 형식**에서만 발생(재현 필요 시 실제 meeting_vote_games 행 확보). #4/#1은 재설계 커밋3에서 강건화 + 하니스 다형식 검증 예정.

**진행**:
- [x] **커밋1 — 구조 재배치**: 섹션 순서 일정→하고싶은→배우고싶은→선호/비선호 요약(신규)→최근참여→모임프로필(맨뒤). "취향보드와 연동 →" 버튼 제거. 선호(meetingStyle)/비선호(profiles.avoid_tags) 읽기전용 요약 칩(둘 다 기존 스코프 데이터, DB 변경 없음). 프로필 저장 시 선호 요약칩 라이브 갱신. 게임추가 버튼 라벨행 인라인 이동. 하니스 검증 완료(순서·요약·저장 무에러).
...
- [x] **#5-1 양방향 연동** (결정: 추가 시 요일 묻기 / 복수+미정 / 기존은 미정): 게임추가 센터모달에 요일선택 단계 추가 — 게임 추가 직후 이번주 7일(월~일) 칩 표시, 복수선택 시 `addMeetingVoteGame`, 해제 시 `removeMeetingVoteGame`, 안 고르면 미정(플래너 미등록). 완료 시 보드 칩에 요일배지 라이브 반영. 지난 요일 disabled. **주의 2건 반영**: ①`meeting_vote_games.game_id`는 INT(bggId)인데 게임 칩은 슬러그 → `window.gameData[slug].bgg.id`로 변환 후 전달(`_toVoteArgs`), bggId 없으면 게임명 custom_name. ②요일 날짜는 `toISOString()`(UTC, KST 하루밀림) 금지 → 로컬 포맷(planner `toDateStr` 동일)으로 계산. FK 제약 없음 확인(meeting_votes 선행 불필요, 독립 insert). 하니스 검증: 요일칩 로컬날짜 정확, vote 등록 game_id 숫자(104), 배지 반영, 미정 시 미등록.
...
- [→이월] **이번 주 모임 섹션 추가 기획** — 날짜별 미니 막대 상세화, 모임 참여 버튼 연결 (낮은 우선순위).

---

### ✅ CHECKPOINT: 모임 기능 개편 — 완료 (2026-07-09)

**문서**: `docs/MEETING_REVAMP_CHECKPOINT.md` (완료, 이식 후보 보존 중)
...
- [x] **2.7단계: 대표 게임** — ① DB 레이어 완료 (getMeetingVoteGames 필드 확장 + setMeetingVoteGamePriority 신설). ② UI 레이어 완료 (막대 대표 우선 정렬, 센터모달 ⭐N 집계·표시, buildGameTags 2차 정렬). ③ 막대 라벨 ⭐ 표시 — is_priority 게임 약칭 앞 ⭐ 프리픽스 (08989de)
  - ~~⚠️ 알려진 이슈: 홈 top-level openDateScheduleModal 경로에서 갱신 안 됨~~ → **해결** cc2b0bc: opts.onDirtyClosed 콜백 패턴, 닫을 때 1회 재조회 (홈·플래너 양쪽)
...
- [x] **6단계: 등록 모달 다주치** — 전체 완료
  ...
  - [→이월] **④ 후보: 내 등록 관리 동선** — 등록은 홈 직행 가능하나 수정·삭제는 플래너 수동 탐색 필요. 방향: Step 1 "등록됨" 칩 클릭 → 해당 날짜 편집 모드(시간/게임 수정 + 삭제). 009 unique 제약·upsert 경로 충돌은 009 Plan에서 선확인.
...
- [→이월] **집계 모달 리디자인** — 센터모달 집계 화면 개편. 설계 논의 선행 필요.
...
- ~~**[관리자 분석 상위 탭 클릭 시 스크롤 정렬]**~~ — 완료 (재검증). requests-admin.html:612 탭 click 핸들러에 `analysisControlsAnchor.scrollIntoView({behavior:'instant', block:'start'})` 이미 구현됨.
...
## c2daace 조사 스냅샷 (2026-07-12, 조사만 — 수정 없음)

**배경**: c2daace(fix: 모임보드 개편 후속 3건) 커밋이 보고한 4개 수정(플래너 매칭·# 표기, 클릭영역 분리, 취향보드 연동, 접기/더보기)이 브라우저에서 전부 미동작. 캐시 문제 아님(하드리로드 확인됨). 원인 조사 결과만 기록, 수정은 하지 않음.

**확인된 사실**:
- `git log --oneline -10` 상 c2daace가 HEAD, `git status` clean, `main`이 `origin/main`과 동일(push 완료) — 배포 브랜치 반영 안 된 것은 아님.
- `git show c2daace --stat` — `assets/js/kakao-auth.js`(81줄 변경), `docs/PROJECT_STATE.md`(5줄) 2개 파일만 변경. 커밋 메시지가 주장한 파일과 실제 변경 파일 일치.
- diff 실제 내용 확인 결과 코드는 **실재함** (허위 보고 아님): `_buildMeetingGameItems(games, ruleSet, maxInitial=2)` 접기/더보기 로직, `.taste-game-item--clickable` 클릭 핸들러에 `.mb-rule-btn`/`.mb-week-badge` 가드, `_canonId`/`_gCanon` 정규화 함수, 주간 배지+정렬+더보기 재빌드 블록 — 모두 kakao-auth.js 2043~2188줄 부근에 존재. `_getGameKeyById`도 709줄에 실제 정의됨(가짜 참조 아님).
- 이 블록 전체는 `openProfilePanel()` 내부 "이번 주 일정 — mini bar" async IIFE(2082~2188줄) 안에서 실행됨. `#mbWeekSection` 존재 여부로만 조건 분기, isMine 여부로 스킵되는 구조는 아닌 것으로 보임(추가 확인 필요).
- 미확인(다음 조사 대상): ①실제 브라우저에서 이 IIFE가 정상 실행되는지(콘솔 에러로 중간에 throw 되는지), ②`_getGameKeyById`가 실제 게임 ID들에 대해 기대한 canonical key를 반환하는지(반환 실패 시 `#`은 사라지지만 여전히 숫자 그대로 표시될 수 있음 — "# 그대로"라는 증상과는 불일치하므로 이 블록이 아예 도달 안 했을 가능성), ③"취향보드 게임" 표시가 이 코드 경로가 아닌 다른 렌더 함수(예: 모임보드가 아닌 다른 화면·다른 postMessage 경로)를 타고 있을 가능성.
- 결론(잠정): 코드는 실재하고 push까지 됐으나 **런타임에서 실행되지 않거나 다른 실행 경로로 우회되고 있을 가능성이 높음** — "커밋만 하고 안 함" 문제가 아니라 "커밋한 코드가 조건상 도달하지 않음" 계열 버그로 추정. A-1 다음 세션에서 Opus xhigh로 브라우저 콘솔 직접 확인(에러 로그, IIFE 진입 여부 breakpoint) 후 진행 요망.
