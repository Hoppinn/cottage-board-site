...
**진행 중 작업 없음** (2026-07-21 세션 ㉓ 종료 시점) · **열린 버그 0건**(§2) · **열린 스모크 6건**(§0 아래 「열린 스모크」).

> 📌 **세션 ㉓의 교훈 — 「열린 버그」 목록의 절반이 죽어 있었다.** 6건을 재현부터 봤더니 **4건이 이미 해결됐거나 애초에 사실이 아니었다**(09:00은 2일 뒤 고쳐진 채 방치 / 사진 개별삭제는 기능이 있었는데 「없다」고 적혀 있었음 / 모서리 음영은 리팩토링 중 해소 / 캐릭터는 문서 3곳이 서로 다른 숫자). **착수 첫 동작을 `git log -S`와 실측으로 잡은 것이 이 판정을 만들었다** — 가설부터 세웠으면 없는 병을 고치고 있었을 것이다. 위 「"0건"은 발견이 아니라 질문이다」와 같은 병이며, **세 번째 반복이다.**
...
>
> 📌 **현재 목표**: **다음 목표를 고르는 것부터**가 첫 판단이다. 아래 표 2~4번이 후보이며 셋은 성격이 완전히 다르다 — 2번은 신규 기능(무거움), 3번은 관리자 화면(가벼움), 4번은 사용자 눈에 안 보이는 리팩토링. **「회원이 보는 화면」이라는 직전 방향을 이어가려면 2번**이고, 그 경우 Plan이 먼저다.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 🔴 **iframe 3배 계상 수정의 사후 확인 — 배포 하루 뒤. 이 세션이 남긴 유일한 숙제** `script-nav.js`에 프레임 가드를 넣었다(2026-07-21). 홈 방문 1회가 `page_sessions` **1행**인지 실측할 것 | [admin-analytics.md](admin-analytics.md) §4 ①·⑦ | 검증 | 수정 근거는 실측이지만 **결과는 실서버에서만 확인된다**(localhost·관리자는 추적 제외라 로컬 재현 불가). 🔎 **사용자에게 먼저 물을 것: "배포하셨나요?"** — 안 했으면 이 항목은 아직 잴 수 없다 |
...
**열린 버그 없음** (2026-07-21 세션 ㉓에서 §2 마지막 2건 종결 — 근거는 git log).

- 「서브시트 상단 모서리 음영」은 **사용자 확인으로 해소**(이전 리팩토링 중 함께 고쳐진 것으로 보임).
- 「금일이용데이터 간헐 미표시」는 **표시 층에서 원인 분기를 제거**했다 — 아래 잔여만 남는다.

> ⚠️ **금일이용데이터가 또 안 보이면 그건 새 증상이다.** 「오늘 흔적이 있으면 0이어도 칩을 그린다」로 바꿨으므로 **0이라 사라지는 경로는 없다.** 그래도 안 보이면 남은 후보는 **에러 수집 부재**(§3 「감지기 갭」 ③ — `console.error`가 우리에게 안 옴)뿐이고, 그건 이미 별도 항목이다. **기각된 가설 4건은 [admin-analytics.md](admin-analytics.md) §6에 있으니 재조사 금지.**
