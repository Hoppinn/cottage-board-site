...
**진행 중 작업 없음** — 2026-07-27 세션 ㊴에서 사용자가 제보 15건을 일괄 등록, 8건 종결(직접방문 집계=정상, 이용시간 카드/막대 불일치=버그아님, 참여자 카운팅 누락=수정 `a7a1880a`, 간식알림 Part1=구현 `10839a00`, referrer 4곳 통일=수정 `2f0e2618`, 업적 알림판 미기록=수정 `3168a4cc`, 알림 읽음 경합=수정 `5747a7a6`, 홈 최근기록 미반영=수정 `37ad9946`). **열린 버그 3건**(§2) · **열린 기능 5건**(§3, Plan 필요 2건 포함) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0** · **열린 스모크 6건**(§0 아래 「열린 스모크」). **미배포 커밋 있음(push는 사용자)**.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | **햄버거(⋯) 메뉴 잘림** | §2 버그 1번 | 버그(CSS) | 해법 패턴이 DESIGN_RULES.md 6-a에 이미 있어 적용만 하면 됨 — 쉬운 것 먼저 |
| 2 | **플래너 지난모임 탭 정렬 순서** | §2 버그 2번 | 버그 | 미조사, 범위 좁아 보임 |
| 3 | **PC 페이지 폭 — game-reviews.html 760 vs 720** | §2 버그 3번 | 확인 우선 | 1440px에선 시각적 차이 없었음 — **사용자가 본 화면이 이건지부터 재확인**, 그 다음 수정 여부 판단 |
| 4 | **간식·음료 요청 Part2(관리자 처리됨 표시 + 요청자 완료알림)** | §3 P1 2번 | 기능/Plan | DB 컬럼 신규 — Plan 모드 먼저 |
| 5 | **모임시간 30분단위 / 게임정리·룰설명 관리자추가 / QR 추천게임 집계** | §3 P1 3~5번 | 기능 | 명확한 스펙, 서로 독립 — 순서 무관하게 진행 가능 |
| 6 | 🚫 **「이날모임 상세창에 등록·수정 버튼」** — 승인 없이 만들었다가 2026-07-22 되돌림(git `d315b314`). 되살리려면 먼저 기획(진입점 개수·복귀 동선) | — | 기획 | 오늘 안건과 무관한 기존 항목, 결정 대기라 낮은 순위 유지 |
| 7 | **유형검사(MBTI식) 신규 기능** | §3 P2 1번 | 기획/Plan | 규모 최대 — 다른 항목 정리 후 |
