...
- **📵 간식·음료/건의 요청 카카오톡(Make) 알림 미수신 — 원인 확인됨 (2026-07-27), 재활성화만 남음** — Make.com 시나리오 `5213346`("Integration Webhooks")이 **처리 중 에러로 자동 비활성화(Inactive)돼 있었다**(사용자가 대시보드 스크린샷으로 확인). 마지막 수정일 6/25, **그 이후 발생한 알림 44건이 대기열(queue)에 쌓인 채 처리 안 되고 있음.** 코드는 무관 — Supabase Database Webhook → 이 시나리오 → 카카오 "나에게 보내기" 전부 대시보드 설정(`voucher_log`도 같은 방식, [db-schema.md](db-schema.md)). **내가 대시보드에 접근할 수단이 없어 여기서부터는 사용자만 진행 가능.**
  - **남은 단계**: ①"INCOMPLETE EXECUTIONS" 탭이나 대기열 항목 하나를 열어 **구체적 에러 메시지 확인**(화면엔 "The reason is: ."로 잘려 안 보였음 — 카카오 토큰 만료가 유력 후보) ②원인 확인 후 **"Inactive" 토글을 켜서 재활성화** ③⚠️ **재활성화 시 밀린 44건이 한꺼번에 처리될 수 있음** — 이미 관리자 화면에서 수동 처리된 오래된 요청들이면 "Show queue"로 먼저 내용 보고 지울지 판단할 것.
  - 에러가 반복되는 근본 원인(예: 토큰 만료)이면 Discord 웹훅 전환이 근본 해결책(Make 시나리오 HTTP 모듈 교체만 하면 됨, 예전부터 나온 안).
  - **재개 조건**: 사용자가 재활성화 완료 + 44건 처리 여부 결정 후 알려주면 종결.
