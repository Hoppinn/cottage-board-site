...
- P2 완료: `player_names`를 쉼표 토큰의 `normalizeNick` 정확 일치로 보정했다. 충돌 키는 연결하지 않고 원문 기록·작성자 권한·모임 데이터는 보존한다. fixture와 읽기 전용 감사로 `덕 지`↔`덕지` 및 부분문자열 오탐 제거를 확인했으며 업적 지급 write는 실행하지 않았다.
