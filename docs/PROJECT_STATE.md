...
- 현재 단계: 1~4순위 코드 수정 및 사용자 브라우저 확인 완료. 5순위 보드 CTA와 6순위 메인 모임 카드 위치 보정 코드는 구현 완료. 프로필 보드의 작성/수정 CTA는 임베드된 모임원 프로필 페이지가 준비된 뒤 그 페이지의 기존 위저드 버튼을 직접 클릭하고, 기록 작성 CTA는 보드 위 iframe 센터모달에서 기록 입력 탭을 열도록 보완했으며 사용자 브라우저 확인 대기
