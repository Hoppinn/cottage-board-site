...
- **`docs/DESIGN_AUDIT.md`** (진행 중 — 작업 재개 전 필독)
  - 2026-06 디자인 감사 / 일회성 임시 문서
  - 플레이기록 색상·구조 개선 대부분 완료 (§1 완료 이력 참조)
  - 게임별 탭 디자인 감사 완료 (사용자 확인)
  - 모임별/게임별 탭 완료, 정보형 페이지(about, price-rules) 여백 개선 완료
  - price-rules 전 섹션 목록화 완료, club-rules 참여 안내로 개편, club-history pr-* 클래스 통일
  - price-rules 꼭 지켜주세요 항목 추가(건물 밖 대화 자제, 과음 후 이용 금지), 결제 칩 브랜드 색상, 위계 간격 전체 개선
  - price-rules: 4개 요약 카드(요금/운영/외부음식/야간정숙) 추가, 결제칩 rgba 뮤트, 푸터 여백
  - club-rules: 3개 인트로 카드 + 섹션 7개→4개(참여방법/일정변경/지인동반/기본매너) 재구조화
  - club-history: 날짜 섹션 기본 접힘(is-open 제거)
  - **다음 작업**: club-history 실제 동작 검증, 푸터 정보 묶음(낮음) (§7 참조)
