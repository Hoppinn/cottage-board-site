...
- [ ] **메인 최근 플레이 미리보기 클릭 비활성** (2026-07-16 기록 → 2026-07-18 **수정 적용, 스모크 대기**) — 메인 "최근 플레이 미리보기" 카드에서 ①게임 썸네일 ②인원 ③사진이 전부 클릭 안 됨. **수정**([index-page.js `initRecentPlay`](../assets/js/index-page.js#L1017)): 게임-리뷰 허브와 동일 진입점 재사용 — 썸네일·게임명·인원(`.pr-player-header`)→`ensureGameSheet`+`openGameRecordSheet(gameKey)`, 사진→공용 `openRecordLightbox`(row에 `data-record` 추가). 리뷰어 닉네임 클릭(→프로필)은 기존 유지. ⚠️ **판단 필요분**: "②인원" 클릭 타깃이 명시 안 돼 있어 **게임시트로** 매핑함(참여자 이름이 여기선 user_id 없는 순수 텍스트라 보드 연결 불가). 스모크에서 이 매핑이 맞는지 확인. **스모크**: 홈 최근 플레이 카드에서 썸네일/게임명/인원 클릭→게임시트, 사진 클릭→라이트박스, +N장→나머지 사진 펼침.
