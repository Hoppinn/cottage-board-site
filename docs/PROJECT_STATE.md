...
- [ ] **[버그] game-reviews 허브 참여자 이름 해석 `p.id` → `p.user_id`** (2026-07-18 발견, 미착수) — [game-reviews.js:501](../assets/js/game-reviews.js#L501) `_profileNickMap`이 `p.id`로 세팅하는데 profiles엔 `id` 컬럼이 없어(kakao 키=`user_id`) **맵이 빈 채**로 돈다 → 허브 기록보드의 참여자 이름 클릭이 **기록 남긴 회원만** 보드로 연결되고 미기록 회원은 비활성. `p.user_id`로 고치면 전원 연결(홈 카드에서 이미 그렇게 정정함). 저위험 1줄. **주의**: `getAllProfiles`가 이미 호출되고 있어 추가 fetch 불필요.
