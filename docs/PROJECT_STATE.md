...
> ✅ **「뽁님 7/11 게임평 ↔ 플레이기록 연결」은 2026-07-22 종결 — 코드 작업 없음, 남은 건 안내뿐. 재등록 금지.** 참여는 이미 `player_names ILIKE` 병합으로 집계되고, 「코멘트 유지 + 남의 세션에 기록 추가」도 이미 가능하다(게임평 모달 → 연동 체크 → `join:` 선택). 근거는 git log.
> - 🚨 **여기서 내가 두 번 틀렸다 — 원인은 조사 부족이 아니라 「필독 문서를 안 본 것」이다.** 「남의 세션 참여」의 경로별 동작(4갈래, 코멘트 삭제 여부가 갈림)은 [js-api.md](js-api.md) `onSubmitCommentModal`·`onSubmitPhotoModal` 줄에 **이미 정확히 적혀 있었다.** 코드만 읽고 대표 경로 하나로 요약하다 틀렸다. **game-sheet 계열을 만질 땐 js-api.md의 해당 줄을 먼저 읽을 것.**
