...
> 🔀 **2026-07-21 재분류.** 사용자 요청으로 순서를 다시 짰다. 기준은 CLAUDE.md B-5·B-6 — **①확인·문구로 닫히는 것 → ②판단이 필요한 버그 → ③기능·구조(Plan) → ④기획**, 그 안에서 **지금 사용자의 관심(모임팟·홈 첫 화면)이 먼저**다. (2026-07-21 세션 ⑱에서 옛 1번 「홈 히어로 미갱신」이 닫혀 번호가 하나씩 당겨졌다.)

| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 📄 **동호회 소개·회칙 요금 문구 정합** — 회칙 77행이 확정 정책과 **정반대로 읽힌다** | §3 P1 | 문구 | **①형**: 정책이 이미 확정돼 코드 없이 닫힌다. 회원이 읽는 규정이라 틀린 채 두면 그대로 오안내 |
| **2** | 🎲 **모임 플래너 동반 지인 인원(`guest_count`)** | §3 P1 | 기능+DB·Plan | **지금 사용자의 관심사**(모임팟 수요가 실제로 발생). 1번과 같은 정책에서 나왔지만 **파일·검증이 달라 묶지 않는다**(A-3) |
...
⚠️ **1과 2는 묶지 않는다.** 같은 요금 정책에서 나왔지만 **1은 정적 HTML 문구, 2는 DB+JS 5개 파일**이라 검증 환경이 다르다.
