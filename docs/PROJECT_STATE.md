...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🔴 **#22 `total_minutes` 증가분 손실(비원자적 read-modify-write)** — #21 조사에서 규명. **표시가 아니라 데이터 정합성** 문제이고 내 보드 「함께한 시간」·업적에도 영향. **착수 전 재현 필요**(기전은 확정, 크기는 미검증) → 그다음 원자적 증가로 전환 | [admin-analytics.md](admin-analytics.md) §4 | Plan / Opus (Red) |
