...
- Play Record Display Family 점수 formatter의 승인 범위·보존 규칙은 [play-record-score-formatter-plan.md](plans/play-record-score-formatter-plan.md)다.
- 플레이기록 게시판·홈 요약·게임정보 목록/게임별 기록은 people/meta presentation을 공유하는 Play Record Display Family다([UI_STRUCTURE.md](UI_STRUCTURE.md), [UI_PATTERNS.md](UI_PATTERNS.md)). 사람 token은 공통 12px·18px line-height·동일 padding 및 micro 12px icon이고, 참가자는 웜그레이, 작성자는 기존 `--green`·`font-weight:600`만 다르며, 인원 표지는 보조정보 tone이다. 게임정보의 목록/게임별 기록은 사람줄 배경을 없애고, 시간·점수를 같은 muted 메타행으로 묶었다. 게임정보·플레이기록 게시판·홈 최근 플레이의 시간은 공통 `play-record-meta-time`(기존 `#8a7868`·600), 점수/구분점은 muted로 통일했다. 브라우저 제어 플러그인의 실행 모듈 누락으로 실제 computed-style/360px 화면 확인은 사용자 실화면 확인 대기다.
- shared UI family 지도는 [UI_STRUCTURE.md](UI_STRUCTURE.md)·[UI_PATTERNS.md](UI_PATTERNS.md)가 정본이다. Play Record 외에도 stable identity/profile entry, meeting participant card, canonical local presentation을 실제 shared renderer/registry가 있는 범위로만 등록했으며, 구조·renderer 전면 통합은 범위 밖이다.
