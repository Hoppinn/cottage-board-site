...
- [ ] **[96]** 게임평 남기기 모달 placeholder → "게임에 대한 평가를 남겨주세요"
- [ ] **[96]** 바텀시트 사진 클릭 → 라이트박스 캡션 첫 줄에 닉네임 표시
- [ ] **[96]** 기록게시판 사진 클릭 → 라이트박스 캡션 첫 줄에 닉네임 표시
- [ ] **[96]** 여러 명 사진 올린 게임 바텀시트 → 헤더 "N명의 사진" / 전체사진 그리드 각 이미지 아래 닉네임
- [ ] **[96]** 기록게시판 게임평 있는 카드 → 게임평 앞에 작은 [닉네임] 태그 표시
- [ ] **[97]** 기록게시판 "사진 추가" 세 번째 탭 표시
- [ ] **[97]** 사진 추가 탭 → 기록 드롭다운 전체 기록 최신순 표시
- [ ] **[97]** 사진 선택 → 그리드 미리보기 → 추가하기 → 해당 기록에 사진 반영
- [ ] **[98]** 기록게시판 사진 3장 초과 시 "+N장" 클릭 → 라이트박스 아닌 가로 펼침
...
- ~~**기록게시판 사진 기록하기 버튼 (세번째 탭)**~~ — 롤백 예정 (142차-97 취소) → 올바른 위치는 게임시트 바텀시트
- **카베르나 게임평 나나 명의 문제** — DB 확인 후 수정
- **[1] 142차-97 롤백** — 사진추가 탭 game-reviews.js에서 제거 (세번째탭이 아닌 게임시트 바텀시트에 위치해야 함)
...
- **[4] 모임별 바텀시트 게임평수정 겹침 버그** — 모임별→게임썸네일 클릭→바텀시트 열릴 때 게임평수정 창이 겹쳐 뜨는 문제 수정
- **[5] 방문자목록 회원탭 표시 개선** — 기본 3개 표시(현재 1개), "더보기 (N명 더)" 개수 수정(현재 132명 더→실제 미표시 수), 닫기버튼 3번째 항목 뒤에 배치
- **[6] 비주얼분석 날짜 필터 → 상단 지표 동기화** — 오늘/7일/30일/전체/날짜별 그래프 변경 시 오늘방문자·7일평균·지난주대비·주요유입 수치도 같이 변경
- **[7] 페이지/회원 분석 기간 필터 개편** — 기본값 최근1주일, 대분류 "최근1주일/전체" 추가, 각 대분류 아래 전체/회원/비회원 서브탭 유지
- **[8] 더루프 사진 "불러오는 중..." 버그** — 게임시트에서 더루프 사진이 로딩만 되고 안 뜨는 문제. game_id 불일치 또는 _fetchGamePhotos 오류 가능성
