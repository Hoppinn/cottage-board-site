...
- [x] 내 보드 프로필 첫 화면 UX 개선 (121차): 대표 캐릭터 ✏ 힌트, 카드 요약 다줄 표기, 함께한 시간 구분선, 음료교환권 중복 제거, 기록보드 항목 클릭→바텀시트
- [x] 내 보드 UX 정리 (122차): 플레이 기록 띄어쓰기, 힌트 아이콘 ⚙, 성장→수집 보드 명칭, 기록 클릭→기록페이지 이동, 알림 그룹화+모두확인제거+0건상태, 캐릭터카드 높이 축소
- [x] UI 소폭 개선 (123차): 칭호 미달성 grayscale 통일, 업적 목표 cur/next 형식, 점수·메모 textarea 디자인 통일
  - 🌱 성장 보드 / ❤️ 취향 보드 (좋아하는+해보고싶은 통합) / 📝 기록 보드 / 🎫 음료교환권 → 2×2 그리드
  - 📊 내 활동 전폭 유지
  - getUserPhotoCount 병렬 로드 추가
- [x] 기록 보드 3섹션 구조 (118차): 플레이기록/게임평/사진 3개 토글, 요약 ul 제거, 기록 페이지 링크 제거
- [x] 취향 보드 게임 표지 클릭 → 바텀시트 (118차): ensureGameSheet() 먼저 호출, gameSheet 변수 null 문제 수정
- [x] 내 보드/서브시트 모바일 상단 잘림 (118차): 100vh → 100dvh fallback 추가 (iOS/Chrome 동적 뷰포트 대응)
- [x] 방문자 카운팅 복구 (118차): 잘못된 GROUP BY date SQL 인서트 22행 삭제, 더미 412행 재삽입. JS 로직(기기당 하루 1회)은 원래 정상이었음
- [x] 내 보드 섹션 UI 전면 재설계 (120차)
  - CSS 버그 수정: profile-char-grid/title-grid > more-wrap에 grid-column:1/-1 추가 (전체보기 레이아웃 깨짐 수정)
  - 업적: goals = 카운트 전용 ("기록 25회" 형식), 목록은 "전체보기 ▾" 토글로 접힘 상태 시작
  - 캐릭터: 미리보기 = 7축별 최고 레벨 캐릭터 1개씩, 전체보기 = 전체 그리드 (preview/body 분리)
  - 칭호: 미리보기 = 5축별 최고 칭호 1개씩, 전체보기 = 전체 그리드
  - 게임도감: preview = count+grade+최근 3개 게임, 전체보기 = 전체 목록
- [x] 내 보드 모든 섹션 "시작 열림 + 미리보기 + 더보기" 패턴 통일 (119차)
  - 기록보드: 플레이기록/게임평 시작 열림 (is-collapsed 제거), 화살표 ▴
  - 취향보드: 좋아하는/해보고싶은 시작 열림 (is-hidden 제거), 화살표 ▴, 게임목록 3개 미리보기 + 더보기
  - 성장보드>업적: 시작 열림 + PREV_ACH=5 미리보기 + 더보기 + 7축 next goal 항상 표시 (profile-ach-goals)
  - 성장보드>캐릭터: 시작 열림 + PREV_CHAR=6 미리보기 + 더보기
  - 성장보드>칭호: 시작 열림 + PREV_TITLE=6 미리보기 + 더보기
- [x] 유저 차단/해제 (어드민)
...
- [x] 방문자 통계 (페이지뷰, 하루 1회 카운트)
  - 113차: 방문자 카운팅 버그 수정 — `__visitor__` 마커 row 방식 도입. 하루 첫 방문 시 1개 삽입, getVisitorStats()는 이것만 카운트. (기존: 페이지뷰 수 = 방문자 수 오기록)
