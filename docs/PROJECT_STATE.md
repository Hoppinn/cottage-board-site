...
- 현재 단계: 1~4순위 코드 수정 및 사용자 브라우저 확인 완료. 5순위 보드 CTA와 6순위 메인 모임 카드 위치 보정 코드는 구현 완료. 프로필 수정 CTA는 `club-intro.html?edit=1`로 기존 프로필을 채운 수정 위저드에 직행하도록 보완했고 사용자 브라우저 확인 대기
...
- 현재 버그: 1~4순위 완료. 5~6순위 CTA의 버튼 배치·내/타인 권한 분기·메인 모임 카드 이동 위치는 사용자 브라우저 확인 대기. 7순위 게임도감 과도한 간격은 아직 미착수.
