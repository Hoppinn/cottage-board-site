...
- 현재 단계: 게임도감 정렬·세로 우선 2열·무모션 토글 위치 보정은 사용자 확인 완료. 메인 보드의 프로필·모임·수집 카드 헤더 구조 조사 전
- 다음 작업: 세 카드의 렌더 경로와 현재 아이콘/제목 배치 규칙을 확인한 뒤, 아이콘 + `XX 보드 >` 한 줄 구조로 통일한다.
...
- 현재 버그: 없음. 이전 게임도감 작업은 사용자 확인으로 완료.
