...
- **개별 '읽음' 버튼이 실제로는 '모두 읽기' 🔴 Plan 진행 중** — [kakao-auth.js:1588](../assets/js/kakao-auth.js#L1588)이 `_markAllNotifSeen`을 호출. 읽음 상태가 `notif_seen_at` **타임스탬프 하나**뿐이고 알림도 DB 행이 아닌 파생 생성물이라 개별 읽음을 저장할 자리가 구조적으로 없음. **사용자 결정: 개별 읽음을 제대로 구현**(Red) → **코드 완료, ⏳ 마이그레이션 실행 + 스모크 대기**.
  ...
  - ⏳ **남은 것 = 스모크뿐**. ⚠️ **선행 조건**: R1 스모크 때 '모두 읽기'를 눌러 `notif_seen_at`이 `2026-07-18 12:43`으로 올라가 있어 **현재 미읽음 알림이 0건** → 읽음 버튼이 렌더되지 않아 그대로는 테스트 불가. 지평선을 되돌려야 한다(DB `notif_seen_at=null` **+ localStorage `cottage_sess_{uid}.notifSeenAt` 삭제** — `effectiveSeenAt`이 둘 중 큰 값이라 한쪽만 지우면 안 됨).
