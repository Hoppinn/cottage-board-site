...
- 현재 단계: 취향 범위의 `장르를 가리지 않음`이 주 취향이 일반 유형일 때 선택되지 않던 버그를 032 마이그레이션과 위저드 동기화로 수정했다. 032 운영 적용·코드 push·실화면 검수가 남았다.
- 다음 작업: 사용자가 032를 운영 DB에 적용한 뒤 새 코드를 배포한다. 그다음 주 취향+범위 any 저장, 상세 출력, 쿠폰 재지급 방지와 360px 화면을 실계정으로 확인한다.
