...
> 📌 **현재 목표**: 표 1번은 **표본 대기**라 사람이 할 게 없다. 실착수할 수 있는 건 **표 2번(dead code 정리)** 하나이고, 그 외는 §5(기획·디자인)이나 실기기가 필요한 「열린 스모크」다.
...
| 2 | 🧹 **dead code 2건 삭제 (리팩토링 세션)** — ①`requests-admin.html:1048` `buildPageMap`(정의 1건 외 참조 0) ②`game-display-adapter.js` `getSearchText`(419) + export(626) + `searchText:` 3줄(459·526·561). **소비처 0건은 2026-07-22 재확인**했다 | 「코드 품질 주석」 | 리팩토링 | 유일하게 남은 실착수 후보. **버그 수정 세션에 섞지 말 것**(구현/리팩토링 분리) |
| 3 | **그 외엔 §5(기획·디자인) 또는 「열린 스모크」(실기기 필요)** | §5 | — | GS5·「진입 N회」 2중 계상은 2026-07-22 종결(git log) |
