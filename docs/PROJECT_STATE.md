...
- ✅ **[버그2-b] 크로스보드 stale — 코드 해결 완료 (2026-07-17 R10b, 스모크만 남음)**. 방향 A(진입 시 DB 재조회 = 단일 소스)로 처리, 스냅샷 임시방편(11e10b8) 제거. 상세·스모크 포인트는 아래 R10b 항목.
...
- **R10b 🟡 코드 완료 · 브라우저 스모크만 남음 (2026-07-17, 커밋 507f2e9·c0cd874·5f82aed)**: 크로스보드 stale을 방향 A(진입 시 DB 재조회 = 단일 소스)로 해결. 취향보드 스냅샷(11e10b8)·`_currentBio` 접근자 제거.
  ...
  - **스모크 포인트(다음 세션 최우선)**: ①취향보드에서 게임 추가/삭제 → 뒤로 → 모임보드에 즉시 반영 ②그 반대 ③취향보드 한줄소개·피하는유형 수정 → 모임보드 선호/비선호 칩 즉시 반영 ④📖 룰 토글 → 모임보드 룰 배지 반영 ⑤모임보드 '비선호 유형 수정 →' 클릭 시 취향보드가 **피하는 유형 섹션으로 스크롤**된 채 열리는지(비동기 렌더로 바뀐 자리 = 회귀 1순위) ⑥거기서 '‹ 모임 보드' 복귀 시 스크롤 위치 복원 ⑦게임시트 좋아요 토스트→취향보드 직행(autoSubsheet 경로) ⑧읽기전용 남의 보드. **동반 확인(이월분)**: R2(취향 게임추가 새로고침 필요) · R4(사진첨부 후 새로고침 필요)가 아직 재현되는지 — 재현되면 원인이 스냅샷이 아니었다는 뜻이므로 별건 등록.
  - 📌 **R10b 세션에서 같이 할 일 — `REFACTOR_CHECKPOINT.md` 정리 (2026-07-17 사용자 지시로 이월)**: 474줄로 최대 문서인데 대부분 완료된 R1~R9·R11·R12 상세라 압축 대상. **2026-07-17에 압축을 시도했다가 멈췄음** — 완료 상세인 줄 알았던 구간에서 **처리 현황 표에 등재된 적 없는 고아 항목 3건**이 나왔기 때문(`squirrel_lv5` 미제작 / `rare/` 경로 문서 미반영 / RPC 재검토 트리거 — 전부 §3로 추출 완료). **같은 지뢰가 더 있을 수 있으니 "완료 같아 보인다"고 통째로 지우지 말 것.**
    ...
    - **R10b 세션에 붙인 이유**: 그때 이 문서를 실제로 열어보게 되므로 맥락을 쥔 채 판단할 수 있음. 이 문서는 **세션 시작마다 읽는 파일이 아니라** 압축 이득이 작고, 잘못 지우면 손해가 큼.
- **R10c ⏳ 대기**: 네비게이션 스택(§3-1 알림 복귀 + §3-2 좋아요 토스트→게임시트 복귀, `openProfilePanel(sub, {backTo:{...}})` 일반화). 신규기능.

**다음 세션 시작점**: **R10b 브라우저 스모크**(위 8개 포인트). 그 다음 후보 = R10c(네비스택) / 감지기 3단계 / REFACTOR_CHECKPOINT 정리.
