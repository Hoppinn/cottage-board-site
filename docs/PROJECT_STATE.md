...
- **📵 간식·음료/건의 요청 카카오톡(Make) 알림 미수신 (2026-07-27 제보 → 같은 날 재확인 "예전엔 왔었다")** — 코드베이스엔 관련 코드가 전혀 없다(Supabase Database Webhook → Make.com 시나리오 `5213346` → 카카오 "나에게 보내기" API, 전부 대시보드 설정. `voucher_log`도 같은 방식 — [db-schema.md](db-schema.md) 참조). **내가 확인·수정할 수단이 없음.**
  - ⚠️ **"전에는 왔다"가 중요한 단서다** — 처음부터 안 됐던 게 아니라 **중간에 끊겼다는 뜻**이라 "설정 안 됨" 계열 가설(테이블 미포함 등)보다 **"있던 게 깨졌다" 계열이 우선**이다. 이번 세션에 `snack_requests`에 컬럼을 두 번 추가했지만(015·016) **Database Webhook은 INSERT 시 그 시점 행 전체를 JSON으로 보내는 방식이라 컬럼 추가로 안 깨진다** — 이 세션 작업이 원인일 가능성은 낮다(단, 확인은 사용자만 가능).
  - **확인 순서 추천**: ①Make.com 시나리오 `5213346` 실행 기록(History)에 최근 실행이 아예 없는지/에러로 찍히는지 — **에러면 원인이 바로 보인다** ②시나리오 자체가 비활성화(꺼짐) 상태인지 — Make는 연속 실패 시 자동으로 끈다 ③카카오 "나에게 보내기" API 토큰 만료 여부 — 카카오 액세스 토큰은 유효기간이 있어 **갱신 없이 시간이 지나면 조용히 실패**할 수 있음(가장 유력, "전엔 됐다"와 가장 잘 맞음) ④Make 무료 플랜 월간 작업량(Operations) 초과.
  - 예전에 Discord 웹훅으로 전환하는 안이 나왔으나 보류 중(Make 시나리오 HTTP 모듈 교체만 하면 됨 — 토큰 만료 문제 자체를 회피할 수 있어 ③이 원인으로 확인되면 이게 근본 해결책).
  - **재개 조건**: 사용자가 위 ①~④ 중 하나라도 확인해 알려주면 그에 맞춰 다음 수를 안내(예: ③이면 Discord 전환 권장, ②면 재활성화 안내).
