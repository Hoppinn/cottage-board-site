...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 🧑 **대표 캐릭터·칭호 변경이 즉시 반영 안 됨**(새로고침해야 바뀜) | §2 | 개별 판단 | 매번 재현되는 회귀 + 사용자가 방금 겪음. 재현부터 |
| 2 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 | 전부 **재현부터** 필요. 하나씩 독립 |
| 3 | 🔔 **관심 기반 묶음 알림** | §3 | Red+Plan | 무거운 신규 기능 |
| 4 | 🟡 **#28 `page_sessions.referrer`도 라벨/pathname 저장** — 외부 유입 체류가 전부 「직접 방문」으로 접힌다. #14와 같은 파일이지만 **다른 화면·다른 판정**이라 분리 | [admin-analytics.md](admin-analytics.md) §4 | 옐로 | 관리자만 보는 화면 |
| 5 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan | 사용자 눈에 안 보이는 리팩토링 |

> 🔀 **2026-07-21 2차 재분류**: 관리자 화면(#28)·리팩토링(GS5)보다 **사용자가 방금 겪은 것**을 앞에 둔다(B-6). 플래너 2건은 같은 날 닫혔다(아래).
> ⚠️ **플래너를 다시 만지면 「열린 스모크」의 「동반 인원 실기기 확인」을 함께 볼 것** — 2026-07-21 Step3 작업은 **쓰기 차단 상태로 DOM만** 봤으므로 그 스모크는 여전히 열려 있다.

> ✅ **「플래너 Step3 게임 등록창 2건」은 2026-07-21 닫힘** — 섹션 간격 + 「지난 모임」 불러오기 버튼. **덤으로 게임 신원 표기 불일치를 고쳤다**(`game_likes`는 슬러그, `meeting_vote_games`는 bggId → 같은 게임을 서로 못 알아봤다). 그 사실의 주소는 [db-schema.md](db-schema.md) `game_likes` 행이고 변환기는 `normalizePickerItem`. 커밋 참조.
