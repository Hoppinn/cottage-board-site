...
- [ ] **[feat] 기록입력 '최신 기록' 버튼 → 최신순 토글로 선택** (2026-07-18 등록, 사용자 아이디어. **판단=할 만함, 구현 보류**) — 현재 1번 행 `↑ 최신 기록 (인원·참여자)` 버튼([game-reviews.js:190](../assets/js/game-reviews.js#L190))은 **가장 최신 내 기록 1건**의 그룹명·인원·참여자만 채운다([:295](../assets/js/game-reviews.js#L295) 핸들러, 데이터는 [:517](../assets/js/game-reviews.js#L517) `_prLatestRecord`=`_myLatest[0]`). 사용자 요청: **직전 1건만 말고 최신순으로 눌러가며(토글) 선택**.
  - **판단 — 저비용·저위험(Sonnet, 소규모 feat)**: 데이터가 **이미 로드돼 있다**. `_myLatest`가 이미 내 기록을 최신순 정렬한 뒤 `[0]`만 취하고 나머지를 버린다([:505~516](../assets/js/game-reviews.js#L505)). 배열 전체를 보관(`_prMyRecents`)하고 인덱스 1개만 두면 클릭마다 다음 기록으로 채우면 끝. DB 추가조회 0.
  - **UX 판단(이게 핵심)**: ⓐ **연속 동일 세팅 dedup 필수** — 같은 모임의 여러 게임 기록은 그룹·인원·참여자가 거의 동일해, 12건을 그냥 순회하면 90%가 같은 값이라 무의미. `(group,count,names)` 시그니처로 **서로 다른 조합만** 순회해야 값이 있다. ⓑ **현재 로드된 게 뭔지 라벨로 보여줄 것**(예: `↑ 3/7 · 7/15 코티지보드 동호회`) — 누를 때마다 동작이 바뀌는 버튼은 숨은 상태라 맹목적 순회는 나쁜 UX. ⓒ 끝에서 최신으로 wrap. **대안**: 버튼 대신 최근 **distinct 세팅 목록 팝오버**로 직접 선택(더 발견적이나 UI 비용 큼). 사용자가 말한 "토글"은 ⓐ+ⓑ 조합이 최소구현.
  - **주의**: `fillCountAndNames`가 기존 태그칩을 지우고 재생성하므로([:285](../assets/js/game-reviews.js#L285)) 재클릭 안전. 2번+ 행의 '위와 동일'은 별개 경로라 무관.
...
- [ ] **메인 최근 플레이 미리보기 클릭 비활성** (2026-07-16 기록 → 2026-07-18 **수정+1차 스모크 반영, 재스모크 대기**) — 메인 "최근 플레이 미리보기" 카드 클릭 불가 건. [index-page.js `initRecentPlay`](../assets/js/index-page.js#L1017). **1차 스모크 피드백 4건 반영**: ①썸네일→게임시트(`openGameRecordSheet`) 유지 ②**게임명은 클릭 대상 아님**(1차엔 눌렸음, 제거) ③**참여자 이름→각 회원 읽기전용 보드**(`openOtherProfileSheet`) — 이름 태그에 `data-nick`, **`getAllProfiles`(전체 회원) 기준 nickname→user_id + 로드된 기록으로 보강** = 기록 없는 회원도 클릭 가능(2차 스모크 반영: 죠르디·춘팝 등 미기록 회원도 걸리게). profiles 조회는 기록 조회와 `Promise.all` 병렬이라 로딩 부담 무시 가능. ⚠️ **profiles엔 `id` 컬럼 없음 → `p.user_id`로 해석**(game-reviews는 `p.id`라 프로필맵이 빈 맵인 채 기록 기반만 돌던 동일 버그 — 여기선 정정). ④**사진 삭제권한**: 이 기록이 내 것/오너면 `data-record.mine=true`로 라이트박스에서 삭제 노출(`openRecordLightbox` onDelete). 리뷰어 닉네임→프로필 유지. ⚠️ **알려진 한계**: 라이트박스 삭제 후 홈 카드는 새로고침 전까지 stale(onAfterDelete 미전달). **재스모크**: 썸네일→시트 / 게임명 안 눌림 / 참여자 이름(기록 없는 회원 포함)→그 사람 보드 / 내 사진 라이트박스에 삭제버튼.
