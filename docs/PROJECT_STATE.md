...
> 📌 **현재 목표**: 다음 큰 덩어리는 **「관리자 분석 P3(드릴다운)」**이고 착수하려면 사용자의 「하자」가 필요하다(Plan 필수, 세션 하나 규모 — 회원 탭 기간 선택·탭 구조 압축이 **같은 UI를 공유**하므로 따로 하면 기간 선택기가 두 벌 된다).
> ✅ **캡션 복사 자리는 2026-07-22 「양쪽 다 ⋯ 메뉴 안」으로 확정 — 재논의 금지.** 두 번 뒤집힌 끝이라 결론만 남긴다: **날짜 헤더에 상시 버튼**(모든 날짜에 깔려 시끄럽다) ❌ / **모임 타이틀에 하나**(캡션은 하루치인데 타이틀은 모임 단위라 어느 날짜인지 못 정한다) ❌ / **각 기록의 ⋯ 메뉴 맨 아래** ✅. **로그인·회원 여부를 보지 않는다**(비회원도 쓰라는 사용자 결정) — 그래서 다른 항목이 없어도 ⋯ 메뉴가 뜬다. 조립은 `play-records-utils.js`가 SSOT고 사본이 생기면 `verify-history-caption.js`가 실패한다.
