...
> 📌 **현재 목표**: 표 1번은 **표본 대기**라 사람이 할 게 없다. **실착수 가능한 코드 작업이 0건**이므로 §5(기획·디자인)이나 실기기가 필요한 「열린 스모크」에서 고른다.
...
| 2 | **그 외엔 §5(기획·디자인) 또는 「열린 스모크」(실기기 필요)** | §5 | — | dead code 2건·GS5·「진입 N회」 2중 계상은 2026-07-22 종결(git log). **실착수 가능한 코드 작업이 비었다** — 다음 세션은 §5에서 고르거나 실기기 스모크를 소화할 것 |
...
> - 📌 **새로 만들 때 지킬 것**: 지난 날짜에서 **막대·상세·모달(보기)은 그대로 열고 등록 행동만 렌더에서 뺀다.** `pointer-events:none`로 막지 말 것 — 2026-07-09 확정 사양이고, 막으면 지난 모임 내용을 못 본다.
> - ⚠️ **`window.buildBarsInCard`가 ✎/✕의 단일 출처**다(홈·플래너·상세모달 3곳 공통). 지난 날짜 판정을 그 함수 밖에서 다시 하지 말 것.
