...
| 2 | 🧹 **P5 청소** — #4·#17 죽은 코드, 죽은 렌더러 3개, #7 x축 눈금, #12 차트 소속. 전부 그린이고 서로 독립이라 한 배치로 | 〃 §4 | 그린 / Sonnet |
| 3 | 📅 **회원 탭 페이지 분포에 기간 필터** (2026-07-19 사용자 요청) — 아래 별도 항목. **#12·#21과 한 화면이라 함께 판단할 것** | §3 | Plan / Opus |
| 4 | 🎯 **P3 — 드릴다운** (이벤트·페이지 → **그걸 한 사람들**로). 원칙 ⑤의 최대 결손, "유기적"의 핵심. **P1·#15·P2로 값과 이름이 정리됐으니 이제 연결할 차례.** 그다음 **P4=C단계**(보드 연동, Red) | 〃 §5 | Plan / Opus |
