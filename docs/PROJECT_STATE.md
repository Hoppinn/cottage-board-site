...
- 현재 단계: 1. 모임보드 미리보기 수정 완료·사용자 화면 확인 대기
- 다음 작업: PC/모바일에서 미리보기 확인. 확인 전에는 2번으로 진행하지 않음.
- 인수인계: 미리보기는 앞으로 예정된 나의 모임 참여 날짜와 각 모임의 내 참여자 카드 정보를 압축해서 보여준다. 일정 날짜는 최대 3개 후 `외 n회`로 줄이며, 평소 프로필 성향은 중복 표시하지 않는다.
- 승인 대기: DB/API 계약 또는 저장 형식 변경이 필요하면 구현 전에 사용자 승인을 받는다.
- 현재 버그: 새로 보고된 열린 버그 없음.

### 다음 시작점

1. [member-board-plan.md](plans/member-board-plan.md)를 읽고 1단계 미리보기 구현을 시작한다.
2. 기존 사용자 확인 대기 항목은 현재 작업과 직접 관련될 때만 함께 판정한다.
3. 모임원보드 개편 완료 후 기존 보류 항목으로 돌아간다.
