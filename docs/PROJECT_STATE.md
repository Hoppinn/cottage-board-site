...
**진행 중 작업 없음** (2026-07-21 세션 ⑲ 종료 시점, **모임 플래너 동반 인원 종결** — 013 적용·검증 완료) · **열린 스모크 5건**(§0 아래 「열린 스모크」).
...
- **세션 ⑲**: 「검사기를 먼저 의심한다」·「자동 검증은 육안 확인을 대체하지 않는다」가 **둘 다 발동**. 전자는 `--negctl`로 기대값을 비틀어 **그 2줄에서만 🔴**이 뜨는 걸 본 뒤 통과를 신뢰한 자리. 후자는 스크린샷을 읽다가 **7/20 등록이 게임명 칸에 「지인팟 4인」이라 적어 지인 수를 신고 중**인 걸 본 자리 — assert만 봤으면 이 기능이 대체하려는 우회가 이미 운영 데이터에 있다는 걸 몰랐다. 「신규 생성 전 기존 검색」도 발동: 문서가 「소비처 5곳」이라 적어둔 것을 grep으로 재확인해 **8곳**임을 확인(`openDatePreviewModal` 누락) — 그대로 믿었으면 그 화면만 옛 숫자를 냈다.
- **재감사 카운터**: 감사 시행이 2026-07-20(세션 ⑭), 세션 ⑲가 **5회차 = 트리거 도달**. 다음 세션 착수 전 재감사할 것(볼 것 4가지는 CLAUDE.md 말미). ⚠️ **세션 ⑰은 카운트하되 감사 자체를 손댄 세션**이다 — CLAUDE.md에서 2건을 커맨드로 분리했다가 `wrap`은 되돌렸고(매 세션 발동 규칙은 상주), 그 판정 기준이 「재감사 때 볼 것」 **4번**으로 들어갔다.
...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | 🚫 **「프로필 보드 4건 묶음」은 묶음이 아니었다** (2026-07-21 독립 진단) — 4건 중 **2건은 이미 닫혀 있었고**(수집보드 스크롤=커밋 db08f332에서 해결됨, 카드 요약=2026-07-21 수정), 남은 2건은 **원인·코드 경로·검증 환경을 하나도 공유하지 않는다**: 박스모달 닫힘은 z-index 레이어 문제(§3), 타인 보드 네비는 표현 통일 문제(§3). **따로 처리할 것** | §3 | 개별 판단 | 원래 대기열 선두였다가 밀린 것 |
...
(비어 있음 — 「동반 인원」은 2026-07-21 종결. 구조는 [js-api.md](js-api.md) `getPartySize`/`sumPartySize`, 컬럼은 [db-schema.md](db-schema.md) 013)
