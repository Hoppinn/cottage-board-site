...
- [ ] **칭호 시스템 V1** (설계 완료, 미구현)
  ...
  - 칭호 계열: 리뷰(✍→👑, 1/10/50/100/300) / 기록(📝→🏛, 10/50/100/200) / 사진(📸→🎨, 1/10/50/100) / 도감계열(🌱→👑, 0~100%) / 방문(☕→👑, 10/30/50/100/300)
  - 희귀칭호: 신작사냥꾼/첫번째개척자/밤의순찰자/이야기꾼 (별도 트리거 필요)
  - 종합등급(코티지초보자~마스터): JS 실시간 계산, DB 저장 불필요
  - 방문 계열·희귀 칭호는 현재 17개 외 신규 트리거 추가 필요
  - 포인트→교환권 전환([4] 보류)과 같은 함수 수정이라 동시 진행 권장
  - 구현 시 Red → Plan 필수
