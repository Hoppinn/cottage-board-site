...
>
> 📌 **현재 목표**: 표 1번은 **데이터 대기**라 사람이 할 게 없고, GS5가 닫히면서 **기술부채 쪽 실착수 후보가 비었다.** 다음 세션은 §5(기획·디자인) 또는 「열린 스모크」에서 고르면 된다.
...
| 2 | **남은 실착수 후보 없음** — §5(기획·디자인)에서 고르거나 「열린 스모크」를 실기기로 닫으면 된다 | §5 | — | GS5·「진입 N회」 2중 계상은 2026-07-22 종결(git log) |
...
- **📊 「진입 N회」 접기 육안 확인 (다음에 관리자 페이지를 열 때)** — 2026-07-22 수정은 **실DB 11,850행에 대한 스크립트 검증만** 거쳤다. 관리자 페이지를 열어 ①회원 카드 「📄 페이지 N개」 펼침의 진입 횟수가 줄었는지 ②콘솔에 에러가 없는지 보면 된다. 기대값은 [admin-analytics.md](admin-analytics.md) §4 ⑧(index 3,951→3,435회 등).
