...
- 현재 단계: 033 운영 적용 완료(사용자 확인). 위저드·프로필 출력·공통 footer 구현 완료. `usual_play_days/times`는 평소 습관, 기존 `available_days/times`는 모임 참여 가능 일정 정본으로 분리했다.
- 다음 작업: 실제 로그인 계정에서 신규·수정 저장과 360×568 위저드 footer를 확인한다.
