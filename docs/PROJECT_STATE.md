...
최종 갱신: 2026-07-27 (세션 ㊴ 종료 — **사용자 제보 15건 전부 처리, 열린 버그 0건.** 완료 내역은 git log(`a7a1880a`~`a56b0be8`). CLAUDE.md도 이 세션 중 2건 자체 감사·수정(세션계약 예외 구멍, 보고 타이밍 모순). 다음 시작점은 §0 표 1번(캡션 복사 역할분리, 설계 확정·조사 완료). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음.** **열린 버그 0건**(§2) · **열린 기능 3건**(§3, Plan 필요 1건 포함) · **열린 스모크 7건**(§0 아래, 카카오 Make 알림 신규 포함) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0**. **미배포 커밋 있음(push는 사용자)**.
...
> 🔄 **캡션 복사 위치는 2026-07-22 결정을 2026-07-27에 다시 바꿨다 — 「재논의 금지」였던 걸 재논의한 것이 아니라 카드 구조가 바뀌어 전제가 달라진 것.** 새 설계·근거는 §3 P1 "캡션 복사 — 역할 분리" 항목(=§0 표 1번) 하나에만 있다. **로그인·회원 여부는 여전히 안 본다**(비회원도 쓰는 결정은 유지). 조립 SSOT는 여전히 `play-records-utils.js`, 검사기 `verify-history-caption.js`는 구조 변경 후 재검증 필요.
