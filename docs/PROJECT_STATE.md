...
최종 갱신: 2026-07-28 (세션 ㊷ 종료 — 간식·음료 상태피커 정정·모임시간 30분단위·건의사항 관리자 가시성(+RLS 버그 발견)·삭제버튼 버그·해금토스트 오타·게임평 줄바꿈·게임평모달 바깥클릭 전부 당일 종결. **열린 버그 1건 — 플래너 막대클릭 모달 게임명 정렬(§2, §0 표 1번, 스크린샷 대기)**. **🚨 push 안 됨 — origin보다 16커밋 앞섬(이번 세션만 14개), 사용자가 직접 push 필요.** **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음.** **열린 버그 1건**(§2 — 플래너 막대클릭 모달 게임명 정렬, 사용자 스크린샷 대기) · **열린 기능 4건**(§3, Plan 필요 3건 — 게임정리·룰설명 관리자입력, 플래너 다른날짜 추가 UX, 모임참여 여러날 동시수정) · **열린 스모크 7건**(§0 아래, 카카오 Make는 사용자가 "나중에"로 보류·재제안 금지) · **관리자 분석 발견 대장 열림 0건** · **열린 코드 덩어리 0**. **🚨 push 안 됨 — origin보다 16커밋 앞섬, 사용자 직접 push 필요.**
...
> ✅ **2026-07-28 세션 ㊷ 종료 시 압축.** 사용자 제보·요청 다수(간식상태피커 정정·모임시간30분·건의사항가시성·삭제버튼·해금토스트·게임평줄바꿈·게임평모달) 당일 조사 즉시 종결(§2·§3 참조) — 표에서 제거. **딱 1건만 미해결로 이월**(아래 표 1번). **항목이 하나씩 닫힐 때마다 이 표에서 그 행을 지운다** — 완료 요약을 표에 쌓지 않는다(CLAUDE.md 「저장 위치」).

| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| 1 | 🔴 **플래너 막대 클릭 시 뜨는 모달의 게임명 목록이 중앙정렬 — 왼쪽정렬 요청 (2026-07-28, 미해결)** | §2 | 버그 | **사용자 스크린샷 대기 중** — 후보 3곳 다 확인했지만 실제로 center인 요소를 못 찾음(반대로 막대 내부를 한 번 잘못 고쳤다가 되돌림, git `abaf02e5`). 스크린샷 받으면 바로 닫힘 — ①확인만 하면 닫히는 것 |
...
| 4 | **모임 참여 여러 날 동시 수정 (2026-07-28 요청, 미조사)** | §3 P1 3번 | 기능/Plan | 착수 전 "퇴초입역시처럼"이 가리키는 화면 사용자 재확인 필요 |
...
**열린 버그 1건** — 아래. 나머지는 2026-07-28 당일 종결(git log 참조).

- [ ] **플래너 막대 클릭 시 뜨는 모달의 게임명 목록이 중앙정렬 — 왼쪽정렬 요청 (2026-07-28 제보, 미해결)** — "막대 클릭해서 뜨는 모달에서 게임명들이 중앙정렬돼있는거를 왼쪽정렬해달라는 말이었어"(사용자). 후보 3곳을 코드 리딩+Playwright 실측으로 확인했지만 **center인 요소를 못 찾음**: ①`day-detail.js` `.sched-bar-game-line`(막대 안 작은 텍스트, `text-align:center`) — 한 번 left로 고쳤다가 **이건 막대 안이라 원래 의도대로 중앙정렬 유지가 맞다**는 사용자 확인으로 되돌림(git `abaf02e5`) ②`openDateMeetingModal`의 `_buildParticipantsHtml`(`.dd-game-list`) — computed text-align이 `start`(=left)로 실측됨, 문제 없음 ③`openDateScheduleModal`(내 일정 편집 모달, `.dd-game-list--editable`) — `justify-content:space-between`이라 이름 자체는 flush-left, 다만 select/star 버튼과의 간격이 넓어 시각적으로 "가운데처럼" 보일 수는 있음(진짜 center는 아님). **재개 조건**: 사용자에게 그 모달 스크린샷 요청함 — 받으면 정확한 요소 특정 후 바로 수정.
