...
**진행 상황**: R1~R4 ✅ · R5 ✅ (2026-07-16) — R5: SC1 조사 결과 `_escapeLike`가 감사 이후 이미 4곳 적용돼 있어 코드변경 없이 검증 종결(백슬래시 정식이스케이프 업그레이드는 위험>이득으로 현행유지). **⚠️ R6 진입 전 실서버 스모크 대기 (R1~R5 누적분, 2026-07-16)**: R6부터 위험도 상승(업적 지급 타이밍)이라 그 전에 브라우저 확인 권장. 확인 포인트 ①**성장보드(R3)**: 내 보드 열어 캐릭터/업적/칭호/도감 카운트가 `N/M`으로 정상 표시되는지(regex 파싱→직접전달로 바꿔서 여기가 깨지면 0/0 등으로 보임) ②**사진 첨부(R4)**: 게임시트 사진/플레이 기록 모달에서 사진 여러 장 추가→취소/저장→다시 열기 반복 시 정상, 콘솔 에러 없음 ③**취향 게임추가(R2)**: 취향보드·모임보드 취향박스에서 게임 검색·추가가 되는지(공용 헬퍼로 합침) ④**알림 읽음(R1 KA5)**: 알림 '모두 읽기'/교환권 확인 동작. 이상 없으면 R6 진행.
...
- ✅ **[버그2 해결, 재확인 권장]** 취향/기록보드 변경(게임 추가·삭제 등)이 **즉시는 반영되나 서브시트 뒤로가기→재진입 시 이전 상태로 되돌아감**(전체 보드 재오픈=DB 재조회 전까지). 사용자 정정으로 정확한 증상 확인: 즉시 append는 정상, **재진입 시 되돌아감**이 진짜 문제. 근본원인: `_openSubSheet`가 **패널 오픈 때 만든 정적 HTML 문자열**(`_tasteInnerHtml`/`_recordInnerHtml`)을 재진입마다 그대로 재주입. 해결: `_openSubSheet`에 `onLeave` 콜백 추가 → 뒤로가기/백드롭으로 나갈 때 **현재 DOM을 문자열 변수(let)로 스냅샷** → 재진입 시 변경분 유지(모든 즉시변경 일괄 보존, 각 변경핸들러 무수정). 부수: `currentAvoidTags`를 DOM(is-active) 기준 도출로 변경(스냅샷 정합), 이전 커밋 0a3bd1d의 반쪽 배열push 제거. bio는 기존 `_currentBio` 경로라 무영향. **한계**: 기록보드에 게임시트 등 **외부 경로** 변경이 반영되려면 별도 re-fetch-on-entry 필요(이번 스냅샷은 서브시트 내부 변경·펼침상태만 보존). 실서버 재확인 권장.
