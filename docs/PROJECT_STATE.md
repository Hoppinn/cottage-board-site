...
| 항목 | 남은 확인 | 재개·판정 경로 |
|---|---|---|
| 이날 모임 상세 UI (2026-08-31) | 날짜 고정 헤더, 게임 조율·참여자별 두 섹션, 참여자 한 명=한 카드 위계와 홈/플래너 미리보기의 인원조율 요약·compact 센터모달이 모바일에서 자연스러운지 | 최근 `cf6d1441`, `f1038820` 및 후속 CSS 커밋. 공용 `day-detail.js`로 미리보기·플래너·상세의 참여 인원/최대 시간 겹침을 통일했다. 이상이 있으면 스크린샷의 URL·뷰포트·비교 기준부터 고정한다. |
