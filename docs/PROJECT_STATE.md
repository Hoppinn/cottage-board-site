...
| 모임일정 삭제 후 재등록 막힘 ("등록됨" 잔류) (2026-07-16 등록 → 2026-07-18 **수정 적용, 스모크 대기**) | **근본 원인 규명·수정**: 플래너는 **미리 로드돼 재사용되는 iframe**인데, 홈 "모임 등록"이 보내는 `cottage-reset-week`([index-page.js:1292](../assets/js/index-page.js#L1292)) 수신 시 club-schedule 핸들러가 **메모리 `allVotes`로 렌더만 하고 DB 재조회를 안 했다**([club-schedule.html:2103](../pages/club/club-schedule.html#L2103)). `allVotes`는 최초 `init()` 1회 로드 + 플래너 **안에서** 등록/취소할 때만 로컬 갱신 → **홈 프리뷰 ✕ 등 iframe 밖 삭제를 iframe이 모름** → 재등록 시 stale 기준 "등록됨" 잔류. 모달/iframe 재사용 원칙 사례. **수정**: 로더 `_loadAllVotes()` 추출 → `cottage-reset-week` 수신 시 재조회하도록. ⚠️ **미해결 잔여 확인 필요**: "홈 미리보기 자체 미반영"은 홈 ✕([index-page.js:1484](../assets/js/index-page.js#L1484))이 `_meetingReload`(loadWeek=매번 DB 재조회) 직접 호출이라 트레이스상 정상 갱신돼야 함 → 스모크에서 이 부분이 남는지 별도 확인. |
