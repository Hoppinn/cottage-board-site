...
최종 갱신: 2026-07-20 (세션 ⑯). **무엇이 닫혔는지는 git log와 도메인 문서에 있다** — 이 문서는 **지금 열려 있는 것**만 갖는다.
...
**진행 중 작업 없음** (2026-07-20 세션 ⑯ 종료 시점, **P3b 종결 = 「관리자 페이지 정리」 목표 종료**) · **열린 스모크 3건**(§0 아래 「열린 스모크」).

📌 **CLAUDE.md 「조건부 유지」 규칙 발동 기록** (재감사 때 판단 근거로 쓸 것 — 기록이 없으면 취향 투표가 된다):
- **세션 ⑮**: 「완료 보고는 중학생 수준으로」 발동 — #27 보고를 "봇 판정이 이름표 목록에 의존했다"로 풀어 쓴 자리. **「닫기 우선」 A-1**(이번에 닫을 것 한 문장)도 세션 계약에서 발동했다. ⚠️ **다만 둘 다 "사고를 막았다"는 증거는 아니다** — 이번엔 막을 사고가 없었다.
- **세션 ⑯**: 「검사기를 먼저 의심한다」 발동 — `verify-drilldown.js`를 `--negctl`(기대값을 일부러 1 비틈)로 먼저 돌려 **정확히 그 한 줄에서만 🔴**이 뜨는 걸 본 뒤에야 "전부 통과"를 믿었다. 「자동 검증은 육안 확인을 대체하지 않는다」도 발동 — 스크린샷에서 `미등록 ID __racetest_nl_…`(#22 통제 실험 잔여 행)이 실데이터 명단에 섞여 있는 게 보였고, 그래서 「이름 없는 user_id를 '회원'이라 부르지 않는다」 규칙이 코드에 들어갔다. **assert만 봤으면 「회원 __racete」로 배포됐을 것이다.**
- **세션 ⑯ 재감사 카운터**: 감사 시행이 2026-07-20(세션 ⑭), 이번이 **2회차**. 트리거는 5회.

### 🎯 다음 세션 시작점

> 🏁 **「관리자 페이지 정리」 목표는 2026-07-20 P3b로 닫혔다** (2026-07-19 시작).
>
> **원래 목적과 대조한 결과** — 사용자 원문은 *"유효 데이터들이 더 직접적이고 **유기적으로** 드러났으면"*이었다. **"직접적"**은 P1~P3(도달률·기준 표기·단일 출처)가, **"유기적"**은 P3b가 채웠다: 이제 화면의 모든 주요 숫자에서 **그 숫자를 만든 사람**으로 내려갈 수 있어 탭이 더는 섬이 아니다. ⚠️ **다만 "유기적"이 다른 뜻일 가능성은 남는다**(지표 간 연결·시각 구조 정리 등). 사용자가 부족하다고 하면 종료 조건을 다시 연다.
>
> 📌 **다음 목표는 미정.** 아래 표는 우선순위 순 대기열이다. (#14는 2026-07-20 종결 — [admin-analytics.md](admin-analytics.md) §4.)

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🚫 **「프로필 보드 4건 묶음」은 묶음이 아니었다** (2026-07-21 독립 진단) — 4건 중 **2건은 이미 닫혀 있었고**(수집보드 스크롤=커밋 db08f332에서 해결됨, 카드 요약=2026-07-21 수정), 남은 2건은 **원인·코드 경로·검증 환경을 하나도 공유하지 않는다**: 박스모달 닫힘은 z-index 레이어 문제(§3), 타인 보드 네비는 표현 통일 문제(§3). **따로 처리할 것** | §3 | 개별 판단 |
| 2 | 🔔 **관심 기반 묶음 알림** | §3 | Red+Plan |
| 3 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 |
| 4 | 🟡 **#28 `page_sessions.referrer`도 라벨/pathname 저장** — 외부 유입 체류가 전부 「직접 방문」으로 접힌다. #14와 같은 파일이지만 **다른 화면·다른 판정**이라 분리 | [admin-analytics.md](admin-analytics.md) §4 | 옐로 |
| 5 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan |
...
| 항목 | 내용 |
|------|------|
| 🔴 **홈 히어로 「최근 플레이」가 갱신 안 됨** (2026-07-21 사용자 보고) | 사진+게임평이 **둘 다 있는 새 기록**을 올렸는데 화면이 옛 기록을 계속 보여준다. ⟹ **영향**: 홈 첫 화면 1장([index.html:228](../index.html#L228)). ⟹ **경로**: [index-page.js:1056](../assets/js/index-page.js#L1056) `records.find(rec => rec.review_text?.trim() && rec.photo_url?.trim()) \|\| records[0]`. **아직 원인 미확정 — 아래는 가설이고 착수 첫 동작은 재현·실측이다.** ①**「게임평」이 어느 테이블인가** — 히어로는 `game_play_records.review_text`를 보는데, 게임평은 `game_reviews.content`라는 **별도 테이블**이 있다([supabase-client.js:1728](../assets/js/supabase-client.js#L1728)). 후자에 썼다면 조건에 영원히 안 걸린다(가장 유력) ②**정렬** — [getAllPlayRecordsForHub:1720](../assets/js/supabase-client.js#L1720)이 `played_at DESC`인데 **`played_at NULL` 기록이 실재한다**(§5 「`played_at NULL` 소급 판단」). Postgres는 `DESC`에서 NULL을 **먼저** 두므로 NULL 기록이 목록 맨 앞을 점유할 수 있다 ③`limit(50)` 밖 ④브라우저 캐시 |
...
| 관리자 페이지 금일이용데이터 | 간헐적 미표시. ⚠️ **"원인 불명, 별도 조사 필요"는 낡은 기재였음** (2026-07-19 정정) — 단일버그 추적은 2026-07-18에 **종료**되고 페이지 전체 감사로 대체됐다. **기각된 가설 4건은 [admin-analytics.md](admin-analytics.md) §6에 근거와 함께 있으니 재조사 금지.** 남은 유력 후보는 ①`duration_sec=0`(표시 조건이 `todayS > 0`이라 0이면 항목이 DOM에서 빠짐) ②에러 수집 부재(`console.error`가 우리에게 안 옴). 📌 **후보 ①의 비회원 몫은 2026-07-20 #3에서 줄었다** — 「오늘」이 `page_sessions` 합만 보던 것을 `anon_sessions`와의 max로 바꿔 **0으로 덮여 사라지던 13세션이 살아났다.** ⚠️ **이 버그가 닫혔다는 뜻은 아니다** — 재현부터 다시 볼 것 |
...
- [ ] **[문구] 동호회 소개·회칙을 확정된 요금 정책에 맞춘다** (2026-07-21) — **회칙에 이미 「지인 동반」 규정이 있고, 방금 확정한 정책과 충돌한다.** ⟹ **영향**: 회원이 읽는 규정 2곳 — [club.html:42](../pages/club/club.html#L42) *"동호회 요금은 5,000원입니다"*(동반 지인·지인팟 요금이 없음), [club-rules.html:73~78](../pages/club/club-rules.html#L73) 「지인 동반」 블록.
  - ⚠️ **실제 충돌 지점**: 회칙 77행 *"모임 미참여자 동반 시 동호회 요금 미적용"* — **확정 정책은 「모임 참여자의 동반 지인은 5,000원 적용」**이다. 이 문장이 "동반 지인 전원 미적용"으로도 읽혀 **정반대 안내가 된다.** 문구를 「모임 참여자와 그 동반 지인 5,000원 / 지인들끼리만 이용은 1인 7,000원 일반 이용」으로 갈라 쓸 것.
  - ⚠️ 회칙 75·76행은 `지인 2인 포함 3인`·`지인팟 3인`을 **텍스트로 신고**하게 돼 있다 — 아래 플래너 작업이 이 중 앞엣것을 **`guest_count` 입력으로 대체**한다. 순서: **이 문구를 먼저 고쳐도 되지만**(정책은 이미 확정), 플래너를 먼저 하면 "플래너에 등록" 한 줄로 끝난다.
  - ⟹ **경로**: 두 파일의 정적 텍스트뿐. 코드·DB 무관. 검증: 두 페이지를 열어 **요금 안내가 서로 모순되지 않는지** 읽는다.

- [ ] **[기능+DB] 모임 플래너 — 동반 지인 인원 지원** (2026-07-21 사용자 요청. 계기: 홈페이지를 **모임팟 잡는 용도**로 쓰려는 사람이 실제로 나타남) — 등록·수정에 **동반 지인 수**만 추가. `meeting_votes`에 `guest_count integer not null default 0` 하나 → **착수 시 Plan 필수**(DB 변경).
  - **요금 정책이 범위를 정한다** (2026-07-21 확정): **모임 참여자와 그 동반 지인 = 1인 5,000원 → 플래너 등록 대상.** **지인들끼리만 이용 = 1인 7,000원 일반 이용 → 플래너에 등록하지 않는다**(합류할 일정이 아니고 할인 확인도 불필요). ⚠️ **`participation_type`·`private_party`·지인팟 분리 렌더·매칭 제외는 전부 폐기됐다** — 첫 스펙은 지인팟도 플래너에 올린다는 전제였고, 그 전제가 요금 정책과 어긋났다.
  - **의도**: **등록 건수 ≠ 방문 인원**을 분리한다. 철수 혼자=1명, 철수+지인 2명=**등록 1건·방문 3명**.
  - ⟹ **영향 대상 — 「N명」을 표시·계산하는 화면 5곳 전부.** `getMeetingVotes` 소비처 실측 **5곳**: 주간 플래너 날짜칩·카드([club-schedule.html:1030](../pages/club/club-schedule.html#L1030)·[:1853](../pages/club/club-schedule.html#L1853)), 월간 캘린더 칩([:1741](../pages/club/club-schedule.html#L1741)), 이날 상세 모달([day-detail.js:484](../assets/js/day-detail.js#L484)·[:830](../assets/js/day-detail.js#L830)), 홈 이번 주 모임([index-page.js:1639](../assets/js/index-page.js#L1639)), 프로필 모임보드 주간/월간([kakao-auth.js:1519](../assets/js/kakao-auth.js#L1519)·[:1721](../assets/js/kakao-auth.js#L1721)). **전부 `.length` 또는 `Set(user_id)` 기반**이라 손대지 않으면 조용히 옛 숫자를 낸다(#15 `visitorKey` 사건과 동형 — 공용 헬퍼 없이 일부만 고치면 그 일부가 다른 답을 낸다).
  - ⟹ **수정·검증 경로**: `getPartySize(vote)` = `1 + guest_count` **공용 헬퍼 하나**를 `supabase-client.js`에 두고 위 5곳이 전부 그것만 쓰게 한다. 최대 난점은 [club-schedule.html:733 `calcOverlap`](../pages/club/club-schedule.html#L733) — **`count`가 `names.length`로 묶여 있어** 지인 수를 실을 자리가 없다(화면엔 이름 1개, 인원은 3명). `names`(표시)와 `count`(인원)를 분리하는 게 이 작업의 유일한 구조 변경이다. 검증: `guest_count=0`인 날짜의 렌더 결과가 **변경 전과 동일**한지 + 5개 화면의 인원 숫자가 서로 일치하는지.
