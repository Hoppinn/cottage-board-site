...
| # | 항목 | 위치 | 등급 | 왜 이 순서 |
|---|------|------|------|---|
| **1** | ⏳ **iframe 3배 계상 수정의 사후 확인 — 표본 대기.** `node scripts/verify-iframe-precondition.js`가 「로그인 상태 홈 방문 11건」이 찼는지 알려준다. 차면 `verify-iframe-rows.js`(`--negctl` 먼저) | [admin-analytics.md](admin-analytics.md) §4 ①·⑦ | 검증 | **2026-07-22 재측정: 표본 7/11건, 판정 불가**(자연 증가 중). 🚨 앞서 `verify-iframe-rows.js`가 **🔴 1건**을 냈지만 **수정 실패가 아니다** — 근거와 다음 지침은 §4 ① 「사후 확인 경과」에 있다. 그걸 읽기 전에 재수사하지 말 것 |
