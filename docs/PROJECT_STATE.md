...
| 항목 | 남은 확인 | 재개·판정 경로 |
|---|---|---|
| 이날 모임 상세 UI (2026-08-31) | 날짜 고정 헤더, 게임 조율·참여자별 두 섹션, 참여자 한 명=한 카드 위계와 홈/플래너 미리보기의 인원조율 요약·compact 센터모달이 모바일에서 자연스러운지 | 최근 `cf6d1441`, `f1038820` 및 후속 CSS 커밋. 공용 `day-detail.js`로 미리보기·플래너·상세의 참여 인원/최대 시간 겹침을 통일했고, 햄버거 active 점과 모달/내 보드 패널 compact 규칙을 조정했다. 사용자 재검증에서 헤더 기준·참여자 간격·음료교환권 카드가 여전히 기준과 다름으로 확인되어, 현재 내 보드 고정헤더를 모바일 하위보드 헤더 기준으로 재정렬하고 대상 카드 선택자를 분리하는 중이다. 사용자 실화면 재확인 전까지 미완료로 유지한다. |
