...
| ~~모바일 "이번주모임 미리보기" 일요일 레이아웃 밀림~~ | ✅ **해결** (2026-07-15). 원인: `#meetingDays`(`.meeting-day-chip` 7개, `flex-wrap:wrap`)가 좁은 모바일 뷰포트에서 컨테이너 폭을 근소하게 초과 — 투표 인원수(`1명`/`2명` 등 텍스트)가 있는 요일 칩만 폭이 넓어져 이번 주처럼 참여자가 몰린 주에만 7번째(일요일) 칩이 다음 줄로 밀림. 1차 수정(gap 6→4px, padding 6px 10px→6px 5px, min-width 36→32px)은 줄바꿈은 해소했으나 **사용자가 실제 화면에서 간격이 너무 좁아졌다고 재지적**(363px 폭 스크린샷) → 2차 수정으로 gap은 원복(6px 유지), padding·min-width만 소폭 축소(6px 10px→6px 7px, 36→34px)해 시각적 간격을 원래에 가깝게 복원하면서 340px 이상 전 구간(320px 제외, 원래 디자인도 320px에서는 줄바꿈됐던 것과 동일 — 회귀 아님) 줄바꿈 해소 재확인(스크린샷 대조 완료) |
