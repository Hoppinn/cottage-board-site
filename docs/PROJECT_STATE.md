...
**세션 ⑫ (2026-07-19)** — 관리자 분석 **P1(도달률) + #15(사람 세기) + P2(기준 표기)** 완료. 커밋 5개(`3b8cd63` P1 / `eefa6b1` #15 / `8c6e7bf`·`acb1da7` P2 / `90ec214` 문서). 상세는 [admin-analytics.md](admin-analytics.md) §5 「이미 끝난 것」·§5-1.
...
| # | 항목 | 위치 | 등급·모델 |
|---|------|------|----------|
| 1 | 🔴 **`record_complete` 추적 보강** — `recordGamePlay` 호출부 6곳 중 [game-reviews.js:469](../assets/js/game-reviews.js#L469) 1곳만 발화, `game-sheet.js` 5곳(816·1948·1962·2036·2641) 무추적 → **`record_start` 231건 대 `record_complete` 1건**. ⚠️ **P3보다 먼저**: 기록 계열 데이터가 비어 있으면 드릴다운해도 내려갈 게 없다 | §3 | 옐로 / Sonnet high |
| 2 | 🧹 **P5 청소** — #4·#17 죽은 코드, 죽은 렌더러 3개, #7 x축 눈금, #12 차트 소속. 전부 그린이고 서로 독립이라 한 배치로 | [admin-analytics.md](admin-analytics.md) §4 | 그린 / Sonnet |
