...
**진행 중 작업 없음** (2026-07-20 종료 시점) · **열린 스모크 3건**(§0 아래 「열린 스모크」).
...
>
> 📐 **순서**: 목표 화면 **안에서** 쉬운 것부터 — 그 쉬운 것은 2026-07-20에 전부 닫혔다. **지금 1번은 #27**(사용자가 보는 숫자의 **기준이 화면마다 다르고**, P3b가 그 위에 얹히므로 먼저다). 그다음 P3b가 목표를 닫는다. #14는 소급 결정이 걸린 별건이라 목표를 닫는 데 필수가 아니다.

| # | 항목 | 위치 | 등급 |
|---|------|------|------|
| **1** | 🔴 **#27 「비회원 N명」 집계 기준 불일치** *(2026-07-20 사용자 신고)* — 요약 탭은 오늘 비회원 14명인데 회원 탭엔 3명. **두 화면이 다른 모집단을 같은 이름으로 센다는 것까지만 확정** — 어느 쪽이 옳은지, 부풀었는지는 **미판정**. **착수 첫 동작은 「비회원」의 정의**(진입 전부 / 세션 시작 / 봇 제외 실이용자) **를 정하고 두 화면을 통일하는 것**. 그 뒤에 원인·부풀림을 측정한다 | [admin-analytics.md](admin-analytics.md) §4 #27 | 조사 |
| 2 | 🎯 **P3b 드릴다운 — 목표를 닫는 유일한 항목.** 이벤트·페이지 숫자에서 **"이걸 한 사람들"**로 내려가는 길. 무겁고 Plan 필요. 선행(#22·#24·R3) 완료. **착수 전 [admin-analytics.md](admin-analytics.md) §1 원칙 ⑤ + §5 P3b를 읽을 것** | 〃 §5 | Plan |
| 3 | 🟡 **#14 `page_sessions.page`가 라벨 저장** — 저장 경로가 **둘**(`script-nav.js` 세션 트래커=한글 라벨 / `_startAnonHeartbeat`=슬러그)이라는 정체가 2026-07-20에 밝혀졌다. 통일 + **과거 행 소급**(불가분 감안) 결정이 필요 | [admin-analytics.md](admin-analytics.md) §4 | 옐로+Plan |
| 4 | 🧩 **프로필 보드 4건 묶음** *(관리자 페이지 종료 후)* — 카드 요약이 편집 미반영 · 박스모달이 닫힘 · 타인 보드 네비게이션 통일 · 수집보드 스크롤 진입점. 네 건 다 같은 서브시트 구조 | §3 | 옐로 |
| 5 | 🔔 **관심 기반 묶음 알림** *(교환권 도배는 2026-07-20 종결)* | §3 | Red+Plan |
| 6 | 단독 버그: 기록보드 09:00 고정 · 서브시트 모서리 음영 · 사진 개별 삭제 불가 · 금일이용데이터 간헐 미표시 · TITLE_DEFS 미배정 칭호 · `squirrel_lv5` 파일 없음 | §2·§3 | 개별 판단 |
| 7 | GS5(escH ~11개) — 착수 시 [REFACTOR_CHECKPOINT.md](REFACTOR_CHECKPOINT.md) 필독 | §3 | Plan |
