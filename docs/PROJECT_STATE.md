...
- **[같은 디자이너 게임 → 뒤로가기]** 같은 디자이너 섹션에서 다른 게임 클릭 시 현재 게임키를 스택에 저장하고, 새 시트에 "← 이전 게임" 뒤로가기 버튼 표시. 클릭 시 이전 게임 시트로 복원. (게임위치 선반 시트의 openShelfSheet 스택 방식 참고)
