...
1. 기능 변경, DB 변경, 데이터 구조 변경, 인증(Auth), Supabase 관련 작업은 반드시 Plan을 먼저 제시하고 승인 후 진행한다.
2. 여러 파일에 걸친 리팩토링이나 구조 변경은 변경 대상 파일과 영향을 먼저 설명한 후 진행한다.
3. UI/CSS 마감 작업은 기능 변경이 없는 경우 Auto mode 진행 가능. 단, 수정 전에 변경 파일과 범위를 먼저 요약한다.
4. 사용자가 명시적으로 요청하지 않은 전면 리디자인, 구조 변경, 네비게이션 변경은 금지.
5. 디자인 원칙은 `docs/DESIGN_RULES.md`를 따른다.
